...
def v2_openapi_and_seams():
    ...
    seams['paths']['/conformance/seams/sample/effect-seams/fire'] = {'post': {'tags': ['Seams'], 'operationId': 'fireEffectSeam',
        ...
        'description': 'The host runs a workflow that drives the named seam once. The scenario then forks the run in `replay` mode and reads `GET /runs/{runId}/effects` on the fork: a seam whose manifest row says `branchReFires: false` MUST NOT produce a new effect attempt there. The witness is the host own Layer-2 ledger, so the leg needs no externally reachable receiver. Without this seam the replay-suppression obligation is only claims-checkable.',
