...
/**
 * RFC 0197 §A.2 R3, registry leg — for every `v2-minor` deprecation row that
 * declares a `retirement.manifest`, how many PUBLISHED manifests still carry
 * the surface. R3's bundle leg is checkable from this repo alone; the registry
 * leg is not, and "we did not look" must not read like "nobody has it". So the
 * census is COMMITTED here with the registry commit and its date, and
 * check-v2-retirement fails a row whose census is missing, null, or older than
 * 30 days — the freshness rule is gap G2.
 *
 * Emits `{}` when there is no row to census (the state at 2.36.0) and `null`
 * when the registry checkout is absent, so the two are never confused.
 */
function registryManifestCensus(dir, root) {
  if (!dir) return null;
  let entries = [];
  try { entries = JSON.parse(readFileSync(join(root, 'spec', 'v1', 'deprecations.json'), 'utf8')).entries ?? []; } catch { return null; }
  const triggersOf = (e) => (Array.isArray(e.removalTrigger) ? e.removalTrigger : e.removalTrigger ? [e.removalTrigger] : []);
  const rows = entries.filter((e) => triggersOf(e).includes('v2-minor') && e.retirement?.manifest);
  let commit = null, commitDate = null;
  try {
    commit = execFileSync('git', ['-C', dir, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
    commitDate = execFileSync('git', ['-C', dir, 'log', '-1', '--format=%cI'], { encoding: 'utf8' }).trim().slice(0, 10);
  } catch { /* a checkout without git history; the date stays null and R3 fails closed */ }
  const at = (doc, path) => { let n = doc; for (const seg of String(path).split('.')) { if (n === null || typeof n !== 'object' || Array.isArray(n)) return undefined; n = n[seg]; } return n; };
  const carries = (found, value) => found !== undefined && (value === undefined ? true : Array.isArray(found) ? found.some((x) => JSON.stringify(x) === JSON.stringify(value)) : JSON.stringify(found) === JSON.stringify(value));
  const out = {};
  for (const e of rows) {
    const spec = e.retirement.manifest;
    const kinds = new Set(spec.kinds ?? []);
    let count = 0, scanned = 0;
    for (const tree of ['v1', 'v2'].map((t) => join(dir, 'registry', t, 'packs')).filter((p) => existsSync(p))) {
      for (const name of readdirSyncFs(tree)) {
        const vdir = join(tree, name, '-');
        if (!existsSync(vdir)) continue;
        for (const f of readdirSyncFs(vdir)) {
          if (!f.endsWith('.json') || f.includes('sbom')) continue;
          let m; try { m = JSON.parse(readFileSync(join(vdir, f), 'utf8')); } catch { continue; }
          if (kinds.size > 0 && !kinds.has(m.kind)) continue;
          scanned++;
          if (carries(at(m, spec.path), spec.value)) count++;
        }
      }
    }
    out[e.id] = { count, manifestsScanned: scanned, registryCommit: commit, registryCommitDate: commitDate };
  }
  return out;
}
...
if (mode === 'write') {
  ...
  const manifest = { $comment: 'GENERATED by scripts/generate-cross-repo-evidence.mjs from the sibling checkouts — the committed record of what every <repo>:<path> evidence pointer in SECURITY/invariants.yaml and conformance/coverage.md resolved to, plus the registry\'s distinct peerDependencies keys (RFC 0177 §B.2 inventory read by check-declaration.mjs), plus the RFC 0197 R3 registryManifestCensus for every v2-minor deprecation row that declares a retirement.manifest. Regenerate after a sibling repo moves or renames a cited file.', generated: new Date().toISOString().slice(0, 10), repos, registryPeerDependencyKeys: registryDir ? registryPeerDependencyKeys(registryDir) : null, registryManifestCensus: registryManifestCensus(registryDir, ROOT), siblingVersions };
