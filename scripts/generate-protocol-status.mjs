...
function parseRfcs() {
  ...
  return listFiles('RFCS', (rel) => /^RFCS\/\d{4}-.+\.md$/.test(rel) && !rel.includes('0000-template') && !/\.(gaps|risks)\.md$/.test(rel))
    .map((rel) => {
      ...
      // RFCS/README.md §"Parked RFCs": an RFC waiting on an EXTERNAL condition
      // rather than on unfinished work here. Derived from the Status cell rather
      // than hand-maintained in the tally, so the two cannot drift.
      const parked = /\(\*\*Parked\*\*\)/.test(text.match(/\|\s*\*\*Status\*\*\s*\|[^\n]*/)?.[0] ?? '');
      return { id, title, status, rel, parked };
    })
...
function renderRfcsReadmeIndex(text, rfcs) {
  ...
  const enumerate = (label) => {
    ...
    const sel = rfcs.filter((r) => r.status === label).sort((a, b) => a.id.localeCompare(b.id));
