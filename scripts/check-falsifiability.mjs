...
// The closed vocabulary is the CLASS; every class may carry a trailing reason,
// as `unwitnessable` always could. A `seam-gated` row that names the seam it is
// gated on is more useful than a bare token, and refusing the reason pushed
// authors back toward the bare one.
...
for (const f of readdirSync(dir).filter((f) => /^\d{4}-.*\.md$/.test(f) && !f.startsWith('0000'))) {
  ...
  // Strict for every v2-era RFC, not only the RFC 0167 family. The two gates
  // disagreed on scope: `check-accepted-predicate` enforces rule 4 against any
  // RFC >= 0167, while this one validated verdicts only for 0167's children —
  // so RFC 0179 and 0187 had their tables enforced by one gate and unchecked by
  // the other. RFC 0187's was three columns with verdicts outside the closed
  // vocabulary, and nothing said so.
  const isFamily = rfc === '0167' || /Part of:\s*RFC 0167/.test(t) || Number(rfc) >= 167;
  ...
  for (const r of rows) {
    ...
    for (const m of r.matchAll(/`([a-z0-9-]+\.test\.ts)`/g)) if (isFamily && !existsSync(join(ROOT, 'conformance', 'src', 'scenarios', m[1])) && !existsSync(join(ROOT, 'conformance', 'src', 'coherence', m[1])) && !/planned|Phase 3|suite 2\.0\.0/i.test(r)) failures.push(`RFCS/${f}: names ${m[1]}, which does not exist and is not marked planned`);
  }
}
