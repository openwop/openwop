...
### feat(rfc-0071): artifact-type packs + AI chat card packs — Draft + Phase-1 spec/schema (2026-05-26)

Files [RFC 0071](RFCS/0071-artifact-type-and-chat-card-packs.md) (`Draft`): two new pack kinds that make the rich artifacts workflow nodes produce — and the prompt-driven chat cards that generate them — distributable, signed, versioned, and capability-negotiable, rather than the host-private free-string registries that MyndHyve and the demo app each reinvented. Generalizes MyndHyve's downstream "Canvas Type" model (artifact types + card templates) into the wire contract while deliberately leaving viewers, field widgets, and export byte-production host-side per `positioning.md`. Lands the **Phase 1 (artifact-type packs)** spec + wire-schema foundation: new `spec/v1/artifact-type-packs.md` (DRAFT); new `schemas/artifact-type-pack-manifest.schema.json` (`kind: "artifact-type"`, peer to the node/workflow-chain/prompt manifest schemas — `node-pack-manifest.schema.json` unchanged); `node-packs.md` §"Manifest format" extended to name the prompt + artifact-type kinds and **the previously-undocumented `PackNode.artifact` field now has normative prose**; `run-event-payloads.schema.json` `artifact.created` gains an optional `registered` boolean (absent ⇒ true, preserving prior semantics). Additive (`COMPATIBILITY.md` §2.1): new optional pack kinds + new optional event field; no existing schema or `MUST` changed. Phase 2 (chat card packs, `kind: "card"`), the `host.artifactTypes` capability advertisement, conformance scenarios, and the reference-host store-side implementation are tracked in the RFC and its `RFCS/registers/0071-*` gap/risk registers. Two High security risks (R1 schema-bomb via distributed Ajv-compiled schemas; R2 prompt injection via card templates) gate promotion to `Active`. See `docs/OPENWOP-CANVAS-TYPE-PACKS-RESEARCH.md` for the prior-art analysis.
