...
### docs: RFC 0058 wall-clock arm now non-steward-adopted (MyndHyve) — curl-verified; stays `Active` (2026-05-26)

After honestly retracting its earlier `maxNodeExecutions` conflation, MyndHyve `workflow-runtime` shipped the real RFC 0058 wall-clock arm. **openwop-side curl-verified** against `https://workflow-runtime-gjw5bcse7a-uc.a.run.app/.well-known/openwop`: `limits.maxRunDurationMs: 600000` now live (`maxLoopIterations: null` honestly absent; `maxNodeExecutions: 100` pre-existing, untouched). MyndHyve attests run-create clamping (`400 validation_error` on over-range) + canonical `cap.breached { kind: 'run-duration' }` + `run_timeout` + RFC 0053 dead-letter on breach, persisted-`startedAt`-anchored elapsed, and a split drift-gate (presence-asserts `maxRunDurationMs`, absence-asserts `maxLoopIterations`); commits `ded1ed43a`/`5f03d5e18`/`8471cbb74`/`4357512b7`. **Disposition: RFC 0058 stays `Active`.** The wall-clock arm (`runTimeoutMs`) has met its bar (reference host + verified non-steward), but the full RFC graduates `Active → Accepted` only when the `maxLoopIterations` arm is enforceable too (rides RFC 0061). Per the RFC 0029 Tier-1/Tier-2 precedent, per-arm progress is recognized in the RFC 0058 Status note + `INTEROP-MATRIX.md` §"round 3" without a misleading binary-list flip (a binary list can't show "0058 Accepted" while `maxLoopIterations` is enforceable nowhere). Docs-only; no schema or wire-shape change.
...
### fix(app): /code-review follow-ups on RFC 0040 Phase 3 (cross-host causation) (2026-05-26)

Two findings from a `/code-review` pass over the RFC 0040 Phase 3 workflow-engine commit. App-only; no schema, wire-shape, or behavior change.

- **`runs.ts`** — documents the `GET /v1/runs/{runId}/ancestry` `cause` limitation. The endpoint reports the nominal same-host value `core.subWorkflow` because `RunRecord` (`src/executor/types.ts`) tracks only `parentRunId` and does not retain which composition primitive created the child — both the `core.subWorkflow` and `core.dispatch` dispatchers set `parentRunId` identically, and the schema enum admits either. Distinguishing them on the wire requires persisting the composition mechanism on `RunRecord` (deferred). Both values denote same-host composition, so the nominal value is accurate at the cross-host boundary this endpoint serves.
- **`discovery.ts` + `runs.ts`** — de-duplicates the RFC 0040 Phase 3 env gate behind a new exported `isPhase3Enabled()` helper (single source of truth: `PHASE_3 === 'true' || PHASE_4 === 'true'`, the additive ladder), used by both the discovery advertisement and the ancestry-endpoint gate so the two cannot drift.
