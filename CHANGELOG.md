...
### feat(host-sqlite): RFC 0031 model-capability refusal — `conformance.modelCapability.insufficient` (2026-05-25)

SQLite advertised the `conformance-model-capability-insufficient` fixture (its fixture filter is prefix-only) but had no handler for the `conformance.modelCapability.insufficient` typeId → it failed `unsupported_node_type` instead of the RFC 0031 refusal `model-capability-insufficient.test.ts` expects. Adds the RFC 0031 §B step 4 / §D refusal case: emit `model.capability.insufficient` (schema-valid payload — nodeId/provider/model/missingCapabilities/fallbackAttempted) BEFORE `node.failed`, then fail with `RunSnapshot.error.code = "capability_not_provided"`, emitting no downstream envelope event (the node never dispatches to a model). **First reference host to demonstrate the RFC 0031 refusal end-to-end** — the Postgres host soft-skips by not advertising the fixture (its `SUPPORTED_NODE_TYPES` filter excludes the typeId). Closes SQLite's last conformance failure (now 0 failed / 1641 passed). Host-only; no protocol-corpus change.
