...
### spec(rfc-0029): promote Active → Accepted on MyndHyve `agentBindings: true` (2026-05-26)

MyndHyve workflow-runtime advertises `capabilities.prompts.agentBindings: true` on revision `workflow-runtime-00217-q7c` (commits `21342a42` + `5f2cfd2a` + `4d7727ba`, re-confirmed in the 2026-05-26 round-3 closure). Curl-verified — `https://workflow-runtime-gjw5bcse7a-uc.a.run.app/.well-known/openwop`. The non-steward host advertisement was the long-pending path-to-`Accepted` criterion (`agent.promptResolved` engine-side emission was already shipping via RFC 0027's prompt-compose seam, `Accepted` 2026-05-23). RFC 0029 `Active → Accepted`; counts shift **Accepted 50 → 51, Active 9 → 8**. README banner + `docs/KNOWN-LIMITS.md` + `docs/PROTOCOL-STATUS.md` synced.
