...
`openwop-auth-saml` (authentication) and `openwop-auth-scim` (provisioning) are independent lanes: SCIM `active:false`/`DELETE` deactivates _its_ provisioned principal, and SAML validates an assertion onto _its_ principal, but nothing ties the two to the same human. On a host advertising **both** profiles this leaves a leaver gap — a person deactivated over SCIM can still authenticate over SAML, because the two lanes map to structurally different RFC 0048 principals. This subsection closes that gap as an **opt-in** obligation, discoverable via `capabilities.auth.subjectLinking`.

The obligations below apply to a host that advertises `openwop-auth-saml` **and** `openwop-auth-scim`. **RFC 0164:** they apply whether or not `capabilities.auth.subjectLinking` is present — the flag is derived from the profile pair, not a gate (see §"Mandatory when both profiles are advertised — RFC 0164" below). Before RFC 0164 they applied only to a host that additionally set `subjectLinking: true`.
...
- If no opaque IdP-stable link key is available for a pair of identities (the IdP asserts no persistent `NameID` / no `externalId`), the host MUST NOT claim the combined leaver guarantee for those identities: it MUST fail closed on the SAML lane for the unlinkable subject (RFC 0164 §A.2 removed the earlier alternative of treating them as independent subjects and omitting `subjectLinking: true`; a host that cannot honour the contract for its deployment as a whole MUST NOT advertise both profiles). A host MUST NOT silently fall back to a mutable/PII key, and MUST NOT advertise `subjectLinking: true` while any admitted identity pair is joined on a non-conforming key.

- **Legacy and anonymous subjects are never linked (RFC 0165 §B.3, §B.6).** A subject whose `issuer` is `urn:openwop:legacy` (synthesized for a run that predates `owner.subject` emission — `auth.md` §"The Subject record") MUST NOT participate in a link, and a SCIM deactivation MUST NOT be taken to deny it. A peer-forwarded, never-authenticated end user on an A2A entry is recorded as `kind: "anonymous"` with the forwarding peer's subject as `actor` and MUST NOT be linked; this narrows RFC 0132 §A.5, which remains in force for an anonymous actor recorded without an `actor`.
...
**Discovery shape:** `capabilities.auth.subjectLinking: true` — since RFC 0164 a **derived** advertisement: a host that advertises both `openwop-auth-saml` and `openwop-auth-scim` in `capabilities.auth.profiles[]` MUST set it `true` (and MUST advertise `subjectLinkKey`); a document that advertises both profiles with the flag absent or `false` fails schema validation and is non-conforming. A host MUST NOT set it `true` unless both profiles are advertised and the obligations above hold. The flag is **deprecated toward v2**: hosts MUST keep emitting it through v1.x, clients SHOULD derive the guarantee from the profile pair and MUST tolerate the flag.
...
#### Mandatory when both profiles are advertised — RFC 0164

**Why this exists.** RFC 0159 landed the leaver contract opt-in and RFC 0163 hardened it, but both left a host free to advertise both profiles and omit `subjectLinking`, shipping the combined-deployment leaver bypass under a conforming banner while the suite recorded `inapplicable`. No production host advertised both profiles when RFC 0164 was filed, so making the obligation follow the profile pair bound nobody and cost nothing; deferring it would have let the first combined host land opted out.

- A host that advertises **both** `openwop-auth-saml` and `openwop-auth-scim` MUST honour every requirement in this subsection (RFC 0159 §A.1–§A.3 and RFC 0163 §A/§B) whether or not `capabilities.auth.subjectLinking` is present.
- For any subject it cannot link (no persistent `NameID` / no `externalId`, no shared IdP trust root, or lanes serving different tenant realms) the host MUST fail closed on the SAML lane for that subject. A host that cannot honour the contract for its deployment as a whole MUST NOT advertise both profiles: advertising fewer profiles than the code implements is conforming; advertising both without the contract is not.
- A host that advertises both profiles MUST advertise `subjectLinking: true` and `subjectLinkKey` (the RFC 0163 closed enum). `capabilities.schema.json` expresses this as a conditional on `profiles[]`; the flag is a derived statement of fact and is deprecated toward v2 (kept through v1.x per `COMPATIBILITY.md` §1).

**Conformance gaps to close:** `auth-subject-link.test.ts` (RFC 0159), gated on both profiles being advertised (RFC 0164; formerly on `capabilities.auth.subjectLinking`), verifies (positive) that a SCIM-provisioned user linked by `externalId` to a SAML persistent `NameID`, once SCIM-deactivated, yields a **denied** SAML decision; and (negative) that a link configured on a mutable key (email) never produces a cross-lane pass. `auth-subject-link-key-class.test.ts` (RFC 0163) verifies the `subjectLinkKey` advertisement unaided and, over a two-trust-root seam (`OPENWOP_TEST_SAML_IDP_URL` + `OPENWOP_TEST_SAML_IDP_URL_B` + `OPENWOP_TEST_SCIM_URL`), that a same-IdP pair links while a cross-IdP identifier collision does not. Both reuse the bundled synthetic SAML IdP (`conformance/src/lib/saml-idp.ts`, whose two-trust-root behaviour is proven server-free in `saml-idp.test.ts`) + a synthetic SCIM payload; the live paths are opt-in via the seams named above. Registered invariants: `subject-link-leaver-deny`, `subject-link-key-class-declared`, `subject-link-same-trust-root`.
