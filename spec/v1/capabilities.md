...
| `replay.sideEffectSuppression`         | `string`                  | **optional v1** | RFC 0140. Declares HOW the host discharges `replay.md` §"Determinism guarantees" caveat 1 on a `mode:"replay"` fork. `recorded-outcome` = side-effecting nodes resolve the source run's recorded outcome or fail closed with `replay_source_missing`, with a default-deny effect-seam guard behind it. `none` (default) = no declared mechanism — **not** permission to re-fire; caveat 1 binds unconditionally. |
