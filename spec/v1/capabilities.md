...
| `replay.supported`                     | `boolean`                 | **optional v1** | RFC 0011. Host implements `POST /v1/runs/{runId}:fork` per `replay.md`. With a non-empty `replay.modes`, this is the `openwop-replay-fork` profile predicate (`profiles.md`).                                                                                                          |
| `replay.modes`                         | `string[]`                | **optional v1** | RFC 0011. Fork modes honored — subset of `["replay","branch"]`. Authoritative; clients MUST NOT infer mode support from the legacy `replay.fork` boolean.                                                                                                                              |
| `replay.sideEffectSuppression`         | `string`                  | **optional v1** | RFC 0140. Declares HOW the host discharges `replay.md` §"Determinism guarantees" caveat 1 on a `mode:"replay"` fork. `recorded-outcome` = side-effecting nodes resolve the source run's recorded outcome or fail closed with `replay_source_missing`, with a default-deny effect-seam guard behind the classifier (§"Side-effect suppression in replay" requirement 5). `none` (default, absent ⇒ this) = no declared mechanism — **not** permission to re-fire; caveat 1 binds unconditionally. |
...
- `selfHosted` (string array, subset of `supported`) — RFC 0108. The subset of `supported` whose entries are operator- or tenant-configured OpenAI-compatible endpoints rather than host-managed connections to known public vendors. See [`aiProviders.selfHosted`](#aiprovidersselfhosted--self-hostedopenai-compatible-provider-class-rfc-0108) below for its three normative rules.
...
**`deferredParameters`** (OPTIONAL sub-object; RFC 0124 / WCP4, `Active`). When `supported: true`, the host offers a capability-gated **deferred-parameter expansion mode** in addition to expansion-time substitution (which stays the default and floor): at drop time it materializes the chain's `parameters` into top-level workflow `variables[]` (author value as `defaultValue`) and rewrites `{{params.<name>}}` into a spec'd runtime binding (PromptTemplate `{{varName}}` with `source:"variable"`, or a variable-sourced PortValue), keeping chain parameters overridable per run via `configurable` while the persisted definition holds ZERO `{{params.*}}` tokens. A parameter marked `x-openwop-sensitive` MUST materialize as a **`source:"secret"`** PromptVariable (BYOK-resolved via `capabilities.secrets`, `[REDACTED]`, never bagged), is deferrable ONLY in a prompt-body position, and **fails closed** (`sensitive_param_not_deferrable`, 422) in any other position or on a host lacking `secrets`/deferred support — never frozen into persisted `config` and never materialized as a plaintext `source:"variable"` binding (both are secret-at-rest / run-bag leaks, SR-1); its per-run supply is a `credentialRef`, not plaintext. Offering deferred mode SHOULD require `prompts.supported: true` with `variable` in `prompts.variableSources`; the sensitive path additionally requires `capabilities.secrets.supported: true`. **RFC 0124 is `Accepted` (2026-07-04) — a host implementing deferred mode MAY now advertise `deferredParameters.supported: true`** (truthful-advertisement: only if it honors the §Security handling above). See [`workflow-chain-packs.md`](./workflow-chain-packs.md) §"Deferred-parameter expansion".

**`hostExpansionSeam`** (OPTIONAL boolean; RFC 0013 erratum, 2026-07-05). A **conformance-only test-seam** advertisement (category: test harness, cf. `observability.testSeams` — NOT a product capability). When `true`, the host serves `POST /v1/host/sample/workflow-chain:expand` returning the `vendor.openwop.workflow-chain-sample` v1.0.0 expansion that `workflow-chain-host-expansion.test.ts` asserts against; absent/`false` ⇒ that live-host scenario soft-skips and the semantic `supported` claim rides the server-free `workflow-chain-expansion.test.ts` legs. **It is distinct from `deferredParameters`** — the RFC 0124 deferred path is witnessed through its own `POST /v1/host/sample/chain/deferred-expand` seam — so advertising `supported` / `deferredParameters.supported` does **not** conscript a host into the RFC 0013 sample-pack seam (whose fixture is not yet bundled into `@openwop/openwop-conformance`; see the RFC 0013 erratum note). A host only advertises `hostExpansionSeam: true` once it actually serves the sample-pack expand seam.
...
Advertising without honoring is a dishonest capability claim;
`OPENWOP_REQUIRE_BEHAVIOR=true` MUST fail it.
...
- `perItemInput` (RFC 0126, OPTIONAL boolean) — when `true`, the host honors `NextWorkerDecision.nextWorkerInputs` (`orchestrator-decision.schema.json`): an index-aligned per-child input array that lets a supervisor fan ONE `childWorkflowId` over N runtime items with distinct per-item inputs (the data-parallel map-over-collection pattern), projected over the RFC 0022 mapping (per-item value wins on key collision). A host advertising `false`/absent, on receiving a non-empty `nextWorkerInputs`, MUST fail closed with `validation_error` and MUST NOT silently drop-and-dispatch N identical children (the correctness/spend hazard RFC 0126 closes). Like the other `dispatch.*` descriptors, `perItemInput` is a prose-documented, schema-permissive capability field (`capabilities.dispatch` is not enumerated in `capabilities.schema.json`; hosts advertise it under the permissive top-level). See `node-packs.md` §core.dispatch.
