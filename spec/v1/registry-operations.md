...
The hosted reference registry is live at [`packs.openwop.dev`](https://packs.openwop.dev) — Stage 1–4 operational maturity deployed 2026-05-12 (WIF auto-deploy, CycloneDX SBOMs, registry CVE feed + OSV scanning, Cloud Monitoring uptime check). As of 2026-07-04 it hosts 94 packs across four trust tiers (`core.openwop.*`, `community.openwop-team.*`, `vendor.openwop.*`, `vendor.myndhyve.*`). See [`docs/PACK-CATALOG.md`](../../docs/PACK-CATALOG.md) for the categorized inventory and [`ROADMAP.md`](../../ROADMAP.md) §"Hosted infrastructure" for the deployment status row.
