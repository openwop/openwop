...
When richer identity is needed, the *adjacent* shape (DID Document, `AgentCard`, gateway record) carries it. `AgentRef` points at one of those when present; it doesn't try to be one.
