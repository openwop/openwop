...
| `exportFormats`      | MAY    | List of export-format **identifiers** (hints) a renderer MAY offer. An entry is a spec-reserved alias (§"Export-format aliases": `pdf`, `pptx`, `docx`, `xlsx`, `md`, `html`, `txt`, `csv`, `json`, `png`, `svg`, `jpeg`, `step`, `stl`, `dxf`), a `vendor.<org>.<format>`/`x-<format>` identifier (the same reserved-core + extension idiom as `requiredModelCapabilities` in `node-packs.md`), or a **media type** (RFC 6838 `type/subtype`, lowercase, no parameters; RFC 0205). Each reserved alias names exactly the media type in §"Export-format aliases"; a consumer **MUST** treat an alias and its media type as the same format, and a manifest **SHOULD NOT** list both. A media type outside that table **SHOULD** be IANA-registered or use an RFC 6838 §3 tree. A pack meant to install on hosts that predate RFC 0205 **SHOULD** keep to the aliases. This spec assigns no byte-level _production_ semantics to any identifier. |
...
Normative (RFC 0205 §C.10). Each reserved alias names exactly one media type (IANA media-types registry, verified 2026-09-22). A consumer MUST treat the two spellings as one format, both when matching a host's `export` advertisement against a type's declared `exportFormats` and when calling `host.artifactTypes.export`.
...
**Legacy identifiers were never wire-conformant.** The `artifactTypeId` pattern in §"The `ArtifactType` declaration" is definitional, not aspirational, and this document grants **no grandfather clause**: a value outside the pattern has never been a conformant published identifier, however long a host has shipped it. Such values are simply _unregistered_ under the tier model above — a legitimate, permanent, first-class status. A host is under **no obligation to migrate** them.
...
**An alias map is a host-internal compatibility shim, not a conformance claim.** A host serving legacy identifiers through an alias is not thereby "conformant under its old names" — the legacy spellings remain non-conformant on the wire; the _canonical_ spellings the alias resolves are what a conformant peer may rely on. A host MUST NOT advertise or imply otherwise.
...
| `export`    | List of export-format identifiers the host can materialize, drawn from declared `exportFormats`. Advisory. Either spelling of a format (RFC 0205 §C.11). |

**What `store: true` claims (scope of the emission MUST).** The claim is **universally quantified over the artifacts of that type, not over the host's production paths**: **every** path by which the host persists an artifact carrying that registered `artifactTypeId` MUST emit `artifact.created`. A host on which _some_ path persists such an artifact silently MUST NOT advertise `store: true` for that type — an advert whose truth depends on which internal path a run happened to take is not a wire contract, because the consumer it exists for (a peer host forwarding an artifact, a UI awaiting one) reads silence and cannot distinguish _persisted-without-event_ from _never-produced_. **The obligation is scoped to registered types**: a path that persists only unregistered artifacts (§"Binding" — a permanent, first-class tier) is outside this facet and does not falsify the advert. Where a host's paths genuinely differ, the honest instrument is the **per-type** `types` map below, not a host-global compromise; a single type produced through both an emitting and a non-emitting path cannot be advertised `store: true` at all until one of those two facts changes.
...
| `exportFormats`    | **Reserved core identifier set + `vendor.*`/`x-` extension.**                                                                                                   | Mirrors the `requiredModelCapabilities` reserved-core + extension idiom; gives interop without a codec spec; media types accepted beside the aliases since RFC 0205. |
