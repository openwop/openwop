...
The `test` op is **excluded** from the patch op set (a fire-and-forget transport frame cannot act on a failed conditional); `move`/`copy` are permitted but OPTIONAL to support. _(Conformance: `a2ui-surface-delta-transport.test.ts`, gated on `a2uiSurface.deltaTransport`.)_
