...
| `core.conformance.a2a-invoke` | **Conformance-only** (2026-08-16). Node of the `conformance-a2a-task-roundtrip` fixture: a host that consumes A2A peers (advertises `a2a.supported` and acts as an A2A client) MUST map it to its A2A bridge node — a **host-extension** node (`a2a.invoke` or similar per `a2a-integration.md` §"State projection"), which is why the fixture cannot name a `core.*` node the corpus specifies. Renamed from `core.a2a.invoke`, a multi-segment `core.*` id that violated the naming convention below and that no host shipped. Hosts that do not consume A2A MUST NOT advertise the fixture; the leg then records `inapplicable`. |
| `core.conformance.mcp-invoke` | **Conformance-only** (2026-08-17). Node of the `conformance-mcp-tool-roundtrip` fixture: a host that consumes MCP (`mcp.supported`, acts as an MCP client) MUST map it to its own MCP invoke bridge node — a **host-extension** node, which is why the fixture cannot name a `core.*` node the corpus specifies — and honour `config.mcp { tool: string, arguments: object }`. Renamed from `core.ai.callPrompt` + `config.mcp`, a VENDOR pack-tier id (the portable-node tier, not the Core Node Set) that no host is obliged to ship and whose real implementation reads `promptId`, not `config.mcp`; the `config.mcp` shape appeared nowhere in `mcp-integration.md` (§Concrete example uses the host-extension `mcpServers`). Hosts that do not consume MCP MUST NOT advertise the fixture; the leg then records `inapplicable`. Same rule and lifecycle as `core.conformance.a2a-invoke`. |
...
A host that does not advertise `capabilities.dispatch.fanOutSupported: true` MUST surface a `validation_error` at `POST /v1/workflows` when a `core.dispatch` node pins `fanOutPolicy: 'parallel'` (mirrors the `workerDispatchModel` / `askUserRouting` registration-time validation). A `joinPolicy` present while `fanOutPolicy !== 'parallel'`, a `joinPolicy.mode` not in `capabilities.dispatch.joinModes`, a `joinPolicy.onChildFailure` not in `capabilities.dispatch.onChildFailureModes` (absent ⇒ `['collect']`; the two join axes are gated independently — `joinModes` for `mode`, `onChildFailureModes` for `onChildFailure`), and a `mode: 'quorum'` missing a valid `quorum` (`1 ≤ quorum ≤ nextWorkerIds.length`) are each registration `validation_error`s.

**Concurrency.** The host dispatches at most `min(maxConcurrency ?? ∞, capabilities.dispatch.maxFanOut ?? ∞)` children at once, in waves — starting a queued child as each in-flight child terminates. The host MUST NOT silently drop children above the ceiling.

**Join (`joinPolicy.mode`).** `'wait-all'` (default) suspends the node until every dispatched child reaches a terminal state. `'quorum'` is satisfied at `quorum` `completed` children; `'first'` at the first `completed`; `'race'` at the first terminal of any kind. When a mode is satisfied with children still in flight, the host MUST cancel them per the [`interrupt-profiles.md`](./interrupt-profiles.md) parent-child cascade.

**Error aggregation (`joinPolicy.onChildFailure`).** `'collect'` (default) lets no individual failure short-circuit the join — the node waits per `mode`, then reports per-child outcomes (downstream edges decide). `'fail-fast'` fails the node on the first `failed`/`cancelled` child and cancels the rest. `'absorb'` records failures but never fails the parent. The `'collect'` default deliberately DIVERGES from the single-child `core.subWorkflow.onChildFailure: 'fail-parent'` default because independent fan-out siblings are the common case (RFC 0118 §Resolved Q1); authors who want fail-parent behavior set `onChildFailure: 'fail-fast'` explicitly.

**Output shape.** Under `fanOutPolicy: 'parallel'` the dispatch node's output replaces the single-child `{ childRunId, childStatus }` with a `children` array plus an aggregate `joinOutcome` (`'satisfied'` / `'failed'` / `'partial'`); each entry is `{ workflowId, childRunId, childStatus, error? }`. RFC 0022 `outputMapping` / `perWorkerOutputMappings` apply per `completed` child; when two children map to the same parent variable, the **last terminal in the parent host's observed wall-clock order** wins, and that order is recorded as `mergeOrder` on the `core.dispatch.join` event so replay is deterministic. Authors SHOULD avoid colliding output keys across parallel children.

**Per-child events + replay.** The parallel fan-out emits the same per-child `node.dispatched` event sequential dispatch emits (RFC 0007 §D5); concurrency makes their relative order non-deterministic, so consumers MUST NOT assume array order. The host additionally emits a `core.dispatch.fanOut` event when the wave begins and a `core.dispatch.join` event when the join is satisfied ([`run-event-payloads.schema.json`](../../schemas/run-event-payloads.schema.json) `$defs.dispatchFanOut` / `$defs.dispatchJoin`; both carry the dispatching node's `nodeId` and set `causationId` to the consumed `runOrchestrator.decided` event). Parallel dispatch is non-deterministic at first execution but MUST be deterministic on **replay**: the host re-folds the recorded `node.dispatched` / `fanOut` / per-child terminals / `join` (including `mergeOrder`) without re-invoking children, and re-applies `outputMapping` in `mergeOrder` — never in `nextWorkerIds` order — so a forked/replayed run reproduces the original parent-variable state (RFC 0041). Each child counts as one dispatch-iteration against `iterationCap` (RFC 0118 §Resolved Q5).
...
- When present, the host MUST project `nextWorkerInputs[i]` into the inputs of the child dispatched for `nextWorkerIds[i]`, **over** the RFC 0022 `inputMapping` / `perWorkerInputMappings` projection — a key present in both resolves to the `nextWorkerInputs[i]` value (per-item literal data is the more specific source and wins).
- `nextWorkerInputs.length` MUST equal `nextWorkerIds.length`. A `next-worker` decision whose lengths differ MUST fail the dispatch node with a `validation_error` and MUST NOT dispatch any child from that decision. (Array-length equality is not JSON-Schema-expressible; this is a runtime MUST.)
- **Capability gate — fail closed.** A host advertising `capabilities.dispatch.perItemInput: true` MUST honor `nextWorkerInputs`. A host **not** advertising it, on consuming a `next-worker` decision carrying a non-empty `nextWorkerInputs`, MUST fail the dispatch node with a `validation_error` and MUST NOT silently drop `nextWorkerInputs` and dispatch N identically-inputted children — silent-drop is the exact correctness/spend hazard this closes, so the gate fails **closed**. (Old hosts reject the field automatically: `NextWorkerDecision` is `additionalProperties: false`, so a strict validator refuses the unknown property.)
- **Homogeneous cardinality is unchanged.** Repeated ids in `nextWorkerIds` already express "one workflow, N children" (RFC 0118; `quorum ≤ nextWorkerIds.length` counts duplicates). This RFC adds no fan-out _mode_ — `fanOutPolicy` / `joinPolicy` / `maxConcurrency` / `iterationCap` / `DispatchConfig` are untouched. Each dispatched child is one dispatch-iteration against `maxFanOut` / `maxConcurrency` / `iterationCap`, exactly as RFC 0118.
- **Replay-deterministic.** `nextWorkerInputs` rides the recorded `runOrchestrator.decided` event's `decision` field; on `:fork`/replay the host MUST re-read the recorded per-item inputs and MUST NOT recompute them (they are frozen at decision time), so a forked/replayed run reproduces byte-identical children — mirroring the RFC 0118 `mergeOrder` replay clause. CP-2 is preserved: per-item input is a scheduling input to child construction, not a run-DAG mutation.
- **Secrets.** A per-item value MUST NOT carry a secret-class value in plaintext here; secret material uses the connection-pack / `credentialRef` channel (RFC 0095), not a per-item input literal.

**Compatibility.** Additive — `nextWorkerInputs` is a new OPTIONAL property on `NextWorkerDecision` (existing decisions omit it, byte-identical); `capabilities.dispatch.perItemInput` is a new OPTIONAL descriptor (absent ⇒ `false`, fail-closed). No event-shape, endpoint, error-code, or MUST change; the decision rides the existing `runOrchestrator.decided` event (no OpenAPI/AsyncAPI diff).
...
**Agent-manifest `role` + the Skill profile (RFC 0131).** An `AgentManifest` (RFC 0003) MAY declare an optional `role` (`"skill" | "assistant"`) that distinguishes a **composable, task-scoped Skill** — a capability invoked via `handoff` (task→return) and composed by an assistant/roster agent — from a **top-level assistant** a client converses with. `role` is **EXPLICIT, never inferred**: `handoff` presence does NOT imply `skill` (an assistant MAY ship a typed entry contract for cross-host interop), and an **absent** `role` leaves the manifest **unconstrained** — exactly today's meaning, byte- and behavior-identical, no profile binds it. A manifest that opts in with `role: "skill"` binds the **Skill profile**, encoded in `agent-manifest.schema.json` as an `if role==="skill"` conditional: it MUST declare `handoff` and MUST constrain `memoryShape` to scratchpad-only (`conversation`/`longTerm` MUST be `false`/absent — persistent + multi-turn memory belong to the composing assistant/roster agent, RFC 0039; a stateful worker undermines replay determinism, RFC 0041). A violating skill manifest **fails schema validation at publish/install** (`pack_validation_failed`) — a malformed manifest (RFC 0003 §C author error), **NOT** an RFC 0072 §C `degraded[]` runtime tier. See [`agent-memory.md`](./agent-memory.md) §D for the memory constraint and the **reject-vs-degrade** contrast against §C. `role` is advisory about kind only — it never widens `toolAllowlist`, `requiresCapabilities`, or any scope. SECURITY invariant: `agent-skill-profile-stateless`.
...
### Vendor extensions on pack manifests (normative — RFC 0138)

A pack manifest MAY carry **host-extension properties** whose names match `^(x-|vendor\.)`, on the manifest root and on each kind's per-item entry object. Their semantics are unspecified by this protocol.
...
- Every other property name remains closed. `additionalProperties: false` still applies, so a misspelled canonical field (`dispalyName`) is still rejected — the hatch admits _declared_ extensions, not arbitrary keys.
...
**Why this exists.** [`host-extensions.md`](./host-extensions.md) §"Vendor-prefixed namespaces" already requires that _"a client receiving an unknown vendor-prefixed field MUST treat it as opaque."_ Before RFC 0138 every pack manifest set `additionalProperties: false` with no pattern escape, so such a field could not legally exist — the corpus mandated a behavior for a case it structurally forbade. A host with a working extension had to choose between shipping it and publishing a conformant pack.

**Trust boundary (normative).** An extension property's value is **pack-authored content** and is therefore **untrusted**, exactly as `form-content-packs.md` §"Trust boundary" treats pack-authored strings. A signature proves authorship, not content safety.

> **"Ignore" means ignore.** A consumer that does not recognize an extension property MUST NOT render it, execute it, interpret it as markup or a templating directive, use it to select a code path, or persist it into a surface where it will later be interpreted. Retaining the raw bytes for round-trip fidelity is permitted; acting on them is not. Enforced as the `pack-manifest-extension-opaque` invariant.

**Differential opacity (normative — RFC 0139).** Where a host exposes a host-sample install seam for a declarative pack kind, that seam MUST return an **observable registration projection** describing what the host registered. Given two manifests identical except that one additionally carries properties matching `^(x-|vendor\.)` that the host does not recognize, the host **MUST** accept both or reject both for the same reason, and the two projections **MUST** be equal once the extension properties themselves are removed from the comparison. Acceptance and registration behavior **MUST NOT** be a function of an unrecognized extension. This is how "ignore" becomes checkable: a suite cannot enumerate a host's rendering and interpretation sinks, but it can require that behavior not vary with the extension at all — which covers every install-time sink at once. Verified by `pack-manifest-extension-opacity.test.ts`. Note the bound: it witnesses install-time sinks, not an extension interpreted at some later moment (RFC 0139 gap G3).

Extension entries carried on **separate** schemas — `agent-manifest.schema.json` (a node pack's `agents[]`) and `prompt-template.schema.json` (a prompt pack's `prompts[]`) — are out of scope for RFC 0138: they are their own contracts with their own compatibility surface, not pack-manifest structure.
...
<!-- normative-example: node-pack-manifest.schema.json -->
```json
{
...
- `400 tarball_too_large` — decompressed bytes exceed the registry's cap (recommended default: 50 MB). The cap is on the _inflated_ size, so a registry MUST evaluate it on the gunzip output (streaming or after inflate), not on the wire body — the conformance probe (`pack-registry-publish.test.ts`, S45) is a ~64 KB gzip that inflates to 64 MiB, which a proxy body limit never sees; a registry whose cap is above 64 MiB (or absent) reaches the tar layer instead and fails that leg.
