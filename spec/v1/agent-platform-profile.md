...
The certifier's floor for this annex (`PROFILE_FLOOR_SCENARIOS['openwop-agent-platform']`, suite ≥ 1.136.3, S42) is exactly this list — `agent-platform-aggregate-evidence`, `agent-manifest-runtime`, `agent-live-runtime-shape`, `tool-descriptor-shape`, `tool-hooks-shape`, `safefetch-behavior`, `provider-usage`, `prompt-list-and-fetch`, `memory-capability-model-shape`, `feedback-capability-shape`, plus `replayDeterminism` when `replay.modes` includes `replay` — so a bundle v2 can evaluate the claim; before S42 the profile was pushed onto `claimedProfiles` by the predicate but had no floor and was reported "NOT certifiable (no floor defined)" on every host, including one on which every constituent scenario passed. The aggregating meta-scenario `agent-platform-profile.test.ts` derives `none`/`partial`/`full` from the discovery payload (§B) and, when `full` is claimed against a live host, asserts every required constituent scenario is in the host's passing set for the reported suite version. It soft-skips the live aggregate (reports `none`) when the floor predicate is unmet.
