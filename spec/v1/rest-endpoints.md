...
- `replay_memory_snapshot_unavailable` — RFC 0039 §B. The host advertises `capabilities.multiAgent.executionModel.version >= 2` AND `capabilities.memory.supported: true` but cannot serve the memory snapshot for the requested `POST /v1/runs/{runId}:fork forkAtEventLogIdx`. Per the MAE-3 contract in `spec/v1/multi-agent-execution.md` §"Replay carry-forward", forks from past event-log indices MUST return memory state as-of that index, not current state; hosts that haven't persisted a snapshot at the requested index MUST refuse the fork rather than silently substitute current memory. `details.forkAtEventLogIdx` SHOULD identify the requested index; `details.oldestAvailableIdx` MAY identify the oldest index for which a snapshot exists so the client can pick a valid fork point.
