...
- The diff MUST be a **pure function** of the two runs' event logs: given the same two logs, every conformant host MUST return the same `divergedAtSeq` and the same ordered `eventDiffs`. Sequence alignment is by event `seq`; the first `seq` whose event differs by canonical comparison sets `divergedAtSeq`. Identical logs MUST yield `divergedAtSeq: null` and an empty `eventDiffs`. This aligns with the determinism contract and the `replay.diverged` event in [`replay.md`](./replay.md).
...
> **Precedence, restated 2026-08-16 (S22 — the "envelope schism").** The **flat** shape above — `error` is a *string* code, `message` a string, everything else under `details` — is canonical for **every HTTP error response** an OpenWOP host returns, on canonical endpoints and on `/v1/host/sample/*` test seams alike. `schemas/error-envelope.schema.json` is authoritative; the TypeScript SDK's `ErrorEnvelope` and every conforming host emit it. Between 2026-06 and 2026-08 a **nested** form — `{ "error": { "code": …, "retriable": … } }` — appeared in this document's own code list (`runner_unavailable`, `residency_unavailable`, `interop_version_unsupported`), in four `host-sample-test-seams.md` seam contracts (§19/§20/§22/§23) and in ~15 scenarios; those were the drift, and re-shaping `error` from string to object would break every conforming host under `COMPATIBILITY.md` §2.2, so **flat wins**. Three other error objects are legitimately nested and are *not* this envelope: `RunSnapshot.error { code, message, retriable? }` (run-level, `run-snapshot.schema.json`), bulk-result items (`{ ok:false, error:{ code } }` above), and JSON-RPC bodies on the MCP / A2A mounts (`{ error: { code, message } }`, their own protocols). The suite reads codes through `conformance/src/lib/error-envelope.ts` (`readErrorCode` / `readRetriable`), which tolerates the legacy nested shape on seams through the first suite minor after 2026-11-10; the seam catalog is corrected to flat and hosts converge their seams (openwop-app H27).

### `details.retriable` convention (REQUIRED where a code declares retriability)

Codes this document marks **retriable** or **not retriable** (`runner_unavailable`: `true`; `interop_version_unsupported`, workload-identity resolution failures: `false`) carry that as `details.retriable` (boolean). There is no top-level `retriable` — `additionalProperties: false` forbids it — and no `error.retriable`. A client that finds `details.retriable === true` MAY retry the same request; absent, it MUST NOT infer retriability from the HTTP status.
...
- `run_timeout` — RFC 0058. A run exceeded its effective `runTimeoutMs` (`min(runTimeoutMs, capabilities.limits.maxRunDurationMs)`). `details.elapsedMs` SHOULD report the observed duration (mirrors `sandbox_timeout`). Pairs with `cap.breached { kind: 'run-duration' }`. The run terminates `failed`. Applies only on hosts advertising `capabilities.limits.maxRunDurationMs`.
- `loop_limit_exceeded` — RFC 0058. A run exceeded its effective `maxLoopIterations` (`min(maxLoopIterations, capabilities.limits.maxLoopIterations)`). `details.iteration` SHOULD report the count reached. Pairs with `cap.breached { kind: 'loop-iterations' }`. The run terminates `failed`. Applies only on hosts advertising `capabilities.multiAgent.executionModel.supported` (the execution loop counts orchestrator turns, RFC 0037; RFC 0061 `version: 5` adds the observable `iteration` counter).
...
- `workspace_conflict` — RFC 0059. A `PUT /v1/host/workspace/files/{path}` carried a stale `If-Match` etag — the file changed since the caller last read it. HTTP status `409`. `details.currentVersion` SHOULD carry the file's live `version` so the client can re-read and retry. Applies only on hosts advertising `capabilities.workspace.supported`.
- `workspace_too_large` — RFC 0059. A workspace `PUT`'s `content` exceeded `capabilities.workspace.maxFileBytes`. HTTP status `413` (hosts MAY use `400` where `413` is impractical). `details.maxFileBytes` SHOULD echo the advertised cap; `details.requestedBytes` MAY identify how much was attempted. Applies only on hosts advertising `capabilities.workspace.supported`.
- `token_budget_exceeded` — RFC 0062. A memory-distillation ("dream") run could not be meaningfully distilled within its effective `distillation.tokenBudget` (`min(distillation.tokenBudget, capabilities.memory.distillation.maxTokenBudget)`). The run MUST fail atomically — **no partial archive is written**. `details.budget` SHOULD echo the budget; `details.minimumRequired` SHOULD report the host's best-effort estimate of the tokens needed (counted against the advertised `tokenizerName`, ±10%). Applies only on hosts advertising `capabilities.memory.distillation.supported`. (Present in the SDK error vocabulary prior to this RFC; RFC 0062 registers it in the spec error-code list.)
- `rate_limited` — too many requests
- `webhook_endpoint_unverified` — RFC 0201 §D. A webhook registration that opted into `standard-webhooks-1` was refused because the endpoint did not echo the verification `challenge` (non-`2xx`, redirect, empty or wrong body, or no answer within 10 seconds). HTTP status `400`; not retriable with the same `Idempotency-Key`. No subscription is persisted.
...
- `replay_context_summary_unavailable` — RFC 0111. The host advertises `capabilities.multiAgent.executionModel.contextBudget.summarization` but cannot serve, as-of the requested `POST /v1/runs/{runId}:fork fromSeq`, the summary artifact that a `context.summarized` event at or after that index references (`summaryRef`). Per `spec/v1/multi-agent-execution.md` §contextBudget a replay MUST reuse the recorded summary and MUST NOT re-summarize, so a host that cannot serve it MUST refuse the fork with this code — the summarization analogue of `replay_memory_snapshot_unavailable`. `details.fromSeq` SHOULD identify the requested index.
...
- `replay_source_missing` — RFC 0140. A **node-failure** code in the run event log, not an HTTP error-envelope code: it rides on `node.failed`, and the `POST /v1/runs/{runId}:fork` request that produced the replay still returns `201`. During a `mode: "replay"` fork, a side-effecting node was reached for which the **source** run has no recorded terminal outcome for the same `(nodeId, attempt)` — because the source diverged upstream, failed early, was cancelled, or never reached the node. Per `spec/v1/replay.md` §"Side-effect suppression in replay" requirement 3, the host MUST fail the node closed with this code; it MUST NOT perform the effect and MUST NOT substitute a synthesized or empty success, because a replay never creates a new side effect. `details.nodeId` SHOULD identify the node; `details.attempt` SHOULD identify the attempt for which no source outcome exists. Applies only on hosts advertising `capabilities.replay.sideEffectSuppression: "recorded-outcome"`.
- `runner_unavailable` — RFC 0122. A per-step dispatch could not be routed to a self-hosted runner: either no runner is registered for the run's owning subject (`spec/v1/self-hosted-runner.md` §Behavior — subject-first match, no cross-subject fallback) or a previously-registered runner went offline mid-run. The error is **retriable** (`retriable: true`) — a runner MAY (re)register and the dispatch can be retried; the run MUST NOT hang. The canonical envelope carries `{ "error": "runner_unavailable", "message": "…", "details": { "retriable": true } }` at HTTP status `>= 400` (flat, per §"Error response shape" — this entry read the nested `{ error: { code, retriable } }` shape from 2026-07 until 2026-08-16, S22); hosts MAY choose the specific 4xx/5xx code (the conformance witness asserts the retriable envelope, not a fixed numeric status — mirrors `run_forbidden` / `capability_required`). A provider-side model/tool error is NOT this code — it rides inside the result frame's `output`; `runner_unavailable` is host-side liveness/routing signalled out-of-band. Applies only on hosts advertising `capabilities.selfHostedRunner.supported`.
- `interop_version_unsupported` — RFC 0152 §B / RFC 0153 §B. A protocol-version negotiation with an interop peer failed and the failure crossed an OpenWOP boundary: either a peer answered the upstream version error (A2A `VersionNotSupportedError`; the MCP unsupported-revision failure) to a request this host originated, or this host refused an inbound request for a version/revision it does not list in `capabilities.a2a.protocolVersions` / `capabilities.mcp.protocolVersions`. Not retriable — mirroring `runner_unavailable`'s documented shape, the envelope carries `{ "error": "interop_version_unsupported", "message": "…", "details": { "retriable": false, "protocol": "a2a"|"mcp", "requested": "…", "supported": [ … ] } }` at HTTP status `>= 400` (flat; the RFC 0152/0153 witnesses assert the envelope, not a fixed numeric status); `details.protocol` MUST be `"a2a"` or `"mcp"`; `details.requested` SHOULD carry the requested version/revision and `details.supported[]` the versions the refusing side listed, when known. The upstream error body is NOT relayed — its closed reason (e.g. `VERSION_NOT_SUPPORTED`) is the only detail projected. HTTP status SHOULD be `400` when this host is the refusing side and `502` when a peer was. See `a2a-integration.md` §"A2A 1.0 versioned composition" §B/§D.7 and `mcp-integration.md`.
- `residency_unavailable` — RFC 0129. A `POST /v1/runs` request carried a `residency.region` the host cannot serve — the region is not in the host's advertised `capabilities.dataResidency.regions[]`. The host MUST reject and create no run (fail-closed; it MUST NOT silently accept-and-ignore the constraint and process out-of-region). The canonical envelope carries `{ "error": "residency_unavailable", "message": "…" }` (flat; the witness reads the code with `readErrorCode`); **HTTP status MUST be one of `400` / `404` / `422`** (the conformance witness asserts the `code` + no-run-created, not a byte-identical numeric — mirrors `capability_required` / `runner_unavailable`, the #815 envelope-not-status ruling). `details.requestedRegion` SHOULD echo the refused region; `details.availableRegions` MAY repeat the advertised `regions[]` (already public). Applies only on hosts advertising `capabilities.dataResidency`; an unadvertised host makes no residency promise (RFC 0129 §3).
