...
- Because group/role membership is dynamic host state, a host MUST resolve membership at decision time and snapshot the resolved subject set so that `POST /v1/runs/{runId}:fork` and replay against a historical checkpoint are deterministic (`replay.md`); it MUST NOT re-resolve membership during replay.
