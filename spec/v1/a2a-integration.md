...
Rules (RFC 0152 §C, invariant `a2a-card-runtime-consistent` — _named, not yet registered_; see §"Conformance"):
...
| `ListTasks` · `GET /tasks?…` | _(new in 1.0)_ | `GET /v1/runs` **scoped to the caller's tenant/workspace/principal**, filtered by `contextId`, `status` (via D.4), `statusTimestampAfter`; paginated (`pageSize`/`pageToken` → the host's run-list pagination); `historyLength`, `includeArtifacts` honoured. Cross-tenant runs **MUST NOT** appear (§E). |
