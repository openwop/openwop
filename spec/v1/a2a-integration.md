...
| `AUTH_REQUIRED`  | `waiting-input` on a `credential` interrupt | **Drift point #3 — retired for this direction by RFC 0199 §D.1.** A run suspended on a `credential` interrupt (`interrupt.md`, gated on `oauth.credentialInterrupt`) MUST project to `AUTH_REQUIRED`, with a `TaskStatus.message` naming the provider and carrying `connectUrl` and `interruptKind: credential`; every other `waiting-input` run stays `INPUT_REQUIRED`. |
| `COMPLETED`      | `completed`                     | Clean.                                                                                                                                                                                                                                |
| `FAILED`         | `failed`                        | Clean.                                                                                                                                                                                                                                |
...
```jsonc
// A2A 0.3 JSON-RPC `message/send` → params.message (camelCase, lowercase role, `kind` discriminators)
{
  "kind": "message",
  "messageId": "msg_002",
  "taskId": "<runId>",
  "role": "user",
  "parts": [{ "kind": "data", "data": { "action": "accept", "feedback": "looks good" } }]
}
```

The `data` part carries an approval resume in the 5-action vocabulary (`interrupt.md` §`kind: "approval"` — `accept` / `reject` / `refine` / `edit-accept` / `ask`); the OpenWOP host translates it to the engine's resume call, taking `decidedBy` from the authenticated caller rather than from the message. The run continues; the next status update flows back to the A2A client. Under the `a2a-1.0` profile the same message is ProtoJSON with no `kind` fields and `role: "ROLE_USER"` (§D.2).
...
- An A2A peer's bearer is an inbound credential: it MUST NOT be attached to any onward hop the host makes on that peer's behalf (`auth.md` §"Onward hops", RFC 0200 §E; invariant `inbound-credential-no-passthrough`).
...
> **Status: additive (2026-07-06, [RFC 0128](../../RFCS/0128-purpose-propagation-permitted-use-labels.md) `Accepted`).**
...
- The label carries purpose _categories_ only — never subject identifiers or consent records; the
...
- **The `metadata.openwop.*` extension shape** — with these exceptions: `metadata.openwop.permittedPurposes` is normative per RFC 0128 (§"Purpose-propagation labels" above), and `metadata.openwop.traceparent` / `.tracestate` are W3C Trace Context strings on `Message.metadata` per RFC 0207 (correlation, never authority; `multi-agent-execution.md` §"W3C tracecontext across MCP + A2A composition"). The rest of the namespace stays host-implementation choice; a future spec annex MAY codify further fields if multiple hosts converge on a pattern.
- **Push notification signing details.** A2A defines the push delivery contract (the push config's `url`, `token`, and `authentication { scheme, credentials }`, sent as an `Authorization` header on delivery); it defines no normative HMAC or body-signing scheme — HMAC appears upstream only as one option in informative guidance. openwop defers to the upstream contract and adds no signing scheme of its own on the A2A path. openwop's own webhook spec (`webhooks.md`) is independent.
...
- **Error taxonomy mapping — inside the A2A layer.** A2A `VersionNotSupportedError` and similar protocol-level errors stay inside the A2A layer and don't surface as openwop `RunEvent` errors. **Narrowed by RFC 0152 §B/§D.7:** when such a failure crosses an OpenWOP _boundary_ it is projected through the canonical error envelope (`interop_version_unsupported` and the D.7 table) — the layer keeps its taxonomy, the boundary keeps its envelope. openwop errors stay inside the run; they project to A2A `FAILED` with `metadata.openwop.error`.
- **AUTH_REQUIRED from a consumed agent.** Hosts that consume A2A agents in `AUTH_REQUIRED` state SHOULD project to `waiting-input` with `metadata.subkind: 'auth'` (unchanged by RFC 0199). A host MAY instead raise a `credential` interrupt when it can obtain the peer's credential through `host.oauth` (RFC 0199 §D.1).
...
To collect the real-impl evidence: run a reference A2A peer (e.g., a `@a2a-js/sdk` server) on a local port, set `OPENWOP_A2A_REAL_PEER_URL=http://localhost:<port>`, run the scenario. The test logs the skill name + response envelope kind (`task` or `message`) so the interop evidence is visible in the CI output. Recorded interop evidence is listed in `INTEROP-MATRIX.md` §"Composition partners".
...
| ------------------------------------------------------ | --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Failure on the OpenWOP side mid-run**                | Task transitions to `failed`.                                         | Run reaches terminal `failed`; `RunSnapshot.error` carries the canonical `RunErrorCode`.                              | The A2A bridge MUST surface the failure as `Task.status.message` — an agent-role `Message` whose text is derived from `RunSnapshot.error.message` (content-free; A2A `Task` has no `error` field). The typed `code` MAY ride the host-extension key `metadata.openwop.error.code` (the §3 `run.failed` carrier) for clients that recognize it; it is not a declared mapping (§D).                                                   |
...
| **Time-skew between A2A and OpenWOP clocks**           | A2A `Task.status.timestamp` is the peer's clock (A2A `Task` has no creation timestamp).                             | OpenWOP `Run.createdAt` is the OpenWOP host's clock.                                                                  | Bridge nodes SHOULD record both timestamps; observers MUST NOT assume monotonicity across the boundary.                                                                                                                                 |
...
> **Status: additive, normative for any host that advertises `capabilities.a2a.durableTasks` (2026-06-14, [RFC 0100](../../RFCS/0100-async-durable-a2a-tasks.md) `Accepted`).** The synchronous `message/send` → poll `tasks/get` round-trip above is unchanged. This section closes the two Future-work items below — the `a2a` capability slot and the `metadata.openwop.*` interrupt-kind shape — and makes the §"State projection" mapping durable + resumable so a host can run a cross-host handoff asynchronously. The run-status → TaskState mapping is **persisted, not changed**.
...
- **Resume across the boundary is unchanged.** An A2A Message reply into an `input-required` Task resolves the HITL interrupt exactly as §4 already specifies (the 5-action approval vocabulary). This section makes the _waiting_ durable + re-attachable; it does not change resume.
...
- **Fire a push on each durable TaskState transition the caller subscribed to** — at minimum on the transitions to `input-required`, `completed`, `failed`, `canceled`. The push body is an A2A `TaskStatusUpdateEvent` (composed per §3); delivery authentication follows the upstream push config (`token` and `authentication { scheme, credentials }`) — A2A defines no HMAC contract, and openwop adds none (§"What OpenWOP does NOT specify about A2A" — unchanged).
...
- An A2A-capable host **MUST** advertise a non-empty `protocolVersions` and a `preferredVersion` present in it. `supported: true` without `protocolVersions` is deprecated: it says the host speaks _some_ A2A and **cannot substantiate a current-A2A claim**.
...
- **Well-known path is unchanged in 1.0**: `/.well-known/agent-card.json` (A2A 1.0.0 §"Discovery"). `agentCardUrl` therefore serves both profiles; the version is discovered _inside_ the card (§C), not from its path.
- **Legacy window (RFC 0152 UQ1, resolved for the date).** A2A 1.0.0 was published **2026-03-12**; the RFC's proposed 12-month window makes **2027-03-12** the date after which a host **SHOULD NOT** advertise `a2a-0.3-legacy` and after which a bare `supported: true` cannot appear in any interop claim. Removal of the 0.3 code path from the corpus is a v2 change or a separately justified upstream-security safety fix (RFC 0152 §Compatibility). The 0.3 _adopter inventory_ (gap G1) is not resolved by picking a date and stays open.
...
| `skills[]` — `{ id, name, description, tags[], examples?, inputModes?, outputModes?, securityRequirements? }` | REQUIRED | The **same workflow registry runtime routing uses**: one skill per invocable workflow, `skills[].id` = the workflow's stable identifier as the host routes it (`workflowId` or slug — pick one and keep it), `inputModes`/`outputModes` = the media types the workflow's input schema and outputs actually accept/produce. A skill absent from routing **MUST NOT** appear; a workflow the caller cannot invoke **MUST NOT** appear in the _extended_ card for that caller (§E). |
| `securitySchemes{}`, `securityRequirements[]` | optional | The auth the endpoint **actually enforces** (RFC 0049/0050 profiles). Listing a scheme the endpoint does not check, or omitting one it does, is card/runtime drift. *Informative (RFC 0200 §F.5):* a host that serves its own OpenAPI document (`GET /v1/openapi.json`, v2 `/openapi.json`) SHOULD generate these two fields from that document's `components.securitySchemes` and per-operation `security` rather than keep them by hand — replacing the placeholder issuer URLs with its advertised issuers and dropping the schemes for lanes it does not advertise. The mapping is one-to-one: OpenAPI `http`/`bearer` → `HTTPAuthSecurityScheme`, `apiKey` → `APIKeySecurityScheme`, `oauth2` → `OAuth2SecurityScheme` (one `OAuthFlows` member per scheme, since A2A's is a `oneof`), `openIdConnect` → `OpenIdConnectSecurityScheme`, `mutualTLS` → `MutualTlsSecurityScheme` (A2A v1.0.1 `a2a.proto` §4.5). |
...
Rules (RFC 0152 §C, invariant `a2a-card-runtime-consistent` — registered, protocol tier, `SECURITY/invariants.yaml`; see §"Conformance"):
...
- `supportedInterfaces[].tenant`: a host **MAY** publish tenant-scoped interfaces. The value is a _routing hint_ for the peer; it **never** selects the tenant of record (§E).
...
Normative for the `a2a-1.0` profile. Field names are the upstream JSON (ProtoJSON, lowerCamelCase) spellings. "→" reads left-to-right for the stated direction. Anything not in these tables is **opaque**: it **MUST** round-trip where the upstream requires it and **MUST NOT** become authority, a prompt segment, a tool call, or a workflow variable without a declared mapping. The only other declared mappings in the corpus are `metadata.openwop.permittedPurposes` (RFC 0128, §"Purpose-propagation labels"), the `metadata.openwop.interrupt` carrier (RFC 0100), and `Message.metadata.openwop.traceparent` / `.tracestate` (RFC 0207).
...
| `ListTasks` · `GET /tasks?…` | _(new in 1.0)_ | The host's tenant-scoped run index (v1 defines no list-runs REST operation; the v2 surface is `listRuns`, RFC 0182) **scoped to the caller's tenant/workspace/principal**, filtered by `contextId`, `status` (via D.4), `statusTimestampAfter`; paginated (`pageSize`/`pageToken` → the host's run-list pagination); `historyLength`, `includeArtifacts` honoured. Cross-tenant runs **MUST NOT** appear (§E). |
...
| `SubscribeToTask` · `POST /tasks/{id}:subscribe` (SSE; the upstream proto binds `GET`, the prose and SDK clients `POST` — a host SHOULD accept both) | `tasks/resubscribe` | RFC 0100 re-attach: replay the current `Task`, then follow. Terminal task ⇒ upstream error (spec §11.3.2). |
...
| `taskId` | Absent ⇒ new run. Present ⇒ **MUST** resolve to the backing run (`taskId == runId`, `a2a-task-state.schema.json`). If that run is `waiting-approval` / `waiting-input`, the message resolves the interrupt exactly as §4 "Resume = A2A Message reply" specifies (5-action approval vocabulary via `metadata.openwop.interrupt`). If the run is `waiting-external`, it is delivered as the RFC 0099 external event. If the run is not suspended, the host **MUST NOT** silently drop the message: it **MUST** either deliver it through a declared input path or return `UnsupportedOperationError`. A **terminal** run is answered by whether the host still retains it. **Retained** (the caller can still read it with `GetTask`) ⇒ the host **MUST** return `UnsupportedOperationError`, per upstream §3.1.1 `SendMessage` and §3.1.2 `SendStreamingMessage` ("Messages sent to Tasks that are in a terminal state … cannot accept further messages"). **Purged or unknown** (`GetTask` would itself answer `TaskNotFoundError`) ⇒ `TaskNotFoundError`, per upstream §3.3.2, whose definition covers a task that is "invalid, expired, or already completed and purged". A task the caller **cannot read** is `TaskNotFoundError` whatever its state, because a terminal-state answer would reveal that it exists (§E). A terminal run is never resurrected. Corrected 2026-09-22; see `COMPATIBILITY.md` §3. |
...
| `auth-required` | `TASK_STATE_AUTH_REQUIRED` | `waiting-input` on a `credential` interrupt (RFC 0199 §D.1) | `waiting-input` + `metadata.subkind: "auth"` (drift #3, reverse only) |
...
| `rejected` | `TASK_STATE_REJECTED` | _(no forward source; a refused `SendMessage` is an error, not a task)_ | `failed` + `reason: "rejected_by_remote"` (drift #4) |
...
| Field | OpenWOP |
| --- | --- |
| `acceptedOutputModes[]` | Media-type filter on outputs; a mode the skill cannot produce ⇒ `ContentTypeNotSupportedError` (upstream `-32005` / `INVALID_ARGUMENT` / `400`) **at request time**, not after the run has done work. |
...
| `taskPushNotificationConfig` — `TaskPushNotificationConfig { tenant?, id, taskId, url (REQUIRED), token?, authentication { scheme (REQUIRED), credentials? } }` | RFC 0100 push config → `A2ATaskState.PushConfig`. `url` **MUST** pass the RFC 0093 webhook-egress SSRF guard before registration (invariant `a2a-push-egress-ssrf`). That guard is the list in `webhooks.md` §"SSRF protection" **in full**, and it has two arms: a **scheme** arm (non-`https://` protocols **MUST** be refused) and an **address** arm (loopback / RFC 1918 / link-local / IPv6 ULA / cloud-metadata **MUST** be refused) — upstream §"Push notification security" says the same. **Both arms apply, not just the address arm**: a host refusing `http://10.0.0.5/push` on the address arm alone, while accepting `http://push.example.com/`, does not satisfy the guard. An abbreviation such as "no private/loopback/link-local target" is shorthand for the referenced list, never its definition. See `COMPATIBILITY.md` §3 for the classification: this is a **Class 3** clarification — a host accepting a plaintext push target was never conforming, and the corpus now says so unambiguously. `token` and `authentication.credentials` are **caller secrets**: they **MUST** be held in the host secret store and referenced, **MUST NOT** enter the run event log, `A2ATaskState` as persisted for `GetTask`, or a debug bundle (SR-1). Push bodies are `StreamResponse`-shaped (`statusUpdate` / `artifactUpdate`) and content-free with respect to run internals. |

**D.7 Errors, both directions.** Upstream 1.0 error catalogue, with the gRPC and HTTP+JSON statuses of A2A **1.0.1** §5.4, ↔ OpenWOP canonical envelope (`error-envelope.schema.json`, codes in `rest-endpoints.md` §Error codes). Left column: what the OpenWOP host returns on its A2A interface; right column: what an OpenWOP caller sees when the host, acting as an A2A client, relays a peer failure across an OpenWOP boundary.

| A2A 1.0 error (`reason`; JSON-RPC / gRPC / HTTP, per 1.0.1) | Host as A2A **server**: emit when | Host as A2A **client**: project as |
| --- | --- | --- |
...
| `TaskNotCancelableError` (`-32002` / `FAILED_PRECONDITION` / `400`) | `CancelTask` on a terminal run. | `run_terminal` (the code `rest-endpoints.md` already uses for already-completed/failed/cancelled runs). |
| `PushNotificationNotSupportedError` (`-32003` / `FAILED_PRECONDITION` / `400`) | Push config on a host with `a2a.pushNotifications` unset. | `capability_required` (`details.requiredCapability: "a2a.pushNotifications"`) |
| `UnsupportedOperationError` (`-32004` / `FAILED_PRECONDITION` / `400`) | An operation or option the host does not implement (e.g. `returnImmediately` without `durableTasks`; message to a non-suspended task with no input path), or a message to a retained terminal task (D.2). | `capability_required` or `validation_error`, whichever names the cause. |
| `ContentTypeNotSupportedError` (`-32005` / `INVALID_ARGUMENT` / `400`) | A part `mediaType` or `acceptedOutputModes` entry the skill cannot handle. | `validation_error` |
| `InvalidAgentResponseError` (`-32006` / `INTERNAL` / `500`) | — (a server does not emit this about itself) | The peer returned a non-conformant response: `validation_error` with `details.protocol: "a2a"`; **MUST NOT** feed the malformed body into run state. |
...
| `VersionNotSupportedError` (`VERSION_NOT_SUPPORTED`; `-32009` / `FAILED_PRECONDITION` / `400`) | §B. | **`interop_version_unsupported`** (§B). |

**Status tolerance (A2A 1.0.0 → 1.0.1).** A2A 1.0.1 corrected six gRPC or HTTP+JSON statuses in its own table: `TaskNotCancelableError` HTTP `409`→`400`; `PushNotificationNotSupportedError`, `UnsupportedOperationError` and `VersionNotSupportedError` gRPC `UNIMPLEMENTED`→`FAILED_PRECONDITION`; `ContentTypeNotSupportedError` HTTP `415`→`400`; and `InvalidAgentResponseError` HTTP `502`→`500`. The **JSON-RPC codes did not change**, so the mandatory JSON-RPC floor (§C) is unaffected. Upstream treats the patch number as carrying no compatibility meaning (§3.6: patch versions "MUST not be considered" in negotiation), so a peer announcing `1.0` may emit either table. Therefore:

- **As server**, on a gRPC or HTTP+JSON interface, a host **SHOULD** emit the 1.0.1 status shown above. A host that emits the 1.0.0 status for one of those six rows remains conforming for the rest of v1.x. The JSON-RPC code and the `reason` are **MUST**-level, as before.
- **As client**, a host **MUST** accept either the 1.0.0 or the 1.0.1 status for those six rows, and **MUST** identify the error by its JSON-RPC code or its `google.rpc.ErrorInfo` `reason` (upstream §10.6 and §11.6 require `ErrorInfo` on gRPC and HTTP+JSON). It **MUST NOT** identify the error by the gRPC or HTTP status alone: under 1.0.1, seven of the nine errors share HTTP `400`.

`COMPATIBILITY.md` §3 records this re-pin as a conformance-affecting correction.

Error `message` text in either direction **MUST NOT** carry stack traces, provider bodies, credentials, or the other side's raw error body (RFC 0152 UQ4: upstream error _details_ are reduced to the closed `reason` and, for version errors, `metadata.supportedVersions` (a comma-joined string — erratum 2026-09-23, RFC 0211 §E); everything else is dropped, not redacted-in-place).

### §E — Identity and security

- **A2A authentication establishes a peer principal; it does not grant OpenWOP authorization.** Before any run is created, read, listed, cancelled, subscribed, or pushed to, the OpenWOP boundary **MUST** resolve tenant, workspace, principal (RFC 0048), scopes (RFC 0049), delegated actor where `auth.workloadIdentity.delegation` is advertised (RFC 0154 §B — the composition RFC 0152 gap G5 named), and audience — and **MUST** decide authorization there, under the same policy as an OpenWOP REST caller. Card `securitySchemes` describe _how_ the peer authenticates; they never describe _what_ it may do.
...
- **Invariants.** `a2a-version-no-silent-downgrade` is registered (protocol tier, `SECURITY/invariants.yaml`, witnessed by `a2a-version-negotiation.test.ts`). `a2a-card-runtime-consistent` (witnessed by `a2a-card-runtime-consistency.test.ts`) and `a2a-peer-no-authority-escalation` (witnessed by `a2a-peer-authority.test.ts` through the §22 invoke seam) are named by RFC 0152 §E and are registered (protocol tier, `SECURITY/invariants.yaml`); their conditions are the MUSTs in §C and this section.
...
- A2A **1.0.1** pin for the gRPC and HTTP+JSON error statuses (§B, D.6, D.7): `https://github.com/a2aproject/A2A/blob/v1.0.1/docs/specification.md` §5.4; release notes `https://github.com/a2aproject/A2A/releases/tag/v1.0.1` ("recent transcoding-related error changes", a2aproject/A2A#1627).
