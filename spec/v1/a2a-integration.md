...
Error `message` text in either direction **MUST NOT** carry stack traces, provider bodies, credentials, or the other side's raw error body (RFC 0152 UQ4: upstream error _details_ are reduced to the closed `reason` and, for version errors, `supportedVersions[]`; everything else is dropped, not redacted-in-place).
