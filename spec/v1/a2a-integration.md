...
> **Informative — A2A 1.0.1.** A2A **1.0.1** (published 2026-05-28) re-mapped the gRPC and/or HTTP+JSON status of six rows of this table: `TaskNotCancelableError` HTTP `409`→`400`; `PushNotificationNotSupportedError`, `UnsupportedOperationError` and `VersionNotSupportedError` gRPC `UNIMPLEMENTED`→`FAILED_PRECONDITION`; `ContentTypeNotSupportedError` HTTP `415`→`400`; `InvalidAgentResponseError` HTTP `502`→`500`. The **JSON-RPC codes are unchanged**, so the mandatory JSON-RPC floor (§C) is unaffected. This table, §B and D.6 remain pinned to 1.0.0 as stated above; re-pinning them is a forthcoming conformance-affecting correction that will be recorded in `COMPATIBILITY.md` §3 when it lands, not an edit made by this note.
