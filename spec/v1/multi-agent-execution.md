...
| `dispatching` | Engine called `POST /v1/runs` (or sub-workflow equivalent) for the child                                         | → `running` (`201 Created` returned + `inputMapping` projection emitted); → `failed` (creation failed before child ran any node) |
| `running`     | Child run is in progress                                                                                         | → `completed` (terminal status); → `failed` (terminal status); → `cancelled` (terminal status)                                  |
...
Hosts MUST NOT silently execute a `confidence < floor` decision without first recording the escalation event AND firing the matching interrupt. The escalation event is `core.workflowChain.confidence-escalated` (see §"Event-payload addition" below) and MUST appear in the run event log BEFORE the interrupt fires AND BEFORE any `core.workflowChain.event` with `phase: "dispatch.began"` for the escalated decision's intended next-worker. **Causation (S32, 2026-08-17).** The escalation event's `causationId` MAY be absent; when present it MUST resolve to an event already on the run's log. It is NOT required to point at a `runOrchestrator.decided`: under the conservative path (RFC 0011 §F CP-1) a low-confidence decision is HELD until human ratification, so no `runOrchestrator.decided` exists on the log before resume — which is exactly why the payload's `originalDecision` carries the `OrchestratorDecision` verbatim. A host that does emit a pre-ratification decided event MAY point at it; a host that honours CP-1 points at nothing or at the preceding node event. (`multi-agent-confidence-escalation.test.ts` required the decided-event causation until suite 1.136.0 and contradicted `orchestratorConservativePath.test.ts` on the same mechanism.)
...
- `summaryRef` is an **artifactId** (a run-produced artifact read via `GET /v1/runs/{runId}/artifacts/{artifactId}`), keeping the event content-free. By direct analogy to the MAE-3 memory carry-forward (§"Replay carry-forward (MAE-3)"), the summary artifact MUST be persisted and readable as-of the iteration's event-log index; a host that cannot serve it at the requested `fromSeq` MUST refuse the fork with `replay_context_summary_unavailable` (the analogue of `replay_memory_snapshot_unavailable`; `rest-endpoints.md` §"Common error codes").
- Summarization that is not recorded this way breaks replay and MUST NOT be advertised under this capability.

**Conformance seam (non-vacuity ceiling).** The assembled transcript is host-internal and never crosses the wire, so conformance observes it via the OPTIONAL `GET /v1/host/sample/agent/transcript-window` seam (`host-sample-test-seams.md`): the host reports its own per-iteration accounting (`tokenCounter`, `tokenCount`, `eventIds`, `summarizedRanges`, and `entries[] { eventId, rendered }` — the exact text fed, REQUIRED for `tokenCounter: "chars"`). The harness recounts the rendered text (for `chars`; other units are advertise-and-attest), and asserts real events + `tokenCount ≤ transcriptTokenBudget` + recent tail (eligible = same `(type, nodeId)` as a fed event) + that the budget was actually under pressure in at least one turn. This proves the host's **declared** transcript accounting is consistent and bounded; it cannot black-box-prove the host feeds nothing additional off-seam — the inherent ceiling of governing host-internal assembly. The capability is advertise-and-attest.
