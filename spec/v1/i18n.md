...
> **Status: Stable · v1.1.** Optional annex for negotiating locale on human-targeted text — interrupt prompts, error envelopes, and host-extension UI strings. Additive to [`interrupt.md`](./interrupt.md) and the error-envelope contract.
...
4. **Use BCP 47 tags throughout.** All locale identifiers (`Accept-Language`, `Content-Language`, `locale` body fields) are [BCP 47](https://www.rfc-editor.org/rfc/rfc5646) language tags. Common tags: `en`, `en-US`, `en-GB`, `ja`, `ja-JP`, `zh-Hans`, `zh-Hant`, `es-419`, `pt-BR`, `de-DE`, `fr-FR`, `ar`, `he`. Tags are compared case-insensitively (RFC 5646 §2.1.1; RFC 4647 §2). Hosts **SHOULD** advertise and emit them in the case RFC 5646 §2.1.1 recommends — lowercase language, titlecase script, uppercase region (`zh-Hant-TW`, `es-419`) — because keyed surfaces such as `localized-content.md` §B store them case-canonically (RFC 0206).
...
- `i18n.defaultLocale` (BCP 47 string; canonical case SHOULD, see rule 4): the locale the host returns when no `Accept-Language` matches `supportedLocales`. Default value if omitted: `"en"`.
- `i18n.supportedLocales` (array of BCP 47 strings; canonical case SHOULD, see rule 4): the locales the host can return for human-facing text. The array MUST contain `defaultLocale`. Order is informational only.
