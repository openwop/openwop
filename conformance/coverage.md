...
| Observability and diagnostics | `cost-attribution.test.ts`, `debugBundle.test.ts`, `otel-emission.test.ts`, `otel-trace-propagation.test.ts`, `metric-emission.test.ts` | A− | OTLP/HTTP-JSON receiver wired 2026-05-11 + OTLP/HTTP-protobuf receiver wired 2026-05-12 (Track 11 follow-up; hand-rolled decoder at `conformance/src/lib/otlp-protobuf.ts`, zero new npm deps). Opt-in via `OPENWOP_OTEL_COLLECTOR=true`; hosts may emit either encoding (`OTEL_EXPORTER_OTLP_PROTOCOL=http/json` or `http/protobuf`). Remaining: OTLP/gRPC (deferred to v1.2+). |
...
| `otel-emission.test.ts` | `openwop.*` OTel spans (`observability.md`) | A− (OTLP/HTTP-JSON + OTLP/HTTP-protobuf both accepted via content-type-routed collector since 2026-05-12) | partial | OTLP/gRPC path (deferred to v1.2+) |
...
| `wasm-pack-*.test.ts` (six scenarios) | `capabilities.nodePackRuntimes.wasm` (`RFCS/0008`) | A (load + invoke + replay + memory cap positive-path + ABI version observational) | full for memory-cap | Memory-cap positive path closed 2026-05-12; ABI-mismatch positive path remains observational pending a deliberately-mismatched pack. |
