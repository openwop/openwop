...
## [2.37.0] — unreleased — the 2.37.0 cycle is open
...
- **Version moved ahead of publication, not with it.** `@openwop/openwop-conformance` and its exact-pinned peer `@openwop/spec-artifacts` move to `2.37.0` now, before any 2.37.0 content lands, because `2.36.1` is published and the published-identity gate refuses a tree whose shipped files differ from the tarball at the same version. The scenarios this minor adds are recorded here as they land. Not tagged, not published; npm still serves `2.36.1`.
