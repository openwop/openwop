...
## [2.41.2] — 2026-09-26 — no scenario changes; the contract peer ships a corrected RFC status label

- **Version moved with the corpus.** `@openwop/openwop-conformance` and its exact-pinned peer `@openwop/spec-artifacts` move to `2.41.2`, because `spec-artifacts` carries `spec/v2/core/node-pack-runtimes.md` with RFC 0203 correctly labelled `Accepted`. No scenario, fixture or requirement changes.
...
- **`context-budget-transcript-bound` can now fail in the ways RFC 0111 names.** It checked that each fed event id existed and was unique, so a wrong `tokenCount`, a window that was not a recent tail, or a budget of 10⁹ the run never reached all passed. The checks now live in the pure checker `src/lib/context-budget.ts` (15 self-tests, one per sabotage). For `tokenCounter: "chars"`, the transcript-window seam must return `entries[] { eventId, rendered }` and the suite sums the rendered Unicode code points, which must equal `tokenCount`. It also checks every id is a real event and the verbatim entries equal `eventIds` in order. For the recent tail, fed events are in log order, none at or after the turn's own `runOrchestrator.decided`, and no newer eligible event (same `(type, nodeId)`) is dropped. A turn with no budget pressure records `partial-witness:`, not a pass. Other token units are advertise-and-attest. See the COMPATIBILITY.md correction on record of 2026-09-26.
...
- **A setting that names a secret is not scrubbed as one.** `evidenceSecretsFromEnv` sweeps every `OPENWOP_*` value whose NAME mentions a key/token/secret/password, and `scrubEvidence` then rewrites that value everywhere in the bundle. openwop-app's RFC 0201 setting `OPENWOP_WEBHOOK_SECRET_ROTATION_OVERLAP_S=60` was swept, the "60" inside `discovery.sha256` was rewritten, the bundle failed `^[0-9a-f]{64}$`, and its deploy certify refused to ship. Values guessed from names are now swept only when credential-shaped (at least 8 characters and not a bare integer); a credential the emitter hands over explicitly is scrubbed whatever its shape. Sabotage: without the floor the new redaction leg fails.
...
- **The harness OIDC issuer is an instrument a host claims, so an honest production bundle is no longer uncertifiable.** Before this, `v2-oidc-id-token-audience` (`0200.id-token-aud`) and `v2-lane-exp-only-bound` (the four `0210.exp-only-*` / `credential-lifetime-code` rows) recorded `blocked` whenever the host did not trust `OPENWOP_TEST_OIDC_ISSUER_URL`. A production host must never trust a test issuer, and one `blocked` row denies certification bundle-wide (RFC 0168 §E.1). Every production bundle advertising an `oidc` or `exp-only` lane was therefore uncertifiable (openwop-app ADR 0745, corpus defect 1). The rows are `seam-gated`, and RFC 0168 §C.1 already rules that an unclaimed suite instrument is `inapplicable`, with `blocked` reserved for a claimed instrument that does not answer. The new `lib/harness-issuer.ts` treats the harness as claimed when the gated lane's advertised `issuers[]` lists it. When the lane does not list it, the row is `inapplicable` and names the issuers the host does trust. When it does, the legs run as before (a failed serve or a refused control is still `blocked`). RFC 0200 and RFC 0210 carry dated correction notes, and `SECURITY/invariants.yaml` `lane-exp-only-lifetime-bounded` is updated. No MUST changes. The residual is stated in the RFC 0210 note: a production bundle does not witness these rows, just as it witnesses no seam-gated row. The witness comes from a cut whose lane lists the harness. **The issuer now has a bind knob:** it listens via `receiverBinding()`, which is loopback, or `0.0.0.0` under `OPENWOP_CONFORMANCE_HARNESS_HOST`, on 2.39.4's `issuerListenPort()`. It waits out `EADDRINUSE` while the other scenario holds the shared URL. Before this, it always bound `127.0.0.1`, so a host in a Linux container reaching it through `host-gateway` found nothing listening (ADR 0745 defect 2; 2.39.4 fixed the port half). **`requirements.json` resolves `const ID` declared inside a test body** (`generate-requirement-registry.mjs`). Before, only module-level constants resolved, so 14 real ids recorded `explicitId: null`: `0210.lane-rule-surfaces-agree` (ADR 0745 defect 4), three `0199.*`, six `0201.*` and four `0214.*`. Those rows were labelled by their title-derived id whenever the leg did not reach its assertion. ADR 0745 defect 5, the reused kid, was already fixed in 2.37.0: every instance mints `openwop-conformance-key-<instance>-<n>`, pinned by `oidc-issuer.test.ts`. New suite self-test `src/lib/harness-issuer.test.ts` covers the claim predicate and the port knob. It is sabotage-proved: dropping the `issuers[]` check makes it fail 2/4.
...
- **Suite `2.40.0`**: 561 scenario files, a minor because two are added. `@openwop/spec-artifacts` moves in lockstep at the same exact pin, and ships the new `webhooks.md` §Durability text and the two invariant rows' threat-model home.

## [2.39.5] — 2026-09-26 — RFC 0213 §B has a deterministic witness, and a host front whose URLs follow the request origin verifies

- **RFC 0213 §B has a deterministic witness: `0213.in-flight-refused-under-hold`.** On a host whose create answers in milliseconds the unaided `v2-idempotency-in-flight` leg records `partial-witness` — same-key creates never overlap. The new leg arms the seams-profile seam `armIdempotencyHold` (`POST /conformance/seams/sample/test/idempotency/hold`, host-sample-test-seams.md §26), which holds the caller's next real create's claim in flight; a concurrent same-key create MUST then be refused `409 idempotency_in_flight` by the host's own path (no retry timing in `details`), and the held create still wins. Seams not advertised ⇒ `inapplicable`; advertised but the path unserved ⇒ `blocked`.
- **`OPENWOP_HOST_PUBLIC_URL` verifies a host whose advertised URLs follow the request origin (fixes #1567).** 2.39.4 required the discovery document fetched through the declared front to be byte-identical to the one fetched over `--base-url`. A host that derives `agentCardUrl`, MCP `serverUrls` and similar values from the REQUEST origin embeds the loopback origin in one and the front's origin in the other, so the check could never pass on exactly the hosts it exists for. On the v2-reference public cut on suite 2.39.4 it was the only blocked row (`0199.credential-interrupt`), and it denied certification of an otherwise 397-pass / 0-fail bundle (openwop-1f). Both origins are now replaced by one placeholder in both documents before the structural comparison, because a host with a public base may embed either origin in either response. Everything else must still match, so a front serving another host is still refused. Checked against real v2-reference discovery fetched both ways (a local boot with a public base); two new self-tests cover the measured shape, and removing the normalisation fails the regression test.
- **The RFC 0170 credential mint/revoke seams are in the seams contract.** `v2-revocation-honored` drives `POST …/auth/credential/mint` and `…/auth/credential/revoke`. RFC 0170 §B.3 names them, and the v2 reference host has served them. Until now neither `api/seams-v2.yaml` nor `host-sample-test-seams.md` defined them, so a host advertising `openwop-conformance-seams-v2` could not know it owed them, and under `--require-behavior` their absence was a hard fail. openwop-app's seams-on in-memory boot hit exactly that on 2026-09-25. Added as `mintLaneCredential` / `revokeLaneCredential`, in the shape the reference host already serves (not a new shape), with the prose bullet beside the RFC 0199 seams. The scenario's `seamAbsent` message now names the contract instead of calling the seam unspecified. No committed bundle changes: MyndHyve's and openwop-app's do not advertise the seams profile (the row is `inapplicable`), and v2-reference's passes it.
...
- **Version moved ahead of publication.** `@openwop/openwop-conformance` and its exact-pinned peer `@openwop/spec-artifacts` move to `2.39.5` because `2.39.4` is tagged. Not tagged, not published.
...
- **A scenario's second credential is the one that gets sent: `workspace-cross-tenant-isolation-blackbox` never authenticated as tenant B.** It passed `OPENWOP_TEST_TENANT_B_API_KEY` as an `Authorization` header without `authenticated: false`. `lib/driver.ts` then overwrote that header with the default `OPENWOP_API_KEY`, so every "tenant B" request was the owner reading its own file. That returns `200`, which the leg records as a cross-tenant leak. openwop-app measured it as a false `executed-fail` on its in-process lane (openwop-app#4107), while a direct HTTP repro with the same two keys answered `404`. The fix is in the driver: a caller-supplied `Authorization` header (any case) is now sent as given and is the only credential on the request. The scenario also passes `authenticated: false` explicitly. It records `blocked` before any assertion when the key is absent, equals `OPENWOP_API_KEY`, or is refused `401`. A sweep of every tenant-B reader found the rest correct: `auth-challenge-no-oracle`, `v2-sse-last-event-id-cursor`, `v2-mcp-mount-map`, `v2-a2a-agent-cards`, `v2-a2a-operation-map` and `v2-oauth-client-pkce-state-iss` (via `bearerInit`) pass `authenticated: false`, and `v2-artifact-a2a-shape` and `v2-mcp-tasks` use raw `fetch`. One scenario outside the tenant-B set had the same defect: `interrupt-auth-required-resume` sent its `OPENWOP_TEST_LOW_SCOPE_KEY` the same way. Its "low-scope" resolve was really a full-scope one, a false miss on the required `403`. That is fixed by the same driver change and made explicit in the scenario. New suite self-test `src/lib/driver-caller-credential.test.ts` stubs `fetch` and pins the rule. It is sabotage-proved: on the old driver its caller-header leg fails.
- **Version moved ahead of publication.** `@openwop/openwop-conformance` and its exact-pinned peer `@openwop/spec-artifacts` move to `2.39.3` because `2.39.2` is tagged and published, and RFC 0215's six gap rows change `spec/v1/gaps.json`, which `@openwop/spec-artifacts` ships. No scenario change.
...
- **`src/lib/v2-projection.ts`: `stripSupported` is now `stripSupportedFlag`, named for the one thing it does** (TODO S1). It removes the retired `supported` flag and nothing else. The corpus generator's function of the same old name (now `projectV1FacetSchema`) also drops `tier`/`experimentalUntil`, folds `supported`-gated if/then into `required`, closes objects and rewrites prose. Under one name, a host reading either assumed the other matched. Each now documents the other. **`stripSupported` stays as a `@deprecated` alias**, because openwop-app's `whd7-v2-projection-parity.test.ts` imports it by that name; it is removed at the next suite major. `carriesUnspliceablePayload` is unchanged, and a self-test now holds it equal to the generator's copy (`scripts/v2-unspliceable.mjs`) on every node of the v1 capabilities schema.
...
## [2.36.0] — 2026-09-23 — the MCP/A2A alignment program, measured: thirteen RFCs, thirty-seven new scenario and coherence files

- **RFC 0210 `Active` — the `exp-only` lane's window is a bound the host enforces, and the two corpus rows that keep the member honest.** New `src/scenarios/v2-lane-exp-only-bound.test.ts` (major 2), gated on an advertised `auth.lanes[]` member with `revocation: "exp-only"` (`inapplicable`, naming what IS advertised, otherwise) **and** `OPENWOP_TEST_OIDC_ISSUER_URL` naming a synthetic issuer the operator configured that lane to trust (`blocked` otherwise — without a token the host would ever accept, "it refused a string" witnesses nothing; a window under 120 s is also `blocked`, because the control token needs room inside it). Three legs, one issuer, one key, one token shape: a control credential at `exp − iat = window − 60` that MUST NOT be refused, a credential at `exp − iat = window + 60` (still unexpired) and one with no `iat` at all that MUST be refused `401 credential_lifetime_exceeded`, and a clock-skewed credential whose total lifetime is inside the window while its remaining lifetime is outside it — the only shape that separates the two bounds, and the one a host enforcing `exp − iat` alone serves. The control is what makes the refusals evidence; a host that refuses every token fails it. New `src/coherence/v2-lane-exp-only-schema.test.ts` (corpus gate, never reaches a bundle) mints `openwop.requirement.0210.exp-only-lane-restricted` — `exp-only` validates on `oauth2` and `oidc`, is refused on `api-key` and `session`, and **each negative is re-validated against a facet copy with only the §C.8 `if`/`then` removed, where it MUST pass**, so it cannot be failing for an unrelated defect in the fixture — and `openwop.requirement.0210.lane-rule-surfaces-agree`, which drives `scripts/check-lane-revocation-rules.mjs` red once per surface in a throwaway corpus copy (an enum member with no §2.2 row; a `LANE_RULES` entry the table does not license) rather than asserting exit 0 on a clean tree. `v2-lane-issuer-advertised.test.ts` gains `exp-only` in its `REVOCATION`, `WINDOWED` and `LANE_RULES` maps, in lockstep with §2.2's table and the facet enum.
- **RFC 0197 — one major-2 scenario and four coherence tests for “v2 surfaces are retired, never reshaped”.** New `src/scenarios/v2-capability-maturity-bounded.test.ts` (major 2, ungated — every v2 host advertises family records; a host advertising none records `blocked`, never a zero-assertion pass) mints `openwop.requirement.0197.maturity-not-overstated`: a family record MUST NOT advertise `status: "stable"` when the `spec/v2/declaration.json` the suite ships calls the family `experimental` or `deprecated`. One-sided on purpose — understating is always permitted — and the declaration is read through `SPEC_V2_DIR`, which resolves to the `@openwop/spec-artifacts` peer in the published layout, so an npm consumer measures against the same bound a contributor does. It can fail today: driven against `evidence/v2-host-bundles/myndhyve.json`'s discovery document, 38 families fail. Four corpus-gate tests mint the claims-check rows and drive each new gate red one input at a time: `src/coherence/v2-surface-monotone-gate.test.ts` (`0197.no-reshape`) — a removed enum member, a removed operation, a `required` entry added to a pre-existing object, an open object closed, and a narrowed `type`, each refused, against two positive controls: an OPTIONAL property added to a closed object passes (§B.5), and an RFC 0209-shaped `anyOf` branch re-cut of the root reports ZERO removals (§A.1 — a `$defs` name is not a surface); `src/coherence/v2-retirement-gate.test.ts` (`0197.retirement-predicate`, `0197.correction-unevidenced`) — nine sabotages against one base fixture the gate calls `retirable`, each of which must be held by ITS OWN predicate and no other, plus §A.4's refusal of a correction whose prior shape a bundle carried or whose census is null; `src/coherence/removal-dates-minor.test.ts` (`0197.retirement-predicate`, `0197.reader-accepts-retired`) — the negative control runs the SAME `removeIn: "2.99"` row twice, once under the pre-0197 `v2.0-cut` trigger (due at once, red) and once under `v2-minor` (not due, green), plus the persisted-class legs where losing the member or its `x-openwop-retired-in` annotation fails; `src/coherence/bundle-maturity-gate.test.ts` (`0197.bundle-maturity-bound` — deliberately NOT the host id, because a corpus ledger row under `maturity-not-overstated` would discharge §C's host-evidence bar without any host having done anything) — the committed bundle re-labelled onto suite 2.36.0 fails with 38, promoting one family in a fixture declaration drops it to 37, and the same content at suite 2.35.1 is reported, not failed (decisions log D1). Fixtures are built in tmpdirs rather than shipped: they only ever feed a spawned gate and would otherwise reach npm consumers who cannot use them (`v2-eos-clock`'s pattern). No scenario added to any profile floor; no pin bump (D3).
- **A lane may advertise only a revocation rule `identity.md` §2.2's row for that lane lists — a tightening, and it fails one committed host's next cut.** `v2-lane-issuer-advertised` gains a third leg minting `openwop.requirement.0170.lane-issuer-advertised.lane-rule`. §2.2 has stated the per-lane rule since RFC 0170 §B.3 and nothing measured it: the facet schema relates `revocation` to `lane` nowhere, so all nine members were valid on all ten lanes, and the two existing legs check only enum membership and window presence. That is how `short-lived` — a rule §2.2 states only for `mtls`, and there as an obligation on the party that **issues** the credential — came to be advertised on a production `oidc` lane whose host performs no revocation re-check at all. **Effect on today's committed bundles: the reference host (`api-key`, `session`, `saml`, `scim`, `workload`) and openwop-app (`api-key`, `session`, `anonymous`) pass unchanged; MyndHyve's next cut fails this row** until its advertisement is corrected (`INTEROP-MATRIX.md` §"Advertised revocation rule per lane"; RFC 0210 §F, register G5). Its committed 2.35.x bundle stays a valid measurement at its own suite version (`COMPATIBILITY.md` §2.3). The `anonymous` lane is exempt and the leg's `inapplicable` message says why: its §2.2 revocation cell is `—` while the facet schema REQUIRES the field, so the table names no legal value for it (RFC 0210 register G1). The map the leg enforces is not a second declaration — the new corpus gate `scripts/check-lane-revocation-rules.mjs` parses §2.2's table and fails when the table, the facet enum and this map disagree. Sabotage: pointing the leg at a host advertising `short-lived` on `oidc` turns the row `executed-fail`, naming the lane and the rule. No version bump (rides the open 2.36.0).
- **RFC 0200 — four new scenarios, one coherence test, and two re-cited legs for the host as an OAuth protected resource.** `v2-protected-resource-metadata` (gated on an `auth.lanes[]` member with `lane ∈ {oauth2, oidc}`; a `404` at the derived well-known URI is an `executed-fail`, never a soft-skip, because the lane is the advertisement and the document is owed): `0200.prm-served` — RFC 9728 metadata answered `200`, unauthenticated, at the RFC 9728 §3/§3.1 path-inserted URI; `0200.prm-consistent` — `resource` identical to the resource identifier, `authorization_servers` set-equal to the URL-form issuers of those lanes, `scopes_supported` (when present) a non-empty string array, and neither sender-constraint flag claimed unless every such lane requires that binding; `0200.mcp-mount-prm` — §A.4, further gated on a mount that really requires an OAuth lane (a host whose api-key lane also opens the mount records `inapplicable` with that reason rather than passing a leg whose premise does not hold). `v2-auth-challenge`: `0200.challenge-401` — `Bearer` + `resource_metadata` on a no-credential refusal with NO error code, `error="invalid_token"` once a credential was presented (RFC 6750 §3.1); `0200.challenge-403-scope` (harness-gated on `OPENWOP_TEST_LOW_SCOPE_KEY`) — `error="insufficient_scope"` with `scope` naming the operation's scope. `auth-challenge-no-oracle` (BOTH_MAJORS) — `0200.no-challenge-on-nondisclosure-404`: the non-disclosure `404` of the tool catalog and the RFC 0074 inventory stays a silent `404`, and a resource-binding `403` carries no `insufficient_scope`; every leg carries a positive control that the route is live for the same credential. `inbound-credential-no-passthrough` (BOTH_MAJORS) — `0200.inbound-credential-no-passthrough`: the suite's own API key plus a cookie and a proxy-auth canary go out on the run-creating request and must appear in no header or raw body captured at the suite-owned webhook receiver; an undelivered hop is `blocked`, never a pass. `v2-oidc-id-token-audience` — `0200.id-token-aud` (gated on an `oidc` lane and `OPENWOP_TEST_OIDC_ISSUER_URL`): a same-audience ID token is admissible (the gate, because from outside "does not trust this issuer" and "refuses ID tokens" are one observation) and one minted for another relying party at the same IdP is `401 audience_mismatch`. `src/coherence/openapi-security-declared.test.ts` mints the `(corpus)` row `0200.openapi-security-declared` by running `scripts/check-openapi-security.mjs`. The major-1 wrong-audience leg of `auth-oauth2-client-credentials` is split into its own `it` and re-cited to `0200.oauth2cc-wrong-aud-401` (non-gating: no v2 cut reaches a major-1 file), and `mcp-current-auth-boundary` gains a non-gating §A.4 leg. Every row was run red against a sabotaged v2-reference build before it was cited.
- **RFC 0199 — three major-2 scenarios and the OAuth legs on two existing ones.** `v2-oauth-client-pkce-state-iss` (gated on `oauth` + `authorization_code` + the seams profile + the advertised synthetic providers) mints `openwop.requirement.0199.pkce-s256`, `.state-single-use`, `.same-user-callback`, `.iss-validated` and `.redirect-uri-fixed`; `v2-credential-interrupt` (also gated on the `conformance-credential` fixture) mints `.credential-interrupt`, `.credential-resume-rechecked`, `.credential-declined` and — on a host advertising `oauth` WITHOUT `credentialInterrupt` — the RFC 0047 §C.3 regression row `.refresh-failure-fails-node`; `v2-oauth-mcp-reach-discovery` mints `.resource-indicator`, `.mcp-pkce-verified` and `.mcp-metadata-bound`. `v2-a2a-operation-map` gains `.a2a-auth-required` and `v2-mcp-mount-map` gains `.mcp-url-mode` and `.form-mode-no-secret`. New `src/lib/oauth-as-double.ts`: a suite-owned authorization-server double (RFC 8414 metadata with switchable `authorization_response_iss_parameter_supported` and `code_challenge_methods_supported`, a PKCE-checking token endpoint that COUNTS every request) and a protected-resource double (RFC 9728), both advertised through `resolvePublicFront` (`OPENWOP_OAUTH_AS_URL`, `_AS2_URL`, `_RESOURCE_URL`). Every "the host refused" row asserts zero token requests at a server the SUITE owns, never the host's own ledger, and carries a positive control on the same path. Every leg needs a fresh Subject from the credential mint seam (a Subject holding a credential would make the missing-credential legs vacuous) — `blocked` without it; the discovery legs are `blocked` without the public fronts, because a connection pack's endpoints are https by schema. New fixtures `conformance-credential`, `conformance-clarification-nested`, `conformance-clarification-sensitive`, catalogued in `fixtures.md`. All 22 sabotage cases were run against a deliberately broken v2-reference build and each turns its row red. `interrupt-approver-routing` registers `credential-reference.schema.json` before compiling the v1 `suspend-request` standalone (`CredentialData.credentialRef` `$ref`s it). No version bump (rides the open 2.36.0).
- **RFC 0207 — trace context across MCP and A2A.** New major-2 `v2-interop-trace-context` (six ids): `0207.mcp-traceparent-carried` — the host's MCP client reached through the production run path (`POST /runs` of `conformance-mcp-client` with the suite's `traceparent`, gated on `mcp.client`) and through the §23 invoke seam; `0207.a2a-traceparent-carried` — the §22 invoke seam into a suite-owned A2A peer; both read the carrier the peer RECORDED and apply the D4 rule (`lib/trace-context.ts` `classifyCarriers`): neither `params._meta.traceparent` / `Message.metadata.openwop.traceparent` nor the HTTP header ⇒ fail, a trace id that is not the suite's ⇒ fail, header-only ⇒ pass (logged `header-only`). `0207.mcp-server-adopts-meta` — `tools/call` on the host's mount with `_meta` trace T and header trace H; observed without OTLP through the outbound call of the run the mount starts (`conformance-mcp-client` as a tool), else through the OTLP collector, else `inapplicable`. `0207.mcp-malformed-ignored` / `.a2a-malformed-ignored` — a garbage value is a normal result, never an error. `0207.a2a-inbound-adopts` — OTLP only. New `otel-mcp-semconv-projection` in `BOTH_MAJORS` (`0207.mcp-semconv-stamped`; `inapplicable` when no `mcp.*` span arrives — the projection is a MAY). `cross-host-traceparent-propagation` (major 1, non-gating) rewritten from two `it.skip` placeholders into runnable legs against the fake MCP server / A2A peer. `debugBundle` gains a v1-only, non-gating span `traceId` / `kind` / `status` leg (`spans: []` ⇒ `inapplicable`). Sabotaged once each against the v2 reference host (neither carrier, fresh trace, prefixed `_meta` key, header adopted over `_meta`, malformed refused — each turns its row red; header-only passes).
- **RFC 0202 — `src/scenarios/v2-a2a-agent-cards.test.ts` (major 2), eight rows for per-agent A2A cards.** Gated on `a2a.agentCards` and a caller inventory with ≥1 `a2aTenant` entry (`inapplicable` otherwise). Rows `openwop.requirement.0202.advertisement` (profile `a2a-1.0`, `agents.manifestRuntime`, public card `extendedAgentCard: true`), `.routing-value` (schema-valid, stable across reads, unique, and containing no tenant/workspace/subject id the host disclosed on a run snapshot — an undisclosed identifier is not caught, G4), `.card-projection` (`name`/`version`/`description` from the entry; the host card's interface set, each with `tenant: R`; equal `capabilities`/`securitySchemes`/`securityRequirements`; ≥1 skill with the A2A REQUIRED fields, within `roster[].workflows` when the entry names them), `.card-sr1` (no `core.conformance.agent-pack` canary on the card; canaries defined in `fixtures.md` §"The per-agent card pack"), `.auth-before-resolve` (unauthenticated real vs never-minted `R`: identical refusal), `.identical-refusal` (`GetExtendedAgentCard` / `SendMessage` / `ListTasks` with tenant B's `R` vs a never-minted value: identical status and body apart from `id`, with a positive control that B's `R` resolves for B), `.public-card-no-r` (header-less and `A2A-Version: 1.0` public cards carry no `R`), `.tenant-of-record` (a task started through `R` is a run the caller reads and tenant B cannot). The two cross-tenant rows need `OPENWOP_TEST_TENANT_B_API_KEY` (optionally `OPENWOP_TEST_TENANT_B_AGENT_ID`); missing ⇒ `blocked`. Rides the open 2.36.0; no pin moves.
- **RFC 0204 — `ctx.mcp` returns MCP results; the catalog's annotation projection.** New major-2 `v2-mcp-client-results` (gated on `mcp.client` + the new fixture `conformance-mcp-client`, node `core.conformance.mcp-client`; `blocked` when the host claims `mcp.client` without the fixture or when its `conformance` binding never reaches the suite's server): `0204.call-tool-verbatim`, `.call-tool-iserror-resolves`, `.list-tools-page`, `.server-health`, `.reject-unknown-server` — each compares the node's `node.completed.outputs.result` with the JSON-RPC result the suite's own server recorded sending (`McpFakeServer.exchanges()`, new). New major-2 `v2-tool-catalog-annotations` (gated on `toolCatalog`, `inapplicable` — not `gateFamily` — when absent, so no strict v2 cut without the family needs a new opt-out): `0204.annotations-derived` (`inapplicable` when no descriptor carries `annotations`) and `.mcp-unclassified-write` (any projected `conformance_readonly_claim_*` MUST be `write`; `inapplicable` without the suite server or an `mcp` source). New coherence test `tool-descriptor-annotations` (`0204.annotations-schema`: 80 descriptors × every single-hint flip, missing hint and `annotations.title`). `lib/mcp-fake-server.ts` (2026-07-28 only): `structured-echo`, `always-error`, `conformance_readonly_claim_<nonce>` (per server instance), and `tools/list` paged 3 + 3 over six tools with `cacheScope: "private"` (was one unpaged `public` page; the legacy handshake is unchanged). G3: `tool-catalog-projection`, `tool-catalog-compact-projection` and `tool-descriptor-shape` are major-aware (`toolCatalogGate`, `toolsPath`, the v2 schemas, v2 event spellings) and join `BOTH_MAJORS`; `tool-catalog-projection` gains an advisory leg (sorted, stable order — met ⇒ pass, not met ⇒ `inapplicable` with the reason, never failed). `mcp-tool-roundtrip` gains the major-1 `raw` leg (`inapplicable` when absent; non-gating). Sabotage run once per row against the v2 reference host on loopback (openwop-examples PR): re-wrap, strip `_meta`, throw on `isError`, merge pages, drop the cursor, drop `annotations`, report `connected`, always `reachable`, resolve `undefined`, `readOnlyHint` on a write tool, omit `openWorldHint`, copy the server's hint into `safetyTier` — each turned its row `executed-fail`; a shuffled catalog turned the advisory leg `inapplicable`; deleting each schema clause turned the coherence row red. No floor or profile change; no pin bump (D3).
- **RFC 0198 — `src/scenarios/v2-mcp-tasks.test.ts` (major 2), thirteen rows for MCP Tasks on the host's mount and the disconnect rule.** Gated on `mcp.serverMount` + profile `mcp-2026-07-28` + `mcp.serverUrls[0]`; the task legs also on the mount's `server/discover` listing `io.modelcontextprotocol/tasks` (`inapplicable` otherwise), while the §G disconnect / no-`notifications/cancelled` legs bind every v2 mount. Ids `openwop.requirement.0198.advertised-via-discover`, `.task-when-nonterminal`, `.task-id-is-run-id`, `.task-status-projection`, `.task-update-resolves-once`, `.task-update-approver-checked`, `.task-get-read-only`, `.task-unreadable-not-found`, `.listen-omits-unreadable`, `.task-cancel-cancels-run`, `.disconnect-cancels-run`, `.task-survives-disconnect`, `.no-server-cancelled`. The cross-tenant legs need `OPENWOP_TEST_TENANT_B_API_KEY` (`blocked` without it) and each carries an owner positive control; the listen leg is `inapplicable` when the host acknowledges not even the owner's own task; the disconnect and SSE-cancel legs need `runList` + the `workflowId` filter. Every row was run red against a sabotaged v2-reference build (one sabotage per row). `fixtures.md` names the three extra fixtures the scenario calls as tools. `v2-interop-map-coherent` gains two `mcp.tasks` sabotages (a deleted `waiting-external` status row; `tasks/update → resolveInterrupt`). No version bump (rides the open 2.36.0).
- **RFC 0205 — the A2A `Part` / `Artifact` rows.** `src/coherence/a2a-parts-schemas.test.ts` mints the four corpus ids (`openwop.requirement.0205.part-schema-upstream`, `.turn-parts-shape`, `.turn-mirror-sync`, `.export-media-type`; spec-repo CI only, never a host bundle): the schemas' members equal the camelCased fields of the pinned `a2a.proto`, and every negative is proven to fail only by the rule under test. Two major-2 scenarios carry the behavioural legs: `v2-artifact-a2a-shape` (`.artifact-a2a-shape`, `.artifact-id-matches-path`, `.artifact-url-part-scoped`; runs inside the new opt-in `conformance-artifact-emit` fixture and negotiates `Accept: application/a2a+json`; an `application/json` answer is `inapplicable`, never a pass) and `v2-conversation-turn-parts` (`.turn-parts-emitted`, the behavioural twin split from `turn-parts-shape`; `inapplicable` when no emitted turn carries `parts`). `artifact-type-pack-manifest-validation` gains a media-type leg; its `PPTX` negative is unchanged. Rides 2.36.0; no pin bump.
- **RFC 0206 — locale-key grammar.** New corpus-gate test `src/coherence/locale-key-grammar.test.ts` mints the six `(corpus)` rows `openwop.requirement.0206.key-grammar`, `.key-grammar-negatives`, `.wire-agreement` (twelve pattern sites across the eight wire files), `.superset` (all 676 + 676×676 RFC 0103 strings, v1 and v2), `.script-family-fallback` and `.old-sections-unchanged` (163 key sets × 8 negotiated tags against a frozen RFC 0103 copy). Sabotage run once each: the old pattern in one v1 schema, the case-insensitive capabilities pattern, `api/openapi.yaml` on the old pattern, a 3-letter-language pattern, the script step deleted, the step widened to any 2-subtag prefix — each turns its row red. New major-2 scenario `v2-content-locale-keys` (`0206.delivery-extended-locale`) writes and delivers an advertised content locale outside the RFC 0103 subset; `inapplicable` with its reason otherwise (no current host advertises one). `resolveSection` is hoisted to `src/lib/localized-content.ts` (one copy, RFC 0206 §B.4, plus the frozen `resolveSectionRfc0103`); `localized-content-delivery` gains four supplementary, non-gating legs. `locale-key-grammar` is added to the certification-bundle id exclusion list.
- **`src/coherence/node-pack-mcp-server-record.test.ts` — RFC 0203's three corpus rows** (`openwop.requirement.0203.mcpserver-subset`, `.language-remote-only`, `.entry-binding`; spec-repo CI only, never a host bundle). Positives validate against `schemas/v2/node-pack-manifest.schema.json` and their `mcpServer` against a vendored upstream MCP Registry `server.json` 2025-12-11 schema pinned by sha256 (`fixtures/node-pack-runtime/upstream/`); ten subset negatives are refused, each only inside `/runtime`; twelve boundary probes check that anything the v2 schema accepts the upstream schema accepts too. `mcpServer` under `wasm` is refused (with a control); an `entry` ≠ `remotes[0].url` is schema-valid and refused by the new reference validator `src/lib/node-pack-runtime.ts` (`remoteEntryBinding`) with `pack_validation_failed`. Sabotage, run once each: `sse` in the remote type, open remote item, open record, `^https?://`, `description` 200, `maxItems` dropped, `if`/`else` deleted, validator equality removed — every one turns a row red. Fixtures catalogued in `fixtures.md` §"Node-pack runtime fixtures". No version bump (rides the open 2.36.0).
...
- **RFC 0201 — four `v2-webhook-*` scenarios for the Standard Webhooks companion scheme**, each gated on `webhooks.signatureAlgorithms` listing `standard-webhooks-1` (`inapplicable` otherwise; `blocked` when the egress guard refuses the loopback receiver). `v2-webhook-standard-webhooks-delivery` (`0201.opt-in-only`, `0201.registration-validated`, `0201.delivery-signed`), `v2-webhook-message-id-stable` (`0201.message-id-stable`), `v2-webhook-endpoint-verification` (`0201.endpoint-verification`), `v2-webhook-secret-rotation` (`0201.secret-rotation`, also gated on `webhooks.secretRotation`). `lib/webhook-receiver.ts` gains a Standard Webhooks verifier written from the 1.0.0 spec recipe — not the upstream library, so a library bug cannot pass a leg — unit-tested against the upstream library's own `sign` vector, and a path-routed modal receiver (`echo` / `no-echo` / `wrong-echo` / `redirect` / `fail2`) so every mode travels through one tunnelled URL. Every row's sabotage was run against the v2 reference host on loopback and fails (openwop-examples PR). No scenario added to any profile floor; no pin bump (D3).
- **RFC 0209 — `ui.a2ui-surface` schema version 2 (A2UI v0.9).** New corpus-gate test `src/coherence/a2ui-v09-profile.test.ts` mints `0209.profile-subset-of-upstream`, `.action-confined`, `.no-secret-input`, `.no-egress-components`, `.catalog-pinned` and `.kind-carve-out` into `evidence/corpus-ledger.json`: every positive validates against `$defs/payloadV2` and each of its messages against the vendored upstream A2UI v0.9 schemas (SHA-256-pinned), and every negative must fail the profile AND pass a copy of the profile with only the restriction under test removed, so it can fail for no other reason (six of the eight are accepted by upstream — the refusal is the profile's). New major-2 scenario `v2-a2ui-v09-surface.test.ts`, gated on the seams profile, a `schemaVersions.kinds["ui.a2ui-surface"]` floor and the new `emit-surface` seam (absent ⇒ `inapplicable`): `0209.version-selects-branch`, `.fold-guarded`, `.catalog-equality`, `.taint-sticky` (with an all-trusted control that MUST advance), `.legacy-readable` (floor-1 hosts only), plus `surface-id-equality` and `recorded-as-recorded`. Fixtures `fixtures/a2ui-v09/` (one positive, eight negatives) and `fixtures/upstream/a2ui-v0.9/` (Apache-2.0, unmodified), both pinned in the pack-contents allowlist. **Not published** — the RFC's comment window was waived by steward override, and the suite waits for the steward's go.
- **RFC 0208 — three major-2 scenarios and one coherence test for the v2 A2A/MCP operation map.** `v2-a2a-operation-map` (gated on `a2a.profiles ∋ a2a-1.0`: contextId inferred, mismatch refused `-32602` with no state change, retained terminal `-32004`, unknown and foreign-tenant `GetTask` answered by the same `-32001`, the state projection read from `interop-map.json`, `-32002`/`-32009`, `SubscribeToTask` refused without `a2a.streaming`, and `ListTasks` scoping under `OPENWOP_TEST_TENANT_B_API_KEY`, `blocked` without it); `v2-mcp-mount-map` (gated on `mcp.serverMount` + `mcp-2026-07-28`: required features read from the map, `server/discover` revisions, header/body agreement, stateless lists, `CallToolResult.isError` for failed runs, `run.started.transport: mcp`, the MRTR server round trip with single-use `requestState`, and the six v1 host-as-server legs ported as v2 twins); `v2-implementation-informational` (server-free: `implementation.url` validates, an unknown key still fails); coherence `v2-interop-map-coherent` (`openwop.requirement.0208.map-coherent`: the gate passes and refuses six sabotaged maps). `fixtures.md` records the two suite requirements — the A2A interface routes exactly one skill, `conformance-approval` (A2A 1.0 carries no skill selector), and fixtures are exposed as MCP tools under their workflowId. Five invariants gain the v2 files in `tests:`. No version bump (the 2.36.0 cycle is open).
...
- **A transport failure inside the watch is now an unreadable observation, never a verdict:** a status that cannot be read is `null`, a log that cannot be read is `readable: false` — both already meant "conclude nothing, poll again" — and the budget still bounds the wait, so a host that never comes back still records no resumption. `Watch.transportErrors` counts them. Unit-tested in `lib/durability-watch.test.ts` (dies after answering and comes back → resumption observed; never comes back → none; an unreadable read never latches completed-un-re-executed).
- **One kill no longer contaminates the rest of the run.** Every seam row now waits for the host to answer at all before probing the seam, and the bound read retries once across a restart. Suite setup retried discovery twice back-to-back, giving a restarting host about a millisecond: each file set up during the restart logged "discovery unreadable" and recorded its fixture gates `blocked`. Setup now waits out a transport failure (refused / reset / closed) for up to 45 s, 1 s apart, without spending an attempt; an HTTP answer and a 20 s timeout still count as attempts, so a host that is up and wrong is not retried into looking right.
- False FAIL only. Suite patch; corpus release stays `2.34.0`. Reported by openwop-app.
...
- **`v2-webhook-delivery-shape` never unregistered anything, and behind a public front that failed a correct host.** On loopback each leg's receiver binds its own ephemeral port, so leg 1's leftover MAJOR-2 subscription delivered to a dead address and nobody noticed. Behind a tunnel every leg shares ONE URL on ONE pinned port: the still-live v2 subscription delivered its v2 rendering into leg 2, and the major-1 leg read it — *"a major-1 run.started payload MUST validate against the V1 definition … engineVersion must be string"* — on a host that had rendered both contracts correctly. It was the ONE failing row of the v2 reference host's first relaxation-free cut (241 pass / 1 fail / 0 blocked). **Reproduced with no ingress at all** by pinning `OPENWOP_WEBHOOK_RECEIVER_PORT` on a loopback run: ephemeral → 3/3, pinned → the major-1 leg fails; after the fix both → 3/3 and the host is left with **0** subscriptions. Every leg now unregisters what it registered, and — belt and braces — a delivery whose `OpenWOP-Webhook-Id` / `X-openwop-Webhook-Id` names a different subscription is not read as this leg's. The file also left a live subscription on every host it ever ran against; MyndHyve's operator had been sweeping them by hand after each tunnelled cut. The other major-2 webhook files were audited and already unregister.
- Suite patch: corpus release stays `2.33.0`.
...
- **A host advertising `a2a` or `mcp` could not be cut relaxation-free.** The A2A fake peer and MCP fake server advertised `http://127.0.0.1:<port>` and nothing else; only the webhook receiver had a public front. So such a host could be measured ONLY with its egress guard relaxed, and a relaxed guard cannot certify — the rule made a host implementing *more* of the protocol *less* able to certify. The two production hosts cut cleanly only because they advertise neither family. New `OPENWOP_A2A_FAKE_PEER_URL` / `OPENWOP_MCP_FAKE_SERVER_URL`, validated by the same rule as `OPENWOP_WEBHOOK_RECEIVER_URL` (now `resolvePublicFront(envName, …)`; `resolveRegistrationUrl` is a wrapper). `hostFacingEndpoint()` is what is handed to the host — every invoke seam's `peerUrl` / `serverUrl`, and the AgentCard's `url` fields — while the suite's own requests to its own fake keep using the loopback `endpoint()`. Twelve scenario files updated.
- **A DECLARED relaxation denied nothing on any real host.** `verifyBundleV3` matched a relaxed obligation to a profile by testing whether the profile **id** contained the family name; the emitter tested whether it contained the **whole obligation string** — two different rules under a comment saying they matched. No real v2 profile id contains a family name, so `webhooks.egress-guard` denied neither `openwop-core-standard` nor anything else. `v2-relaxation-recorded` stayed green because its fixture profile was called `openwop-webhooks`, the one id the rule could match. New `profilesRelaxedBy()` in `lib/v2-profiles.ts` reads ownership from where it was always stated — a profile's `predicate.families` in `spec/v2/profiles.json` — and is the ONE function both the emitter and the verifier call. The id-named arm is kept, so nothing denied today is un-denied. New leg `openwop.requirement.0173.relaxation-recorded.owned-by-registry` asserts against the real `openwop-core-standard`. **Sabotage-proven: restoring the old rule leaves the old leg green and reddens only the new one** — which is the blindness, reproduced.
- **NEW `v2-webhook-egress-refusal.test.ts` — the egress guard's refusal, at major 2, for the first time.** `webhooks.md` §SSRF is a plain MUST with a registered code, and no major-2 row asserted it: `http-client-ssrf` and `webhook-negative` never left the major-1 lane, so a v2 bundle could certify `openwop-core-standard` on a host whose guard refused nothing. Eight registration probes, one per class the sentence names (non-`https`, IPv4 and IPv6 loopback, `localhost`, two RFC 1918 ranges, link-local / cloud metadata, IPv6 ULA); each MUST answer `400 webhook_url_rejected`. Registration-time only — no probe is a destination the suite owns, nothing is delivered, an accepted registration is deleted at once. New id `openwop.requirement.0171.webhook-egress-refused`.
- **An UNDECLARED relaxation is now detectable, and denied.** This README said the suite "cannot detect an undeclared one". For the egress guard that was never true: a host that accepts `https://127.0.0.1/…` has shown on the wire that its guard is open. Three outcomes — all refused → `executed-pass`; accepted with a `webhooks.*` relaxation DECLARED → `inapplicable`, the declaration quoted, the owning profile denied by the declaration; accepted with NOTHING declared → `executed-fail`, which `profilesDeniedByObservedRelaxation()` turns into a denial of every profile built on `webhooks` in both the emitter (`certified: false`) and `--verify` (new rejection `undeclared-relaxation-observed`, scoped to the profile). Declaring nothing is no longer a way around the rule.
- **Measured on the v2 reference host in all three postures:** guarded → pass (8/8 refused, right code); `OPENWOP_WEBHOOK_ALLOW_PRIVATE=1` with nothing declared → **fail, "ACCEPTED 8 of 8"** — that flag opens the scheme check and the metadata address too, not only loopback; same flag with the relaxation declared → `inapplicable`. The first attempt at that measurement reported pass/pass/pass, because the harness had not killed the first host and all three runs hit the guarded one; it now refuses to measure while the port is held.
- `coverage.md`: new row; and the `v2-durability-recovery` row is corrected — it still said `duplicate-delivery` was "counted per effect identity" and that the log was read from `/events`, both wrong since 2.32.0.
- **Expect this release to un-certify a loopback cut.** That is its purpose. A host cut through public fronts (both production hosts) is unaffected.
- Suite pinned to corpus `2.33.0`.
...
- **`v2-durability-recovery`: both kill rows observe until the host's DECLARED recovery bound elapses.** They read the log once, the instant discovery answered again — demanding resumption within ~0 ms of the listener returning, a fast bound RFC 0158 rejects. A host whose sweeper first ticks 5 s after boot against a derived 65 s bound failed, then resumed correctly ten seconds later. The bound (`GET /host/durability/bound`) is a ceiling for waiting only; 60 s fallback when unserved; the governing per-class figure is read off the seam's own response (`recoveryBoundMs`) when the host names one. The observation ceiling is 240 s by default and **operator-raisable** (`OPENWOP_DURABILITY_OBSERVATION_CEILING_MS`): a fixed ceiling would have moved the single read's defect from 0 s to 240 s and made a §B.6-conformant 12.5-minute leased-class bound uncertifiable. A declared bound over the ceiling records `blocked` with that precondition named, not fail. The row detail records the observed kill → resumption interval.
...
- **`duplicate-delivery` counts the effect where it lands.** It counted rows per identity on `GET /runs/{id}/effects` — and a ledger keyed on effect identity admits at most one row per identity *by construction*, so the assertion held whatever the effect did. openwop-app sabotage-tested it before building to it: dedup claim forced to always win, then the effect emitted **twice per fire**; the per-identity count stayed 1 and the row stayed green. §C asks for invocation counts; the projection exposes identities. The scenario now starts its own receiver, passes `effectUrl` to the seam, and asserts **exactly one arrival**; the seam MUST stage work performing exactly one outbound effect addressed there and deliver it twice. The ledger read stays as a labelled consistency check. It also sends no `workflowId` (`conformance-noop` records no effect, so the row could only record `blocked`). Receiver reachability is a declared operator precondition, as for the webhook rows.
- **The run log is read from `GET /runs/{runId}/events/poll`, not `/events`.** At major 2 `/events` is `streamRunEvents`, SSE only; `pollRunEvents` is the JSON read. The scenario parsed the stream path as JSON, so on a host serving exactly what the OpenAPI states the read came back null and **`poison-exhaustion` resolved `executed-pass` with `partial-witness: blocked`** — its load-bearing clause never ran. Measured on the v2 reference host; openwop-app content-negotiates the stream path, which is why it never showed there. An unreadable log on a kill row is now `blocked`, never "nothing resumed".
- Four of the five durability defects were found by the openwop-app host reading the scenario against its own mechanism **before building the seam**; the fifth by the v2 reference host's first run. No host had witnessed the seam rows, so no bundle's seam rows are invalidated — but the reference host's committed `poison-exhaustion` pass was vacuous and is superseded by its next cut.
- **`--certify` no longer hands the bundle signing key to the vitest child.** The child's environment was `{ ...process.env }`, so `OPENWOP_BUNDLE_SIGNING_KEY` — the PKCS8 PEM — went with it, though only the parent signs. Reported by a host operator mid-cut: on macOS `npm exec` rewrites `process.title` and a process listing spills into the child's environ, so `ps` printed the signing key in full into a terminal and transcript. New `src/lib/child-env.ts`; both spawn sites use it, and a self-test asserts on `cli.ts`'s source that no child environment is a bare spread of `process.env`. The key **id** is not secret and stays. **If you listed processes during a `--certify` on ≤ 2.31.1, treat that key as seen and rotate it.**
...
- `aiProviders.input` / `.policies` restored to their v1 closed shapes — the v2 override had made both `additionalProperties: true` with no properties. Suite pinned to corpus `2.28.0`.
