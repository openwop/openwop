...
## [2.39.5] — unreleased — RFC 0215's prior-art survey and threat-model home ship in the contract peer
...
- **Version moved ahead of publication.** `@openwop/openwop-conformance` and its exact-pinned peer `@openwop/spec-artifacts` move to `2.39.5` because `2.39.4` is tagged, and RFC 0215's two closed gap rows change `spec/v1/gaps.json`, which the peer ships. Not tagged, not published.
