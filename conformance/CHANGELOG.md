...
## [2.38.0] — unreleased — the 2.38.0 cycle is open (RFC 0214 and RFC 0212 add legs and a scenario file, so the open 2.37.2 patch cycle becomes a minor)
...
- **Version moved ahead of publication.** `@openwop/openwop-conformance` and its exact-pinned peer `@openwop/spec-artifacts` move to `2.37.2` because `2.37.1` is tagged and publishing. Not tagged, not published.
