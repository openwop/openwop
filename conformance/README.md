...
Current source tree: 97 scenario files. Use [`coverage.md`](./coverage.md) for current grade/gap tracking.
...
| Auth profiles | Capability-shape scenarios for `openwop-auth-api-key-rotation`, `openwop-auth-oauth2-client-credentials`, and `openwop-auth-oidc-user-bearer` shipped 2026-05-11 under RFC 0010, with a synthetic OIDC issuer harness at `conformance/src/lib/oidc-issuer.ts`. Remaining: opt-in `auth-mtls.test.ts` scenario + live-IdP positive-path validation against at least one reference host. |
