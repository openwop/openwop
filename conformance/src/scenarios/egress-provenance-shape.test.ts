...
describe('egress-provenance-shape: CredentialProvenance (RFC 0079 §A, server-free)', () => {
  ...
  it('a conforming descriptor validates', () => {
    expect(
      ...
      req('openwop.it.egress-provenance-shape.a-conforming-descriptor-validates', 'host-capabilities.md §"Credential provenance + egress policy"', 'a conforming CredentialProvenance MUST validate'),
    ).toBe(true);
  });

  it('audiences: [] is rejected (minItems:1 — a credential needs ≥1 audience to bind)', () => {
    expect(validate({ credentialId: 'c', issuer: 'host', audiences: [] }), req('openwop.it.egress-provenance-shape.audiences-is-rejected-minitems-1-a-credential-needs-1-audience-to-bind', 'RFC 0079 §A', 'audiences MUST have ≥1 entry')).toBe(false);
  });

  it('a missing REQUIRED credentialId is rejected', () => {
    expect(validate({ issuer: 'host', audiences: ['a'] }), req('openwop.it.egress-provenance-shape.a-missing-required-credentialid-is-rejected', 'RFC 0079 §A', 'credentialId is REQUIRED')).toBe(false);
  });

  it('an unknown property is rejected (additionalProperties:false)', () => {
    expect(validate({ credentialId: 'c', issuer: 'host', audiences: ['a'], secretValue: 'sk-live-xxx' }), req('openwop.it.egress-provenance-shape.an-unknown-property-is-rejected-additionalproperties-false', 'RFC 0079 §A', 'CredentialProvenance MUST be additionalProperties:false')).toBe(false);
  });
  ...
  it('declares no secret-value property (egress-decision-no-secret-leak)', () => {
    ...
    for (const p of props) {
      expect(
        ...
        req('openwop.it.egress-provenance-shape.declares-no-secret-value-property-egress-decision-no-secret-leak', 'SECURITY invariant egress-decision-no-secret-leak', `CredentialProvenance MUST NOT declare a secret-bearing property (${p})`),
      ).toBe(false);
    }
  });
});
...
describe('egress-provenance-shape: egress.decided event (RFC 0079 §B, server-free)', () => {
  ...
  it('validates a content-free decision and enforces the decision enum + required fields', () => {
    expect(validate({ decision: 'allowed', destination: 'api.stripe.com', credentialId: 'cred-stripe-1', reason: 'ok' }), req('openwop.it.egress-provenance-shape.validates-a-content-free-decision-and-enforces-the-decision-enum-required-fields', 'RFC 0079 §B', 'a conforming egress.decided MUST validate')).toBe(true);
    expect(validate({ decision: 'denied', destination: 'attacker.example', credentialId: 'cred-stripe-1', reason: 'out-of-audience' }), req('openwop.it.egress-provenance-shape.validates-a-content-free-decision-and-enforces-the-decision-enum-required-fields', 'RFC 0079 §B', 'a denied decision MUST validate')).toBe(true);
    expect(validate({ decision: 'ok', destination: 'a' }), req('openwop.it.egress-provenance-shape.validates-a-content-free-decision-and-enforces-the-decision-enum-required-fields', 'RFC 0079 §B', 'the canonical allow value is "allowed", not "ok"')).toBe(false);
    expect(validate({ destination: 'a' }), req('openwop.it.egress-provenance-shape.validates-a-content-free-decision-and-enforces-the-decision-enum-required-fields', 'RFC 0079 §B', 'decision is REQUIRED')).toBe(false);
    expect(validate({ decision: 'allowed' }), req('openwop.it.egress-provenance-shape.validates-a-content-free-decision-and-enforces-the-decision-enum-required-fields', 'RFC 0079 §B', 'destination is REQUIRED')).toBe(false);
  });

  it('reason is a CLOSED enum — a free-form / URL-bearing reason is rejected (egress-decision-no-secret-leak)', () => {
    expect(validate({ decision: 'denied', destination: 'attacker.example', reason: 'out-of-audience' }), req('openwop.it.egress-provenance-shape.reason-is-a-closed-enum-a-free-form-url-bearing-reason-is-rejected-egress-decisi', 'egress-decision-no-secret-leak', 'a conventional reason code MUST validate')).toBe(true);
    expect(
      validate({ decision: 'denied', destination: 'attacker.example', reason: 'https://attacker.example/leak?token=sk-live-xxx' }),
      req('openwop.it.egress-provenance-shape.reason-is-a-closed-enum-a-free-form-url-bearing-reason-is-rejected-egress-decisi', 'egress-decision-no-secret-leak', 'a free-form / URL-bearing reason MUST be rejected (it would defeat the content-free guarantee)'),
    ).toBe(false);
  });
  ...
  it('the egressDecided $def declares no secret-value property', () => {
    ...
    for (const p of Object.keys(def)) {
      expect(SECRET_PROP_NAMES.includes(p.toLowerCase()), req('openwop.it.egress-provenance-shape.the-egressdecided-def-declares-no-secret-value-property', 'egress-decision-no-secret-leak', `egress.decided MUST NOT declare a secret-bearing property (${p})`)).toBe(false);
    }
  });
  ...
  it('egress.decided is in the RunEventType enum', () => {
    ...
    expect(enumVals.includes('egress.decided'), req('openwop.it.egress-provenance-shape.egress-decided-is-in-the-runeventtype-enum', 'run-event.schema.json', 'egress.decided MUST be in the RunEventType enum')).toBe(true);
  });
});
...
describe('egress-provenance-shape: capability advertisement (RFC 0079 §D, server-free)', () => {
  it('capabilities.httpClient.egressPolicy is declared with its sub-flags', () => {
    ...
    expect(egressPolicy, req('openwop.it.egress-provenance-shape.capabilities-httpclient-egresspolicy-is-declared-with-its-sub-flags', 'capabilities.md §httpClient', 'httpClient.egressPolicy MUST be declared')).toBeDefined();
    for (const flag of ['supported', 'decisions']) {
      expect(egressPolicy?.properties?.[flag], req('openwop.it.egress-provenance-shape.capabilities-httpclient-egresspolicy-is-declared-with-its-sub-flags', 'host-capabilities.md §"Credential provenance + egress policy"', `egressPolicy.${flag} MUST be declared`)).toBeDefined();
    }
  });
});
