...
describe('discovery: /.well-known/openwop', () => {
  it('returns 200 with required Capabilities fields per capabilities.md §2', async () => {
    ...
    expect(res.status, req('openwop.it.discovery.returns-200-with-required-capabilities-fields-per-capabilities-md-2', 
      'capabilities.md §2',
      'discovery endpoint MUST be reachable without auth and return 200',
    )).toBe(200);

    const body = res.json as Record<string, unknown> | undefined;
    expect(body, req('openwop.it.discovery.returns-200-with-required-capabilities-fields-per-capabilities-md-2', 'capabilities.md §2', 'response MUST be JSON')).toBeDefined();
    ...
    for (const required of ['protocolVersion', 'supportedEnvelopes', 'schemaVersions', 'limits']) {
      expect(body?.[required], req('openwop.it.discovery.returns-200-with-required-capabilities-fields-per-capabilities-md-2', 
        'capabilities.md §3',
  ...
  it('serves Cache-Control per capabilities.md §4 (caching guidance)', async () => {
    ...
    expect(cacheControl, req('openwop.it.discovery.serves-cache-control-per-capabilities-md-4-caching-guidance', 
      'capabilities.md §4',
  ...
  it('IF a standard ETag is present, a matching If-None-Match yields 304 (RFC 0165 §C.2 — presence-gated)', async () => {
    ...
    expect(etag.trim().length, req('openwop.it.discovery.if-a-standard-etag-is-present-a-matching-if-none-match-yields-304-rfc-0165-c-2-p', 'RFC 0165 §C.2', 'ETag, when present, MUST be a non-empty validator')).toBeGreaterThan(0);
    const second = await driver.get('/.well-known/openwop', { authenticated: false, headers: { 'If-None-Match': etag } });
    expect(second.status, req('openwop.it.discovery.if-a-standard-etag-is-present-a-matching-if-none-match-yields-304-rfc-0165-c-2-p', 'capabilities-change-detection.md §"Cache validators"', 'a matching If-None-Match SHOULD yield 304 Not Modified')).toBe(304);
  });
  ...
  it('IF Capabilities-Etag is present, it is non-empty and stable within the cache window', async () => {
    ...
    if (firstEtag === null) {
      expect(firstEtag, req('openwop.it.discovery.if-capabilities-etag-is-present-it-is-non-empty-and-stable-within-the-cache-wind', 
    ...
    expect(secondEtag, req('openwop.it.discovery.if-capabilities-etag-is-present-it-is-non-empty-and-stable-within-the-cache-wind', 
  ...
  it('declares non-zero limits per capabilities.md §3 (CapabilityLimiter shape)', async () => {
    ...
    expect(limits, req('openwop.it.discovery.declares-non-zero-limits-per-capabilities-md-3-capabilitylimiter-shape', 
      'capabilities.md §3',
    ...
    for (const k of ['clarificationRounds', 'schemaRounds', 'envelopesPerTurn']) {
      const v = limits?.[k];
      expect(typeof v, req('openwop.it.discovery.declares-non-zero-limits-per-capabilities-md-3-capabilitylimiter-shape', 
        'capabilities.md §3',
        `limits.${k} MUST be a non-negative integer`,
      )).toBe('number');
      expect(v ?? -1, req('openwop.it.discovery.declares-non-zero-limits-per-capabilities-md-3-capabilitylimiter-shape', 
        'capabilities.md §3',
        `limits.${k} MUST be >= 0`,
...
describe('discovery: /.well-known/openwop fixtures field shape per RFC 0003', () => {
  it('IF fixtures is present, it MUST be a string[] of unique non-empty entries', async () => {
    ...
    for (const entry of arr) {
      expect(typeof entry, req('openwop.it.discovery.if-fixtures-is-present-it-must-be-a-string-of-unique-non-empty-entries', 
        'capabilities.md §`fixtures` (RFC 0003)',
        'every fixtures entry MUST be a string',
      )).toBe('string');
      expect((entry as string).length, req('openwop.it.discovery.if-fixtures-is-present-it-must-be-a-string-of-unique-non-empty-entries', 
        'capabilities.md §`fixtures` (RFC 0003)',
    ...
    expect(unique.size, req('openwop.it.discovery.if-fixtures-is-present-it-must-be-a-string-of-unique-non-empty-entries', 
      'capabilities.md §`fixtures` (RFC 0003)',
...
describe('discovery: /v1/openapi.json', () => {
  it('returns 200 with a parseable OpenAPI 3.1 document', async () => {
    ...
    expect(res.status, req('openwop.it.discovery.returns-200-with-a-parseable-openapi-3-1-document', 
      'rest-endpoints.md',
      'self-describing OpenAPI endpoint MUST return 200',
    )).toBe(200);

    const body = res.json as { openapi?: string } | undefined;
    expect(body?.openapi, req('openwop.it.discovery.returns-200-with-a-parseable-openapi-3-1-document', 
      'rest-endpoints.md',
...
describe('discovery: auth-scoped capability shape', () => {
  it('host claiming auth-scoped discovery advertises required fields', async () => {
    ...
    expect(disc?.authScoped?.supported, req('openwop.it.discovery.host-claiming-auth-scoped-discovery-advertises-required-fields', 
    ...
    if (disc?.authScoped?.mode !== undefined) {
      expect(
        ...
        req('openwop.it.discovery.host-claiming-auth-scoped-discovery-advertises-required-fields', 
...
describe('discovery: auth-scoped view satisfies base schema', () => {
  it('authenticated discovery preserves required Capabilities fields', async () => {
    ...
    expect(res.status, req('openwop.it.discovery.authenticated-discovery-preserves-required-capabilities-fields', 
      'capabilities-change-detection.md §"Scoped capability views"',
      'authenticated discovery MUST return 200',
    )).toBe(200);

    const body = res.json as Record<string, unknown> | undefined;
    expect(body, req('openwop.it.discovery.authenticated-discovery-preserves-required-capabilities-fields', 'capabilities-change-detection.md §"Scoped capability views"', 'authenticated discovery body MUST be JSON')).toBeDefined();
    ...
    for (const required of [
      ...
      expect(body?.[required], req('openwop.it.discovery.authenticated-discovery-preserves-required-capabilities-fields', 
        'capabilities-change-detection.md §"Scoped capability views"',
...
describe('discovery: auth-scoped is not an authorization oracle', () => {
  it('unauthorized key MUST NOT reveal capabilities outside its authorization', async () => {
    ...
    if (!unauthorizedKey) {
      ...
      return softSkip('blocked', 'precondition not met — `!unauthorizedKey` returned early ([discovery: auth-scoped] OPENWOP_TEST_UNAUTHORIZED_API_KEY not supplied; skipping authorization-oracle probe) (seam, prior step, or fixture unavailable)');
    }
    ...
    if (unauthorized.status === 401 || unauthorized.status === 403) {
      ...
      return softSkip('blocked', 'precondition not met — `unauthorized.status === 401 || unauthorized.status === 403` returned early (Host rejected the unauthorized key outright — that\'s fine. The oracle probe is moot when the host refuses the bearer.…');
    }
    ...
    expect(extras.length, req('openwop.it.discovery.unauthorized-key-must-not-reveal-capabilities-outside-its-authorization', 
      'capabilities-change-detection.md §"Scoped capability views"',
