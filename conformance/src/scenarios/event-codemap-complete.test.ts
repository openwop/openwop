...
describe.skipIf(CODEMAP === null || !existsSync(CODEMAP))('event-codemap-complete: spec/v1/event-codemap.json ↔ run-event.schema.json (corpus coherence)', () => {
  ...
  it('every RunEventType enum member has exactly one row, and every row names an enum member', () => {
    ...
    for (const t of enumTypes) expect(rowsByV1.get(t), `enum member ${t} MUST have exactly one codemap row`).toBe(1);
    for (const r of map.rows) expect(enumTypes, `codemap row ${r.v1} MUST be an enum member`).toContain(r.v1);
    expect(map.rows.length).toBe(enumTypes.length);
  });

  it('every payloadDef exists in run-event-payloads.schema.json $defs', () => {
    for (const r of map.rows) {
      expect(r.payloadDef, `${r.v1} MUST be keyed to a payload $def`).not.toBeNull();
      expect(defs[r.payloadDef as string], `${r.v1}: $defs.${r.payloadDef} MUST exist`).toBeDefined();
    }
  });
  ...
  it('every proposed v2 name is unique, grammatical, two segments, and never under the reserved `core.` prefix', () => {
    ...
    for (const r of map.rows) {
      expect(grammar.test(r.v2), `${r.v1} → ${r.v2} MUST match the v2 naming grammar`).toBe(true);
      expect(r.v2.split('.').length, `${r.v2} MUST have exactly two segments`).toBe(2);
      expect(r.v2.startsWith('core.'), `${r.v2} MUST NOT use the reserved core. prefix`).toBe(false);
      expect(seen.has(r.v2), `${r.v2} is proposed for two different v1 types`).toBe(false);
      seen.add(r.v2);
    }
  });
});
