/**
 * v2-umbrella-codemod-legs — RFC 0167 §D.3 (corpus wrapper).
 *
...
describe('v2-umbrella-codemod-legs (RFC 0167 §D.3)', () => {
  it('the corpus gate script exits 0', () => {
    if (V1_DIR === null) return softSkip('inapplicable', 'not a spec checkout');
    const r0 = spawnSync('node', [join(root, 'scripts', 'check-codemods.mjs')], { cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 });
    expect(r0.status, req('openwop.requirement.0167.codemod-legs', 'RFC 0167 §D.3', `every codemod carries positive, negative, idempotence and self-sabotage legs — ${tail(r0)}`)).toBe(0);
  }, 180_000);
});
