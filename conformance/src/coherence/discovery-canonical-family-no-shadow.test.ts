...
describe.skipIf(V1_DIR === null)('RFC 0149 §E — canonical families are not shadowed', () => {
  ...
  it('the canonical family list and the example corpus are both non-empty', () => {
    ...
    expect(canonicalFamilies().size, req('openwop.it.discovery-canonical-family-no-shadow.the-canonical-family-list-and-the-example-corpus-are-both-non-empty', 'RFC 0149 §E', 'capabilities.schema.json MUST declare families')).toBeGreaterThan(50);
    expect(fencedObjects(v1Dir).length, req('openwop.it.discovery-canonical-family-no-shadow.the-canonical-family-list-and-the-example-corpus-are-both-non-empty', 'RFC 0149 §E', 'spec/v1 MUST contain parseable json examples')).toBeGreaterThan(20);
  });
  ...
  it('no vendor-namespaced object re-declares a canonical family', () => {
    ...
    expect(
      ...
      req('openwop.it.discovery-canonical-family-no-shadow.no-vendor-namespaced-object-re-declares-a-canonical-family', 'RFC 0149 §E', 'RFC 0149 §E: a canonical family name inside `x-host-*` / `vendor.*` / `private.*` shadows ' +
  ...
  it('no discovery example carries credential material', () => {
    ...
    expect(
      findings,
      req('openwop.it.discovery-canonical-family-no-shadow.no-discovery-example-carries-credential-material', 'RFC 0149 §E', 'RFC 0149 §E: discovery is served credential-free to anonymous callers, so a real-shaped ' +
