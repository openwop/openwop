/**
 * v2-umbrella-codemod-at-active — RFC 0167 §D.3 (corpus wrapper).
 *
 ...
 *
 * @see RFCS/0167-openwop-v2-umbrella.md §D.3
 * @see scripts/check-codemods.mjs
 */
...
describe('v2-umbrella-codemod-at-active (RFC 0167 §D.3)', () => {
  it('the corpus gate script exits 0', () => {
    if (V1_DIR === null) return softSkip('inapplicable', 'not a spec checkout');
    const r0 = spawnSync('node', [join(root, 'scripts', 'check-codemods.mjs'), '--at-active'], { cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 });
    expect(r0.status, req('openwop.requirement.0167.codemod-at-active', 'RFC 0167 §D.3', `every codemod-able row is codemod-able while its RFC is still Active — ${tail(r0)}`)).toBe(0);
  }, 180_000);
});
