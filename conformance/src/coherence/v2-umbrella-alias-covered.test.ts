/**
 * v2-umbrella-alias-covered — RFC 0167 §E.2 (corpus wrapper).
 *
 ...
 *
 * @see RFCS/0167-openwop-v2-umbrella.md §E.2
 * @see scripts/check-alias-coverage.mjs
 */
...
describe('v2-umbrella-alias-covered (RFC 0167 §E.2)', () => {
  it('the corpus gate script exits 0', () => {
    if (V1_DIR === null) return softSkip('inapplicable', 'not a spec checkout');
    const r0 = spawnSync('node', [join(root, 'scripts', 'check-alias-coverage.mjs')], { cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 });
    expect(r0.status, req('openwop.requirement.0167.alias-covered', 'RFC 0167 §E.2', `every visible-kind deprecation row has an alias detector — ${tail(r0)}`)).toBe(0);
  }, 180_000);
});
