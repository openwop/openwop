...
describe.skipIf(V1_DIR === null)('RFC 0150 §B — Layer-2 effect identity is retry-stable', () => {
  ...
  it('the normative composition block is found at all', () => {
    ...
    expect(
      ...
      req('openwop.it.effect-identity-composition.the-normative-composition-block-is-found-at-all', 'RFC 0150 §B', `spec/v1/idempotency.md MUST carry a fenced key composition under "${HEADING}"`),
    ).not.toBeNull();
  });
  ...
  it('the retry counter does not participate in the identity', () => {
    ...
    expect(
      ...
      req('openwop.it.effect-identity-composition.the-retry-counter-does-not-participate-in-the-identity', 'RFC 0150 §B', 'RFC 0150 §B: `attempt` is separate telemetry and MUST NOT participate in the logical ID. ' +
  ...
  it('the identity is domain-separated and version-tagged', () => {
    ...
    expect(
      block,
      req('openwop.it.effect-identity-composition.the-identity-is-domain-separated-and-version-tagged', 'RFC 0150 §B', 'RFC 0150 §B: the preimage MUST open with the `openwop:activity:v2` domain tag so a v1 ' +
  ...
  it('the identity binds the tenant', () => {
    ...
    expect(
      block,
      req('openwop.it.effect-identity-composition.the-identity-binds-the-tenant', 'RFC 0150 §B', 'RFC 0150 §B: `tenantId` is part of the preimage. Without it two tenants that collide on ' +
  ...
  it('the ordinal is documented as stable across retries', () => {
    const block = compositionBlock(doc) ?? '';
    expect(block, req('openwop.it.effect-identity-composition.the-ordinal-is-documented-as-stable-across-retries', 'RFC 0150 §B', 'RFC 0150 §B: the preimage carries `logicalInvocationOrdinal`')).toContain(
  ...
  it('the composition agrees with the claim that a retried call reuses the identity', () => {
    ...
    expect(
      ...
      req('openwop.it.effect-identity-composition.the-composition-agrees-with-the-claim-that-a-retried-call-reuses-the-identity', 'RFC 0150 §B', 'spec/v1/idempotency.md §"Composition: how the layers compose" MUST keep the guarantee ' +
