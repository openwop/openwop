...
describe('RFC 0152 §A — A2A versioned discovery', () => {
  ...
  it('a versioned advertisement validates', () => {
    expect(
      ...
      req('openwop.it.versioned-composition-profiles.a-versioned-advertisement-validates', 'RFC 0152', JSON.stringify(validate.errors)),
    ).toBe(true);
  });
  ...
  it('a legacy-only host can say so explicitly', () => {
    ...
    expect(
      ...
      req('openwop.it.versioned-composition-profiles.a-legacy-only-host-can-say-so-explicitly', 'RFC 0152', JSON.stringify(validate.errors)),
    ).toBe(true);
  });
  ...
  it('an empty version list is rejected', () => {
    expect(
      ...
      req('openwop.it.versioned-composition-profiles.an-empty-version-list-is-rejected', 'RFC 0152', 'RFC 0152 §A: an A2A-capable host MUST advertise a NON-EMPTY `protocolVersions`. An empty ' +
  ...
  it('duplicate versions are rejected', () => {
    expect(validate(a2a({ protocolVersions: ['1.0', '1.0'] })), req('openwop.it.versioned-composition-profiles.duplicate-versions-are-rejected', 'RFC 0152', 'duplicate versions are rejected')).toBe(false);
  });
  ...
  it('a malformed version is rejected', () => {
    for (const v of ['1', 'v1.0', 'latest', '1.0.0']) {
      expect(
        ...
        req('openwop.it.versioned-composition-profiles.a-malformed-version-is-rejected', 'RFC 0152', `RFC 0152 §A: '${v}' is not an A2A major.minor version`),
      ).toBe(false);
    }
  });

  it('a malformed profile id is rejected', () => {
    expect(validate(a2a({ protocolVersions: ['1.0'], profiles: ['a2a-latest'] })), req('openwop.it.versioned-composition-profiles.a-malformed-profile-id-is-rejected', 'RFC 0152', 'a malformed profile id is rejected')).toBe(false);
...
describe('RFC 0153 §A — MCP versioned discovery', () => {
  ...
  it('a versioned advertisement validates', () => {
    expect(
      ...
      req('openwop.it.versioned-composition-profiles.a-versioned-advertisement-validates~2', 'RFC 0152', JSON.stringify(validate.errors)),
    ).toBe(true);
  });
  ...
  it('the v1.0 baseline `serverUrls` (required by mcp-discoverability when supported:true) validates — path or absolute URL, non-empty, no duplicates', () => {
    ...
    expect(validate({ supported: true, serverUrls: [] }), req('openwop.it.versioned-composition-profiles.the-v1-0-baseline-serverurls-required-by-mcp-discoverability-when-supported-true', 'RFC 0152', 'empty serverUrls is the bare `supported: true` problem')).toBe(false);
    expect(validate({ supported: true, serverUrls: ['/a', '/a'] }), req('openwop.it.versioned-composition-profiles.the-v1-0-baseline-serverurls-required-by-mcp-discoverability-when-supported-true', 'RFC 0152', 'duplicates')).toBe(false);
    expect(validate({ supported: true, serverUrls: [''] }), req('openwop.it.versioned-composition-profiles.the-v1-0-baseline-serverurls-required-by-mcp-discoverability-when-supported-true', 'RFC 0152', 'an empty string is not a mount')).toBe(false);
  });
  ...
  it('versions use MCP date form exactly', () => {
    ...
    for (const v of ['latest', '2026-7-28', '2026-07', 'v2026-07-28']) {
      expect(
        ...
        req('openwop.it.versioned-composition-profiles.versions-use-mcp-date-form-exactly', 'RFC 0152', `RFC 0153 §A: '${v}' is not MCP's date form`),
      ).toBe(false);
    }
  ...
  it('the feature list is closed', () => {
    expect(
      ...
      req('openwop.it.versioned-composition-profiles.the-feature-list-is-closed', 'RFC 0152', 'RFC 0153 §A: an unrecognized feature name is indistinguishable from a typo, and a peer ' +
        'that silently ignores one has negotiated a capability neither side implements.'),
    ).toBe(false);
  });

  it('an empty version list is rejected', () => {
    expect(validate({ supported: true, protocolVersions: [] }), req('openwop.it.versioned-composition-profiles.an-empty-version-list-is-rejected~2', 'RFC 0152', 'an empty version list is rejected')).toBe(false);
  });
});
  ...
  it('both RFCs keep their upstream-peer acceptance items unticked and annotated', () => {
    ...
    for (const [file, needle] of [
      ...
      expect(
        ...
        req('openwop.it.versioned-composition-profiles.both-rfcs-keep-their-upstream-peer-acceptance-items-unticked-and-annotated', 'RFC 0152', `${file}: the upstream-peer item MUST remain unticked and annotated — no peer is contacted ` +
