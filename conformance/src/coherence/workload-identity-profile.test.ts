...
describe('RFC 0154 §A — workload identity shape', () => {
  ...
  it('a verified identity validates', () => {
    expect(
      ...
      req('openwop.it.workload-identity-profile.a-verified-identity-validates', 'RFC 0154 §A/§B', JSON.stringify(validate.errors)),
    ).toBe(true);
  });
  ...
  it('the object is closed — credential material cannot be smuggled in', () => {
    ...
    for (const extra of [
      ...
      expect(
        ...
        req('openwop.it.workload-identity-profile.the-object-is-closed-credential-material-cannot-be-smuggled-in', 'RFC 0154 §A/§B', `RFC 0154 §A: raw credentials MUST NOT enter the identity object — rejected \`${Object.keys(extra)[0]}\``),
      ).toBe(false);
    }
  });
  ...
  it('the scheme is a closed enum', () => {
    expect(
      ...
      req('openwop.it.workload-identity-profile.the-scheme-is-a-closed-enum', 'RFC 0154 §A/§B', 'RFC 0154 §A: an unrecognized scheme is a verification path nobody implemented. Accepting ' +
        'the NAME without the verification is exactly the failure this profile prevents.'),
    ).toBe(false);
  });

  it('subject is required and non-empty', () => {
    expect(validate({ scheme: 'spiffe' }), req('openwop.it.workload-identity-profile.subject-is-required-and-non-empty', 'RFC 0154 §A/§B', 'subject is required and non-empty')).toBe(false);
    expect(validate({ scheme: 'spiffe', subject: '' })).toBe(false);
  });

  it('a key binding carries a digest reference, never a key', () => {
    expect(validate({ scheme: 'mtls-san', subject: 'x', keyBinding: { method: 'mtls', thumbprintRef: DIGEST } }), req('openwop.it.workload-identity-profile.a-key-binding-carries-a-digest-reference-never-a-key', 'RFC 0154 §A/§B', 'a key binding carries a digest reference, never a key')).toBe(true);
    expect(
...
describe('RFC 0154 §B — delegated actor chain', () => {
  ...
  it('a verified delegation validates', () => {
    expect(
      ...
      req('openwop.it.workload-identity-profile.a-verified-delegation-validates', 'RFC 0154 §A/§B', JSON.stringify(validate.errors)),
    ).toBe(true);
  });
  ...
  it('audience is required on a delegation', () => {
    ...
    expect(
      ...
      req('openwop.it.workload-identity-profile.audience-is-required-on-a-delegation', 'RFC 0154 §A/§B', 'RFC 0154 §B: a delegation without an audience cannot be checked against this host.'),
    ).toBe(false);
  });
  ...
  it('an empty chain is rejected', () => {
    expect(
      ...
      req('openwop.it.workload-identity-profile.an-empty-chain-is-rejected', 'RFC 0154 §A/§B', 'RFC 0154 §B: a delegation context with no chain claims a delegation nobody can inspect.'),
    ).toBe(false);
  });
  ...
  it('proofRef is a digest reference, not the proof', () => {
    expect(
      ...
      req('openwop.it.workload-identity-profile.proofref-is-a-digest-reference-not-the-proof', 'RFC 0154 §A/§B', 'RFC 0154 §B: `proofRef` is a REFERENCE. A JWT pasted here would put the proof itself into ' +
        'every event and span this object reaches.'),
    ).toBe(false);
  });

  it('a chain hop cannot carry extra fields', () => {
    expect(validate({ ...base, delegation: { chain: [{ subject: 'x', token: 'y' }], audience: 'h' } }), req('openwop.it.workload-identity-profile.a-chain-hop-cannot-carry-extra-fields', 'RFC 0154 §A/§B', 'a chain hop cannot carry extra fields')).toBe(false);
  });
  ...
  it('a chain hop MAY carry its verified scopes, as a set of non-empty strings', () => {
    ...
    expect(validate({ ...base, delegation: { chain: [{ subject: 'x', scopes: ['runs:read'] }, { subject: 'y', scopes: ['runs:read'] }], audience: 'h' } }), req('openwop.it.workload-identity-profile.a-chain-hop-may-carry-its-verified-scopes-as-a-set-of-non-empty-strings', 'RFC 0154 §A/§B', 'a chain hop MAY carry its verified scopes, as a set of non-empty strings')).toBe(true);
  ...
  it('the behavioral and external-audit items stay unticked and annotated', () => {
    ...
    for (const needle of [
      ...
      expect(
        ...
        req('openwop.it.workload-identity-profile.the-behavioral-and-external-audit-items-stay-unticked-and-annotated', 'RFC 0154 §A/§B', `RFC 0154: "${needle}" MUST remain unticked and annotated — a schema cannot demonstrate ` +
