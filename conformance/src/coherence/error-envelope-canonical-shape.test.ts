...
describe('S22 — the canonical HTTP error envelope is flat', () => {
  ...
  it('the schema says `error` is a string, requires `message`, and forbids other top-level keys', () => {
    expect(validate({ error: 'runner_unavailable', message: 'no runner', details: { retriable: true } }), req('openwop.it.error-envelope-canonical-shape.the-schema-says-error-is-a-string-requires-message-and-forbids-other-top-level-k', 'rest-endpoints.md', 'the schema says `error` is a string, requires `message`, and forbids other top-level keys')).toBe(true);
  ...
  it('readErrorCode / readRetriable read the canonical shape, and tolerate the legacy nested shape only as legacy', () => {
    ...
    expect(readErrorCode(flat), req('openwop.it.error-envelope-canonical-shape.readerrorcode-readretriable-read-the-canonical-shape-and-tolerate-the-legacy-nes', 'rest-endpoints.md', 'readErrorCode / readRetriable read the canonical shape, and tolerate the legacy nested shape only as legacy')).toBe('interop_version_unsupported');
  ...
  it.skipIf(V1_DIR === null)('rest-endpoints.md states the precedence, the details.retriable convention, and carries no nested code-list entry', () => {
    ...
    expect(prose, req('openwop.it.error-envelope-canonical-shape.rest-endpoints-md-states-the-precedence-the-details-retriable-convention-and-car', 'rest-endpoints.md', 'rest-endpoints.md states the precedence, the details.retriable convention, and carries no nested code-list entry')).toContain('flat** shape above');
