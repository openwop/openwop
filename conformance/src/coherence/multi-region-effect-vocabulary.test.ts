...
describe.skipIf(V1_DIR === null)('RFC 0150 §D — multi-region effect vocabulary', () => {
  ...
  it('the idempotency capability family is found at all', () => {
    ...
    expect(idem, req('openwop.it.multi-region-effect-vocabulary.the-idempotency-capability-family-is-found-at-all', 'RFC 0150 §D', 'capabilities.schema.json MUST declare an `idempotency` family')).not.toBeNull();
    expect(
      literals(idem?.properties?.['crossRegion']).length,
      req('openwop.it.multi-region-effect-vocabulary.the-idempotency-capability-family-is-found-at-all', 'RFC 0150 §D', 'the `crossRegion` enum MUST be non-empty'),
    ).toBeGreaterThan(0);
  });
  ...
  it('crossRegion offers the three RFC 0150 §D postures', () => {
    ...
    expect(
      [...values].sort(),
      req('openwop.it.multi-region-effect-vocabulary.crossregion-offers-the-three-rfc-0150-d-postures', 'RFC 0150 §D', 'RFC 0150 §D: the canonical postures are `single-region` (no cross-region guarantee), ' +
  ...
  it('crossRegion does not carry `strict`, which was a latency claim in a safety slot', () => {
    ...
    expect(
      ...
      req('openwop.it.multi-region-effect-vocabulary.crossregion-does-not-carry-strict-which-was-a-latency-claim-in-a-safety-slot', 'RFC 0150 §D', 'RFC 0150 §D: `strict` promised bounded read-visibility, not effect authorization — a host ' +
  ...
  it('partitionRecoveryStrategy offers no time-ordered rule', () => {
    ...
    expect(
      timeOrdered,
      req('openwop.it.multi-region-effect-vocabulary.partitionrecoverystrategy-offers-no-time-ordered-rule', 'RFC 0150 §D', 'RFC 0150 §D: under a partition there is no shared clock, so a time-ordered rule cannot ' +
  ...
  it('partitionRecoveryStrategy names the rule the annex actually requires', () => {
    ...
    expect(
      values,
      req('openwop.it.multi-region-effect-vocabulary.partitionrecoverystrategy-names-the-rule-the-annex-actually-requires', 'RFC 0150 §D', 'the annex MUSTs lex-min(runId) convergence, so that rule MUST be nameable in the ' +
  ...
  it('the spec states that reconciliation does not authorize effects', () => {
    ...
    expect(
      ...
      req('openwop.it.multi-region-effect-vocabulary.the-spec-states-that-reconciliation-does-not-authorize-effects', 'RFC 0150 §D', 'RFC 0150 §D: run-record reconciliation and permission to issue effects are separate. ' +
  ...
  it('a host that can neither fence nor rely on provider dedup must say so', () => {
    ...
    expect(
      ...
      req('openwop.it.multi-region-effect-vocabulary.a-host-that-can-neither-fence-nor-rely-on-provider-dedup-must-say-so', 'RFC 0150 §D', 'RFC 0150 §D: absent a fencing token or a provider guaranteeing duplicate suppression, a ' +
  ...
  it('the capability advertisement example parses as JSON', () => {
    ...
    expect(blocks.length, req('openwop.it.multi-region-effect-vocabulary.the-capability-advertisement-example-parses-as-json', 'RFC 0150 §D', 'idempotency.md MUST contain fenced json examples to check')).toBeGreaterThan(0);
