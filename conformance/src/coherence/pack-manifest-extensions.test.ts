...
describe('pack-manifest-extensions: every manifest admits the hatch (RFC 0138, server-free)', () => {
  it('every pack kind is covered — the enumeration is not a naming-convention glob', () => {
    ...
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.every-pack-kind-is-covered-the-enumeration-is-not-a-naming-convention-glob', 'RFC 0138', `expected 9 in-scope manifests, found ${manifestFiles.length}: ${manifestFiles.join(', ')}. A new pack kind MUST be added here, or listed in OUT_OF_SCOPE with a reason.`),
    ).toBe(9);
  });
  ...
  for (const f of manifestFiles) {
    it(`${f.replace(/-?(pack-)?manifest\.schema\.json$/, '')} — manifest ROOT carries the extension hatch`, () => {
      ...
      expect(
        ...
        req('openwop.it.pack-manifest-extensions.manifest-root-carries-the-extension-hatch', 'node-packs.md §Vendor extensions on pack manifests', `${f} root MUST admit ^(x-|vendor\\.) so the host-extensions.md opacity MUST is satisfiable`),
      ).toBe(true);
    });
  }
});
...
describe('pack-manifest-extensions: the hatch is NARROW — typos still rejected (RFC 0138)', () => {
  ...
  it('an `x-` extension on an entry VALIDATES (the motivating case)', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.an-x-extension-on-an-entry-validates-the-motivating-case', 'node-packs.md §Vendor extensions on pack manifests', 'a host extension MUST NOT make the pack unpublishable'),
    ).toBe(true);
  });
  ...
  it('a `vendor.` extension on an entry VALIDATES', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.a-vendor-extension-on-an-entry-validates', 'host-extensions.md §Vendor-prefixed namespaces', 'vendor-prefixed fields are legitimate'),
    ).toBe(true);
  });
  ...
  it('a MISSPELLED canonical field is STILL REJECTED', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.a-misspelled-canonical-field-is-still-rejected', 
  ...
  it('an unextended pack still validates (RFC 0138 is additive)', () => {
    expect(validate(pack({})), req('openwop.it.pack-manifest-extensions.an-unextended-pack-still-validates-rfc-0138-is-additive', 'COMPATIBILITY.md §2.1', 'existing manifests validate unchanged')).toBe(true);
  });
    ...
    for (const f of manifestFiles) {
      for (const node of closedObjects(load(f))) {
        for (const pat of Object.keys((node['patternProperties'] as Record<string, unknown>) ?? {})) {
          ...
          expect(re.test('dispalyName'), req('openwop.it.pack-manifest-extensions.the-hatch-pattern-is-not-accidentally-permissive', 'RFC 0138', `${f}: pattern ${pat} MUST NOT match a bare canonical-looking key`)).toBe(false);
          expect(re.test('x-anything'), req('openwop.it.pack-manifest-extensions.the-hatch-pattern-is-not-accidentally-permissive', 'RFC 0138', `${f}: pattern ${pat} admits x- extensions`)).toBe(true);
        }
      }
    }
  });
...
describe('pack-manifest-extensions: the corpus states opacity + trust normatively (RFC 0138)', () => {
  ...
  it.skipIf(V1_DIR === null)('an unrecognized extension MUST be ignored, not rejected', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.an-unrecognized-extension-must-be-ignored-not-rejected', 'node-packs.md §Vendor extensions on pack manifests', 'unrecognized extensions MUST be ignored, never a rejection reason'),
    ).toBe(true);
  });
  ...
  it.skipIf(V1_DIR === null)('"ignore" is defined — not render, execute, interpret, or persist-for-later', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.ignore-is-defined-not-render-execute-interpret-or-persist-for-later', 'node-packs.md §Vendor extensions on pack manifests', '"ignore" means ignore — the hatch MUST NOT become a rendering or execution channel'),
    ).toBe(true);
    expect(
      /pack-authored content/i.test(packsDoc) && /untrusted/i.test(packsDoc),
      req('openwop.it.pack-manifest-extensions.ignore-is-defined-not-render-execute-interpret-or-persist-for-later', 'node-packs.md §Vendor extensions on pack manifests', 'an extension value is pack-authored, therefore untrusted'),
    ).toBe(true);
  });
  ...
  it.skipIf(V1_DIR === null)('extensions are NOT a capability-negotiation channel', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.extensions-are-not-a-capability-negotiation-channel', 'node-packs.md §Vendor extensions on pack manifests', 'a host MUST NOT infer support from an extension property'),
    ).toBe(true);
  });
  ...
  it.skipIf(V1_DIR === null)('host-extensions.md still carries the opacity MUST the hatch exists to satisfy', () => {
    expect(
      ...
      req('openwop.it.pack-manifest-extensions.host-extensions-md-still-carries-the-opacity-must-the-hatch-exists-to-satisfy', 'host-extensions.md §Vendor-prefixed namespaces', 'the MUST that motivated RFC 0138 is still present'),
    ).toBe(true);
  });
});
