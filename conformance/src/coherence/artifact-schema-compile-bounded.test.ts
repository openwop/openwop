...
describe('artifact-schema-compile-bounded: contract present in the corpus (RFC 0071, server-free)', () => {
  ...
  it.skipIf(V1_DIR === null)('artifact-type-packs.md declares the bounded-compilation MUST', () => {
    expect(
      ...
      req('openwop.it.artifact-schema-compile-bounded.artifact-type-packs-md-declares-the-bounded-compilation-must', 'artifact-type-packs.md', 'a "Bounded schema compilation" section MUST exist'),
    ).toBe(true);
    expect(
      /MUST bound/i.test(artifactDoc) && /MUST reject/i.test(artifactDoc),
      req('openwop.it.artifact-schema-compile-bounded.artifact-type-packs-md-declares-the-bounded-compilation-must', 'artifact-type-packs.md §"Bounded schema compilation"', 'host MUST bound + MUST reject over-limit schemas'),
    ).toBe(true);
    // The three structural axes + the timeout MUST all be named.
    for (const axis of [/byte size/i, /\$ref/i, /keyword/i, /timeout/i]) {
      expect(axis.test(artifactDoc), req('openwop.it.artifact-schema-compile-bounded.artifact-type-packs-md-declares-the-bounded-compilation-must', 'artifact-type-packs.md', `bound axis ${axis} MUST be named`)).toBe(true);
    }
  });
  ...
  it.skipIf(V1_DIR === null)('host-capabilities.md §host.artifactTypes references the bound', () => {
    expect(
      ...
      req('openwop.it.artifact-schema-compile-bounded.host-capabilities-md-host-artifacttypes-references-the-bound', 'host-capabilities.md §host.artifactTypes', 'MUST reference the bounded-compilation invariant'),
    ).toBe(true);
  });
});
...
describe('artifact-schema-compile-bounded: a finite bound catches schema bombs (RFC 0071, server-free)', () => {
  ...
  it('admits a benign artifact schema', () => {
    ...
    expect(exceedsBounds(benign), req('openwop.it.artifact-schema-compile-bounded.admits-a-benign-artifact-schema', 'artifact-type-packs.md', 'a legitimate artifact schema MUST NOT be rejected')).toBe(false);
  });
  ...
  it('rejects a $ref-depth bomb', () => {
    ...
    expect(exceedsBounds({ type: 'object', properties: { deep: node } }), req('openwop.it.artifact-schema-compile-bounded.rejects-a-ref-depth-bomb', 'threat-model-node-packs.md', 'a $ref-depth bomb MUST be rejected')).toBe(true);
  });

  it('rejects a keyword-count bomb', () => {
    const props: Record<string, unknown> = {};
    for (let i = 0; i < LIMITS.maxKeywords + 100; i++) props[`p${i}`] = { type: 'string' };
    expect(exceedsBounds({ type: 'object', properties: props }), req('openwop.it.artifact-schema-compile-bounded.rejects-a-keyword-count-bomb', 'threat-model-node-packs.md', 'a keyword-count bomb MUST be rejected')).toBe(true);
  });

  it('rejects an oversized schema', () => {
    const huge = { type: 'object', description: 'x'.repeat(LIMITS.maxBytes + 1) };
    expect(exceedsBounds(huge), req('openwop.it.artifact-schema-compile-bounded.rejects-an-oversized-schema', 'threat-model-node-packs.md', 'an over-size schema MUST be rejected')).toBe(true);
  });
});
