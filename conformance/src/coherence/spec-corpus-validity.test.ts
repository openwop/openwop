...
describe('spec-corpus: JSON Schemas compile under Ajv2020', () => {
  ...
  it('finds at least three schemas (workflow-definition, run-event, suspend-request)', () => {
    expect(schemaFiles.length, req('openwop.it.spec-corpus-validity.finds-at-least-three-schemas-workflow-definition-run-event-suspend-request', 'spec-corpus-validity.test.ts (no spec citation in file)', 'finds at least three schemas (workflow-definition, run-event, suspend-request)')).toBeGreaterThanOrEqual(3);
  ...
  for (const file of schemaFiles) {
    it(`${file} parses + compiles`, () => {
      ...
      expect(schema['$id'], req('openwop.it.spec-corpus-validity.parses-compiles', 'spec-corpus-validity.test.ts (no spec citation in file)', `${file} $id MUST match its canonical openwop.dev URL`)).toBe(
...
describe('spec-corpus: schemas/README.md index matches schema files', () => {
  ...
  it('schemas/README.md exists next to schema files', () => {
    expect(existsSync(schemasReadmePath), req('openwop.it.spec-corpus-validity.schemas-readme-md-exists-next-to-schema-files', 'spec-corpus-validity.test.ts (no spec citation in file)', 'schemas/README.md MUST exist')).toBe(true);
  });
  ...
  it('schemas/README.md lists every *.schema.json exactly once', () => {
    ...
    expect(tableStart, req('openwop.it.spec-corpus-validity.schemas-readme-md-lists-every-schema-json-exactly-once', 'spec-corpus-validity.test.ts (no spec citation in file)', 'schemas/README.md MUST include the schema index table')).toBeGreaterThanOrEqual(0);
    expect(tableEnd, req('openwop.it.spec-corpus-validity.schemas-readme-md-lists-every-schema-json-exactly-once', 'spec-corpus-validity.test.ts (no spec citation in file)', 'schemas/README.md schema index MUST precede validation instructions')).toBeGreaterThan(tableStart);
    ...
    for (const file of schemaFiles) {
      ...
      expect(
        occurrences,
        req('openwop.it.spec-corpus-validity.schemas-readme-md-lists-every-schema-json-exactly-once', 'spec-corpus-validity.test.ts (no spec citation in file)', `schemas/README.md MUST list ${file} exactly once`),
      ).toBe(1);
    }
    ...
    for (const file of mentioned) {
      expect(
        schemaFiles,
        req('openwop.it.spec-corpus-validity.schemas-readme-md-lists-every-schema-json-exactly-once', 'spec-corpus-validity.test.ts (no spec citation in file)', `schemas/README.md lists ${file}, but no matching schema file exists`),
      ).toContain(file);
    }
  });
});
...
describe('spec-corpus: RunEventType payload index matches event enum', () => {
  ...
  it('payload type-index keys exactly match RunEventType enum values', () => {
    ...
    expect(indexedTypes, req('openwop.it.spec-corpus-validity.payload-type-index-keys-exactly-match-runeventtype-enum-values', 'spec-corpus-validity.test.ts (no spec citation in file)', 'run-event-payloads.schema.json _typeIndex MUST cover every RunEventType')).toEqual(
      runEventTypes,
    );
  });
  ...
  it('payload type-index refs point to declared payload $defs', () => {
    const defs = payloadSchema.$defs as Record<string, unknown> | undefined;
    expect(defs, req('openwop.it.spec-corpus-validity.payload-type-index-refs-point-to-declared-payload-defs', 'spec-corpus-validity.test.ts (no spec citation in file)', 'run-event-payloads.schema.json MUST declare $defs')).toBeDefined();

    for (const [eventType, entry] of Object.entries(typeIndexProperties())) {
      const ref = (entry as Record<string, unknown>).$ref;
      expect(typeof ref, req('openwop.it.spec-corpus-validity.payload-type-index-refs-point-to-declared-payload-defs', 'spec-corpus-validity.test.ts (no spec citation in file)', `_typeIndex.${eventType} MUST be a $ref`)).toBe('string');
      const defName = String(ref).match(/^#\/\$defs\/([A-Za-z0-9_-]+)$/)?.[1];
      expect(defName, req('openwop.it.spec-corpus-validity.payload-type-index-refs-point-to-declared-payload-defs', 'spec-corpus-validity.test.ts (no spec citation in file)', `_typeIndex.${eventType} MUST reference #/$defs/<name>`)).toBeDefined();
      expect(
        Object.prototype.hasOwnProperty.call(defs ?? {}, defName ?? ''),
        req('openwop.it.spec-corpus-validity.payload-type-index-refs-point-to-declared-payload-defs', 'spec-corpus-validity.test.ts (no spec citation in file)', `_typeIndex.${eventType} references missing payload definition "${defName}"`),
      ).toBe(true);
    }
  });
  ...
  it('payload schema description states the current RunEventType variant count', () => {
    ...
    expect(typeof description, req('openwop.it.spec-corpus-validity.payload-schema-description-states-the-current-runeventtype-variant-count', 'spec-corpus-validity.test.ts (no spec citation in file)', 'run-event-payloads.schema.json MUST carry a description')).toBe('string');
    expect(
      description,
      req('openwop.it.spec-corpus-validity.payload-schema-description-states-the-current-runeventtype-variant-count', 'spec-corpus-validity.test.ts (no spec citation in file)', 'run-event-payloads.schema.json description MUST state the current RunEventType variant count'),
...
describe('spec-corpus: OpenAPI 3.1 spec is structurally valid', () => {
  ...
  it('exists', () => {
    expect(existsSync(openapiPath), req('openwop.it.spec-corpus-validity.exists', 'spec-corpus-validity.test.ts (no spec citation in file)', 'exists')).toBe(true);
  });

  it('declares openapi: 3.1 + required top-level keys', () => {
    const { topLevelKeys, raw } = readYamlHeader(openapiPath);
    expect(topLevelKeys.has('openapi'), req('openwop.it.spec-corpus-validity.declares-openapi-3-1-required-top-level-keys', 'spec-corpus-validity.test.ts (no spec citation in file)', 'declares openapi: 3.1 + required top-level keys')).toBe(true);
  ...
  it('every $ref to ../schemas/*.json resolves to a real file', () => {
    ...
    for (const ref of refs) {
      ...
      expect(existsSync(abs), req('openwop.it.spec-corpus-validity.every-ref-to-schemas-json-resolves-to-a-real-file', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI $ref points at missing file: ${ref}`)).toBe(true);
    }
  });
  ...
  it('operationIds are unique', () => {
    ...
    expect(operationIds.length, req('openwop.it.spec-corpus-validity.operationids-are-unique', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST declare operationIds for public routes')).toBeGreaterThan(0);
    expect(duplicates, req('openwop.it.spec-corpus-validity.operationids-are-unique', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI operationIds MUST be unique; duplicates: ${duplicates.join(', ')}`)).toEqual([]);
  });
  ...
  it('every operation tag is declared in the top-level tags list', () => {
    ...
    expect(declaredTags.size, req('openwop.it.spec-corpus-validity.every-operation-tag-is-declared-in-the-top-level-tags-list', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST declare at least one top-level tag')).toBeGreaterThan(0);
    expect(operationTags.length, req('openwop.it.spec-corpus-validity.every-operation-tag-is-declared-in-the-top-level-tags-list', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI operations MUST carry tags')).toBeGreaterThan(0);

    for (const tag of operationTags) {
      expect(
        declaredTags.has(tag),
        req('openwop.it.spec-corpus-validity.every-operation-tag-is-declared-in-the-top-level-tags-list', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI operation tag "${tag}" MUST be declared in the top-level tags list`),
      ).toBe(true);
    }
  });
  ...
  it('declares ApiKeyAuth as the global default security requirement', () => {
    ...
    expect(raw, req('openwop.it.spec-corpus-validity.declares-apikeyauth-as-the-global-default-security-requirement', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST declare global ApiKeyAuth security')).toMatch(
      /^security:\n\s+- ApiKeyAuth:\s*\[\]\s*$/m,
    );
    expect(raw, req('openwop.it.spec-corpus-validity.declares-apikeyauth-as-the-global-default-security-requirement', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST define ApiKeyAuth as an HTTP bearer security scheme')).toMatch(
  ...
  it('only documented public or signed-token operations clear security', () => {
    ...
    expect(operations.length, req('openwop.it.spec-corpus-validity.only-documented-public-or-signed-token-operations-clear-security', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST expose operations')).toBeGreaterThan(0);

    for (const operation of operations) {
      expect(
        operation.clearsSecurity,
        req('openwop.it.spec-corpus-validity.only-documented-public-or-signed-token-operations-clear-security', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI operation ${operation.operationId} security override MUST match its public/signed-token status`),
  ...
  it('protected operations document canonical 401 and 403 auth failure responses', () => {
    ...
    expect(operations.length, req('openwop.it.spec-corpus-validity.protected-operations-document-canonical-401-and-403-auth-failure-responses', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST expose operations')).toBeGreaterThan(0);

    for (const operation of operations) {
      if (operation.clearsSecurity) continue;
      expect(
        operation.responseStatusCodes,
        req('openwop.it.spec-corpus-validity.protected-operations-document-canonical-401-and-403-auth-failure-responses', 'spec-corpus-validity.test.ts (no spec citation in file)', `Protected OpenAPI operation ${operation.operationId} MUST document 401 Unauthenticated`),
      ).toContain('401');
      expect(
        operation.responseStatusCodes,
        req('openwop.it.spec-corpus-validity.protected-operations-document-canonical-401-and-403-auth-failure-responses', 'spec-corpus-validity.test.ts (no spec citation in file)', `Protected OpenAPI operation ${operation.operationId} MUST document 403 Forbidden`),
      ).toContain('403');
    }
  });
  ...
  it('declares both the literal /v1/agents/{roster,org-chart} routes and the {agentId} param route', () => {
    const { raw } = readYamlHeader(openapiPath);
    expect(raw, req('openwop.it.spec-corpus-validity.declares-both-the-literal-v1-agents-roster-org-chart-routes-and-the-agentid-para', 'spec-corpus-validity.test.ts (no spec citation in file)', 'declares both the literal /v1/agents/{roster,org-chart} routes and the {agentId} param route')).toContain('/v1/agents/{agentId}:');
  ...
  it('every /v1/agents/{agentId} param excludes the reserved literals (roster, org-chart)', () => {
    ...
    expect(paramRoutes, req('openwop.it.spec-corpus-validity.every-v1-agents-agentid-param-excludes-the-reserved-literals-roster-org-chart', 'spec-corpus-validity.test.ts (no spec citation in file)', 'expected ≥2 /v1/agents/{agentId...} routes (base + /deployments)')).toBeGreaterThanOrEqual(2);
    expect(
      exclusions,
      req('openwop.it.spec-corpus-validity.every-v1-agents-agentid-param-excludes-the-reserved-literals-roster-org-chart', 'spec-corpus-validity.test.ts (no spec citation in file)', 'each /v1/agents/{agentId} param schema MUST exclude the reserved literals via a (?!roster$|org-chart$) lookahead'),
  ...
  it('the reserved-literal exclusion pattern rejects roster/org-chart and accepts a real agentId', () => {
    ...
    expect(re.test('roster'), req('openwop.it.spec-corpus-validity.the-reserved-literal-exclusion-pattern-rejects-roster-org-chart-and-accepts-a-re', 'spec-corpus-validity.test.ts (no spec citation in file)', 'roster MUST NOT match the {agentId} param')).toBe(false);
    expect(re.test('org-chart'), req('openwop.it.spec-corpus-validity.the-reserved-literal-exclusion-pattern-rejects-roster-org-chart-and-accepts-a-re', 'spec-corpus-validity.test.ts (no spec citation in file)', 'org-chart MUST NOT match the {agentId} param')).toBe(false);
  ...
  it('the agent-manifest agentId pattern can never produce a reserved literal (defense in depth)', () => {
    ...
    expect(typeof pattern, req('openwop.it.spec-corpus-validity.the-agent-manifest-agentid-pattern-can-never-produce-a-reserved-literal-defense', 'spec-corpus-validity.test.ts (no spec citation in file)', 'agent-manifest.schema.json MUST constrain agentId with a pattern')).toBe('string');
    const re = new RegExp(pattern as string);
    expect(re.test('roster'), req('openwop.it.spec-corpus-validity.the-agent-manifest-agentid-pattern-can-never-produce-a-reserved-literal-defense', 'spec-corpus-validity.test.ts (no spec citation in file)', 'manifest agentId MUST NOT permit the reserved literal `roster`')).toBe(false);
    expect(re.test('org-chart'), req('openwop.it.spec-corpus-validity.the-agent-manifest-agentid-pattern-can-never-produce-a-reserved-literal-defense', 'spec-corpus-validity.test.ts (no spec citation in file)', 'manifest agentId MUST NOT permit the reserved literal `org-chart`')).toBe(false);
    expect(re.test('core.example.pack.agent')).toBe(true);
  });
  ...
  it('typed error specializations compose the canonical Error schema', () => {
    ...
    for (const schemaName of ['RunClaimConflict', 'UnsupportedStreamMode']) {
      ...
      expect(
        block,
        req('openwop.it.spec-corpus-validity.typed-error-specializations-compose-the-canonical-error-schema', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI ${schemaName} MUST compose the canonical Error schema`),
      ).toContain("- $ref: '#/components/schemas/Error'");
      expect(
        block,
        req('openwop.it.spec-corpus-validity.typed-error-specializations-compose-the-canonical-error-schema', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI ${schemaName} MUST keep typed metadata under details`),
      ).toMatch(/^\s{12}details:\s*$/m);
      expect(
        block,
        req('openwop.it.spec-corpus-validity.typed-error-specializations-compose-the-canonical-error-schema', 'spec-corpus-validity.test.ts (no spec citation in file)', `OpenAPI ${schemaName} MUST require the canonical error/message/details top-level fields`),
...
describe.skipIf(COVERAGE_DOC_PATH === null)('spec-corpus: OpenAPI operation coverage map', () => {
  ...
  it('every OpenAPI operationId is represented in conformance/coverage.md', () => {
    ...
    expect(operationIds.length, req('openwop.it.spec-corpus-validity.every-openapi-operationid-is-represented-in-conformance-coverage-md', 'spec-corpus-validity.test.ts (no spec citation in file)', 'OpenAPI MUST declare operationIds for public routes')).toBeGreaterThan(0);

    for (const operationId of operationIds) {
      expect(
        coverage,
        req('openwop.it.spec-corpus-validity.every-openapi-operationid-is-represented-in-conformance-coverage-md', 'spec-corpus-validity.test.ts (no spec citation in file)', `conformance/coverage.md MUST mention OpenAPI operationId "${operationId}"`),
...
describe.skipIf(V1_DIR === null)('spec-corpus: REST endpoint catalog matches OpenAPI paths', () => {
  ...
  it('every OpenAPI operation has a matching method/path row in rest-endpoints.md', () => {
    ...
    expect(operations.length, req('openwop.it.spec-corpus-validity.every-openapi-operation-has-a-matching-method-path-row-in-rest-endpoints-md', 'rest-endpoints.md', 'OpenAPI MUST expose operations')).toBeGreaterThan(0);
    expect(catalogRows.length, req('openwop.it.spec-corpus-validity.every-openapi-operation-has-a-matching-method-path-row-in-rest-endpoints-md', 'rest-endpoints.md', 'rest-endpoints.md MUST include endpoint catalog rows')).toBeGreaterThan(0);

    for (const operation of operations) {
      const key = `${operation.method} ${operation.path}`;
      expect(
        catalogKeys.has(key),
        req('openwop.it.spec-corpus-validity.every-openapi-operation-has-a-matching-method-path-row-in-rest-endpoints-md', 'rest-endpoints.md', `rest-endpoints.md MUST document OpenAPI operation ${operation.operationId} as ${key}`),
      ).toBe(true);
    }
  });
  ...
  it('REST catalog auth/scope columns match OpenAPI security overrides', () => {
    ...
    for (const operation of operations) {
      ...
      expect(row, req('openwop.it.spec-corpus-validity.rest-catalog-auth-scope-columns-match-openapi-security-overrides', 'spec-corpus-validity.test.ts (no spec citation in file)', `rest-endpoints.md MUST document ${operation.method.toUpperCase()} ${operation.path}`)).toBeDefined();
      if (!row) continue;

      if (operation.clearsSecurity) {
        expect(
          ['None', 'Signed token'],
          req('openwop.it.spec-corpus-validity.rest-catalog-auth-scope-columns-match-openapi-security-overrides', 'spec-corpus-validity.test.ts (no spec citation in file)', `${operation.operationId} clears OpenAPI security, so rest-endpoints.md MUST mark auth as None or Signed token`),
        ).toContain(row.auth);
        expect(
          row.scope,
          req('openwop.it.spec-corpus-validity.rest-catalog-auth-scope-columns-match-openapi-security-overrides', 'spec-corpus-validity.test.ts (no spec citation in file)', `${operation.operationId} clears OpenAPI security, so rest-endpoints.md MUST mark scope as None`),
        ).toBe('None');
      } else {
        expect(
          row.auth,
          req('openwop.it.spec-corpus-validity.rest-catalog-auth-scope-columns-match-openapi-security-overrides', 'spec-corpus-validity.test.ts (no spec citation in file)', `${operation.operationId} inherits OpenAPI ApiKeyAuth, so rest-endpoints.md MUST mark auth as API key`),
        ).toBe('API key');
        expect(
          row.scope,
          req('openwop.it.spec-corpus-validity.rest-catalog-auth-scope-columns-match-openapi-security-overrides', 'spec-corpus-validity.test.ts (no spec citation in file)', `${operation.operationId} inherits OpenAPI ApiKeyAuth, so rest-endpoints.md MUST name a non-empty non-None scope`),
        ).not.toBe('None');
      }
    }
  });
...
describe.skipIf(V1_DIR === null)('spec-corpus: error examples use canonical details slot', () => {
  ...
  for (const file of docsToCheck) {
    it(`${file} does not document retry/conflict metadata as top-level error fields`, () => {
      ...
      expect(
        content,
        req('openwop.it.spec-corpus-validity.does-not-document-retry-conflict-metadata-as-top-level-error-fields', 'spec-corpus-validity.test.ts (no spec citation in file)', `${file} MUST NOT show retryAfter as a top-level error field; use details.retryAfter`),
      ).not.toMatch(/\{\s*(?:[^{}]|\{[^{}]*\})*error:[^{}]*retryAfter[^{}]*\}/);
      expect(
        content,
        req('openwop.it.spec-corpus-validity.does-not-document-retry-conflict-metadata-as-top-level-error-fields', 'spec-corpus-validity.test.ts (no spec citation in file)', `${file} MUST NOT show activeRunId/activeHost as top-level error fields; use details.{activeRunId,activeHost}`),
...
describe.skipIf(
  ...
  () => {
    ...
    it('TypeScript exports HTTP_ERROR_CODES and isHttpErrorCode', () => {
      ...
      expect(source, req('openwop.it.spec-corpus-validity.typescript-exports-http-error-codes-and-ishttperrorcode', 'spec-corpus-validity.test.ts (no spec citation in file)', 'TypeScript SDK MUST export HTTP_ERROR_CODES')).toContain('export const HTTP_ERROR_CODES');
      expect(source, req('openwop.it.spec-corpus-validity.typescript-exports-http-error-codes-and-ishttperrorcode', 'spec-corpus-validity.test.ts (no spec citation in file)', 'TypeScript SDK MUST export isHttpErrorCode')).toContain('export function isHttpErrorCode');
    });

    it('Python exports HTTP_ERROR_CODES and is_http_error_code', () => {
      const source = readFileSync(sdkSources.python, 'utf8');

      expect(source, req('openwop.it.spec-corpus-validity.python-exports-http-error-codes-and-is-http-error-code', 'spec-corpus-validity.test.ts (no spec citation in file)', 'Python SDK MUST export HTTP_ERROR_CODES')).toContain('HTTP_ERROR_CODES = frozenset');
      expect(source, req('openwop.it.spec-corpus-validity.python-exports-http-error-codes-and-is-http-error-code', 'spec-corpus-validity.test.ts (no spec citation in file)', 'Python SDK MUST export is_http_error_code')).toContain('def is_http_error_code');
    });

    it('Go exports HTTPErrorCodes and IsHTTPErrorCode', () => {
      const source = readFileSync(sdkSources.go, 'utf8');

      expect(source, req('openwop.it.spec-corpus-validity.go-exports-httperrorcodes-and-ishttperrorcode', 'spec-corpus-validity.test.ts (no spec citation in file)', 'Go SDK MUST export HTTPErrorCodes')).toContain('var HTTPErrorCodes = []string');
      expect(source, req('openwop.it.spec-corpus-validity.go-exports-httperrorcodes-and-ishttperrorcode', 'spec-corpus-validity.test.ts (no spec citation in file)', 'Go SDK MUST export IsHTTPErrorCode')).toContain('func IsHTTPErrorCode');
    });

    it('SDK READMEs document the HTTP error helper surface', () => {
      expect(readFileSync(sdkReadmes.typescript, 'utf8'), req('openwop.it.spec-corpus-validity.sdk-readmes-document-the-http-error-helper-surface', 'spec-corpus-validity.test.ts (no spec citation in file)', 'SDK READMEs document the HTTP error helper surface')).toContain('HTTP_ERROR_CODES');
    ...
    it('SDK changelogs mention the HTTP error helper surface', () => {
      expect(readFileSync(sdkChangelogs.typescript, 'utf8'), req('openwop.it.spec-corpus-validity.sdk-changelogs-mention-the-http-error-helper-surface', 'spec-corpus-validity.test.ts (no spec citation in file)', 'SDK changelogs mention the HTTP error helper surface')).toContain('HTTP_ERROR_CODES');
    ...
    it('TypeScript dist exports the HTTP error helper surface', () => {
      for (const [label, path] of Object.entries(typescriptDist)) {
        expect(existsSync(path), req('openwop.it.spec-corpus-validity.typescript-dist-exports-the-http-error-helper-surface', 'spec-corpus-validity.test.ts (no spec citation in file)', `TypeScript dist artifact ${label} MUST exist`)).toBe(true);
      }
    ...
    it('TypeScript dist metadata uses openwop branding', () => {
      for (const path of Object.values(typescriptDist)) {
        ...
        expect(source, req('openwop.it.spec-corpus-validity.typescript-dist-metadata-uses-openwop-branding', 'spec-corpus-validity.test.ts (no spec citation in file)', `${path} MUST NOT contain legacy MyndHyve package names`)).not.toContain('@myndhyve');
        expect(source, req('openwop.it.spec-corpus-validity.typescript-dist-metadata-uses-openwop-branding', 'spec-corpus-validity.test.ts (no spec citation in file)', `${path} MUST use @openwop package naming`)).toContain('@openwop');
      }
    });
    ...
    it('TypeScript dist source maps point back to src and avoid legacy branding', () => {
      for (const [label, path] of Object.entries(typescriptDistMaps)) {
        expect(existsSync(path), req('openwop.it.spec-corpus-validity.typescript-dist-source-maps-point-back-to-src-and-avoid-legacy-branding', 'spec-corpus-validity.test.ts (no spec citation in file)', `TypeScript dist source map ${label} MUST exist`)).toBe(true);
        const sourceMap = readJson(path) as { sources?: unknown };
        const raw = readFileSync(path, 'utf8');

        expect(raw, req('openwop.it.spec-corpus-validity.typescript-dist-source-maps-point-back-to-src-and-avoid-legacy-branding', 'spec-corpus-validity.test.ts (no spec citation in file)', `${path} MUST NOT contain legacy MyndHyve package names`)).not.toContain('@myndhyve');
        expect(Array.isArray(sourceMap.sources), req('openwop.it.spec-corpus-validity.typescript-dist-source-maps-point-back-to-src-and-avoid-legacy-branding', 'spec-corpus-validity.test.ts (no spec citation in file)', `${path} MUST declare source files`)).toBe(true);
        expect(
          (sourceMap.sources as string[]).every((source) => source.startsWith('../src/')),
          req('openwop.it.spec-corpus-validity.typescript-dist-source-maps-point-back-to-src-and-avoid-legacy-branding', 'spec-corpus-validity.test.ts (no spec citation in file)', `${path} MUST map to TypeScript source files under ../src`),
        ).toBe(true);
      }
    });
    ...
    for (const code of [
      ...
      it(`all SDK HTTP error helpers include ${code}`, () => {
        for (const [sdk, path] of Object.entries(sdkSources)) {
          const source = readFileSync(path, 'utf8');
          expect(source, req('openwop.it.spec-corpus-validity.all-sdk-http-error-helpers-include', 'spec-corpus-validity.test.ts (no spec citation in file)', `${sdk} SDK MUST include canonical REST code ${code}`)).toContain(code);
        }
      });
    }
  },
...
describe.skipIf(SCENARIOS_DIR === null || CONFORMANCE_README_PATH === null)(
  ...
  () => {
    ...
    it('README suite count equals src/scenarios/*.test.ts count', () => {
      ...
      expect(scenarioFiles.length, req('openwop.it.spec-corpus-validity.readme-suite-count-equals-src-scenarios-test-ts-count', 'spec-corpus-validity.test.ts (no spec citation in file)', 'conformance suite MUST contain scenario test files')).toBeGreaterThan(0);
      expect(
        readme,
        req('openwop.it.spec-corpus-validity.readme-suite-count-equals-src-scenarios-test-ts-count', 'spec-corpus-validity.test.ts (no spec citation in file)', 'conformance/README.md MUST state the current scenario-file count in "What\'s Covered"'),
      ).toContain(`The current suite has ${scenarioFiles.length} scenario files under \`src/scenarios/\`.`);
      expect(
        readme,
        req('openwop.it.spec-corpus-validity.readme-suite-count-equals-src-scenarios-test-ts-count', 'spec-corpus-validity.test.ts (no spec citation in file)', 'conformance/README.md MUST state the current scenario-file count near historical notes'),
...
describe('spec-corpus: AsyncAPI 3.1 spec is structurally valid', () => {
  ...
  it('exists', () => {
    expect(existsSync(asyncapiPath), req('openwop.it.spec-corpus-validity.exists~2', 'spec-corpus-validity.test.ts (no spec citation in file)', 'exists')).toBe(true);
  });

  it('declares asyncapi: 3.1 + required top-level keys', () => {
    const { topLevelKeys, raw } = readYamlHeader(asyncapiPath);
    expect(topLevelKeys.has('asyncapi'), req('openwop.it.spec-corpus-validity.declares-asyncapi-3-1-required-top-level-keys', 'spec-corpus-validity.test.ts (no spec citation in file)', 'declares asyncapi: 3.1 + required top-level keys')).toBe(true);
    expect(topLevelKeys.has('info')).toBe(true);
  ...
  it('every $ref to ../schemas/*.json resolves to a real file', () => {
    ...
    for (const ref of refs) {
      const abs = pathResolve(API_DIR, ref.split('#')[0] ?? ref);
      expect(existsSync(abs), req('openwop.it.spec-corpus-validity.every-ref-to-schemas-json-resolves-to-a-real-file~2', 'spec-corpus-validity.test.ts (no spec citation in file)', `AsyncAPI $ref points at missing file: ${ref}`)).toBe(true);
    }
  });
  ...
  it('named RunEventDoc messages use event names from run-event.schema.json', () => {
    ...
    expect(messageNames.length, req('openwop.it.spec-corpus-validity.named-runeventdoc-messages-use-event-names-from-run-event-schema-json', 'spec-corpus-validity.test.ts (no spec citation in file)', 'AsyncAPI MUST declare named SSE messages')).toBeGreaterThan(0);

    for (const name of messageNames) {
      if (syntheticMessageNames.has(name)) continue;
      expect(
        runEventTypes.has(name),
        req('openwop.it.spec-corpus-validity.named-runeventdoc-messages-use-event-names-from-run-event-schema-json', 'spec-corpus-validity.test.ts (no spec citation in file)', `AsyncAPI message name "${name}" MUST exist in run-event.schema.json RunEventType enum, or be documented as synthetic`),
      ).toBe(true);
    }
  });
  ...
  it('operation channel refs point to declared channels', () => {
    ...
    expect(channels.size, req('openwop.it.spec-corpus-validity.operation-channel-refs-point-to-declared-channels', 'spec-corpus-validity.test.ts (no spec citation in file)', 'AsyncAPI MUST declare channels')).toBeGreaterThan(0);
    expect(channelRefs.length, req('openwop.it.spec-corpus-validity.operation-channel-refs-point-to-declared-channels', 'spec-corpus-validity.test.ts (no spec citation in file)', 'AsyncAPI operations MUST reference channels')).toBeGreaterThan(0);

    for (const ref of channelRefs) {
      expect(
        channels.has(ref),
        req('openwop.it.spec-corpus-validity.operation-channel-refs-point-to-declared-channels', 'spec-corpus-validity.test.ts (no spec citation in file)', `AsyncAPI operation references missing channel "${ref}"`),
      ).toBe(true);
    }
  });
  ...
  it('channel keys and message names are unique', () => {
    ...
    expect(duplicateChannels, req('openwop.it.spec-corpus-validity.channel-keys-and-message-names-are-unique', 'spec-corpus-validity.test.ts (no spec citation in file)', `AsyncAPI channel keys MUST be unique; duplicates: ${duplicateChannels.join(', ')}`)).toEqual([]);
    expect(duplicateMessages, req('openwop.it.spec-corpus-validity.channel-keys-and-message-names-are-unique', 'spec-corpus-validity.test.ts (no spec citation in file)', `AsyncAPI message names MUST be unique; duplicates: ${duplicateMessages.join(', ')}`)).toEqual([]);
  });
});
...
describe.skipIf(V1_DIR === null)('spec-corpus: prose docs carry a Status: legend tag', () => {
  ...
  it('finds the expected prose doc set', () => {
    ...
    expect(proseFiles.length, req('openwop.it.spec-corpus-validity.finds-the-expected-prose-doc-set', 'spec-corpus-validity.test.ts (no spec citation in file)', 'finds the expected prose doc set')).toBeGreaterThanOrEqual(11);
  });
  ...
  for (const file of proseFiles) {
    it(`${file} declares a Status: tag (STUB / DRAFT / OUTLINE / FINAL | Stable / Stabilizing / Draft / Experimental)`, () => {
      ...
      expect(
        content,
        req('openwop.it.spec-corpus-validity.declares-a-status-tag-stub-draft-outline-final-stable-stabilizing-draft-experime', 'spec-corpus-validity.test.ts (no spec citation in file)', `${file} must include a "Status:" legend tag near its header`),
...
describe.skipIf(README_PATH === null)('spec-corpus: local Markdown links resolve', () => {
  ...
  it('finds Markdown files to check', () => {
    expect(markdownFiles.length, req('openwop.it.spec-corpus-validity.finds-markdown-files-to-check', 'spec-corpus-validity.test.ts (no spec citation in file)', 'repo checkout should contain Markdown docs')).toBeGreaterThan(0);
  });
...
describe.skipIf(README_PATH === null)('spec-corpus: public docs avoid private implementation breadcrumbs', () => {
  ...
  it('scans public docs and schemas', () => {
    expect(publicTextFiles.length, req('openwop.it.spec-corpus-validity.scans-public-docs-and-schemas', 'spec-corpus-validity.test.ts (no spec citation in file)', 'public docs/schemas list MUST be non-empty')).toBeGreaterThan(0);
  });
  ...
  for (const file of publicTextFiles) {
    ...
    it(`${relFile} has no private implementation or pre-v1 breadcrumbs`, () => {
      ...
      for (const { label, pattern } of banned) {
        expect(text, req('openwop.it.spec-corpus-validity.has-no-private-implementation-or-pre-v1-breadcrumbs', 'spec-corpus-validity.test.ts (no spec citation in file)', `${relFile} MUST NOT contain ${label}`)).not.toMatch(pattern);
      }
    });
  }
});
...
describe.skipIf(FIXTURES_DOC_PATH === null)('spec-corpus: fixtures.json catalog matches fixtures.md', () => {
  ...
  it('every fixture id mentioned in fixtures.md has a corresponding JSON', () => {
    ...
    for (const cite of cited) {
      expect(
        fixtureJsonFiles,
        req('openwop.it.spec-corpus-validity.every-fixture-id-mentioned-in-fixtures-md-has-a-corresponding-json', 'spec-corpus-validity.test.ts (no spec citation in file)', `fixtures.md cites fixture id "${cite}" but no matching ${cite}.json exists`),
      ).toContain(cite);
    }
  });
  ...
  it('every fixture JSON file is referenced by fixtures.md', () => {
    ...
    for (const id of fixtureJsonFiles) {
      expect(
        doc,
        req('openwop.it.spec-corpus-validity.every-fixture-json-file-is-referenced-by-fixtures-md', 'spec-corpus-validity.test.ts (no spec citation in file)', `fixture ${id}.json exists but fixtures.md does not document it`),
      ).toContain(id);
    }
  });
});
...
describe('spec-corpus: RFC 0089 conformance certification bundle + binding rule', () => {
  ...
  it('a sample bundle validates against conformance-certification-bundle.schema.json', () => {
    ...
    expect(ok, req('openwop.it.spec-corpus-validity.a-sample-bundle-validates-against-conformance-certification-bundle-schema-json', 'spec-corpus-validity.test.ts (no spec citation in file)', JSON.stringify(validate.errors))).toBe(true);
  });

  it('verifyBundle ACCEPTS a claim that is discovery-derivable AND floor-proven (§B)', () => {
    const r = verifyBundle(sampleBundle);
    expect(r.valid, req('openwop.it.spec-corpus-validity.verifybundle-accepts-a-claim-that-is-discovery-derivable-and-floor-proven-b', 'spec-corpus-validity.test.ts (no spec citation in file)', 'verifyBundle ACCEPTS a claim that is discovery-derivable AND floor-proven (§B)')).toBe(true);
  ...
  it('verifyBundle REJECTS a profile its discovery document does not derive (§B(1))', () => {
    ...
    expect(r.valid, req('openwop.it.spec-corpus-validity.verifybundle-rejects-a-profile-its-discovery-document-does-not-derive-b-1', 'spec-corpus-validity.test.ts (no spec citation in file)', 'verifyBundle REJECTS a profile its discovery document does not derive (§B(1))')).toBe(false);
  ...
  it('verifyBundle REJECTS a bundle missing a floor scenario (§B(2))', () => {
    ...
    expect(r.valid, req('openwop.it.spec-corpus-validity.verifybundle-rejects-a-bundle-missing-a-floor-scenario-b-2', 'spec-corpus-validity.test.ts (no spec citation in file)', 'verifyBundle REJECTS a bundle missing a floor scenario (§B(2))')).toBe(false);
...
describe.skipIf(RFC0089_BUNDLE_PATH === null || !existsSync(RFC0089_BUNDLE_PATH))('spec-corpus: RFC 0089 committed reference-host certification bundle', () => {
  ...
  it('the committed bundle file exists (generated by --certify)', () => {
    expect(existsSync(bundlePath), req('openwop.it.spec-corpus-validity.the-committed-bundle-file-exists-generated-by-certify', 'spec-corpus-validity.test.ts (no spec citation in file)', `expected a committed reference bundle at ${bundlePath}`)).toBe(true);
  });
  ...
  it('the committed reference bundle validates against the bundle schema (§A)', () => {
    ...
    expect(ok, req('openwop.it.spec-corpus-validity.the-committed-reference-bundle-validates-against-the-bundle-schema-a', 'spec-corpus-validity.test.ts (no spec citation in file)', JSON.stringify(validate.errors))).toBe(true);
  });
  ...
  it('verifyBundle REJECTS the committed reference bundle — it is `invalidated` (RFC 0148 §D)', () => {
    ...
    expect(r.valid, req('openwop.it.spec-corpus-validity.verifybundle-rejects-the-committed-reference-bundle-it-is-invalidated-rfc-0148-d', 'RFC 0148 §D', 'the committed v1 bundle is invalidated, not merely historical')).toBe(false);

    const sse = r.verdicts.find((v) => v.profile === 'openwop-stream-sse');
    expect(sse?.floorProven, req('openwop.it.spec-corpus-validity.verifybundle-rejects-the-committed-reference-bundle-it-is-invalidated-rfc-0148-d', 'RFC 0148 §D', 'profiles.md §openwop-stream-sse: predicate AND those scenarios pass')).toBe(false);
    expect(sse?.missingFloor, req('openwop.it.spec-corpus-validity.verifybundle-rejects-the-committed-reference-bundle-it-is-invalidated-rfc-0148-d', 'RFC 0148 §D', 'its own results.failed lists the stream-modes scenarios')).toContain(
  ...
  it('discovery.sha256 is the canonical-JSON SHA-256 of the captured discovery.document', () => {
    ...
    expect(bundle.discovery.sha256, req('openwop.it.spec-corpus-validity.discovery-sha256-is-the-canonical-json-sha-256-of-the-captured-discovery-documen', 'spec-corpus-validity.test.ts (no spec citation in file)', 'discovery.sha256 is the canonical-JSON SHA-256 of the captured discovery.document')).toBe(recomputed);
  });
});
...
describe('spec-corpus: createRun composed request schema is satisfiable (RFC 0094 §A)', () => {
  ...
  it('the requestBody closes at the composition (unevaluatedProperties), not inside a branch', () => {
    ...
    expect(
      block,
      req('openwop.it.spec-corpus-validity.the-requestbody-closes-at-the-composition-unevaluatedproperties-not-inside-a-bra', 'spec-corpus-validity.test.ts (no spec citation in file)', 'RFC 0094 §A: the composed createRun request schema MUST be closed with `unevaluatedProperties: false`'),
  ...
  it('canonical createRun bodies PASS the composed schema; an undeclared property FAILS', () => {
    ...
    expect(inlineProps, req('openwop.it.spec-corpus-validity.canonical-createrun-bodies-pass-the-composed-schema-an-undeclared-property-fails', 'spec-corpus-validity.test.ts (no spec citation in file)', 'the inline branch MUST declare workflowId')).toContain('workflowId');
    ...
    expect(
      ...
      req('openwop.it.spec-corpus-validity.canonical-createrun-bodies-pass-the-composed-schema-an-undeclared-property-fails', 'spec-corpus-validity.test.ts (no spec citation in file)', 'RFC 0094 §A: an undeclared property MUST still fail at the composed level (unevaluatedProperties: false)'),
    ).toBe(false);
  });
});
