/**
 * v2-umbrella-child-parts — RFC 0167 §C (corpus wrapper).
 *
 ...
 *
 * @see RFCS/0167-openwop-v2-umbrella.md §C
 * @see scripts/check-migrations.mjs
 * @see scripts/check-waiver-ledger.mjs
 */
...
describe('v2-umbrella-child-parts (RFC 0167 §C)', () => {
  it('the corpus gate script exits 0', () => {
    if (V1_DIR === null) return softSkip('inapplicable', 'not a spec checkout');
    // §C's row cites TWO scripts — `check-migrations.mjs` for part (b) and
    // `check-waiver-ledger.mjs` for part (f) — so the id asserts both. Binding it
    // to only one would mint an id whose assertion is narrower than its row,
    // which is the defect this whole exercise exists to remove.
    const r0 = spawnSync('node', [join(root, 'scripts', 'check-migrations.mjs')], { cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 });
    expect(r0.status, req('openwop.requirement.0167.child-parts', 'RFC 0167 §C', `part (b): every migration row resolves — ${tail(r0)}`)).toBe(0);
    const r1 = spawnSync('node', [join(root, 'scripts', 'check-waiver-ledger.mjs')], { cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 });
    expect(r1.status, req('openwop.requirement.0167.child-parts', 'RFC 0167 §C', `part (f): every waived child is in the ledger — ${tail(r1)}`)).toBe(0);
  }, 180_000);
});
