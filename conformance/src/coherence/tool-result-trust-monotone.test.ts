...
describe('tool-result-trust-monotone: COMPLETENESS — no unfenced ingress (RFC 0143 part 1)', () => {
  ...
  it.skipIf(REPO_ROOT === null)('every invariant the §4 STRIDE table cites actually exists (no dangling fence)', () => {
    ...
    expect(cited.length, req('openwop.it.tool-result-trust-monotone.every-invariant-the-4-stride-table-cites-actually-exists-no-dangling-fence', 'threat-model §4', 'the STRIDE table cites fencing invariants')).toBeGreaterThan(5);
    ...
    expect(
      dangling,
      req('openwop.it.tool-result-trust-monotone.every-invariant-the-4-stride-table-cites-actually-exists-no-dangling-fence', 'threat-model §4', `every invariant-shaped id cited in §4 MUST exist in invariants.yaml — dangling: ${dangling.join(', ')}`),
    ).toEqual([]);
  });
  ...
  it.skipIf(REPO_ROOT === null)('the curated floor of prompt-ingress surfaces each has a real fencing invariant', () => {
    ...
    for (const [surface, inv] of Object.entries(FLOOR)) {
      ...
      expect(
        present,
        req('openwop.it.tool-result-trust-monotone.the-curated-floor-of-prompt-ingress-surfaces-each-has-a-real-fencing-invariant', 'threat-model §5 / invariants.yaml', `the "${surface}" ingress MUST retain a fencing invariant (${inv.join(' | ')})`),
      ).toEqual(inv);
    }
  });
  ...
  it.skipIf(REPO_ROOT === null)('the general rule itself is a declared invariant', () => {
    expect(
      ...
      req('openwop.it.tool-result-trust-monotone.the-general-rule-itself-is-a-declared-invariant', 'invariants.yaml', 'RFC 0143 mints the general monotone-composition invariant the point-invariants instance'),
    ).toBe(true);
  });
});
...
describe('tool-result-trust-monotone: the MEET is stated normatively (RFC 0143 part 2)', () => {
  it.skipIf(REPO_ROOT === null)('§2a exists and states monotone composition (the meet)', () => {
    expect(/## 2a\. Trust is monotone through composition/i.test(TM), req('openwop.it.tool-result-trust-monotone.2a-exists-and-states-monotone-composition-the-meet', 'threat-model §2a', 'the general-rule section exists')).toBe(true);
    expect(
      /MUST be the \*\*meet\*\*|MUST be the meet of its inputs/i.test(TM),
      req('openwop.it.tool-result-trust-monotone.2a-exists-and-states-monotone-composition-the-meet', 'threat-model §2a', 'a composed segment\'s trust MUST be the MEET of its inputs — one untrusted input taints the segment'),
    ).toBe(true);
  });
  ...
  it.skipIf(REPO_ROOT === null)('"a tool ran" is explicitly NOT a basis for trust', () => {
    expect(
      ...
      req('openwop.it.tool-result-trust-monotone.a-tool-ran-is-explicitly-not-a-basis-for-trust', 'threat-model §2a', 'untrusted-by-default — a tool having run is not a trust basis'),
    ).toBe(true);
  });

  it.skipIf(REPO_ROOT === null)('no laundering through storage, and both strategies are blessed', () => {
    expect(/does not launder it/i.test(TM), req('openwop.it.tool-result-trust-monotone.no-laundering-through-storage-and-both-strategies-are-blessed', 'threat-model §2a', 'persist-then-recall does not launder (closes FRMD-F1-1)')).toBe(true);
    expect(/dynamic coarse-grained propagation/i.test(TM) && /static reader classification/i.test(TM), req('openwop.it.tool-result-trust-monotone.no-laundering-through-storage-and-both-strategies-are-blessed', 'threat-model §2a', 'both conforming strategies named')).toBe(true);
    expect(/[Vv]alue-granular taint.{0,60}NOT required/i.test(TM), req('openwop.it.tool-result-trust-monotone.no-laundering-through-storage-and-both-strategies-are-blessed', 'threat-model §2a', 'value-granular taint is NOT required — the floor is the coarse meet')).toBe(true);
  });

  it.skipIf(REPO_ROOT === null)('the structurally-isolated-reader carve-out is present and narrow', () => {
    expect(/structurally isolated/i.test(TM), req('openwop.it.tool-result-trust-monotone.the-structurally-isolated-reader-carve-out-is-present-and-narrow', 'threat-model §2a', 'the isolation carve-out exists so correct sandboxing is not punished')).toBe(true);
    expect(/missing.{0,20}`?"?trusted"?`?.{0,40}violat|absence of a trust decision is\W{0,4}\s*`?untrusted`?/i.test(TM), req('openwop.it.tool-result-trust-monotone.the-structurally-isolated-reader-carve-out-is-present-and-narrow', 'threat-model §2a', 'the fail-open (missing ⇒ trusted) default is the violation')).toBe(true);
  });
});
...
describe('tool-result-trust-monotone: the instances name the general rule (RFC 0143 part 3)', () => {
  it.skipIf(V1_DIR === null)('ai-envelope.md §"Trust boundary" cross-references §2a / RFC 0143', () => {
    expect(
      ...
      req('openwop.it.tool-result-trust-monotone.ai-envelope-md-trust-boundary-cross-references-2a-rfc-0143', 'ai-envelope.md §"Trust boundary"', 'the MCP/A2A rule cites the general rule as its instance, so it cannot drift into contradicting it'),
    ).toBe(true);
  });
});
