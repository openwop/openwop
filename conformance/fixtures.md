...
- **MCP Tasks (RFC 0198).** `v2-mcp-tasks` also calls `conformance-approval-approvers`, `conformance-delay` (with `arguments.delayMs`) and `conformance-cancellable` (with `arguments.delayMs`) as tools, each under its workflowId, so the task, disconnect and SSE-cancel legs have a suspending, a non-listed-approver, a short and a long run to drive. The disconnect and SSE-cancel legs also need `runList` with the `workflowId` filter, to find a run the client never received a handle for. A fixture missing from `fixtures[]` records `blocked` for the leg that calls it; without `runList` those two legs are `inapplicable`.
