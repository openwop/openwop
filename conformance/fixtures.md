...
| Credential                                | `conformance-credential`                                                                        | RFC 0199 §C — one node declaring `auth { type: oauth2, provider: synthetic, scopes }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `waiting-input` (credential interrupt), or the node fails without the facet          | unbounded (suspends)         |
...
| Context-Budget Multi-Turn                 | `conformance-context-budget-multiturn`                                                          | RFC 0111 — **superseded as the witness gate in suite 2.41.0.** Drives the supervisor through `mockDecisions`, i.e. a mock supervisor — exactly what RFC 0111 §Scope forbids from advertising `contextBudget`, so no honest host can witness against it. Kept (v1 compatibility: hosts may still list it); both RFC 0111 scenarios now gate on `conformance-context-budget-live`                                                                                                                                              | `completed`                                                                         | ≤ 60s                        |
| Context-Budget Live Orchestrator | `conformance-context-budget-live` | RFC 0111 witness (suite 2.41.0) — `core.orchestrator.supervisor` driven by the host's LIVE model (no mock keys; `config.instructions` is the objective), dispatching `conformance-noop` until it terminates, bounded by `settings.maxLoopbackIterations: 8`. The host reports each iteration's fed transcript through `GET /v1/host/sample/agent/transcript-window` including `entries[] { eventId, rendered }`; `context-budget-transcript-bound` recounts `chars`, checks real events + the recent tail + budget PRESSURE (no pressure ⇒ `partial-witness`); `context-summarization-replay` replays and compares the fed summary text byte for byte. A host whose supervisor has no live model MUST NOT advertise this fixture | `completed` | ≤ 300s |
...
| v2 `ctx.mcp` Client                       | `conformance-mcp-client`                                                                        | RFC 0204 — one `core.conformance.mcp-client` node (conformance-RESERVED) calls the host's `ctx.mcp.<method>` with the run's inputs and completes with `outputs.result` = the resolved value verbatim; a rejection fails the node with its code. See §"The `ctx.mcp` fixture (RFC 0204)" | `completed`, or `failed` (`not_found`) for an unknown `serverId` | ≤ 30s |
...
- **Expected behavior**:
  1. Run reaches `running` inside the delay.
  2. Client cancels; `effect` therefore records NO terminal outcome.
  3. Client forks `mode: "replay"`.
  4. The fork MUST NOT emit `node.completed` for `effect` (completing it means the effect was performed).
  5. The fork MUST emit `node.failed` for `effect` with `error.code: "replay_source_missing"` (`replay.md` §"Side-effect suppression in replay" requirement 3).
- **Why the fail-closed path**: a node replayed from a *recorded* outcome is indistinguishable in the event log from one that re-fired and recorded the same thing. A node with NO recorded outcome is not — a non-suppressing host completes it, a suppressing host cannot. That asymmetry is what lets this scenario run with no operator-side effect instrumentation.
...
## `conformance-credential` (RFC 0199 — the `credential` interrupt)

- **Purpose**: give the `credential` interrupt, and RFC 0047 §C.3's fail-the-node default, one fixture. One `conformance.oauth.use` node declares `auth: { type: "oauth2", provider: "synthetic", scopes: ["openwop.read"] }`.
- **Opt-in**: a host advertises the fixture only when it advertises `oauth` with the `synthetic` provider; unadvertised makes `v2-credential-interrupt` record `blocked`.
- **Inputs**: none.
- **Behavior**:
  1. Run by a Subject with no credential for `synthetic`, the node either suspends on a `credential` interrupt (`oauth.credentialInterrupt` advertised — snapshot `waiting-input`, `CredentialData` carrying `provider`, `scopes`, `reason: "missing"` and an https host-owned `connectUrl`) or fails (RFC 0047 §C.3, unchanged).
  2. The grant completes through `connectUrl` and the host's own callback; the host then resolves the interrupt itself with `{ outcome: "authorized" }` and the run continues.
  3. A caller resolve of `authorized` while no credential resolves is `400 validation_error`; `declined` fails the node with `connector_auth_declined`.
  4. After the `oauth/expire-refresh` seam (and a provider that refuses the refresh), a terminal refresh failure emits `connector.auth-expired` and then either suspends with `reason: "expired"` or fails the node with `connector_auth_expired`.
- **Timing**: unbounded (it suspends).

## `conformance-clarification-nested` / `conformance-clarification-sensitive` (RFC 0199 §D.2(d) probes)

- **Purpose**: the two schemas an MCP server mount MUST NOT bridge in form mode — one whose answer is an object (not a flat primitive), one marked `format: "password"`. `v2-mcp-mount-map` calls them as tools with and without `elicitation.url` and asserts no `mode: "form"` appears, with the flat `conformance-clarification` as the positive control.
- **Note**: the sensitive fixture exists to be REFUSED. Authoring a clarification that solicits credential material is itself forbidden (RFC 0199 §C.6); it ships only as the machine-checkable probe.
- **Timing**: unbounded (they suspend).
...
- **MCP Tasks (RFC 0198).** `v2-mcp-tasks` also calls the approvers variant of the approval fixture (a host-defined approval gate whose `approversList` names a principal the suite's bearer is not, advertised under the id `v2-approver-enforced` names), `conformance-delay` (with `arguments.delayMs`) and `conformance-cancellable` (with `arguments.delayMs`) as tools, each under its workflowId, so the task, disconnect and SSE-cancel legs have a non-listed-approver, a short and a long run to drive. The disconnect and SSE-cancel legs also need `runList` with the `workflowId` filter, to find a run the client never received a handle for. A fixture missing from `fixtures[]` records `blocked` for the leg that calls it; without `runList` those two legs are `inapplicable`.

---

## The `ctx.mcp` fixture (RFC 0204)

`conformance-mcp-client` is what `v2-mcp-client-results` drives. Its node `core.conformance.mcp-client` is conformance-RESERVED: a host that advertises `mcp.client` maps it to a node that calls its own `ctx.mcp` (`spec/v2/core/host-services.md` §`mcp`) and nothing else, and a host that does not MUST NOT advertise the fixture.

- **Inputs.** `method` ∈ `callTool` | `listTools` | `readResource` | `serverHealth`; `serverId`; `name` and `arguments` (callTool); `cursor` (listTools); `uri` (readResource).
- **Output.** The node's `node.completed.outputs.result` MUST be the resolved value verbatim (the v2 RunSnapshot carries no run outputs, so the observation path is the event log). A rejection fails the node, and `node.failed.error.code` is the rejection's code (`not_found` for an unknown `serverId`).
...
RFC 0199 adds `provider.issuer`, `provider.pkce` and the two issuer-less providers (`issuerlessProviders[]`) the distinct-redirect-URI leg needs; the endpoints a host actually talks to are supplied at run time by the `oauth/authorize-start` seam, which points them at the suite's authorization-server double (`src/lib/oauth-as-double.ts`).
