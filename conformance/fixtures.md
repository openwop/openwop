...
| Prompt All Four Kinds | `conformance-prompt-all-four-kinds` | RFC 0027 §A four-kind dispatch coverage. Single `mock-ai` node with one ref per `PromptKind` (`systemPromptRef`, `userPromptRef`, `schemaHintPromptRef`, single-entry `fewShotPromptRefs[]`); host MUST emit one `agent.promptResolved` per kind AND one `prompt.composed` per composition. Capability-gated on `capabilities.prompts.supported`. | `completed` | ≤ 10s |
...
| `conformance-prompt-few-shot` | `conformance.prompt.few-shot` | `kind: "few-shot"` template carrying two Q/A exemplar pairs. Variable-free. Used by the `prompt-all-four-kinds-events` scenario to exercise the few-shot dispatch slot (real provider dispatchers route each exemplar to a separate assistant/user message pair; the mock LLM concatenates them into the composed body). |
