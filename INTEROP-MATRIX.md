...
| **Postgres** (reference example, PARTIAL) | Multi-process durability path + the eventual `production`-profile claimant. Run-lifecycle slice works (discovery + run create + terminal poll + cancel + events poll + idempotency replay). Audit / interrupts / webhooks / observability / SSE are deferred to follow-up sessions per module. | `examples/hosts/postgres/` | `openwop-core` (partial — no SSE) | `minimal` (until production-profile ports land) | Not claimed | `examples/hosts/postgres/README.md` + in-process `test/lifecycle.test.ts` via pglite |
