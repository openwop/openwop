...
> **Last updated:** 2026-05-12 (Phase C round 1 — Postgres parity catch-up)
...
| **Python in-memory** (reference example) | Cross-language portability proof / Python 3.11 stdlib-only port of the TypeScript in-memory host. **Conformance verified 2026-05-12:** 670/782 (85.7%) default-mode pass against `@openwop/openwop-conformance`. 50 failures are all capability-gated scenarios where the host advertises fixtures whose runtime behavior isn't implemented yet (interrupts, bulk-cancel, BYOK, pack-registry, cap-breach, channel-ttl, etc.) — same honesty-cleanup posture SQLite resolved in Phase A. Every scenario the host's claimed profiles (`openwop-core` + stream surfaces) gate on passes. | `examples/hosts/python/` | `openwop-core` · `openwop-stream-sse` · `openwop-stream-poll` | `minimal` | Not claimed | `examples/hosts/python/conformance.md` |
...
| Python reference | 670 | 50 | 32 | 30 | 782 | **85.7%** (measured 2026-05-12) |
...
Python reference's 50 default-mode failures (measured 2026-05-12) decompose into the same category SQLite was in pre-Phase-A: capability-gated scenarios where the host advertises the underlying fixture but doesn't yet implement the runtime behavior. Categories: 7 interrupt scenarios (approval / clarification / quorum / auth-required / external-event / parent-child cascade — Python host advertises the interrupt fixtures but doesn't claim the matching `openwop-interrupt-*` profiles), 4 bulk-cancel (`POST /v1/runs:bulk-cancel` not yet implemented; new endpoint from Phase B 2026-05-12), 3 BYOK (secrets resolution not implemented), 3 pack-registry (registry endpoints not exposed), 2 cap-breach (recursion limit not enforced), 2 interrupt-cascade-cancel, plus singletons for channel-ttl pruning, conversation-capability refusal, dispatch-loop, identity-passthrough, pause-resume race, append-ordering, route-coverage, plus assorted scoped capabilities. None of the failures touch the `openwop-core` + `openwop-stream-*` profiles the host actually claims — those scenarios pass cleanly. Closing the 50 is a Python-host honesty-cleanup pass (mirroring SQLite's Phase A close-out) — stop advertising fixtures whose runtime behavior isn't implemented yet. Out of scope for the cross-language portability proof itself.
