...
| **SQLite** (reference example) | Single-machine durability / process-restart-safe walkthrough | `examples/hosts/sqlite/` | `openwop-core` · `openwop-stream-sse` · `openwop-stream-poll` · `openwop-audit-log-integrity` · `openwop-interrupt-quorum` · `openwop-interrupt-auth-required` · `openwop-interrupt-external-event` · `openwop-interrupt-cascade-cancel` · `openwop-auth-api-key-rotation` · `openwop-auth-oauth2-client-credentials` · `openwop-auth-oidc-user-bearer` · `openwop-auth-mtls` (audit + interrupts since 2026-05-11; four production-auth profiles since 2026-05-12 under RFC 0010 — rotation verified end-to-end via `OPENWOP_SECONDARY_API_KEY`; OAuth2-CC + OIDC + mTLS advertised at capability-shape level pending live-IdP / TLS-terminator wiring) | `minimal` | Not claimed | `examples/hosts/sqlite/conformance.md` |
