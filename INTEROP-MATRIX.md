...
| **SQLite** (reference example) | Single-machine durability / process-restart-safe walkthrough | `examples/hosts/sqlite/` | `openwop-core` · `openwop-stream-sse` · `openwop-stream-poll` · `openwop-audit-log-integrity` · `openwop-interrupt-quorum` · `openwop-interrupt-auth-required` · `openwop-interrupt-external-event` · `openwop-interrupt-cascade-cancel` · `openwop-auth-api-key-rotation` · `openwop-auth-oauth2-client-credentials` · `openwop-auth-oidc-user-bearer` · `openwop-auth-mtls`. **Conformance posture (production-auth, since 2026-05-12 under RFC 0010):** `openwop-auth-api-key-rotation` is **verified end-to-end** (two-key overlap + canary-redaction + grace-window advertisement, exercised via `OPENWOP_SECONDARY_API_KEY` against the host's dual-candidate `checkAuth`). `openwop-auth-oauth2-client-credentials`, `openwop-auth-oidc-user-bearer`, and `openwop-auth-mtls` are advertised at **capability-shape only** — the host does not yet parse JWTs or terminate TLS in its reference HTTP listener; behavior portions of the corresponding scenarios soft-skip on this host. Audit-log-integrity + interrupt profiles unchanged since 2026-05-11. | `minimal` | Not claimed | `examples/hosts/sqlite/conformance.md` |
