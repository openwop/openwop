...
| **Created** | 2026-06-07 |
| **Updated** | 2026-06-08 (`Active → Accepted` — non-steward host **MyndHyve** `workflow-runtime` (revision `00265-4p7`) seeds `core.conformance.degraded-cap-agent` (a `requiresCapabilities` key the host does not advertise) and the published `@openwop/openwop-conformance@1.21.0` `agent-capability-degraded-projection.test.ts` passes non-vacuously under `OPENWOP_REQUIRE_BEHAVIOR=true` + `OPENWOP_DEGRADED_CAPABILITY_AGENT_ID`. See [Amendment record](#amendment-record). |
...
- **Steward-curl-confirmed** (openwop-app-1, read-only key): `GET /v1/agents` → that agent carries a non-empty `degraded: ["x-host.conformance.unsupported"]` (§B; reuses the RFC 0072 §C `degraded[]` marker), while all-met agents omit it.
- MyndHyve PR #153. 2026-06-07 (`Draft → Active` — comment window waived per `GOVERNANCE.md` single-maintainer lazy consensus during the bootstrap phase, after a steward wire-shape review.
- The additive surface (the `AgentManifest.requiresCapabilities[]` field + the degraded projection reusing the existing RFC 0072 §C `degraded[]` inventory marker) is landed and validates: `spec-corpus-validity` green and the always-on `agent-requires-capabilities-shape.test.ts` passes; a manifest without the field stays valid (absent ⇒ no requirements).
