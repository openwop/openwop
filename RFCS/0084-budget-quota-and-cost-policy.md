...
- discovery doc root (steward-curl-verified field-for-field; budget has no normative read endpoint per UQ3, so the bar is the live advertisement + the green non-vacuous gated scenario + the §C-ordered durable trace).
- The gated behavioral scenario deferred at `Draft → Active` was the steward prerequisite, authored + published in `@openwop/openwop-conformance@1.16.0` (`budget-enforcement`, PR #444 + #445); MyndHyve passes it **non-vacuously under `OPENWOP_REQUIRE_BEHAVIOR=true`** (`1 passed / 1 passed`, 2204ms real-network, zero soft-skip lines) — the §C/§D enforcement proven on a 12-event durable trace: `budget.reserved → budget.consumed`(projected OFF the RFC 0026 `provider.usage` stream, no double-count)`→ budget.threshold.crossed{percent:80} → budget.exhausted → cap.breached{kind:"budget-cost"} → run.failed{error:"budget_exhausted"}` (reserved < threshold < exhausted < cap.breached < run.failed); `model-denied → budget_model_denied` refused pre-call (`modelCalled:false`, no `provider.usage`); `advisory → completed`, budget._ emitted without stopping; all `budget._` content-free (`budget-no-pricing-leak`).
- **Schema-hygiene fix folded in (compatible widening):** `run-event-payloads.schema.json#/$defs/capBreached` `limit`/`observed`relaxed`integer → number`so`cap.breached{kind:"budget-cost"}` can carry the same float dollars (`1.02`) the `budget.exhausted`payload already does (§D's example +`maxCostUsd`are dollars) — every prior integer payload still validates (integers ⊂ numbers), so no producer/consumer breaks.
- No other wire-shape change. —`Draft → Active`(2026-05-30) — steward acceptance, comment window waived per`GOVERNANCE.md`single-maintainer lazy consensus after MyndHyve (non-steward) wire-shape review; wire shapes now locked.
- All 5 Unresolved questions resolved as proposed: UQ1 host MAY coalesce`budget.consumed`but MUST emit reserved+threshold+exhausted; UQ2 consumed values are recorded facts → replay exhaustion point deterministic even if live pricing changed; UQ3 non-run-scoped budgets are host-config only at v1.x (no`GET /v1/budgets`); UQ4 `budget.maxRetries`is a ceiling over the RFC 0009 retry count, not a separate mechanism; UQ5`interrupt`resume carries an additive budget delta audited via a second`budget.reserved`.
- NEW `spec/v1/budget-policy.md`+`schemas/budget-policy.schema.json`+ the reserved`budget`run-options key + four content-free`budget._`events + four`cap.breached{budget-_}`kinds +`capabilities.budget`+`limits.maxBudget{Tokens,CostUsd}`+ the protocol-tier`budget-no-pricing-leak`invariant +`budget-policy-shape.test.ts`landed.
- The §E orthogonality seam with RFC 0058 (wall-time/iterations) is normative — zero dimension overlap, shared only via`cap.breached`. The behavioral enforcement scenario, the `budget_exhausted`/`budget_model_denied`OpenAPI error codes, and the reference-host accounting deferred to`Active → Accepted`.).
