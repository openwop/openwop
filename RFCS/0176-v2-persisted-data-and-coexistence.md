...
**§C.1** `/.well-known/openwop` is one resource. Its representation is selected by `OpenWOP-Version` per RFC 0172 §A.3: no header ⇒ the v1 document (with `protocolVersions[]` additive, RFC 0165 §A) through the overlap; `OpenWOP-Version: 2` ⇒ the closed v2 root (RFC 0169 §A.4). A single fetch answers the major the client speaks and names the other. The charter's "per-major sub-objects in one document" is not adopted: a v2 closed root cannot contain a v1 sub-object without reopening itself (adversarial review 3).
