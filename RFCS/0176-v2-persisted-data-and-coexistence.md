...
| §A.8 an era-`3` log reads on `/v1/…` in v1 vocabulary, via the same codemap row inverted | the codemap is a bijection (118 rows, 118 distinct on each side) and a host verifies that at load | a host serving both majors with runs created after the cut | witnessable — gated on the overlap |
...
- [x] `Active → Accepted` (Phase 3): `spec/v2/core/persistence.md`; `spec/v2/event-codemap.json` in the 2.0.0 tarball; the seven scenarios; the three consumers pinned. — evidence: the corpus half is present and green. **`reason:` the openwop-app clause is dropped from the tick and recorded here.** Two of its four ids are `executed-pass` on openwop-app — `well-known-one-resource` (and `.v2-representation`) and one leg of `v1-signed-webhook-accepted`. The other two, `v1-events-translated` and `fork-a-v1-run`, record `inapplicable` with the reason "seams profile not advertised (conformance.seamsProfile !== openwop-conformance-seams-v2)". This is the distinction `GOVERNANCE.md` §"Acceptance evidence tiers" insists on: **the seam is not mounted, which is not the same fact as the seam does not exist.** openwop-app has built the seam and does not mount it on the production revision (INTEROP-MATRIX Gap A), so it CAN witness this on a tagged side revision and has not yet; that is a deployment choice, not a build gap. MyndHyve records the identical `inapplicable`. All four ids are `executed-pass` tier-1 on the reference host, which advertises the seams profile.
