...
| `Rejected`   | Maintainers declined it (RFC 0166 §D.1). The file stays with the reason in `Updated`; the number is never reused. |
...
The RFC-authoring workflow produces two companion registers alongside substantial RFCs: `registers/<nnnn>-<slug>.gaps.md` (open design gaps discovered during authoring, each with an owner/disposition) and `registers/<nnnn>-<slug>.risks.md` (identified risks with severity and mitigation). Both live in [`RFCS/registers/`](./registers/), keyed to the RFC number (RFC 0174 §C.1, 2026-09-03: the 27 pairs that lived in `RFCS/` root were moved; `scripts/check-rfc-status-coherence.mjs` fails a register elsewhere).
...
Current tally: **Accepted 158 · Active 9 · Draft 1** (168 RFCs, excluding the `0000` template; Active = 0035 Parked, 0111 Parked, 0121 Parked, 0158, 0167, 0169, 0172, 0174, 0178; Draft = 0038 Parked).

Of the 10 non-`Accepted` RFCs, **4 are [Parked](#parked-rfcs)** on a named external tripwire and **6 await repository work**.
...
| [0163](./0163-subject-linking-hardening.md) | Subject-linking hardening — a **declarable, witnessable** link-key class (`capabilities.auth.subjectLinkKey`, a closed enum of allowed classes only) plus a **same-IdP trust-root MUST** before a SAML⟷SCIM link may form. The additive follow-on to RFC 0159 that converts its §A.2/§A.4 negative-existence claims-check into a positive advertisement and closes its cross-IdP collision gap. | `Accepted` |
| [0164](./0164-mandatory-subject-linking.md) | Mandatory SCIM ⟷ SAML subject linking for combined hosts — the RFC 0159/0163 leaver contract follows the **profile pair** (`openwop-auth-saml` + `openwop-auth-scim`); `capabilities.auth.subjectLinking` becomes a derived advertisement that MUST be `true` when both are advertised, deprecated toward v2. Classified `additive`: no production host advertised both profiles at filing. | `Accepted` |
| [0165](./0165-v2-preparation-wire-shapes.md) | v2 preparation — additive wire shapes: root `protocolVersions[]` (strict grammar, contains `protocolVersion`, reserved for v2 major negotiation); optional `owner.subject` Subject record (`schemas/subject.schema.json`: issuer-scoped, lane-typed, `keyClass` iff saml/scim, bounded `actor` chain) on `RunSnapshot` and the `run.started` echo, with the legacy-subject rule (`urn:openwop:legacy`, never linkable), fork copy of tenant + subject, and the A2A anonymous-actor rule; SHOULD-level dual emission of the `OpenWOP-*` webhook headers and a standard `ETag` on discovery. Additive; window waived under GOVERNANCE §"Sole-steward operation". `Active → Accepted` gates on the openwop-app + MyndHyve host legs | `Accepted` |
| [0166](./0166-register-dispositions-terminal-states-witness-classes.md) | Register dispositions, terminal states, witness classes — a closed disposition token at the head of every gap (`open|closed|transferred:|carried:|externally-gated:`) and risk (`open|mitigated|accepted|closed|transferred:`) register row, machine-checked (no terminal-status RFC with an open gap row); `spec/v1/gaps.json` as the one gap namespace (`openwop.gap.<rfc>.<n>`); `witness` class on every `SECURITY/invariants.yaml` entry and `extensions.json` record (initial classification mechanical and marked; three ratchets); `Rejected` as a reachable RFC status; the RFC 0147 high-risk cohort published as `docs/RETROSPECTIVE-QUEUE.md`; RFC 0155 §D's uncalibrated 12/4 budget repealed in favour of the witness requirement. Process, additive; window waived under GOVERNANCE §"Sole-steward operation" | `Accepted` |
| [0167](./0167-openwop-v2-umbrella.md) | **OpenWOP v2 — the program RFC.** Six axioms; eleven children (0168–0178) mapped 1:1 to the charter's C.1–C.11; every version axis (18) and alias (28 detectors, 26 register rows) enumerated with a disposition; `spec/v1/migrations.json` as the v1→v2 migration register (27 rows) checked against each child's table; a codemod contract with a three-part negative control and a self-sabotaging runner (`codemods/`, first codemod: the `capabilities` wrapper removal); the per-consumer migration plan for the nine repositories; the Phase 2 exit and the v2.0 cut gates. Adds program RFCs and children to RFC 0001 §3. `breaking` (v2; nothing in v1.x changes). Window waived under GOVERNANCE §"Sole-steward operation"; RFC 0147 §A.6 overridden and named. `Accepted` only at the cut. | `Active` |
| [0169](./0169-v2-discovery-and-capabilities.md) | **v2 discovery and capabilities (RFC 0167 child C.2).** One capability record type `{status, since, until?, witness, …facets}` on a closed root; one declaration file generating the schema, the pack peer-dependency identifiers and the spec anchors, retiring the four unschema'd registries; profiles derived in one generated registry with technical/adoption maturity and an `externally-gated` disposition; the wrapper, dotted mirrors, `contractProvenance`, `Capabilities-Etag`, `auth.subjectLinking`, bare A2A/MCP `supported`, `replay.fork` and the `openwop-core` alias removed. Migration rows C2.1–C2.10; codemod `openwop.codemod.discovery-document-v2`. `breaking` (v2). Window waived. | `Active` |
| [0172](./0172-v2-versioning-and-release.md) | **v2 versioning and release (RFC 0167 child C.5).** Major negotiation by `protocolVersions[]` + root `preferredVersion` + the `OpenWOP-Version` header on a bare origin with unversioned v2 path keys (no `/v2/` path space; v1 keys kept through the overlap); all 18 axes dispositioned (`engineVersion` unified — codemod `openwop.codemod.engine-version-unify`); `schemas/v2/` under `/spec/v2/`; the corpus tag as the only release event with every human-surface version generated. Retracts `rest-endpoints.md` §Versioning bullets 1–3, the gRPC `openwop.v2.Engine` analogy and AsyncAPI `pathname: /v1`. Migration rows C5.1–C5.9. `breaking` (v2). Window waived. | `Active` |
| [0174](./0174-v2-governance.md) | **v2 governance (RFC 0167 child C.7).** The six states used (supersession flips in the same PR; rejections filed; no number without a file); `Active → Accepted` as a machine predicate; waiver authority checked at merge (`check-waiver-authority.mjs`; the approval-count waiver recorded); registers in one directory (27 stray pairs moved), `carried:<self>` a ratchet and refused where the prose says CLOSED (45 rows re-tokened `closed`); document `Status:` banners derived from the owning RFC (four graduated; `self-hosted-runner.md` no longer forbids what RFC 0122 permits); RFC 0158's ladder as the maturity template; the front door under budget; prose gap tables into `gaps.json`; the host-inventory rule restated. Migration rows C7.1–C7.4. `breaking` (v2). Window waived. | `Active` |
| [0178](./0178-v2-assurance-registers-and-deprecation-machinery.md) | **v2 assurance registers and deprecation machinery (RFC 0167 child C.11).** `deprecations.json` normative in v2 (generated `deprecated: true` + `x-openwop-remove-in`; removal dates enforced at merge; the pack-manifest `deprecated` wire field renamed to avoid the keyword); `gaps.json` rows with a real witness class and a requirement id, `G<n>` as an alias until 2.1; falsifiability tables parsed and checked (`check-falsifiability.mjs`); contradicting gaps fail (`check-gap-contradictions.mjs`, first catch closed); every deprecation kind has a detector; schemas/README maturity words checked against RFC status; redocly step citation corrected. Migration rows C11.1–C11.6. `breaking` (v2). Window waived. | `Active` |
## See also
