...
**§B.1** `Active → Accepted` is computed by `scripts/check-accepted-predicate.mjs` (Phase 3 for v2 RFCs; v1.x RFCs are grandfathered as recorded): every acceptance-criteria box ticked or carrying a stated reason (the existing `rfc-lifecycle-coherence` leg); no `open` gap row (RFC 0166); a tier named in `Updated` (GOVERNANCE §"Acceptance evidence tiers"); a cited bundle ACCOUNTING for each requirement id the RFC's falsifiability table names (RFC 0148) — **either** at least one `executed-pass` row, **or**, when the row's verdict cell declares a non-executable witness class (`seam-gated`, `negative-existence`, `unwitnessable`, or `witnessable — gated` naming the gate), a non-pass row on a certified bundle carrying a reason. The second branch exists because the first alone refuses a corpus that did everything right: RFC 0177 §E.4's `form-when-reuses-edge-conditions` is recorded `inapplicable` with the reason “profile not advertised in the captured discovery set”, `check-cut-gates.mjs` §G.2 accepts exactly that, and a rule demanding `executed-pass` regardless leaves the honest move and the checkable move in conflict — whose only resolution is to mint a vacuous leg, the very defect this predicate exists to refuse. What the second branch never admits is SILENCE: the suite MUST have reached the leg and recorded why. **A falsifiability row MAY name a requirement id minted under another RFC's number** — twelve rows already do, this RFC's own included — when the row asserts the SAME requirement that id already witnesses, and the Observable cell names the owning RFC or the gate they share. Ownership does not move: the id stays minted and maintained under the RFC whose number it carries, and a citing RFC MUST NOT restate or narrow its meaning. What a cross-citation never admits is a NEW normative requirement borrowing an old witness — if the citing row states a rule the cited id's assertion does not exercise, the row is unwitnessed and needs its own id, however much gate machinery the two share. Nor may a row cite an id whose owning RFC is not `Active` or `Accepted`, or one carrying no `executed-pass` row of its own: a citation is never stronger than the evidence it points at, and a chain of citations to nothing is the vacuous pass with extra steps. For a program child, the parent `Active` — and the parent flips LAST, since a parent that is `Accepted` is no longer `Active` and would otherwise strand every child still awaiting its own flip. A flip that fails the predicate fails the gate.
...
**§E.1** RFC 0158's ladder (`durable-single-instance → multi-instance → multi-region-qualified`, each rung named with its evidence, no discovery field) is the template for every per-host maturity claim in v2: a rung is evidence, never an advertisement. **§E.2** `spec/v2/core/` is under 25,000 words (`check-core-budget.mjs`, Phase 3). **§E.2a (2026-09-17).** The ceiling stays at 25,000 and the rule already practised is made explicit: **core prose carries the MUSTs; rationale, measurement and history live in schema `description` / `$comment` fields, which the budget does not count.** Three core MUSTs landed in one week at 24,999 of 25,000 by moving their arguments into `ids.schema.json`, `run-event-payloads.schema.json` and `suspend-request.schema.json`; that is the forcing function working, and raising the ceiling would remove it. Two guards follow from the near-miss it also produced — one compression briefly changed `MUST NOT rewrite era-2 rows in place` to `MUST NOT rewrite era-2 rows` before review caught it: **(i)** a budget trim MUST NOT alter a normative clause — diff the RFC 2119 sentences of the edited section before and after, and pay from connectives, parentheticals and restatements only; **(ii)** a paragraph that duplicates a schema description is deleted in favour of the schema, not compressed. Raising the ceiling was considered and rejected: 16 core RFCs have landed against it, and the near-miss came from hurry, not from the number. Every `spec/v2/ext/<name>/` document declares `witness` and both maturity axes in its header; a MUST with `witness: unwitnessable` may not appear in `core/`; a document whose only witness is "deferred to Active → Accepted" enters `ext/` or is deleted. `/v1/goals`, `/v1/export`, `/v1/import` enter `api/v2/openapi.yaml` or their RFCs' MUSTs are demoted — decided in C.1 with the seams profile. **§E.3** The 43 "Open spec gaps" tables are absorbed into `gaps.json` in Phase 3 (`generate-gaps.mjs` gains `spec/v2/**` as a source with a 2-column adapter) and deleted from prose; a gap citing an artifact that contradicts it fails (C.11).
...
- [ ] `Active → Accepted` (Phase 3): `check-accepted-predicate.mjs` and `check-core-budget.mjs` in the gate; the 43 prose gap tables absorbed and deleted; `spec/v2/core/` under budget with every `ext/` header declared; §B.4 restated in `spec/v2/core/`. — met at the RC: both checks run in `scripts/openwop-check.sh` stage 10; the 43 prose gap tables are absorbed into `spec/v1/spec-gaps.json` (196 rows across 43 documents, both row shapes mapped, ids `(doc, row-order)`-derived) and deleted from prose, each document left carrying its absorption pointer, which `generate-gaps.mjs --check` verifies; `spec/v2/core/` measures 20,899 of 25,000 words across 20 documents and every one of the 13 `ext/` families declares `witness` and both maturity axes in its README header (`check-declaration.mjs` leg 5); §B.4 is restated at `spec/v2/core/overview.md` §"v1 end-of-support". deferred: nothing.
