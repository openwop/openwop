...
| **Created**       | 2026-09-22                                                      |
| **Updated**       | 2026-09-26 — **`Active → Accepted`, provisional pending RFC 0156 §B retrospective review** (it went `Active` under a waived window; register row `not-reviewed`). Evidence tier: tier-1 — the v2 reference host (openwop-examples), a reference example and not a production host; single host witness: the certified public v2-reference cut on published 2.40.1 (openwop#1591, evidence commit `7c7f61dd`; build `commit:265fca7`, witness `d8750e4b8117`, file sha256 `5939c270e27e`, signed `v2-reference-4`, 403 pass / 0 fail / 0 blocked, relaxations `[]`, RFC 0158 rung `durable-single-instance`, steward `--verify --host-key` with the published suite → VERIFIED) records `0205.artifact-a2a-shape` (3), `.artifact-id-matches-path` (2) and `.turn-parts-emitted` (1) `executed-pass`, and `.export-alias-equivalence` `inapplicable` with its §C.11 falsifiability reason. MyndHyve's certified 2.39.5 bundle (openwop#1585) records both host rows `inapplicable` — it does not advertise the `conformance-artifact-emit` fixture and emits no A2A-shaped turns — so it is not a witness · 2026-09-22 (`Draft → Active` in the filing PR. **Comment window waived** (additive, 7-day) by the steward on 2026-09-22 under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md` §"Bootstrap-phase RFC waivers". RFC 0147 §A.6 does not apply. The RFC adds two optional representations and widens one string grammar. Its single isolation sentence, §A.3, restates for the native read the outbound `url` rule that `a2a-integration.md` D.3 (RFC 0152, `Accepted`) already imposes on the same artifact over A2A. No identity, authorization, isolation, idempotency, replay, external-effect or certification rule is created or relaxed, and persisted turns are read exactly as before.) · **Updated 2026-09-22 — amended per implementation review** (`review/arch-impl.md` R4, R6; Status unchanged: the `(corpus)` rows are minted by a coherence test in `conformance/src/coherence/`, where the corpus ledger is emitted, not by scenario server-free legs; `turn-parts-shape` is split into the server-free `turn-parts-shape` and the behavioural `turn-parts-emitted`, so no bundle satisfies the behavioural box vacuously; the scenarios ride the open 2.36.0 suite (decisions log D3)) |
...
- **New coherence test (planned): `conformance/src/coherence/a2a-parts-schemas.test.ts`.** It carries every server-free leg: `part.schema.json` and `artifact.schema.json` against the A2A examples, both turn defs, the mirror check and both manifest schemas' `exportFormats`. It mints the four `(corpus)` ids of the Falsifiability table, because `evidence/corpus-ledger.json` is emitted only from `conformance/src/coherence/` (`check-spec-coherence.mjs`) and `check-accepted-predicate` satisfies a `(corpus)` row from that ledger alone. No scenario file mints a `(corpus)` id.
...
- **New major-2 scenario (planned): `v2-conversation-turn-parts.test.ts`.**
  - One gated behavioural leg, minting `openwop.requirement.0205.turn-parts-emitted` (not `turn-parts-shape`, which is the server-free id). It runs on hosts that advertise `conversationPrimitive` and a conversation fixture. Every `conversation.exchanged` turn that carries `parts` must validate. The leg is `inapplicable`, with that reason, if no turn does.
- **New legs on `artifact-type-pack-manifest-validation.test.ts`** (planned, major 1): media-type positives, and the alias-table entries validate. They are supplementary; `export-media-type` is minted by the coherence test above.
- **Suite.** The scenarios ride the open, unpublished 2.36.0 suite and `@openwop/spec-artifacts` 2.36.0 (decisions log D3). No PR in this batch bumps a version pin; the single publish is the steward's call.
...
| §B.5 `parts`, when present, is a valid `Part[]` (`openwop.requirement.0205.turn-parts-shape`) | a turn `{…, parts: [{text:'a', raw:'YQ=='}]}` (two content members) or `parts: []` fails both v2 turn defs; `[{text:'a'}]` passes (planned coherence test `a2a-parts-schemas.test.ts`) | the suite, unaided (corpus) | witnessable — unaided (corpus): sabotage by dropping the `oneOf` from `part.schema.json` and the two-member negative passes |
| §B.5 a host's emitted turns carry valid `parts` (`openwop.requirement.0205.turn-parts-emitted`) | a `conversation.exchanged` turn in a live run's log whose `parts` fails the v2 turn def (planned `v2-conversation-turn-parts.test.ts`) | the suite, on a host advertising `conversationPrimitive` and a conversation fixture | witnessable — gated on that advertisement; `inapplicable`, with that reason, when no emitted turn carries `parts`, **never** `executed-pass`. The behavioural twin of `turn-parts-shape`, split so a server-free pass cannot stand in for it |
...
- [x] At least one host (MyndHyve, which advertises `artifactTypes` and `conversationPrimitive` in its committed v2 bundle, or the v2 reference host) records `openwop.requirement.0205.artifact-a2a-shape` or `openwop.requirement.0205.turn-parts-emitted` `executed-pass` on a behavioural leg (`turn-parts-shape` is server-free and never satisfies this box). Otherwise the RFC records why no host does and is accepted on the corpus rows (tier: corpus gate). *Not yet:* the v2 reference host implements both (openwop-examples v2-reference, RFC 0205 host PR) and records `artifact-a2a-shape`, `artifact-id-matches-path` and `turn-parts-emitted` `executed-pass` on a local loopback boot against the unpublished 2.36.0 tree; no committed bundle carries them until a cut on the published suite. — **Met 2026-09-26 by the v2 reference host:** the certified public v2-reference cut on published 2.40.1 (openwop#1591, evidence commit `7c7f61dd`; build `commit:265fca7`, witness `d8750e4b8117`, file sha256 `5939c270e27e`, signed `v2-reference-4`, 403 pass / 0 fail / 0 blocked, relaxations `[]`, RFC 0158 rung `durable-single-instance`, steward `--verify --host-key` with the published suite → VERIFIED) records `artifact-a2a-shape` `executed-pass` (3 assertions), `artifact-id-matches-path` (2) and `turn-parts-emitted` (1), each on a behavioural leg; `artifact-url-part-scoped` is `inapplicable` (the host emits no `url` Part, register G3) and `export-alias-equivalence` `inapplicable` (§C.11: unwitnessable on the wire). The same rows first passed on the certified 2.39.5 cut (openwop#1575). MyndHyve, named first here, is **not** a witness: its certified 2.39.5 bundle (openwop#1585) records both host rows `inapplicable` (no `conformance-artifact-emit` fixture; no A2A-shaped turns).
