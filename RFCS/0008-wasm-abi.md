...
**Multi-value vs packed-i64 encoding.** WebAssembly multi-value returns are first-class in the spec but require the `multivalue` target feature, which is not enabled by default on stable Rust's `wasm32-unknown-unknown` toolchain (as of 2026-05). Conformant modules MAY satisfy any `(result i32 i32)` signature in this RFC using *either* native multi-value OR a packed `i64` encoding: low 32 bits = ptr, high 32 bits = len. Hosts MUST support both:
