...
- extension), and passed both gated behavioral scenarios non-vacuously under `OPENWOP_REQUIRE_BEHAVIOR=true` vs published conformance 1.12.0 — `org-position-no-authority-escalation` (the §B projector strips every authority field off the live wire) + `agent-org-chart-scoping` (acyclic tree, cross-tenant 404, §D roll-up.
- dedup).
- Steward independently curl-verified the live host at rev `workflow-runtime-00426-lur` (api.myndhyve.ai).
- Mirror of the RFC 0086/0077 graduation (#396).) <br> 2026-05-30 (Draft → Active — wire surface landed: NEW `agent-org-chart.schema.json` (departments/roles/members over RFC 0086 `rosterId`s + acyclic `reportsTo`; NO authority field, every object `additionalProperties:false`) + additive `agents.orgChart` capability (REQUIRES `agents.roster`; `departmentNesting`/`responsibilityView`) + NEW normative `spec/v1/agent-org-chart.md` (§A record / §B the load-bearing non-authority invariant / §C tenant scoping / §D responsibility roll-up / §E capability) + always-on `agent-org-chart-shape.test.ts` (incl. the structural §B assertions: the schema rejects a `scopes`/`canDispatch`/`permissions`/`authority` field; a member's key set is descriptive-only) + NEW protocol-tier SECURITY invariant `org-position-no-authority-escalation`.
- NO new RunEventType (the org-chart is structure + a read, not an event surface — §Alt-5).
- The reference impl shipped as a host-extension at `/v1/host/sample/org-chart` (apps/workflow-engine, #371). The `GET /v1/agents/org-chart` endpoint, SDK helpers, behavioral non-authority + scoping scenario, and reference-host normative wiring defer to `Active → Accepted` per the RFC 0086/0077/0082 precedent.).
