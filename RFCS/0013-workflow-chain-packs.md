...
**Tracked follow-up (closes this RFC's real Phase-3 debt, independent of RFC 0124):** bundle `vendor.openwop.workflow-chain-sample` into `@openwop/openwop-conformance` as a loadable fixture (so the scenario loads the pack instead of hardcoding its expansion), wire the in-memory host's existing `expandChain()` to the `/…:expand` route + advertise `hostExpansionSeam: true`, then re-check the acceptance item above. Only then is the cross-host host-expansion contract witnessed by a serving host.
...
- [ ] Reference host expansion implementation. **⚠️ Erratum (2026-07-05): this item was recorded as done but is NOT landed.** The in-memory reference host does **not** serve `POST /v1/host/sample/workflow-chain:expand` and does **not** advertise the capability (`examples/hosts/in-memory/src/server.ts` has no chain-expansion route), and the `vendor.openwop.workflow-chain-sample` fixture is **not** bundled into `@openwop/openwop-conformance` (it lives only in `openwop-examples` + as hardcoded assertions inside `workflow-chain-host-expansion.test.ts`). So the live-host scenario **soft-skips against every host today** — its positive legs are unwitnessed. What IS landed: the per-host pure-function smoke (`examples/hosts/in-memory/test/workflow-chain-expansion.test.ts`, 5 cases) and the four **server-free** scenarios (`workflow-chain-{expansion,pack-manifest-validation,pack-signature-verification,unresolvable-typeid}.test.ts`), which witness the expansion algorithm itself. The live-host wire witness remains **open** — see the erratum note below. (RFC 0124's deferred-parameter path is separately witnessed via `workflow-chain-deferred-parameters.test.ts` + `/v1/host/sample/chain/deferred-expand`; it does NOT depend on this seam.)
