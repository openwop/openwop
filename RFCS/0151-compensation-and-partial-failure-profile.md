...
- [ ] Threat models and invariants land. (2026-08-16: threat model landed — `SECURITY/threat-model-compensation.md`, indexed in `SECURITY.md`; `compensation-replay-no-refire` registered and re-homed to it. Carried: `compensation-effect-id-retry-stable`, `compensation-tenant-authority-bound`, `compensation-input-recorded-facts-only` — named, not registered, because the §21 seams do not yet exercise those threats; the threat model §7 names the seam extension each needs.)
