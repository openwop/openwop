...
| **Created** | 2026-05-19 |
| **Updated** | 2026-05-29 (`Active → Accepted`: every acceptance-criteria item is satisfied and the surface is now **verified end-to-end against a running host**. See [Amendment record](#amendment-record). |
...
- The reference impl at `apps/workflow-engine/backend/typescript/src/routes/packs-test.ts` (547 lines — `PUT/GET/DELETE /v1/packs-test/{name}/-/{version}[.tgz                                                                                                                                                                       \| .sig]`+`POST /v1/packs-test/reset`, validation pipeline in spec order, isolated in-memory catalog) is mounted + advertised behind`OPENWOP_PACKS_TEST_NAMESPACE_ENABLED=true`, with`discovery.ts`emitting`capabilities.packs.testMode.{supported: true, isolated: true, catalogResetEndpoint, scopes}`.
- Booted locally, all **26 scenarios pass under strict`OPENWOP_REQUIRE_BEHAVIOR=true`** —`pack-registry-publish.test.ts`(the 25-code publish-error catalog) +`pack-registry-isolation.test.ts`(the §C point-1 test-catalog isolation invariant) — i.e. they genuinely run, not soft-skip.
- Per RFC 0001 §3 (implementation landed + conformance updated), both criteria met; no explicit non-steward bar.
- MyndHyve operates a`PackRegistryStorage`surface and has flagged`packs.testMode`as plausibly in scope (pending the`/v1/packs-test/\*` contract) — a future second-host advertisement would strengthen, not gate, this status.).
