...
- [ ] `spec/v1/localized-content.md` merged (data model, per-section merge, delivery + admin endpoints, capability advertisement, security invariants, Status legend).
- [ ] The four schemas + the `content` capability block merged; schema discipline (`ajv`/`redocly`) green.
- [ ] `api/openapi.yaml` paths added (public delivery + admin CRUD), `Vary`/`Cache-Control`/`Content-Language` documented; `redocly` green.
- [ ] ≥1 conformance scenario covering the new surface, capability-gated, in a published `@openwop/openwop-conformance` release; suite version bumped.
- [ ] CHANGELOG `[Unreleased]` entry; INTEROP-MATRIX row reflects the reference host's **actual** pass (emit/advertise honesty).
- [ ] Reference host implements and passes the new scenarios (after HOST-1 annex prerequisite), or the RFC explicitly defers reference-host implementation.
- [ ] Register sweep: every open row in `registers/0103-localized-content-surface.{gaps,risks}.md` closed, transferred, or carried forward as a named open gap here.
