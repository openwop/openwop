...
| **Created** | 2026-05-22 |
| **Updated** | 2026-06-01 (`Active → Accepted`) — graduated as a **steward-side mechanism-codification with an AMENDED acceptance criterion** (the RFC 0069 precedent — a corpus-level mechanism with nothing behavioral for a non-steward host to advertise — composed with the RFC 0054 / 0080 amended-criterion precedent for deterministic, non-security, read-only metadata). See [Amendment record](#amendment-record). |
...
**Amended 2026-06-01 (architect-gated, David-confirmed) — the RFC 0080 precedent.** RFC 0042 is a _stability-signaling metadata convention_, not a behavioral capability: it has no host-side logic, so the live "advertise `tier:"experimental"`" direction is mechanically-equivalent-by-construction to the schema + the deterministic `experimentalGate()` helper, AND the only honest way to make it non-vacuous would require a host to mark a _stable_ capability `experimental` — a false-stability signal the honest-advertisement invariant forbids (the same wall as RFC 0080's degraded keystone). The two original criteria that can only be met by fabrication — _"Reference workflow-engine advertises at least one capability sub-block with `tier: "experimental"`"_ and _"At least one non-steward implementer signals adoption"_ — are therefore **struck**; the deliverable is the mechanism itself, and the field is exercised the first time a host honestly holds an `Active`-RFC-preview capability to mark.
...
- **Architect-gated decision; David-confirmed.** RFC 0042 is a _stability-signaling convention_, not a behavioral capability: its guarantee decomposes entirely into (a) the schema `tier` enum + conditional-required `experimentalUntil` (`capabilities.schema.json`), (b) the deterministic `experimentalGate()` helper (soft-skip under default mode / strict under `OPENWOP_REQUIRE_EXPERIMENTAL=true`, `conformance/src/lib/behavior-gate.ts`), (c) the always-on server-free `experimental-tier-shape` §A shape group + the §C `openwop-experimental` derived profile prose — all green on `main`.
- There is **no host-side logic**: a host "exercising" 0042 merely writes the string `"tier":"experimental"` into its discovery JSON, which proves nothing the schema does not already guarantee well-formed, and the soft-skip it triggers is the helper's deterministic branch, not host behavior — so the live "advertise `tier:experimental`" direction is **mechanically-equivalent-by-construction** to the schema+helper.
- Moreover, the only honest way to make it non-vacuous would require a host to mark a _stable, Accepted-RFC-backed_ capability `experimental` — an **active false-stability signal on the live wire** that the honest-advertisement invariant (the same one behind RFC 0080's degraded keystone) forbids; the non-steward adopter MyndHyve correctly **refused to fabricate** one (its `multiAgent.executionModel` is `version:5`, stable, every constituent RFC Accepted).
- The field is genuinely exercised the first time a host honestly has an `Active`-RFC-preview capability to mark — a real future event, never a fabricated one.
- **Honest disclosure:** the schema currently anchors `tier`/`experimentalUntil` ONLY on `multiAgent.executionModel`; every other capability sub-block keeps `additionalProperties:false` with no `tier` property and would reject one today, so §A's "every capability advertisement" is wire-enabled incrementally per the §"Per-RFC application" SHOULD — generalizing the schema is clean additive follow-up,.
- not a graduation blocker.
- 2026-05-29 (`Draft → Active` — steward acceptance, comment window waived per GOVERNANCE.md lazy consensus.
- Wire surface **reconciled**: §A/§B/§D normative prose landed in `capabilities.md` §"Capability stability tier" + §C `openwop-experimental` profile in `profiles.md`; the schema `tier`/`experimentalUntil` on `multiAgent.executionModel`, the `experimental-tier-shape` conformance scenario, the `experimentalGate` helper, and the INTEROP column were already on `main` — landed incidentally under commit `45678c49`, not a 0042 PR — and are now backed by prose.
- Extending `tier` to the other §"Per-RFC application" sub-blocks is a per-host follow-on SHOULD, not a graduation blocker.).
