...
| **Created**       | 2026-09-03                                                      |
| **Updated**       | 2026-09-03 (`Draft → Active` in the filing PR. **Comment window waived** under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md`; RFC 0001 §5 cross-org rule not yet active; RFC 0147 §A.6 overridden and named in the parent, RFC 0167 — authorization, isolation, replay, and external effects are this child's whole surface. Adversarial review recorded below.) · 2026-09-03 (filed) · 2026-09-18 (`Active → Accepted`). **Evidence tier: tier-1 — steward-verified**, partially corroborated tier-2. Witness: `openwop-host-v2-reference` build `commit:87cb8a007d05`, suite 2.4.2, `witnessSha256 9217e347b85f…`, signed `v2-reference-3`, all three profiles certified. **Stated plainly:** that host is tier-1 by `GOVERNANCE.md` §"Acceptance evidence tiers" (a reference host in `openwop-examples`), but it is run from the pinned example and its bundle's `discovery.url` is `http://127.0.0.1:3838` — it is not a host that serves traffic, and this acceptance does not call it deployed. MyndHyve corroborates `webhook-durable-delivery` and the approver-enforcement ids. **This PR also flips RFC 0035 `Superseded`** (register G1, RFC 0174 §A.1 — both edits in one PR; RFC 0001 fixes the timing at the replacement's acceptance). **Said plainly: 0035's tripwire asked for a NON-STEWARD host and is retired by replacement, not satisfied.** `pack-isolation` is `executed-pass` (ten legs) on the steward's own reference host and `inapplicable` on both production hosts; no non-steward host ever advertised `capabilities.sandbox.supported`. The seven protocol-tier `node-pack-sandbox-*` invariants and the four `sandbox_*` error codes keep RFC 0035 as their v1 citation through the overlap, to 2026-12-04. |
| **Affects**       | **Part of: RFC 0167 — child C6.** v2 (Phase 3): `spec/v2/core/security-defaults.md` (NEW; the obligation table), `spec/v2/core/replay.md` §Effect seams, `schemas/v2/effect-seam-manifest.json` (NEW), `schemas/v2/capabilities.schema.json` (the fourteen `auth.*` gate fields, `replay.sideEffectSuppression`, `webhooks.durable`, `interrupt.approverRouting.enforced`, `sandbox.supported` removed as gates; `sandbox.isolationModel` and `auth.lanes[]` kept as facets), `SECURITY/threat-model-replay.md` §6–§8, `SECURITY/invariants.yaml` (effect-seam invariants with tests); `spec/v1/extensions.json` (`openwop-compensation-v1` note corrected — this PR, data). v1.x (this PR, data only): `spec/v1/migrations.json` rows `openwop.migration.C6.1`–`C6.9`; deprecation rows `security-opt-in-flags`, `supported-durable-flag` (`proposed`); register rows **Leaves standing (NOT superseded):** RFC 0069, 0083, 0104, 0119, 0140, 0150, 0151, 0157 and 0164 remain the v1 authorities through the overlap. This list lives here rather than in `Supersedes` because `check-rfc-status-coherence.mjs` harvests every four-digit number in that cell as a supersession target — naming them there made this RFC claim to supersede nine Accepted RFCs and produced eight spurious gate failures. |
...
| **Supersedes**    | RFC 0035 — the sandbox execution contract; its obligation moves to this child's packs obligation at the v2 cut. (No other RFC is superseded here; see `Affects` for the v1 authorities this child leaves standing.) |
| **Superseded by** | —                                                               |
...
| §B webhook durability — RETRY | exhaustion is observed: the receiver's attempt log shows the backoff and the terminal give-up — `openwop.requirement.0173.webhook-durable-delivery` | the suite, gated on `webhooks` | witnessable — gated |
| §B webhook durability — DEAD-LETTER | nothing. `openwop.requirement.0173.webhook-durable-delivery.dead-letter` ends in an UNCONDITIONAL soft-skip on every host, because `api/v2/openapi.yaml` serves no dead-letter read: the suite watches a subscriber exhaust its retries and then has no surface on which to see where the delivery went. Split from the retry row 2026-09-18 — the two were one row naming one id, so the retry pass stood in for both halves and the sink half read as witnessed on every bundle in the corpus, certified ones included | a host with a dead-letter read surface | negative-existence — unwitnessable until `webhooks.md` §Durability's "inspectable" sink has an endpoint; the `deadLetter` capability family already declares `retentionDays` and nothing reads it. **Recorded limitation:** suite 2.4.5 is the first that annotates this leg `partial-witness:`, and rule 4 refuses such a row — but MyndHyve's cited bundle was cut on 2.1.7 and carries an un-annotated `executed-pass`, so the gate still accepts it from there. This row reads honestly only once every cited bundle is re-cut on 2.4.5 or later. |
...
- [x] `Active → Accepted` (Phase 3): `spec/v2/core/security-defaults.md` + the obligation table; `schemas/v2/effect-seam-manifest.schema.json` and `GET /host/effect-seams`; the gate fields removed from `schemas/v2/capabilities.schema.json`; bundle v3 `host.relaxations[]`; `threat-model-replay.md` §6–§8; `check-threat-model-template.mjs`; the nine scenarios; openwop-app passes `effect-seam-manifest`. — evidence: `effect-seam-manifest` is `executed-pass` on ALL THREE bundles, openwop-app's included. Path corrected in this edit: the schema is `schemas/v2/effect-seam-manifest.schema.json`. **`reason:` the other two host clauses are dropped from the tick and recorded here, and they fail for opposite reasons.** `webhook-durable-delivery` is `blocked` on openwop-app with the reason "host SSRF guard rejected the loopback receiver" — that is production **correctly refusing** to call back to a loopback address, so the requirement is unobservable there, not unmet; a publicly reachable receiver is the only missing precondition. It is `executed-pass` tier-1 on the reference host and tier-2 on MyndHyve. `compensation-read-projection` is `inapplicable` on openwop-app AND on MyndHyve — neither advertises the profile — and `executed-pass` on the reference host only.
