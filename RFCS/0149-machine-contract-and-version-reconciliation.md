...
| **Created** | 2026-08-11 |
| **Updated** | 2026-08-16 (UQ3 resolved by measurement — one deployed `1.0.0` exists (MyndHyve tier-2), no corpus normalization; INTEROP row caveated.) 2026-08-16 (§B canonical-typo lint landed — UQ2 measured then decided: distance one on discovery-shaped root keys, `spec/v1` + RFCs ≥ 0149; 0 findings; acceptance item "authoring lint" checked.) 2026-08-16 (§D example extraction landed — See [Amendment record](#amendment-record). |
...
**Example extraction landed 2026-08-16** (suite `1.116.0`) as `normative-example-extraction.test.ts` with an explicit declaration rather than a heuristic: `<!-- normative-example: <name>.schema.json -->` on the line above a fence marks a whole-instance example; the suite extracts and validates every declared example, and fails on any undeclared whole-instance example. Measured before landing: 183 fenced JSON blocks in `spec/v1`, 111 strict-JSON, 26 whole instances of a schema — those 26 are declared; the rest are fragments and stay prose. "Into fixtures" is satisfied in-process (extraction at test time against the same schema files) rather than by committing copies, so the prose stays the single source.
...
2. ~~What edit-distance rule avoids false positives for legitimate extension names?~~ **Resolved 2026-08-16 — measured, then decided.** Plain distance-one over every fenced root object in `spec/v1` + `RFCS/` (218) produced six near-misses (`ts`/`fs`, `agent`/`agents`, `secret`/`secrets`, `context`/`content`, `prompt`/`prompts`, `schemaVersion`/`schemaVersions`), all keys of event or run objects. So the rule is distance one applied to **discovery-shaped** objects only — every root key canonical, vendor-namespaced, the legacy wrapper, or within one edit of a canonical family, and at least one canonical-or-near — in `spec/v1` and RFCs numbered ≥ 0149 (older RFCs are the dated record, the wrapper leg's 0073 logic). 53 discovery-shaped objects, 0 findings; a typo-only snippet (`{ "compensaton": … }`) is caught. Landed as the third leg of `capability-example-root-layout.test.ts` with the predicate pinned.
3. ~~Is a temporary `1.0.0` normalization warning required for any deployed host?~~ **Resolved 2026-08-16 by measurement: no normalization, and yes there is a deployed `1.0.0`.** The four reference hosts advertise `1.0` and openwop-app `1.1` (`app.openwop.dev/api/.well-known/openwop`, deploy `8b2cc5ab`), but the tier-2 sibling `api.myndhyve.ai` served `"1.0.0"` on 2026-08-16. A consumer-side normalization (`1.0.0` → `1.0`) would re-open exactly the ambiguity §C closed — a consumer could no longer tell a patch convention from a typo from a different protocol — so the corpus does not add one; under the shipped predicate that host does not derive `openwop-discovery-core` and fails `protocol-version-grammar` until it advertises `1.0`, and `INTEROP-MATRIX.md` says so on its row. The fix is one string on the host.
...
- [x] OpenAPI paths resolve correctly and match AsyncAPI/SDKs. (Resolution landed — §A, `openapi-resolved-paths.test.ts`. **2026-08-16, suite `1.114.0 -> 1.115.0`: the AsyncAPI/SDK parity half landed** — `openapi-asyncapi-sdk-parity.test.ts`: (1) `spec/v1/operation-path-manifest.json`, the generated canonical operation-path manifest (`scripts/generate-operation-path-manifest.mjs`, `--check` in `openwop:check`), re-derived in-test from `api/openapi.yaml` and compared byte-for-byte, every row exactly one `/v1`; (2) every AsyncAPI channel `address` resolves under the AsyncAPI server `pathname` to a GET path key of the OpenAPI document with exactly one `/v1`; (3) the SDK half reaches into `openwop-sdks` when reachable (`OPENWOP_SDKS_DIR` or the sibling checkout): `sdk/parity-expectations.json` covers every canonical operation with the same method + resolved path, declares no orphan, and the TypeScript host client (`client.ts` + `sse.ts`) issues every ts-typed path and no `/v1` path outside the contract or `/v1/host/*`. When the SDK tree is not reachable the requirement `openwop.requirement.rfc0149.sdk-path-parity` is recorded `blocked` with the reason (RFC 0148 §A) rather than passing vacuously — that is the honest shape of a cross-repository requirement in a single-repository runner. Witnessed green against the local `openwop-sdks` checkout: 47 canonical + 5 host-extension + 4 test-catalog operations, zero drift.)
- [x] All normative discovery examples use root layout and validate. (Root layout landed — §B, all eight `spec/v1` examples unwrapped. **"And validate" landed 2026-08-16, suite `1.115.0 -> 1.116.0`** — `normative-example-extraction.test.ts`: a fenced example becomes a validated instance by declaring its schema with `<!-- normative-example: <name>.schema.json -->` on the line above the fence; every declared example is extracted at test time and validated against `schemas/<name>` with the same Ajv registration the rest of the suite uses, and every fenced block that validates as a whole instance of some non-permissive schema MUST be declared (the inverse guard — sabotage-verified both ways). 26 whole-document examples across 19 `spec/v1` docs declared in the same change (`capabilities`, `error-envelope` ×14, the six pack-manifest kinds, `run-snapshot`, `prompt-ref`, `localized-content-section`, `audit-verify-result`); the one whole discovery example (`capabilities.md`) is declared against `capabilities.schema.json`, validates, and carries no credential/tenant keys (§E leg). Extraction is in-process rather than committed under `fixtures/` — a committed copy of a prose example is a second source of truth that drifts, which is what §D exists to remove; `CONTRIBUTING.md` §Prose specs states the authoring rule.)
- [x] Authoring lint catches wrappers, canonical typos, and shadows without closing runtime discovery. (Wrappers landed §B; shadows landed §E; **canonical typos landed 2026-08-16** — UQ2 resolved by measurement (see above), third leg of `capability-example-root-layout.test.ts`, discovery-shaped predicate pinned, 0 findings in scope. Runtime discovery untouched: the lint reads the corpus, not a host.)
- [x] Version grammar landed — §C, enforced in the schema `pattern`, `profiles.ts`, and `version-negotiation.md`. No migration warning phase: with no deployed hosts, the steward's call was to enforce directly rather than warn-then-enforce, so the 90-day step this item anticipated does not apply.
- [ ] Lifecycle coherence gate passes across the corpus. (Landed for RFCs >= 0147 — `rfc-lifecycle-coherence.test.ts`. Deliberately not retroactive: 200 bare items across earlier RFCs would need annotating first, and a gate that fires 200 times on its first run gets disabled rather than fixed. See `docs/RFC-LIFECYCLE-COHERENCE.md`.)
- [x] CHANGELOG, SDK path manifest, and version-negotiation runbook updated. (CHANGELOG and runbook landed. **2026-08-16: the SDK path manifest landed on the corpus side** as `spec/v1/operation-path-manifest.json` — published FROM the corpus for SDK repositories to consume, rather than generated INTO `openwop-sdks`; the consuming half is `openwop-sdks` `sdk/parity-expectations.json`, which `openapi-asyncapi-sdk-parity.test.ts` checks against the manifest when the repository is reachable. `rest-endpoints.md` §Versioning documents the manifest.)

## Amendment record

Change history relocated from the `Updated` metadata cell (newest first).

- `normative-example-extraction.test.ts`, 26 examples declared, suite `1.116.0`; AsyncAPI/SDK parity half landed — `openapi-asyncapi-sdk-parity.test.ts` + `spec/v1/operation-path-manifest.json`, suite `1.114.0 -> 1.115.0`; see the acceptance items).
- 2026-08-12 (`Active` -> `Accepted`; 7-day comment window waived by the steward per `MAINTAINERS.md` §"Bootstrap-phase RFC waivers".
- **Landed:** §A canonical URL resolution — 44 operations resolved `/v1/v1/*`, plus discovery itself; fixed with `openapi-resolved-paths.test.ts` (#942). §B wrapper lint with all eight normative examples unwrapped, `capability-example-root-layout.test.ts` (#945). §D feasibility measured and the checkbox hypothesis corrected (#946, #948).
- Suite `1.76.0`. §C `protocolVersion` grammar — the field was specified three incompatible ways at once (schema `minLength: 1`, predicate `startsWith('1.')`, prose calling it semver), so `"1.0.0"` both validated AND derived `openwop-core`; now `^(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)$` in the schema, `profiles.ts`, and `version-negotiation.md` §"Protocol version grammar", with a leg asserting the schema pattern and the predicate are the same string.
- Closes gap V2, open since v1.0. Suite `1.79.0`. §E `discovery-canonical-family-no-shadow` — a canonical family name nested inside `x-host-*` / `vendor.*` / `private.*` shadows what a consumer negotiates on; `host-extensions.md`'s opacity rule binds the consumer, not the host publishing the collision.
- Corpus was already clean, so it landed sabotage-verified as a guardrail. Suite `1.80.0`. §D's lifecycle gate — `rfc-lifecycle-coherence.test.ts`, enforcing annotated-vs-bare on `Accepted` RFCs >= 0147, with all 67 bare items across 0147-0156 annotated to land it and a leg asserting the rule binds RFC 0149 itself.
- Suite `1.81.0`. **Carried forward, not closed:** §D's example-extraction clause (measured; the annotated-vs-bare rule is recommended, not enforced), and §E's `discovery-canonical-family-no-shadow` invariant.).
