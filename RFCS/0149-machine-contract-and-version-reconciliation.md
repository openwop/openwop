...
- [ ] All normative discovery examples use root layout and validate. (Root layout landed — §B, all eight `spec/v1` examples unwrapped. "And validate" is §D's example-extraction clause, which is carried: a fenced example is still prose to every validator in the corpus.)
