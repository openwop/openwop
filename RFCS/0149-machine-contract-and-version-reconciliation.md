...
| **Created** | 2026-08-11 |
| **Updated** | 2026-08-16 (UQ3 resolved by measurement — one deployed `1.0.0` exists (MyndHyve tier-2), no corpus normalization; INTEROP row caveated.) 2026-08-16 (§B canonical-typo lint landed — UQ2 measured then decided: distance one on discovery-shaped root keys, `spec/v1` + RFCs ≥ 0149; 0 findings; acceptance item "authoring lint" checked.) 2026-08-16 (§D example extraction landed — `normative-example-extraction.test.ts`, 26 examples declared, suite `1.116.0`; AsyncAPI/SDK parity half landed — `openapi-asyncapi-sdk-parity.test.ts` + `spec/v1/operation-path-manifest.json`, suite `1.114.0 -> 1.115.0`; see the acceptance items) · 2026-08-12 (`Active` -> `Accepted`; 7-day comment window waived by the steward per `MAINTAINERS.md` §"Bootstrap-phase RFC waivers". **Landed:** §A canonical URL resolution — 44 operations resolved `/v1/v1/*`, plus discovery itself; fixed with `openapi-resolved-paths.test.ts` (#942). §B wrapper lint with all eight normative examples unwrapped, `capability-example-root-layout.test.ts` (#945). §D feasibility measured and the checkbox hypothesis corrected (#946, #948). Suite `1.76.0`. §C `protocolVersion` grammar — the field was specified three incompatible ways at once (schema `minLength: 1`, predicate `startsWith('1.')`, prose calling it semver), so `"1.0.0"` both validated AND derived `openwop-core`; now `^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$` in the schema, `profiles.ts`, and `version-negotiation.md` §"Protocol version grammar", with a leg asserting the schema pattern and the predicate are the same string. Closes gap V2, open since v1.0. Suite `1.79.0`. §E `discovery-canonical-family-no-shadow` — a canonical family name nested inside `x-host-*` / `vendor.*` / `private.*` shadows what a consumer negotiates on; `host-extensions.md`'s opacity rule binds the consumer, not the host publishing the collision. Corpus was already clean, so it landed sabotage-verified as a guardrail. Suite `1.80.0`. §D's lifecycle gate — `rfc-lifecycle-coherence.test.ts`, enforcing annotated-vs-bare on `Accepted` RFCs >= 0147, with all 67 bare items across 0147-0156 annotated to land it and a leg asserting the rule binds RFC 0149 itself. Suite `1.81.0`. **Carried forward, not closed:** §D's example-extraction clause (measured; the annotated-vs-bare rule is recommended, not enforced), and §E's `discovery-canonical-family-no-shadow` invariant.) |
