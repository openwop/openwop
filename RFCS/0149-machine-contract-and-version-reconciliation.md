...
- [ ] OpenAPI paths resolve correctly and match AsyncAPI/SDKs. (Resolution landed — §A, `openapi-resolved-paths.test.ts`. The AsyncAPI/SDK parity half is carried: `openapi-asyncapi-sdk-parity.test.ts` is unwritten, and it reaches into `openwop-sdks`.)
...
- [ ] CHANGELOG, SDK path manifest, and version-negotiation runbook updated. (CHANGELOG and runbook landed. The SDK path manifest is carried — it is generated into `openwop-sdks`, a separate repository.)
