...
| §A.3 cross-tenant fair share (SHOULD) | tenant B's delivery arrives while tenant A holds attempts open past its floor | the suite, with an operator-supplied `OPENWOP_TEST_TENANT_B_API_KEY` | unwitnessed — a SHOULD; no row is minted (gap register G4, decided). A row that cannot fail does not belong in a certification bundle |
