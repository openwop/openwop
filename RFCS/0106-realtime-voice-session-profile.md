...
1. **Should `voice.*` events be gated by an advertised flag or purely by emission?** §D proposes emission-gated (clients ignore unknown types). An explicit `realtimeVoice.events: "supported"` flag would let a client _discover_ the taxonomy before a session. Decide before `Active`.
2. **Where does the turn-event taxonomy normatively live** — extend `stream-modes.md` / `channels-and-reducers.md`, or a new `spec/v1/voice-session.md`? A new doc is cleaner if the profile grows (telephony, multi-party voice); an extension is lighter for the §D floor. Decide at `Active`.
3. **Interaction with RFC 0101 multi-party group conversation.** Does a live voice session in a multi-party room need per-speaker turn attribution on `voice.turn_commit` (a `subject` field)? Defer to a follow-on or pin a `subject?` now?
4. **A `scale-profiles.md` voice-latency profile.** Latency is non-normative here (§E). Should a separate profile RFC make first-audio-byte a measurable (advisory) scale target? Named as a follow-on, not in this RFC.
5. **Streaming-synthesis chunk transport.** §C returns base64 chunks inline over the async stream; for high chunk rates a `streamRef`-per-session or a single growing `mediaRef` may be better. Confirm the inline-base64-chunk shape at `Active`.
6. **Author confirmation of the distillation (G10).** §A–§F are this skill's synthesis of a research _survey_ + a deep-dive web/codebase pass — not the author's hand-authored design. Author must confirm the four resolutions above match intent before `Draft → Active` locks them.
...
- [ ] Remaining Unresolved questions resolved + author confirms the four Draft resolutions (G10) before `Draft → Active` (which locks the wire shapes).
- [ ] `Active → Accepted`: a reference host advertises `aiProviders.realtimeVoice` + passes the gated scenarios non-vacuously.
