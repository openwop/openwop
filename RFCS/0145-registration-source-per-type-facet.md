...
| ID | Gap |
|---|---|
| G1 | **Cross-surface agreement (requirement 3) — leg built, live witness outstanding.** Leg B now asserts that a host's advertised `registrationSource` for a type equals the value it emits on `artifact.created`, driven through the RFC 0142 host-sample seam. It was deliberately withheld at first because, against a corpus where no host advertised the facet, it would have gone green by finding nothing; a host now advertises it, so the leg has something real to compare. **Non-vacuity is proven, but against a STUB, not a host:** advert/event agreement passes, disagreement reds, and an event omitting the field reds — the last because absent asserts *unspecified* provenance (requirement 2), which disagrees with an advert naming one. **The gap therefore narrows rather than closes: what remains is a live run.** It closes when the leg is run non-vacuously against a host advertising the facet — the reference host advertises it as of `cbfbf9d4a`, but pins the suite at `^1.68.2`, which predates this leg. |
