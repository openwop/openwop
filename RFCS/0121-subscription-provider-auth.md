...
| **Created**       | 2026-07-01                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Updated**       | 2026-07-01 — Parked pending the tripwire in [Unresolved question 1](#unresolved-questions): a documented, provider-specific legal/ToS review clearing at least one consumer-AI-subscription's terms for this reuse pattern, OR that provider's own sanctioned third-party integration path. Un-parks when either lands.                                                                                                                                                                                     |
...
- **No reference-host commitment.** This RFC does not propose or require any `openwop-examples` or `openwop-app` implementation while `Draft (Parked)`. In particular, **no openwop-app ADR should be authored for this surface until Unresolved question 1 has a documented resolution** — this inverts the usual RFC-then-ADR sequencing (cf. RFC 0108 → ADR 0121 in `openwop-app`) deliberately, because here the legal precondition sits *upstream* of any implementation decision, not downstream of it.
...
- **Cross-cut:** none identified against `WORKFLOW-PROTOCOL-openwop-PLAN.md` — this is a Draft, additive-only wire vocabulary change with no reference-host obligation yet.

## Acceptance criteria

Promotion `Draft (Parked) → Active` requires, in addition to the standard checklist:

- [ ] **Unresolved question 1 has a documented resolution** for at least one named provider (legal/ToS review citation, or the provider's own sanctioned integration-path documentation) — the un-parking tripwire.
...
- [ ] Reference host implements and passes the new scenarios, **or** (permitted while Parked, and likely even at initial Active promotion) this RFC explicitly defers reference-host implementation pending a chosen §C acquisition shape.

## References

- `omnigent-ai/omnigent` (github.com/omnigent-ai/omnigent) — prior art: documented credential kinds include API key, **Subscription** (Claude Pro/Max, ChatGPT via official CLIs), Gateway, and Databricks workspace profile; the harness architecture that wraps official vendor CLIs as subprocesses is the closest real-world analog to §C shape 2.
...
- openwop-app competitive feature-gap analysis (conversation-derived, 2026-07-01) — the originating motivation, comparing `omnigent-ai/omnigent`'s feature surface against openwop-app's current BYOK/API-key-only model.
