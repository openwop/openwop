...
| **Created** | 2026-07-01 |
| **Updated** | **2026-09-26 — witness PAUSED (steward decision).** The reference host is built and live but dark: openwop-app #4136 (Copilot provider, ADR 0757) and #4137/#4138; the OAuth App is registered and its client is set on the backend. The remaining criterion cannot be witnessed honestly yet: every Copilot call from the steward account — the SDK with a scopeless or a broad-scoped token, the standalone CLI, and Copilot's own first-party `/login` — is refused `403 not authorized to use this Copilot feature` although Copilot Free and the CLI policy show enabled (ADR 0757 §Registration). Scope and the OAuth App are not the variable; the account's entitlement is. Criterion 2 is NOT to be met on the advert-shape and scope-rejection rows alone: they pass without any Copilot call succeeding, so they cannot witness that the cleared mechanism works (a row that cannot fail is not a witness). **Resumes when** an account for which Copilot's own CLI answers a prompt authorizes the OAuth App; then the sidecar is enabled, one real turn is witnessed, and the certifying cut runs. · **2026-09-26 — UN-PARKED: the steward hold is lifted for GitHub Copilot individual plans (Free / Pro / Pro+), by the steward's named go.** The tripwire below fired: Unresolved question 1 now has a documented resolution for one named provider, through the criterion's own second branch — **the provider's own sanctioned integration-path documentation**, GitHub's Copilot SDK authentication guide (see UQ1). **This is not a legal review and claims no legal clearance.** No other provider is cleared: Anthropic's and Google's current terms expressly prohibit the pattern and OpenAI's are silent. The remaining `Accepted` gate is reference-host work (criterion 2), which is repository work, so the RFC is no longer Parked (`RFCS/README.md` §"Parked RFCs" rule 3). The rule that no host may advertise `subscription` without a documented clearance **still binds for every provider other than GitHub Copilot**. **Previously — Parked 2026-08-13 — tripwire:** Unresolved Question 1 resolved — a legal/ToS citation for at least one named provider. **Steward hold reaffirmed 2026-08-13** when asked directly; 6 of 8 `Accepted` criteria met and the remaining two both hinge on UQ1. Not waiting on repository work. **2026-08-13 — hold REAFFIRMED by the steward.** Asked directly whether to resolve, waive, or withdraw, the steward chose to keep 0121 held at `Active`. This is a **standing decision, not an open action item**: 6 of 8 `Accepted` criteria are met and the remaining two both hinge on UQ1, which is a legal question the repository cannot answer. **Do not re-raise 0121 as closable work** — it is closed as a decision and open as a status, and those are different things. 2026-07-01 — Draft → Active (bootstrap-phase steward waiver; 7-day comment window waived; see [Status history](#status-history)). Unresolved question 1 (legal/ToS clearance) — originally the un-parking tripwire — is **re-scoped** as an Active → Accepted and implementation precondition: no host may advertise or implement `subscription` mode until it is documented as cleared for a named provider. |
...
1. **Legal/ToS clearance (the un-parking tripwire).** Does any major consumer-AI-subscription provider's terms of service permit a third-party host to make API-shaped calls under a reused personal login — or does the provider instead offer its own sanctioned third-party integration path for this pattern? **Not resolved by this RFC.** No implementation, ADR, or ID host advertisement should proceed until this has a documented answer for at least the specific provider(s) targeted. This is **re-scoped** from the original Draft → Active un-parking tripwire to an **Active → Accepted / implementation precondition**: the wire vocabulary is `Active`, but no host may advertise or implement `subscription` mode, and no reference host or ADR may proceed, until this has a documented answer.
   - **Resolved for one provider, 2026-09-26: GitHub Copilot (individual plans).** GitHub's own Copilot SDK authentication guide (https://docs.github.com/en/copilot/how-tos/copilot-sdk/auth/authenticate, fetched 2026-09-26) documents a GitHub OAuth App method with *Copilot Subscription Required: Yes*: "This enables your application to make Copilot API requests on behalf of users who authorize your app." Its listed uses include "SaaS applications building on top of Copilot" and "Any multi-user application where you need to make requests on behalf of different users". That is the provider's own sanctioned third-party integration path for exactly this pattern — the second branch of this question — and it is shape 2 (§C): the SDK drives the provider's own client. The page names no OAuth scopes; the least scope a host needs is determined empirically (`SECURITY/threat-model-provider-policy.md` §4.8).
   - **Not cleared, same date:** **Anthropic** — "Anthropic does not permit third-party developers to offer Claude.ai login into their own applications, or to route requests through Free, Pro, or Max plan credentials on behalf of their users." (https://code.claude.com/docs/en/legal-and-compliance; the same page permits an end user to sign in to the *unmodified Claude Code binary* hosted by a platform, which is a hosted agent, not this RFC's provider-call pattern). **Google** — "Directly accessing the services powering Gemini CLI (for example, the Gemini Code Assist service) using third-party software, tools, or services (for example, using OpenClaw with Gemini CLI OAuth) is a violation of applicable terms and policies." (https://geminicli.com/docs/resources/tos-privacy/). **OpenAI** — the terms are silent on third-party use of "Sign in with ChatGPT"; silence is not a sanctioned path, so it is not cleared.
   - **Reading notes (informative, not normative).** "Non-metered" in §Summary contrasts a personal plan with a **per-call-metered API key**; a consumer plan with an included allowance and optional overage billing is still a personal plan in this sense. **Org-paid seats** (Copilot Business / Enterprise) are governed by the organization's Copilot policy: a host may serve them only as that policy allows, and they are not the cleared case recorded here.
...
- [x] **Unresolved question 1 has a documented resolution** — **satisfied 2026-09-26 for GitHub Copilot (individual plans)** by the criterion's second branch, the provider's own sanctioned integration-path documentation (https://docs.github.com/en/copilot/how-tos/copilot-sdk/auth/authenticate; see UQ1). Not a legal review; no other provider is cleared. _History:_ _(**Steward hold reaffirmed 2026-08-13.** Presented with four options — resolve with a provider citation, flip on the at-own-risk waiver, withdraw the RFC, or keep it held — the steward kept it **held**. Flipping would assert a clearance nobody has and put ToS exposure on operators and end-users; RFC 0147 §A.5 forbids exactly this shape of `Accepted` elsewhere, and 0121 gets no exemption for being inconvenient. **This item is not waiting on repository work.**)_ for at least one named provider (legal/ToS review citation, or the provider's own sanctioned integration-path documentation) — re-scoped from the original un-parking tripwire; **gated all implementation** UNLESS waived. _(No citation exists as of 2026-07-01; UQ1 remains **UNRESOLVED**. The steward has instead issued an **at-own-risk waiver** of the implementation gate — see [Status history](#status-history) — accepting that operators/end-users bear the ToS/legal risk. This item stays open: it is **waived, not satisfied**.)_
...
### UQ1 resolved for GitHub Copilot — un-parked (2026-09-26)

The steward, presented with provider research (each quote re-fetched from the primary source on 2026-09-26), named **GitHub Copilot individual plans** as the cleared provider and lifted the 2026-08-13 hold for that provider only. The citation is GitHub's own sanctioned integration-path documentation, not a legal review. In the same decision the steward directed that the reference app's at-own-risk path stop offering `subscription` for `anthropic` and `google`, whose terms now expressly prohibit it. Criterion 2 remains open: the reference host must advertise `subscription` for Copilot and pass `aiproviders-subscription-scope`, with the acquisition mechanism following gap G2's ruling (an official-client harness).
