...
| **Status** | `Active` |
| **Track status** | §A (`runtime.requires[]`) **`Accepted` 2026-05-29** — verified live on a non-steward host (MyndHyve); §B (`ctx.http.safeFetch`) `Active` — wire surface landed, reference-impl pending. RFC `Active` overall until §B graduates. |
...
- [ ] `core.openwop.http@2.0.0` ships the feature-detection consumer path (`ctx.http?.safeFetch` with fallback) **and** a non-steward host adopts it — MyndHyve committed to the host `ctx.http.safeFetch` adapter + flipping `net.outbound` into its grant-set (which unblocks its `ads-publish-*` packs from the §A install refusal). This is the remaining `Active → Accepted` gate for §B.
