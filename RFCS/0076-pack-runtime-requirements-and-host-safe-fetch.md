...
- [ ] **Remaining §B `Active → Accepted` gate:** registry republish of the signed `core.openwop.http@2.0.0` tarball (distribution) **and** a non-steward host adopting `ctx.http.safeFetch` — MyndHyve committed to the host adapter + flipping `net.outbound` into its grant-set (which unblocks its `ads-publish-*` packs from the §A install refusal).
