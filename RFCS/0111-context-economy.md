...
| **Created**       | 2026-06-26                                                                                     |
| **Updated**       | 2026-09-26 — **amended in place (additive): the fork refusal names its code.** The summary-unavailable refusal said only "mirroring `replay_memory_snapshot_unavailable`" and named no code, so the first implementing host had to emit a vendor code for a condition this RFC owns. It is now `replay_context_summary_unavailable` (`409`, not retriable), registered in `spec/v2/errors.json` since 2.42 and listed in v1 `rest-endpoints.md` §"Common error codes". It binds only hosts advertising `contextBudget.summarization` (none in production at amendment time), so no existing pass changes. No conformance leg asserts the code: a black-box suite cannot make a host's summary artifact unavailable at `fromSeq`, so the code is advertise-and-attest, like the off-seam ceiling of the transcript seam. · **Parked 2026-08-13 — tripwire:** a host running **real orchestrator-loop model turns** advertises `contextBudget` and produces a non-vacuous witness. No steward host does; the mock supervisor is forbidden from advertising it by this RFC's own §Scope, and openwop-app confirmed on 2026-08-13 that it runs no real orchestrator turns. **All five acceptance boxes are ticked and the RFC still does not graduate** — criterion 5's tick covers *deferring* reference-host implementation for `Active`, and the same sentence names this tripwire as the `Accepted` gate. Counting the boxes says ready; reading them says not yet. 2026-06-26                                                                                     |
...
The seam returns the host's own accounting of what it fed the model that iteration. Conformance: drive enough turns that an *unbounded* transcript would exceed the budget, then assert (a) `tokenCount ≤ transcriptTokenBudget`; (b) the harness independently re-computes the token sum and confirms the host's reported `tokenCount` is internally consistent and ≤ budget — ***corrected 2026-09-26*** (`COMPATIBILITY.md`, an assertion no implementation could satisfy: events are content-free, so the event log gives the harness nothing to count). The seam now also returns `entries[] { eventId, rendered }`, the exact text fed per item; for `tokenCounter: "chars"` it is REQUIRED and the harness sums the rendered code points, which MUST equal `tokenCount`; every entry is a real event of the run and the verbatim entries are `eventIds` in order. For `o200k_base`, `cl100k_base` and `host-defined` the recount is advertise-and-attest (the suite ships no tokenizer). No host `MUST` moved; (c) `eventIds` are the recent tail (no older event included while a newer eligible one is dropped — *eligible* meaning sharing `(type, nodeId)` with an event fed that turn, defined 2026-09-26 because the RFC had left it undefined); and (d) every `summarizedRanges` entry has a matching `context.summarized` event. A full witness also needs **pressure** — at least one turn where the budget evicted or summarized an older eligible event; a run whose transcript never reaches the advertised budget records `partial-witness`, not a pass (suite 2.41.0).
...
- `replay_context_summary_unavailable` (amended 2026-09-26) is a new registered error code: it names the refusal this RFC already required, binds only hosts advertising `contextBudget.summarization`, and registering a code is additive (`COMPATIBILITY.md` §2.1; precedent: the 2.36 registry rows added by RFCs 0199/0201/0210).
