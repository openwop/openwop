...
- [x] Spec and bundle-v2 schema merged. (2026-08-16: checked — the schema, the emitter, the consumer verifier and `conformance-certification.md` §C are all on `main`. Earlier annotation: Schema landed — `schemas/certification-bundle-v2.schema.json` with per-requirement dispositions, required `blocked` total, and provenance digests, plus `certification-bundle-v2.test.ts`. Carried: spec prose, and the runner does not yet EMIT v2.)
...
- [x] Strict mode fails advertised missing behavior. (§B's advertise-vs-opt-out `MUST NOT` throws rather than warning, and gate decisions record `skipped` / `inapplicable` ledger dispositions. **2026-08-16, suite `1.118.0`+: the mid-scenario missing seam is covered** — `seamAbsent(reason)` (`lib/soft-skip.ts`) records `blocked` in default mode and THROWS under `OPENWOP_REQUIRE_BEHAVIOR=true`; `behaviorGatePresent` does the same for a seam helper that returned nothing; a 403 is no longer a pass (three §B legs had asserted `.not.toBe(404)`). Pinned by `conformance-advertised-seam-required.test.ts` (1.123.0).)
- [x] New scenarios and fixtures pass, including sabotage tests. (Landed in full 2026-08-16 — see the end of this note. Earlier: Partially landed — `certification-floor-enforcement.test.ts` is sabotage-verified. **2026-08-16, suite `1.115.0`:** the last two named §Conformance scenarios landed — `certification-bundle-non-vacuous.test.ts` (consumer re-derivation `verifyBundleV2`: missing witness / vacuous pass / duplicate requirement / tampered totals / invented disposition / reason-missing all REJECT the evidence; an honest `blocked` or `executed-fail` on a required row is VALID evidence that does not certify; discovery-only and undefined-floor claims handled) and `certification-bundle-redaction.test.ts` (`scrubEvidence` at the emitter — values AND keys, longest-secret-first, empties ignored — `evidenceSecretsFromEnv`, and the verifier's `secret-canary` rejection; `--certify` now scrubs the finished v1/v2 document with the handed credential + every `OPENWOP_*` key/token/secret + the conformance canary before validating and writing, and self-verifies with the consumer verifier). The fixture shapes the RFC names (valid-v2, early-return, missing-floor, duplicate-requirement, tampered-witness, secret-canary) are built INLINE against the live core-standard floor rather than committed under `fixtures/`, matching `certification-bundle-v2.test.ts`; **2026-08-16, suite `1.123.0`: `conformance-execution-witness.test.ts` and `conformance-advertised-seam-required.test.ts` are on disk** — the runner's file record is now the pure `resolveFileRecord` (pinned at runner / emitter / verifier layers with sabotage vectors), and the strict-mode rule is pinned for `behaviorGate`, `behaviorGatePresent` and `seamAbsent` with the env toggled in-process. Every scenario RFC 0148 §Conformance names exists and is sabotage-verified.)
- [x] Historic bundle inventory and invalidation/reissue record published. (Inventory landed — `docs/CERTIFICATION-BUNDLE-INVENTORY.md`. **Reissue landed 2026-08-16** — inventory rows 2–5, `openwop-examples#14`. Earlier annotation, kept for the record: reissue was carried and blocked on `openwop-examples`, whose committed bundle is formally invalidated and currently unowned.)
- [x] Reference hosts publish valid v2 bundles. (2026-08-16: **landed** — `openwop-examples#14` commits `certification-bundle-v2.json` for all four reference hosts (in-memory 1.1.7, SQLite 1.1.7, Postgres 1.0.0 on pglite, Python 1.0.0), generated by suite `1.116.0` `--certify --bundle-version 2` with the ledger on; each validates against the v2 schema and passes the consumer verifier's evidence checks. "Valid" ≠ "all green": every bundle carries `blocked > 0` and `openwop-node-packs` is rejected on all four (`pack-registry*` legs pass with zero assertions) — recorded in `docs/CERTIFICATION-BUNDLE-INVENTORY.md` rows 2–5 and the `INTEROP-MATRIX.md` host rows. The earlier annotation's "against a passing run" was the wrong bar: §A/§C exist so that a not-passing run is still valid evidence.)
- [x] CHANGELOG, coverage guide, interop matrix, and migration runbook updated. (CHANGELOG landed. Interop matrix swept 2026-08-16 — the four reference-host rows carry their bundle-v2 verdicts and the header defines the evidence vocabulary. **Coverage guide landed** — `conformance/coverage.md` §"Requirement dispositions and certification". **Migration runbook landed** — `version-negotiation.md` §"Certification-evidence migration (RFC 0148 — bundle v1 → v2)" with the operator / consumer / reference-host / suite rows and the 2026-11-10 window end.)
