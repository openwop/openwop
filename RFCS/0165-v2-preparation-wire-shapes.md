...
| **Created**       | 2026-09-02                                                      |
| **Updated**       | 2026-09-03 (`Active → Accepted` — **tier-1 + tier-2 evidence** per `GOVERNANCE.md` §"Acceptance evidence tiers": openwop-app #3631 (`2578e025c`, ADR 0625) and MyndHyve `workflow-runtime` myndhyve/myndhyve#238 (`8a972d193`) each advertise `protocolVersions`, persist and emit `owner.subject` on new runs with the §B.3 legacy synthesis on old ones, copy tenant + subject (and principal) onto forks, dual-emit the `OpenWOP-*` header family and serve a standard `ETag` / `304` on discovery; the openwop-sdks webhook-helper defect is fixed and released (TS 1.9.0 / Py 1.7.0 / Go v1.6.0). MyndHyve's leg also drops `openwop-auth-scim` from its advertisement — the RFC 0164 erratum remediation (register G1 of 0164 → G5 here). Both hosts are steward-owned; not independent-organization evidence.) · 2026-09-02 (`Draft → Active` in the filing PR: the three shapes, the Subject schema, the prose rules, two invariants, three new scenarios and four added legs landed together at suite 1.155.0; the additive comment window waived per `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md`; RFC 0147 §A.6 overridden and named in §Compatibility. `Active → Accepted` gates on the openwop-app and MyndHyve host legs.) · 2026-09-02 (filed) **2026-09-04 — `Accepted → Active`, reverted.** The `Active → Accepted` criterion below says both hosts *advertise* `protocolVersions`; that is a claim about a live surface, and it was flipped on MERGED PRs. Measured 2026-09-04: `https://api.myndhyve.ai/.well-known/openwop` has no `protocolVersions`, `auth.profiles` is still the pre-erratum pair, and the `ETag` is Express's default weak validator rather than the strong sha256 this RFC specifies; `https://app.openwop.dev/api/.well-known/openwop` has no `protocolVersions` either. Neither host serves it. The gated scenarios record `inapplicable` rather than fail, so a bundle looks clean while witnessing nothing — which is why this was not caught by the evidence machinery and had to be found by fetching the hosts. Restored to `Active` until both hosts DEPLOY, not merge.
...
- [ ] `Active → Accepted`: openwop-app (tier-1) and MyndHyve (tier-2) each advertise `protocolVersions`, emit `owner.subject` on new runs with the legacy rule on old ones, dual-emit headers and `ETag`, and pass the gated scenarios non-vacuously; SDK helpers fixed and released. (openwop-app #3631 `2578e025c` ADR 0625; myndhyve/myndhyve#238 `8a972d193`; openwop-sdks #33 — see § Implementation record) **Reverted 2026-09-04 — deferred: neither host serves it.** The PRs merged; the revisions did not ship. `advertise` is a live-surface claim and a merged PR is not one. Re-tick when both hosts' deployed `/.well-known/openwop` carries `protocolVersions` and the RFC 0165 §C.2 strong `ETag`, witnessed by a bundle rather than by a commit SHA.
