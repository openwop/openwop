...
| **Created**       | 2026-09-02                                                      |
| **Updated**       | 2026-09-04 (**the acceptance below was granted on the wrong evidence and is now re-grounded on the right one; the status does not change but its justification does**. The 2026-09-03 entry cites *merged PRs* for a claim about a live surface. On 2026-09-04 both hosts were fetched and neither served it: the tier-1 host was fifteen commits behind its own main and the motivating merge had never run outside CI. That finding produced `GOVERNANCE.md` §"Acceptance evidence tiers"'s new rule — **a merge is a promise; only a deployment is a witness** — and a revert to `Active` was prepared. **The revert was never opened as a PR and never landed**, so the RFC stood at `Accepted` on merged-PR evidence for a further day while this steward stated otherwise. That error is recorded here rather than quietly closed, because the rule it violated is one this RFC's own remediation created. The acceptance is now witnessed: openwop-app revision `00691-hay` serves `protocolVersions ["1.1","2.0"]`, `preferredVersion 1.1`, `signingKeys[]` and `eventLogSchemaVersion 3` on the live wire; its **signed bundle v3** (`suite 2.0.0-rc.18`, `--target-major 1`, witness `9420baa1ae9a`, 1971 rows / 5741 assertions, `host.build commit:77b8351c`, signature `openwop-app-self-2026-09-04` resolving in the live discovery document) records `protocol-versions-array` **34 assertions**, `owner-subject-shape` **30**, `owner-subject-echo` **20**, every row `executed-pass`; and the steward **re-ran all three independently against the deployed host with a separate `conformance-verify` credential** — 18 tests, `executed-pass 19`, including the run-creating `owner-subject-echo` legs that could not be reached earlier. MyndHyve's leg is witnessed by its own signed bundle (witness `11254a4525df`, 2066 rows / 7312 assertions, `executedFail 0`) carrying the same three scenarios at 16 / 15 / 10 assertions across four corpus versions. Both hosts remain steward-owned; still not independent-organization evidence.) · 2026-09-03 (`Active → Accepted` — **tier-1 + tier-2 evidence** per `GOVERNANCE.md` §"Acceptance evidence tiers": openwop-app #3631 (`2578e025c`, ADR 0625) and MyndHyve `workflow-runtime` myndhyve/myndhyve#238 (`8a972d193`) each advertise `protocolVersions`, persist and emit `owner.subject` on new runs with the §B.3 legacy synthesis on old ones, copy tenant + subject (and principal) onto forks, dual-emit the `OpenWOP-*` header family and serve a standard `ETag` / `304` on discovery; the openwop-sdks webhook-helper defect is fixed and released (TS 1.9.0 / Py 1.7.0 / Go v1.6.0). MyndHyve's leg also drops `openwop-auth-scim` from its advertisement — the RFC 0164 erratum remediation (register G1 of 0164 → G5 here). Both hosts are steward-owned; not independent-organization evidence.) · 2026-09-02 (`Draft → Active` in the filing PR: the three shapes, the Subject schema, the prose rules, two invariants, three new scenarios and four added legs landed together at suite 1.155.0; the additive comment window waived per `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md`; RFC 0147 §A.6 overridden and named in §Compatibility. `Active → Accepted` gates on the openwop-app and MyndHyve host legs.) · 2026-09-02 (filed)
...
- [x] `Active → Accepted`: openwop-app (tier-1) and MyndHyve (tier-2) each advertise `protocolVersions`, emit `owner.subject` on new runs with the legacy rule on old ones, dual-emit headers and `ETag`, and pass the gated scenarios non-vacuously; SDK helpers fixed and released. (openwop-app #3631 `2578e025c` ADR 0625; myndhyve/myndhyve#238 `8a972d193`; openwop-sdks #33 — see § Implementation record)

## Implementation record

| Item | Where | Status |
| --- | --- | --- |
| RFC text, `subject.schema.json`, both `owner` references, root `protocolVersions`, prose rules (§A–§C), deprecations register entries, two invariants, three scenarios + four legs, `Draft → Active` | `../openwop` #1173 (`69a420b0`), suite 1.155.0 | merged |
| SDK webhook-helper fix (`sha256=` prefix, `X-openwop-*` names, `OpenWOP-*` accepted; `Subject` + `protocolVersions` types) | openwop-sdks #33 — TS 1.9.0 / Py 1.7.0 / Go v1.6.0 | released |
| openwop-app (tier-1) host leg — `protocolVersions`, `subject_json` on runs, subject minted per lane, legacy synthesis on reads, fork copy, `OpenWOP-*` dual emission, discovery `ETag`/`304`, suite pin 1.156.0 | openwop-app #3631 (`2578e025c`), ADR 0625 | merged |
| MyndHyve (tier-2) host leg — `protocolVersions`, `RunDoc.subject` minted from the Firebase / API-key / service lanes (`host/ownerSubject.ts`), legacy synthesis, `run.started` owner echo, fork copies `initiatedBy` + subject, `OpenWOP-*` dual emission, discovery `ETag`/`304`, `auth.profiles` narrowed to `openwop-auth-saml` (RFC 0164 erratum), suite pin 1.156.0 | myndhyve/myndhyve#238 (`8a972d193`) | merged |
