...
- **New major-2 scenario (planned): `v2-a2ui-v09-surface.test.ts`.**
  - Server-free legs against `schemas/v2/envelopes/ui.a2ui-surface.schema.json` and against vendored upstream A2UI v0.9 schemas in `conformance/fixtures/upstream/a2ui-v0.9/`, pinned by SHA-256.
  - Seam-gated behavioural legs through the v2 seams profile (an emit-surface seam equivalent to v1's), `inapplicable` where the seam is absent.
...
| §A.2 catalog pinned (`openwop.requirement.0209.catalog-pinned`) | a payload `catalogId` other than the basic-catalog URI fails; on a seam host, a `createSurface.catalogId` differing from the payload's is refused | the suite, unaided (corpus) for the enum; seam-gated for the equality | witnessable — seam-gated for the equality (the emit-surface seam); the enum half unaided |
...
- [ ] `v2-a2ui-v09-surface.test.ts` ships in a published suite. Every corpus row records `executed-pass`.
- [ ] A committed v2 host bundle records `0209.version-selects-branch`, `.fold-guarded` and `.taint-sticky` at `executed-pass`. The expected host is openwop-app (tier-1; it renders A2UI in v1 and has the v1 emit seam) or the v2 reference host.
