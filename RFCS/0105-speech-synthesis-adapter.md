...
| **Created**       | 2026-06-20                                                                                                                                                                                                                                                                    |
| **Updated**       | 2026-06-21 (`Active → Accepted` — graduated on dual non-steward host behavioral evidence vs `@openwop/openwop-conformance@1.29.0`: openwop-app reference host rev `00265-wtx` (MiniMax, `url` arm) + MyndHyve non-steward witness rev `00280-dcs` / `api.myndhyve.ai` (Gemini, `base64` arm), both passing `speech-synthesis-roundtrip` non-vacuously under `OPENWOP_REQUIRE_BEHAVIOR=true` + steward-curl-verified, honest provider/asset-arm divergence on the identical locked shape). 2026-06-20 `Draft → Active` (7-day comment window waived; all four Unresolved questions resolved in-RFC because `Active` locks the wire shape).                                                                  |
