...
| **Created** | 2026-05-18 |
| **Updated** | 2026-05-18 (Active → Accepted: HVMAP-1a/1b/1c graduated from `it.todo()` placeholders to live behavioral tests against the Postgres reference host alongside the supervisor-mock extension (`config.mockDispatchPlan` on `core.orchestrator.supervisor` — §"Unresolved questions" #6, lands in this cycle). HVMAP-2 already lived behavioral. The Postgres reference host advertises `capabilities.agents.dispatchMapping: true` + `capabilities.subWorkflow.inputMapping: true` and implements both surfaces end-to-end. Remaining `it.todo` cases — unset-variable projection, refusal-on-missing-capability, mid-run propagation suppression, child-failed/cancelled outputMapping skip — are honest deferrals requiring conformance-harness infrastructure beyond this RFC's scope (capability-toggle hook, deterministic-fail child fixture, cancellable child fixture, fixture variant omitting defaultValues) and tracked in `conformance/coverage.md` as future work. Prior history: Draft → Active 2026-05-18 via bootstrap-phase waiver per `CONTRIBUTING.md` §"Bootstrap-phase notes"; additive RFC; schema deltas + spec prose + placeholder scenarios + capability advertisements landed at `Draft` in commits cf7df05 + 02a84e1; reference-host impl landed in a8a8594.) |
...
- [x] Four new conformance scenarios + four fixtures land in `conformance/`. (2026-05-18 — HVMAP-1a / HVMAP-1b / HVMAP-1c / HVMAP-2 all graduated to live behavioral tests against the Postgres reference host alongside the supervisor-mock extension; happy-path coverage complete. Negative-path `it.todo` blocks — unset-variable projection (HVMAP-1a-null), capability-refusal (HVMAP-1a-refusal / HVMAP-2-refusal), child-failed/cancelled outputMapping skip (HVMAP-1b-failed/cancelled), per-worker mapping override precedence (HVMAP-1c-override), mid-run no-propagation (HVMAP-2-no-midrun-propagation), unset-parent-input (HVMAP-2-unset) — remain `it.todo()` and require conformance-harness extensions (capability-toggle hook, deterministic-fail child fixture, cancellable child fixture, fixture variants omitting defaultValues) tracked in `conformance/coverage.md` as future work.)
...
All acceptance criteria met. Negative-path conformance coverage extensions tracked separately in `conformance/coverage.md`.
