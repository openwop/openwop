...
| **Created** | 2026-05-29 |
| **Updated** | 2026-06-01 (`Active → Accepted` — graduated on a **non-steward host**, with an **amended acceptance criterion** (see §"Acceptance criteria"). See [Amendment record](#amendment-record). |
...
- MyndHyve `workflow-runtime` (rev `workflow-runtime-00445-bem @ 100%`, live on `https://api.myndhyve.ai`) advertises `memory {supported:true, writable:true, ttlSupported:true, search:{supported:true, modes:["filter"]}, retention:{ttl:true, forget:true}, attribution:{supported:true, emitsWriteEvents:true}}` at the discovery doc root (steward-curl-verified field-for-field — the §A reconciled model on the live wire), and serves the §C degraded projection on the NORMATIVE `GET /v1/agents`.
- The gated `memory-degraded-projection` scenario (shipped in `@openwop/openwop-conformance@1.15.0`) passes **non-vacuously under `OPENWOP_REQUIRE_BEHAVIOR=true`** (1967ms, zero soft-skip): a real non-empty inventory (the manifest agent `core.openwop.agents.deep-research.default`, `memoryShape:{scratchpad:true, longTerm:true}`) is **correctly NOT stamped** — it requests `longTerm`, the host provides long-term durability (Firestore) ⇒ `memoryDegraded` absent, the §C-1 non-degraded direction of the iff-contract proven on the live wire.
- **Amendment (RFC 0054 precedent):** the §C degraded-STAMP direction (a degraded entry MUST carry `memoryDegraded:true` + the closed-enum `degradedMemoryDimensions`) is **not exercised on MyndHyve** because its MemoryAdapter durably honors every requestable memory dimension (single source of truth `host/memoryDimensions.ts` drives BOTH the advertisement and the projection), so no honest agent can be degraded — and MyndHyve correctly **refuses to fabricate** a degraded agent, which would under-claim a durability it genuinely provides (the dishonest-advertisement direction).
- The degraded projection is a **deterministic, read-only pure function** of `(memoryShape, host capability model)` — the RFC 0054 category (mechanically certified by the suite against any conformant host), NOT a security/isolation claim where independent implementation is load-bearing.
- Per that precedent the Accepted criterion is amended from "a host exercises the degraded stamp" to "a host advertises the §A model + serves the §C projection + passes the iff-contract's non-degraded direction non-vacuously, with the degraded-STAMP direction exercised the first time an honest host that genuinely lacks a dimension adopts it." Rationale: requiring the degraded stamp now would block a complete, mechanically-certifiable read projection on the existence of a host that honestly lacks a memory dimension AND serves `GET /v1/agents` AND advertises `memory.supported` — none currently does (the steward workflow-engine serves the inventory but honestly advertises `memory.supported:false`; the in-memory host is honestly ephemeral but serves no inventory), so an indefinite gate would penalize MyndHyve's maximal durability.
- A future honest lacking-a-dimension host (steward or non-steward) exercising the degraded stamp is a post-Accepted strengthening, not a gate.
- No wire-shape change at graduation (the `memory.{search,retention,writable}` dimensions + the `memoryDegraded`/`degradedMemoryDimensions` inventory fields shipped at `Draft → Active`). — `Draft → Active` (2026-05-30) — steward acceptance, comment window waived per `GOVERNANCE.md` single-maintainer lazy consensus after MyndHyve (non-steward) wire-shape review; wire shapes now locked.
- All 4 Unresolved questions resolved as proposed: UQ1 `writable` absent ⇒ writable + read-only host sets `false`; UQ2 `degradedMemoryDimensions` uses the §A dimension names (closed enum), not the `memoryShape` keys; UQ3 `forget` is a host-managed mutation outside the replay envelope (`replay.md` §"Recorded-fact events"); UQ4 ship base `openwop-memory`, defer `-search`/`-managed` tiers. §A dimension table + §B query-endpoint decision + §C degraded-projection + the additive `memory.{writable,search,retention}` + inventory `memoryDegraded`/`degradedMemoryDimensions` + the `openwop-memory` profile + `memory-capability-model-shape.test.ts` landed.
- Reference-host degraded-projection + behavioral `memory-degraded-projection.test.ts` deferred to `Active → Accepted`.).
