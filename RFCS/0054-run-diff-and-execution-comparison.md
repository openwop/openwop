...
1. **Cross-host diff.** Can `against` reference a run on a _different_ host (federated diff)? Out of scope for v1 (both runs are same-host); revisit with A2A if pulled.
...
- [x] A reference host implements the endpoint and passes identical + fork-divergence (+ state-shape + access-boundary). **(Path to `Accepted` — criterion amended 2026-05-29.)** The steward workflow-engine reference host (`routes/runs.ts` `computeRunDiff`) passes all four `run-diff.test.ts` scenarios non-vacuously. The original criterion required a _non-steward_ host; it was amended to a steward-reference-host + strict-conformance bar because this is a read-only deterministic diff whose interop guarantee is mechanically certified by the conformance suite (see the `Updated` field for the full rationale + the MyndHyve opt-out). Non-steward cross-host corroboration remains a welcome post-Accepted strengthening.

## Amendment record

Change history relocated from the `Updated` metadata cell (newest first).

- The three Unresolved questions are resolved below; UQ3's exclusion list is now pinned normatively in `rest-endpoints.md` §:diff. `Active → Accepted` awaits a non-steward host advertising the endpoint + passing the identical + fork-divergence scenarios.).
- **2026-05-29 (Active → Accepted):** graduated on the steward workflow-engine reference host.
- The `GET /v1/runs/{runId}:diff` endpoint (`apps/workflow-engine/backend/typescript/src/routes/runs.ts`, `computeRunDiff`) was verified to pass all four `conformance/src/scenarios/run-diff.test.ts` scenarios non-vacuously (self-diff → `divergedAtSeq: null` + empty `eventDiffs`; structurally-different runs → non-null `divergedAtSeq` aligned to `eventDiffs[0]`; `stateDiff` shape + redaction-safe; access boundary → 400 on missing `against`, 404 on nonexistent).
- **The Accepted criterion is amended** (see §"Acceptance criteria"): from "a non-steward host implements + passes" to "the steward reference host implements + passes strict conformance." Rationale: RFC 0054 is a read-only, deterministic, pure-function-of-event-logs diff — the interop guarantee (same two logs ⇒ same `divergedAtSeq`) is fully captured by the conformance suite running against _any_ conformant host, unlike a security/isolation claim where independent implementation is load-bearing for credibility.
- The designated non-steward adopter (MyndHyve workflow studio) documented an opt-out (`INTEROP-MATRIX.md` §"Capability adoption — RFC 0045–0054 cohort", no UI demand), so an indefinite non-steward gate would block a complete, verified, mechanically-certified read endpoint on external product demand that may never materialize.
- This aligns 0054 with the reference-host graduation precedent (0056/0059/0060/0062/0063/0064). A future non-steward adopter corroborating cross-host determinism is a post-Accepted strengthening, not a gate.).
