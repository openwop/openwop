...
| **Created**       | 2026-09-22                                                      |
| **Updated**       | 2026-09-26 — **clarified, Status unchanged**: §B.3(c)'s membership check treats an `https` issuer's empty path as equal to `/` (RFC 3986 §6.2.3) and nothing else; (d) and `iss` stay exact. Measured on Google's Calendar MCP server, whose PRM lists `https://accounts.google.com/` while the issuer is `https://accounts.google.com`. Earlier: 2026-09-26 — **clarified, Status unchanged**: §B binds host-configured MCP-reach providers as well as pack-declared ones, and §C reads through a bound credential reference (the two ambiguities openwop-app-ce's gap survey raised; `spec/v2/core/oauth.md` carries both). Earlier: 2026-09-22 — filed `Draft → Active`; **comment window waived** by the steward on 2026-09-22 under GOVERNANCE.md §"Sole-steward operation" — an explicit **steward override of RFC 0147 §A.6** (precedent: RFC 0194), which forbids bootstrap waiver language from shortening the window for an RFC of this risk class; it is outside the `MAINTAINERS.md` waiver grant, recorded there as an override, not as a routine waiver (this RFC affects identity and authorization: who may complete a grant, and what a run may ask a user for). Acceptance under this override is provisional and the §B review is owed (RFC 0156 register, `docs/WAIVER-RETROSPECTIVE-REGISTER.md`). **Updated 2026-09-22 — amended per implementation review** (`review/arch-impl.md` R1, R3, R9; Status unchanged): §E.2's schema conditional is replaced by the host-side rule of Unresolved question 3, because the G4 census found six published `reach.mcp` + `oauth2` manifests without `issuer`; every Falsifiability id is now minted by a major-2 scenario (the v1 legs stay as non-gating coverage); the `authorize-start` seam must call the production URL builder. |
...
*Clarified 2026-09-26 (owner ruling, openwop-app-ce's gap survey item 2):* §B binds equally a provider the host itself configures to reach an MCP server with an OAuth grant. The threats §B answers (token audience confusion; authorization-server metadata substitution) come from calling the MCP server with the token, not from where the provider was declared. For such a provider, the configured server URL stands in for `reach.mcp.server.url` and the configured issuer for `provider.auth.issuer`; §B.4 pinning runs when the host loads that configuration; and one with no configured issuer, or without `S256`, is refused the grant as §E.2 refuses an issuer-less pack.
...
3. **Discovery verifies; it never selects.** The host MUST obtain the MCP server's Protected Resource Metadata (RFC 9728), because MCP clients "**MUST** use OAuth 2.0 Protected Resource Metadata for authorization server discovery" (Authorization §Overview 4). The host MUST then:
   ...
   - (c) require `authorization_servers[]` to contain the manifest's `provider.auth.issuer`. The host MUST NOT authorize a provider of this combination whose manifest declares no `issuer`: (c) cannot hold, so the grant is refused with `connection_auth_metadata_mismatch` (§E.2; the schema keeps `issuer` optional);
     - *Clarified 2026-09-26 (owner ruling, measured on Google Calendar MCP):* (c) compares after exactly one normalization. An `https` issuer identifier with an empty path is equal to the same identifier with path `/` (RFC 3986 §6.2.3). No other part is normalized: host case, port, any other path, query and fragment all compare exactly. The reason is a real server. `https://calendarmcp.googleapis.com/.well-known/oauth-protected-resource/mcp/v1` lists `authorization_servers: ["https://accounts.google.com/"]`, while `https://accounts.google.com`'s metadata declares `issuer: "https://accounts.google.com"`. Under exact comparison, every configured value failed either (c) or (d), so a conforming host refused every grant. The two equal values share one origin, so discovery gains no power to choose a server. (d) and the RFC 9207 `iss` check stay exact against the configured issuer, which is therefore configured without the slash.
...
4. **Resolution.** The resume value is closed: `{ "outcome": "authorized" | "declined" }`. It carries no credential. `resumeSchema` rejects any other property, so a credential cannot be submitted through it.
   ...
   - *Clarified 2026-09-26 (owner ruling, gap survey item 4):* a host that binds a node to one credential reference (a connection, say) reads §C.2's "resolves for the run's Subject, provider and scopes" as that bound reference resolving with the node's scopes. The interrupt carries it as `credentialRef` (opaque, RFC 0046; it MUST NOT encode secret material), §C.4's re-check is that the bound reference now resolves with those scopes, and `connectUrl` still completes only for the initiating Subject. A host's own permission to re-authorize a shared connection applies in addition, never instead.
...
- [x] The scenarios above ship in a **published** suite (they ride the unpublished 2.36.0), and each is shown able to fail by its named sabotage. **Sabotage done 2026-09-22**: 22 cases run against a deliberately broken v2 reference host, each turning its row red (recorded in the implementation PR); publication is the remaining half. **Both halves now hold.** Publication verified 2026-09-24 against the PUBLISHED tarball, not the checkout: `npm pack @openwop/openwop-conformance@2.37.0` carries `src/scenarios/v2-oauth-client-pkce-state-iss.test.ts`, `v2-credential-interrupt.test.ts` and `v2-oauth-mcp-reach-discovery.test.ts`. The suite rode 2.36.0 unpublished; 2.36.0 and 2.37.0 have both since published, and 2.36.x is superseded for cutting by 2.37.0 (openwop#1506, #1513).
- [ ] A committed host bundle carries `openwop.requirement.0199.pkce-s256`, `.state-single-use`, `.same-user-callback`, `.iss-validated` and `.credential-interrupt` at `executed-pass`, nothing relaxed. At least one host must be production (tier-2 MyndHyve, or tier-1 openwop-app if it adopts `oauth`). If the seams run on a 0%-traffic side revision, it is built from the production image and `host.build` records it as a side revision; the bundle notes state that `authorize-start` calls the production authorization-URL builder. — **Status 2026-09-25, not ticked:** the host rows are all witnessed on the certified public v2-reference bundle on published suite 2.39.5 (`evidence/v2-host-bundles/openwop-host-v2-reference.json`, openwop#1575; witness `09587c853ea2`, 0 fail / 0 blocked, relaxations `[]`, all three profiles certified) (`.pkce-s256`, `.state-single-use`, `.same-user-callback`, `.iss-validated`, `.credential-interrupt` `executed-pass`; `.credential-interrupt` became witnessable once openwop#1567/#1571 let a host with a public front be measured). The box also requires **a production host**, and the v2 reference host is a reference example. The open item is therefore a production OAuth-client witness: MyndHyve, which advertises `oauth` but not `conformance.seamsProfile`, would need seams-v2 with `oauth/authorize-start` and `expire-refresh` mounted on its production builder and refresh path (R9); openwop-app, if it adopts `oauth`; or a steward change to this criterion. The status stays `Active`.
- [x] `.mcp-url-mode` and `.form-mode-no-secret` `executed-pass` on a committed **v2** bundle of a host with a v2 MCP mount (the v2 reference host, after RFC 0208's mount). The v1 legs on openwop-app's v1 bridge are supplementary and do not satisfy this box. — the certified public v2-reference bundle on published suite 2.39.5 (`evidence/v2-host-bundles/openwop-host-v2-reference.json`, openwop#1575; witness `09587c853ea2`, 0 fail / 0 blocked, relaxations `[]`, all three profiles certified): `.mcp-url-mode` (5 assertions) and `.form-mode-no-secret` (13) `executed-pass` on the v2 reference host's v2 MCP mount.
