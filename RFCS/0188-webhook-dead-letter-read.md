...
| **Created**       | 2026-09-18                                                      |
| **Updated**       | 2026-09-18 (`Draft → Active` in the filing PR. **Comment window waived** (additive, 7-day) by the steward under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md` §"Bootstrap-phase RFC waivers". RFC 0147 §A.6 does not apply: no identity or auth surface is narrowed — one new optional read is added, gated on a facet no host advertises yet.)  · **Active → Accepted 2026-09-19.** Evidence tier: tier-1 — steward-verified (the reference host in `openwop-examples`, GOVERNANCE §"Acceptance evidence tiers"). The reference host advertises `webhooks.deadLetter` and serves the canonical §A read; `openwop.requirement.0188.dead-letter-read` and `.dead-letter-content-free` both record `executed-pass` on a bundle with **0 blocked** and all three profiles certified (suite 2.25.0, witness `c27516af1841`, openwop/openwop-examples#63). The host had served the path as a vendor extension all along, but its SHAPE did not conform — advertising the facet over it would have published the vacuous claim RFC 0193 exists to stop, so the projection was rewritten to §A first. The first cut after advertising was refused certification by `0188.dead-letter-content-free`, which read a fresh subscription's empty sink and asserted nothing; that leg could never have passed on any host and is fixed in suite 2.25.0 (openwop/openwop#1432).|
...
- [x] `Active → Accepted`: `openwop.requirement.0188.*` `executed-pass` on a certified host bundle. The reference host already serves the path as a vendor extension, so its work is to advertise the facet, move the path off `extensions.<org>.deadLetterRead`, and re-cut.
