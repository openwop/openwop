...
1. **Force-expire hook normation.** Should this RFC normate the URL and method of the test force-expire endpoint (e.g., `POST /v1/admin/test/expire/{runId}` gated on `capabilities.testing.testKeyPrefix`), or leave it host-private (operator supplies via env var, as drafted)? The §C "host-private + env-var" approach matches `OPENWOP_RESTART_DURING_RUN_HOST_DIR` in `restart-during-run.test.ts` but adds operator setup friction.

2. **Retry-After value range.** `production-profile.md` says "MUST equal `Retry-After` header in seconds" but doesn't bound the value. Should the schema add `retryAfterSeconds.maximum`? Hosts that return `Retry-After: 3600` are technically conformant; downstream clients may interpret that as misbehavior. Out of scope for this RFC if the answer is "no normative bound"; otherwise needs a sentence in `production-profile.md`.

3. **Inflight cap discoverability vs. probing.** `inflightCap` is advertised in §A. An alternative is letting the suite probe via Little's Law (issue increasing concurrency until 503 fires). Probing is operationally robust but slower and uses more host resources during CI. Advertised cap is the chosen default; reviewers may prefer probing.

4. **Postgres host advertisement update.** When this RFC moves to `Active`, does the Postgres host advertise `production.supported: true` immediately, or wait until both new conformance scenarios pass against it? Recommended order: scenarios land in suite at `1.X.0`, Postgres host updates `/.well-known/openwop` in the same release, INTEROP-MATRIX row gains the profile claim in the same commit. Open for review.
