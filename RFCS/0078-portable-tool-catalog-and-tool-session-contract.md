...
| **Created** | 2026-05-29 |
| **Updated** | 2026-06-01 (`Active → Accepted` — graduated on a **non-steward host**. MyndHyve `workflow-runtime` (rev `workflow-runtime-00441-pid @ 100%`, live on `https://api.myndhyve.ai`) advertises `toolCatalog {supported:true, sources:["node-pack","mcp","host-extension"], sessionLifecycle:true}` at the discovery doc root and serves the normative `GET /v1/tools` + `GET /v1/tools/{toolId}` (steward-curl-verified `401` unauth on both — routes mounted, not `/v1/host/sample/*`; authed `GET /v1/tools` → a bare `ToolDescriptor[]`, by-id → the descriptor, unknown-id → `404`). See [Amendment record](#amendment-record). |
...
The spec is the right place because _discovery shape_, _requirement vocabulary_, and _safety-tier taxonomy_ are cross-host interop concerns a client depends on to render a tool picker, pre-flight missing credentials, and warn on dangerous tools. The per-surface _implementation_ (how the host enumerates its MCP servers, resolves connector actions, etc.) stays a host choice; this RFC pins the projection shape, the capability gate, and the descriptor contract.
...
Both are **additive optional endpoints**; a host that omits `toolCatalog` returns `404`/`501` and stays v1-compliant. The catalog is a _projection_ — it does not define a new invocation path. A tool is invoked exactly as today (agent dispatch with `toolAllowlist`, `core.dispatch`, MCP call); the catalog only _describes_ it.
...
Most tools are single-shot (one call → one result, already covered by RFC 0064's `agentToolCalled`/`agentToolReturned`). A _tool session_ models a multi-step interaction (e.g. an MCP server that opens a stateful connection, or a connector OAuth dance mid-call). The lifecycle is content-free observability over the existing call events:
...
- **RFC 0064 tool hooks** — the per-call authorization + `agentToolCalled`/`agentToolReturned` events are unchanged; the catalog is the _static_ projection, RFC 0064 is the _runtime_ signal. `auth.scopes` declares what RFC 0064 §C enforces.
...
**Additive** (`COMPATIBILITY.md` §2.1). New optional `capabilities.toolCatalog` block; two new read-only endpoints behind it (a host that omits the capability `404`/`501`s them, staying v1-compliant); a new `tool-descriptor.schema.json`; optional `tool.session.*` events (additive RunEventTypes, folded best-effort). No existing field, endpoint, event, or invocation path changes — the catalog _describes_ existing tools, it does not redefine how they run. No conformance pass is invalidated.
...
2. **Fold tool discovery into `GET /v1/agents` (RFC 0072).** Rejected — agents and tools are distinct surfaces (an agent _uses_ tools; a tool isn't an agent). Overloading the agent inventory would conflate the two and break the clean `toolAllowlist` → `ToolDescriptor` resolution.
...
- The two gated behavioral scenarios each §Conformance named but deferred at `Draft → Active` were authored + published in `@openwop/openwop-conformance@1.14.0` (`tool-catalog-projection` + `tool-session-lifecycle`, PR #425) and MyndHyve passes both **non-vacuously under `OPENWOP_REQUIRE_BEHAVIOR=true`** — `tool-catalog-projection` (the §F-2 authorization-scoped projection: a no-`tools:*`-scope principal sees only the no-scope baseline tool, every scoped descriptor withheld; unknown OR unauthorized id both → `404`, indistinguishable) + `tool-session-lifecycle` (the §D `tool.session.opened`→`agent.toolCalled`/`agent.toolReturned`→`tool.session.closed` bracket, one shared `sessionId`, content-free, persisted to the durable event log).
- The SDK `tools.*` helpers stay deferred (the gated scenario drives the raw conformance driver, not the SDK).
...
- NEW `spec/v1/tool-catalog.md` + `schemas/tool-descriptor.schema.json` + `capabilities.toolCatalog` + the two content-free `tool.session.{opened,closed}` events + `tool-descriptor-shape.test.ts` landed.
- The `GET /v1/tools` OpenAPI surface, SDK helpers, behavioral projection/session scenarios, and reference-host catalog deferred to `Active → Accepted`.).
