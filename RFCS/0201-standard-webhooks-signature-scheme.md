...
| §D.13–§D.14 endpoint verification refuses without consent | `…0201.endpoint-verification`: **no-echo** and **wrong-echo** → `400 webhook_endpoint_unverified`, and a following run produces **zero** deliveries to that URL. **echo** → `201`. The verification request verifies under §C.9 with the supplied secret and carries no `OpenWOP-Event-Type`. A host that returns `201` and verifies afterwards fails | the suite, gated | witnessable — gated |
