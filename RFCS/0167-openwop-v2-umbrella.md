...
| openwop-sdks | TS 1.9.0 / Py 1.7.0 / Go v1.6.0; operation-path manifest consumer | 3 | v2-only sibling majors beside the untouched v1 packages: `sdk/typescript-v2` (`@openwop/openwop@2.0.0-rc.1`), `sdk/python-v2` (`openwop-client==2.0.0rc1`), `go/v2` (tag `go/v2/v2.0.0-rc.1`); parity gate reads `spec/v2/path-manifest.json` — 51 of 51 operations typed in each. Open at the RC as a draft PR pending the corpus tag for its `CORPUS_TAG` bump. |
| openwop-examples | four hosts, nine clients, pins from `^1.20` to `^1.73` | 3 | **corrected 2026-09-04**: neither the in-memory nor the sqlite host can be retrofitted to the v2 floor (no fork substrate, no packs, no Subject — 111 and 108 scenarios blocked respectively), so the deliverable is a new `examples/hosts/v2-reference` written FROM `spec/v2/core/` rather than from v1 host code. It is the §F Witness and Front-door evidence, and building it is what found seven corpus and six suite defects. The other three hosts stay on the 1.x line through the overlap. |
| openwop-registry | 282 signed manifests; `/v1` prefix; 13 manifest schemas | 3 | `registry/v2/` under `ed25519-canonical-json`; all 109 core packs carry an `engines` ceiling; peer-dependency keys rewritten through the alias table. The signed tree is producible only by the CI job (the signing key is a CI-only secret), so the RC lands the inputs, scripts and workflow — not a signed tree. Draft PR pending the corpus tag. |
| openwop-site | copies `schemas/` to `public/spec/v1/` | 3 | **landed**: the generator iterates `SPEC_TREES`, `/spec/v2/` renders 40 pages, and `schemas/v2/` is served at its own `$id` instead of nesting under `/spec/v1/v2/`. `/spec/latest` stays v1 until GA — a release-candidate tree is not what an unqualified "latest" should resolve to. |
...
- [x] `Active → Accepted` — §G.2 on the v2.0 release candidate; every child `Accepted`. (Phase 3 cut.) — evidence: **both halves, 2026-09-18.** Every one of the eleven program children is `Accepted` (0168–0178). §G.2: `node scripts/check-cut-gates.mjs --host-bundle evidence/v2-host-bundles/openwop-host-v2-reference.json` → `0 failed, 0 blocked → exit 0`, all ten predicate groups passing — Identity, Registers, Closure, Deprecation, Paths, Codemods, Waiver, Witness, Coexistence, Front door. **Two things §G.2's green does NOT mean, stated so no one reads more into it than it says.** (1) It passes on the reference host's bundle only; run against either production host's committed bundle it fails several groups — openwop-app's is uncertified, MyndHyve's measured suite 2.1.7 and predates five of the 79 scenario files applicable at major 2. (2) Its Witness group audits the 140 requirement ids the generated registry declares for major-2 scenarios; before 2.4.3 the same gate harvested only literal `req('…')` call sites and audited 113, so a green §G.2 recorded before then covered less than this one does.
