...
- [ ] `Active → Accepted` (Phase 3): declaration file + generator + `schemas/v2/capabilities.schema.json` + `profiles.json` landed; the five v2 scenarios in suite 2.0.0; the four v1 registries retired; an example host passes `capabilities-root-closed` from the front door. — met at the RC: the declaration file (`spec/v2/declaration.json`, 86 family rows / 17 metadata keys / 3 profiles), the generator (`scripts/generate-from-declaration.mjs`), the generated `schemas/v2/capabilities.schema.json`, the generated `profiles.json` (written to `spec/v2/profiles.json`), the five v2 witnesses in suite 2.0.0-rc.1 — three scenarios (`v2-capabilities-root-closed`, `v2-capability-record-shape`, `v2-profiles-derived-only`) and two corpus wrappers (`v2-declaration-parity`, `v2-externally-gated-never-stable`), the latter two carrying `executed-pass` rows in `evidence/corpus-ledger.json` — and the four v1 registries (`extensions.json`, `core-standard-manifest.json`, `capability-declaration-classes.json`, `operation-path-manifest.json`) retired as v2 sources: every v1 root key is anchored in the declaration file and `check-declaration.mjs` fails a reserved-undefined slot. deferred: the four registry files themselves stay under `spec/v1/` through the v1 overlap and are deleted at v1 end-of-support (reason: they remain the v1.x sources, and no v2 artifact reads them); the front-door `capabilities-root-closed` pass is recorded from the v2-reference host built out of `spec/v2/core/` (commit `a830ce1a`), whose signed bundle is a Phase 4 entry item (Phase 4).
