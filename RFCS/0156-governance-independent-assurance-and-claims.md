...
- [x] Assurance manifest and claims CI gate land across public surfaces. (**2026-08-16: landed** — `docs/ASSURANCE-STATUS.json` + `.md`, DERIVED by `scripts/generate-assurance-status.mjs` from `MAINTAINERS.md`, the RFC waiver record, `SECURITY/external-audit-*`, `INTEROP-MATRIX.md`, `conformance/package.json` / `CHANGELOG.md`, and every `RFCS/registers/*.risks.md`; each section names its source, `--check` in `openwop:check` step 6 fails on drift, and the same check scans README / ROADMAP / governance / security / compatibility / INTEROP-MATRIX / docs / conformance-README for any RFC 0147 §A claim token the manifest does not permit (negated, quoted, or evidence-bar-naming uses exempt). Today: 1 maintainer / 1 org, 41 waivers / 0 retrospective reviews, audit unscheduled, no Tier-3 host, 46 open Critical/High program risks, **0 of 7 claims permitted**. "Across public surfaces" here means this repository; the site repo is not scanned from here.)
- [ ] Security SLA source and recurring review cadence are operational. (Carried. The manifest now records the audit tracker and the last corpus release date, which is the input a cadence check would read.)
