...
- **P0-2 relaxation** — MUST→SHOULD is backward-compatible: existing strict (`additionalProperties:false`) schemas still conform; the relaxation only _permits_ open schemas.
...
1. Should `validation: "closed"` ever be _required_ for a type a consumer treats as a cross-host contract, or is advisory-only sufficient? (Current: advisory; default `open`.)
