...
- [ ] At least one reference host implements + advertises `capabilities.sandbox`. Two viable paths: (a) NEW `examples/hosts/wasm-sandbox/` directory; (b) Postgres reference host extension using `wasmtime-postgres`. Either passes the 8 scenarios with the behavioral assertions actually running (not `it.todo`). (Path-to-Accepted.)
