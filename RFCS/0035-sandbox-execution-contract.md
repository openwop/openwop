...
- The sole declared non-steward adopter has honestly opted out (`no-untrusted-packs`).
- Repository work is complete: the real-isolation `wasm-sandbox` reference host, the portable `sandbox-wasm-*` scenarios, and the 7 protocol-tier invariants all landed.
- 2026-06-01 (docs correction — the §Acceptance-criteria item on the `SECURITY/invariants.yaml` graduation was stale: the 7 `node-pack-sandbox-*` rows (excl.
- `no-eval`) **have** graduated `reference-impl → protocol`, backed by the portable `conformance/src/scenarios/sandbox-wasm-{isolation,timeout}.test.ts` driving the real-WebAssembly `examples/hosts/wasm-sandbox/` fixtures (`check-security-invariants` green).
- The RFC's own `Active → Accepted` status is unchanged — it stays `Active`, gated only on a non-steward host that genuinely runs untrusted pack code adopting `capabilities.sandbox`; the invariant-tier graduation and the RFC status flip are independent tracks, and only the latter remains open.
- No wire-shape or schema change.).
- 2026-05-21 (Draft → Active same-day: `capabilities.sandbox` block landed in `schemas/capabilities.schema.json` with the 5 fields (supported + isolationModel anyOf + allowedHostCalls + memoryLimitBytes + wallClockLimitMs); `spec/v1/host-capabilities.md` §"Sandbox execution contract (RFC 0035)" added with the 8-row failure-mode invariant table + capability advertisement + error-code table; `spec/v1/rest-endpoints.md` §"Common error codes" gains the 4 new codes (`sandbox_memory_exceeded`, `sandbox_timeout`, `sandbox_capability_denied`, `sandbox_escape_attempt`).
- The 8 `node-pack-sandbox-*` rows in `SECURITY/invariants.yaml` stay at `tier: reference-impl` for now; graduation to `tier: protocol` is gated on a sandbox-executing reference host advertising + the 8 matching conformance scenarios per §D. Path to `Accepted`: first non-steward host advertises + passes the 8 scenarios.).
