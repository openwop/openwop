...
- [ ] `SECURITY/invariants.yaml` 7 `node-pack-sandbox-*` rows (excl. the JS-specific `no-eval`) graduate from `reference-impl → protocol`. **NOT YET graduated — the immediate next step.** The real-isolation host + its 11/11 behavioral suite now exist, so the proof is real, BUT `scripts/check-security-invariants.sh` requires a protocol-tier invariant's backing test to live under `conformance/src/scenarios/`. Graduation lands with a portable sandbox conformance scenario there (driving the committed fixtures), sequenced deliberately rather than asserted prematurely — cf. the reverted `5864a2f` premature graduation, whose lesson was exactly *don't flip the tier ahead of a real backing assertion*.
