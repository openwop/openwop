...
Path to `Active → Accepted`: at least one **non-steward** host advertises the capability AND passes the §B probes (the `sandbox-wasm-{isolation,timeout}` scenarios + the advertisement-shape probes). **As of 2026-06-01 there is no candidate.** The sole declared non-steward adopter (MyndHyve) has formally opted out of RFC 0035 — see `INTEROP-MATRIX.md` §"Deferred / opt-out (MyndHyve round-3)", rationale *no-untrusted-packs*: its host does not execute untrusted pack code, so a sandbox boundary is not load-bearing for it (an honest opt-out, not a gap). RFC 0035 therefore stays `Active` until a non-steward host that *does* run untrusted pack code in a real-isolation sandbox (wasm/process/container) advertises `capabilities.sandbox.supported: true` and passes the §B probes. The steward `node:vm` MVP is the wire-contract demonstrator; by construction it cannot satisfy the non-steward bar. The `SECURITY/invariants.yaml` promotion (`reference-impl → protocol`) is **independent of, and no longer blocked by, this adoption gate** — it landed once the real-isolation `examples/hosts/wasm-sandbox/` host + the portable `sandbox-wasm-*` scenarios closed the "no reference host executes pack-loaded typeIds in a sandbox" rationale (7 rows now protocol-tier; see the acceptance-criteria item above). What remains gated on a non-steward adopter is only RFC 0035's own `Active → Accepted` *status* flip.
