...
- [ ] `Active → Accepted` (Phase 3): `generate-deprecation-annotations.mjs` and `check-removal-dates.mjs` in the gate over `schemas/v2/` and `api/v2/`; every v2 gap row with a witness and a requirement id; the mirrors generated or `$ref`'d; the redocly suppressions re-enabled.
