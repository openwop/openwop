...
| **Created**       | 2026-09-16                                                      |
| **Updated**       | 2026-09-16 (`Draft` → `Active`; **comment window waived** (additive, 7-day) by the steward under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md` §\"Bootstrap-phase RFC waivers\". **RFC 0147 §A.6 overridden and named:** this RFC touches identity (the bound-id path spelling), which §A.6 says MUST complete the full window. Override grounds: nothing accepted today stops being accepted (additive — a second spelling beside `%2F`), the defect was live on a tier-1 host's front door, and suite 2.2.0 already enforces the scenario, so the Draft status contradicted a shipped MUST. Counterfactual: had any host's accepted spelling been narrowed, `safety-fix` and the full window. `Accepted` waits on the reference host's evidence bundle.) · 2026-09-17 (`Active → Accepted`). **Evidence tier: tier-1 — steward-verified.** **Accept side, measured on the production front door** at 2026-09-17T01:57Z against `app.openwop.dev` (openwop-app `0ab209b0e`, revision `00706-f8w`), unauthenticated, `OpenWOP-Version: 2`: `GET /api/runs/some-tenant~2F<uuid>` → **403 `id_tenant_mismatch`**, and the legacy `%2F` spelling → 403 `id_tenant_mismatch`, while a bare unknown uuid → 404 `not_found`, and a malformed escape (`sometenant~2`, no second hex digit) → **400 `validation_error`** (the malformed rule, §A.1; reproduced independently by openwop-app-1 at 01:58Z). The 403 means the segment was decoded as a tenant-bound id and the tenant check fired. The same probe answered 404 on the pre-RFC build `108f166fe` (measured 2026-09-16 by openwop-app-1), so the 404→403 move is the decode proof. **Emit side and the unaided scenario:** `v2-bound-id-path-projection` is `executed-pass` in the openwop-app major-2 lane on the deployed commit (`deploy.sh` ratchet at `0ab209b0e`: 73 pass / 0 red; first green at openwop-app #3887, ADR 0705). The host mirror is byte-equal to the suite oracle (`bound-id-projection-parity`, 9/9, #3904). Second deployed witness: kicktodo.com `bd73008`, where legs A–C showed `~2F` at 403 where it was 404 before 2.2.0, and leg D showed the emitted `eventsUrl`/`statusUrl` carrying `~2F` with neither `%2F` nor `%2f`, and the link followed 200. Recorded honestly: the served certification bundle lists `openwop.requirement.0184.bound-id-path-projection` as `inapplicable`, because the `--certify` lane runs at major 1 and this scenario is registered at major 2. That is a gap in the bundle's coverage, not a failed witness; making the certify lane cover major 2 is an openwop-app follow-up. No signed-in production run was created to observe the emitted link on `app.openwop.dev` itself: anonymous-visitor runs are unbound by design (openwop-app ADR 0704), and the emit path is the same code kicktodo.com leg D measured.) |
