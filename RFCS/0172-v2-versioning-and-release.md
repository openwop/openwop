...
- [x] `Active → Accepted` (Phase 3): `spec/v2/core/versioning.md`; root `preferredVersion` in `schemas/v2/`; the three retractions; `check-path-parity.mjs`; generated `info.version`; `spec/v2/path-manifest.json`; the five scenarios in suite 2.0.0; dual-stack passing on openwop-app from both majors. — evidence: all seven declared requirement ids are `executed-pass` on ALL THREE committed bundles, and `openwop.requirement.0172.dual-stack-negotiation` passes all four legs **on openwop-app itself**, which is what this criterion asks for by name. `check-path-parity.mjs` green. Path corrected in this edit: the operation set is `spec/v2/path-manifest.json`; no `spec/v2/operation-path-manifest.json` exists or ever did.
