...
- **Lever 2 — outside the Core floor, pending black-box proof.** For `Accepted` capabilities whose **behavioral** conformance is still proven through a test seam, soft-skip, or simulator rather than the production wire. They stay `Accepted` (no de-grade — that would be a wire regression) but sit **outside** the Core floor until their black-box production-path proof lands, at which point they graduate **into** it. As of this RFC: prompt-resolution chain (RFC 0029), OTel secret-redaction (RFC 0034), multi-region idempotency + cross-engine ordering (RFC 0036), sandbox execution (RFC 0035), workspace cross-tenant isolation (RFC 0059). The annex (`core-standard-profile.md` §"Extensions outside the floor") tracks each with its graduation condition.
