...
**Program RFCs and children (RFC 0167 §C, 2026-09-03).** A change too large for one witness MAY be filed as a *program RFC* (the parent) with *child RFCs*, each a full RFC with its own number, register, falsifiability table and `Active → Accepted` evidence. A child carries `Part of: RFC NNNN — child <label>` in its `Affects` row. Children flip `Active` and `Accepted` independently; the parent's `Accepted` predicate is *every child `Accepted`* plus the parent's own stated gates, and the parent MUST NOT flip `Accepted` earlier. A parent flips `Active` before any child does (a child binding a rule under a `Draft` parent has no waiver to stand on). The parent is where the comment-window waiver, the cross-organization note and any overridden rule are recorded; each child repeats the waiver literal and takes its own ledger row.

**Supersession, rejection, numbering (RFC 0174 §A, 2026-09-03).** A superseding RFC MUST flip the superseded RFC's `Status` to `Superseded` and add the forward pointer in the same PR (`scripts/check-rfc-status-coherence.mjs`); a `Supersedes` cell that begins with `—` or says *amends* / *corrects* records a partial amendment and is not a supersession. A declined proposal is filed as `Rejected` with its reason. A number is never reserved without a file; a renumber leaves a `Withdrawn` file with a forward pointer. A normative change to an `Accepted` RFC is a new RFC that supersedes the section, never an edit in place; editorial corrections and register sweeps remain in-place. `Active → Accepted` is a machine predicate (RFC 0174 §B.1; `scripts/check-accepted-predicate.mjs` from the v2 cut), and waiver authority is checked at merge (`scripts/check-waiver-authority.mjs`).
