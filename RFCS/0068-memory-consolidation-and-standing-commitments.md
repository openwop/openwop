...
1. **Replay determinism through a consolidation pass.** This is the keystone open question. A consolidation pass is a read-modify-write of memory; if a run *reads* consolidated memory, does deterministic replay (RFC 0041) still hold? Two stances: (a) consolidation is a *host-managed background mutation* like RFC 0062 distillation — a replayed run re-reads the log-recorded snapshot, not live memory, so the pass is outside the replay envelope; or (b) if a run's behavior depends on *when* a pass ran relative to its reads, the ordering must be captured. Proposed resolution: consolidation MUST be modeled as a host-managed mutation whose effect a run sees only via the deterministic read-snapshot (option a), so `agent.memory.consolidated` is an observability event, not a replay input — but this MUST be confirmed against RFC 0041 §C before `Active`. If a host wants a run to *trigger* consolidation mid-run, that path is explicitly out of scope at `Draft`.
2. **Idempotence tolerance.** §D requires a second pass over an unchanged corpus to be a no-op. Is exact `inputCount == outputCount` the right bar, or is a small tolerance needed for hosts whose strengthening pass rewrites timestamps? Proposed: exact for merge/dedup; confirm a tolerance for strengthen-only passes before `Active`.
3. **Commitment lifecycle observability.** Should there be a `commitment.inferred` event (when the host *creates* a commitment) and a `commitment.cancelled` event (when the user revokes it), or is `commitment.fired` sufficient for v1.x? Proposed: `fired`-only at `Draft`; add the lifecycle events in a follow-up if an implementer needs the full audit trail. Decide before `Active`.
4. **Consolidation + write-attribution interaction.** When consolidation writes a merged entry, should it also emit RFC 0057 `memory.written` for the new entry, or only `agent.memory.consolidated`? Proposed: emit both when the host advertises both capabilities (the `memory.written` attributes the new entry to the consolidation pass as the "node"). Confirm the attribution `nodeId` convention before `Active`.
...
- [ ] `agent-memory.md` §"Background consolidation" + §"Inferred commitments" (the §D normative contracts).
- [ ] `capabilities.schema.json` carries `agents.memoryConsolidation` + `agents.commitments`; `run-event-payloads.schema.json` carries `agentMemoryConsolidated` + `commitmentFired` defs + `_typeIndex` entries; `run-event.schema.json` RunEventType enum gains the two event names.
- [ ] 3 conformance scenarios (shape always-on; consolidation idempotence + commitment fire-once gated on capability).
- [ ] CHANGELOG entry under `[1.1.5 — unreleased]` + INTEROP-MATRIX row.
- [ ] Reference host implements + passes the behavioral scenarios, OR this RFC explicitly defers reference-host implementation (it does — shape ships; behavioral assertions soft-skip until a host advertises).
