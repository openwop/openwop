...
| **Created** | 2026-07-02 |
| **Updated** | 2026-07-06 (`Active → Accepted` — the schema/spec surface this RFC's own §Updated defined as the Active→Accepted work is landed: `provider.vendor` added to `schemas/connection-pack-manifest.schema.json` (optional `type:string, minLength:1`), the §Manifest clause 16 prose in `spec/v1/connection-packs.md`, and the three schema-validation conformance legs in `connection-pack-manifest-valid.test.ts` (accept-with-`vendor` / accept-omitting / reject-non-string) — GREEN. See [Amendment record](#amendment-record). |
...
- Per the RFC's own §Conformance, "the schema-validation legs suffice": `vendor` gates no capability and carries no wire behavior, so the observable contract IS the schema shape (witnessed), and the only further MUSTs — catalog *rendering* SHOULD-group / MUST-fallback-to-`displayName` — are inherently wire-unobservable (the 0128/0129 §4 unobservable-runtime class: the wire enforces the observable contract, the rendered grouping is declared behavior).
- Host/registry adoption (openwop-app ADR 0183/0186; `packs.openwop.dev` facet) is explicitly *downstream* of Accepted per §Migration, NOT a graduation precondition. Single-witness bootstrap steward waiver; the registry `vendor` facet + a pack-delivered connector grouped live are non-blocking adoption follow-ons).
- 2026-07-06 (Draft→Active — bootstrap steward waiver, 7-day window waived; additive presentational-only field, no wire behavior. Implementation of the `provider.vendor` schema/spec surface is the Active→Accepted work).
- 2026-07-02 filed `Draft`.
