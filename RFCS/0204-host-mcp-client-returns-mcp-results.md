...
Word count (`check-core-budget.mjs`'s whitespace split): `host-services.md` +136 (section) +1 (marker); `tool-catalog.md` 308 as filed, 306 as landed (correction (1) above). **Measured on the implementation tree: 25,132 → 25,575 (+443), cap 29,200 → 29,400 (`toolCatalog` homed) = net +243.** `mcp` is already homed (`interop.md`); adding `host-services.md` to its `normativeText` grants nothing.
