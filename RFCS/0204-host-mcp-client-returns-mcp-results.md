...
| §D.13 order (SHOULD) | `openwop.requirement.0204.catalog-order` — two reads, same order, sorted by `toolId` | the suite, unaided | witnessable — unaided; advisory only (a SHOULD), not a certification row |
