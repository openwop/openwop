...
- [ ] A committed v2 host bundle carries the §A rows at `executed-pass`; a committed v2 host bundle (MyndHyve preferred, or the v2 reference host with a real `toolCatalog` whose `sources` include `mcp` and project the suite's fake MCP server) carries `.annotations-derived` and `.mcp-unclassified-write` at `executed-pass` — `annotations-derived` must be non-vacuous (at least one descriptor carries `annotations`).
