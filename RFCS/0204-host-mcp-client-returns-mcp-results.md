...
| **Created**       | 2026-09-22                                                      |
| **Updated**       | 2026-09-22 (filed `Draft`) · 2026-09-22 — `Draft → Active` in the filing PR. **Comment window waived** (additive, 7-day) by the steward on 2026-09-22 under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md` §"Bootstrap-phase RFC waivers". RFC 0147 §A.6 does not apply: the RFC changes the *shape* a pack receives from an MCP call and the metadata a catalog publishes; it adds no outbound call, changes no idempotency or replay rule (a `callTool` is an outbound effect path governed by `replay.md`'s effect-seam rules exactly as before), and touches no identity, tenancy or certification surface. The one new safety rule (§D.3, an unclassified MCP tool is `write`) narrows what a host may claim; it grants nothing. · **Updated 2026-09-22 — amended per implementation review** (`review/arch-impl.md` R3; Status unchanged: the major-1-only `v1-raw-verbatim` MAY and the `catalog-order` SHOULD are advisory rows that carry no requirement id for `check-accepted-predicate` rule 4; the annotation rows may be witnessed by the v2 reference host with a real `toolCatalog` as well as by MyndHyve) · **Updated 2026-09-22 — implemented** (Status unchanged). Three corrections found implementing it, none changing a MUST's intent: (1) the standard `GET /tools` body is a bare `ToolDescriptor[]`, as `api/openapi.yaml` / `api/v2/openapi.yaml` `listTools` define it and as MyndHyve serves it (RFC 0078's own Updated note records the bare array); the quoted `tool-catalog.md` text said `{ tools: ToolDescriptor[] }`, which is v1 prose that contradicts the v1 wire, so v2 now states the wire shape (−2 words); (2) the v2 RunSnapshot carries no run outputs, so the `ctx.mcp` rows observe `node.completed.outputs.result` on the event log, not `GET /runs/{runId}`; (3) the fake server's list is six tools paged 3 + 3 (the three existing tools stay listed) rather than a three-tool 2 + 1. (4) The unanswering binding is `conformance.down`, not `conformance-down`: `conformance/fixtures.md`'s catalog gate reads every `conformance-*` token as a fixture id. (5) The `tool-catalog.md` banner cites only the Accepted RFC 0078 and RFC 0112 (a `Stable` document may cite only `Accepted` RFCs, `check-rfc-status-coherence`); RFC 0204 is named in its "Why this exists" (RFC 0203 precedent). One corpus row added (`0204.annotations-schema`, the descriptor schema enforces §D.12). MCP 2026-07-28 `schema.ts` re-fetched 2026-09-22 (sha256 `742750af0bb8…`): `CallToolResult`, `ListToolsResult extends PaginatedResult, CacheableResult`, `DiscoverResult extends CacheableResult`, `ToolAnnotations` defaults as cited. · 2026-09-24 (`Active → Accepted`). Evidence tier: tier-1 — the v2 reference host (openwop-examples), a reference example and not a production host; single witness, the certified public v2-reference bundle on published suite 2.38.0 (`evidence/v2-host-bundles/openwop-host-v2-reference.json`, openwop#1542; build `commit:1157625f`, witness `669d926abf45`, 385 executed-pass / 0 fail / 0 blocked, relaxations `[]`, all three profiles certified). |
...
Word count (`check-core-budget.mjs`'s whitespace split): `host-services.md` +136 (section) +1 (marker); `tool-catalog.md` 308 as filed, 306 as landed (correction (1) above). **Measured on the merged implementation tree: 25,385 → 25,828 (+443), cap 29,200 → 29,400 (`toolCatalog` homed) = net +243.** `mcp` is already homed (`interop.md`); adding `host-services.md` to its `normativeText` grants nothing.
...
- NEW `conformance/src/scenarios/v2-mcp-client-results.test.ts` (major 2), gated on `mcp.client` and the new fixture `conformance-mcp-client` (node `core.conformance.mcp-client`, conformance-RESERVED: the host maps it to a node whose output is the resolved `ctx.mcp.*` value verbatim, and whose run inputs select `method`, `serverId`, `name`, `arguments`, `cursor`, `uri`). The node's `node.completed.outputs.result` on the event log (`GET /runs/{runId}/events/poll`) is the normative observation path, so the rows are `witnessable-gated`, not seam-gated; the expected value in every row is the JSON-RPC result the suite's own server recorded sending. The suite's fake MCP server (`OPENWOP_MCP_FAKE_SERVER=true`) is extended with `structured-echo`, `always-error`, `conformance_readonly_claim_<nonce>`, and a six-tool list paged 3 + 3 with `outputSchema`, `annotations`, `ttlMs` and `cacheScope: "private"`; the host binds `conformance` to it and `conformance.down` to an address nothing answers.
...
- The existing `tool-catalog-projection`, `tool-catalog-compact-projection` and `tool-descriptor-shape` scenarios are registered for both majors (G3), with a stable-order advisory leg in `tool-catalog-projection`. The move to `BOTH_MAJORS` is admitted only once each file is major-aware (no hard-coded `/v1/` paths, no `.supported` gate), which `generate-scenario-majors.mjs` enforces; until then the §D.10 row has no major-2 witness.
...
| §B.8 v1 `raw` (v1 only; leg id `0204.v1-raw-verbatim` in the major-1 `mcp-tool-roundtrip` scenario) | when the v1 fixture output carries `raw`: equals the fake server's result, and `isError` agrees | the suite, v1 fixture `conformance-mcp-tool-roundtrip`, at major 1 | witnessable — gated at major 1 only; `inapplicable` when `raw` is absent. **Non-gating**: a v1 MAY, outside the Acceptance box, and no v2 cut reaches a major-1 file, so the row carries no requirement id for rule 4 |
...
| §D.12 mapping (schema) | `openwop.requirement.0204.annotations-schema` — over all 80 `safetyTier` × `replayPolicy` × `egress` combinations, `schemas/v2/tool-descriptor.schema.json` accepts the table's annotations and refuses every single-hint flip, every missing hint and `annotations.title` (`conformance/src/coherence/tool-descriptor-annotations.test.ts`) | the suite, unaided | witnessable — unaided (corpus) |
| §D.10 restated MUSTs | existing `tool-catalog-*` / `tool-descriptor-shape` rows at major 2 | the suite | witnessable-gated (G3) |
| §D.13 order (SHOULD) | advisory leg id `0204.catalog-order` — two reads, same order, sorted by `toolId` | the suite, unaided | witnessable — unaided; advisory only (a SHOULD), not a certification row, so it carries no requirement id for rule 4 |
...
- [x] Spec text merged: `host-services.md` §`mcp`; `tool-catalog.md`; facet, descriptor schema and declaration; v1 `raw`; the two `fetchMCP` notes; normative-home baseline lowered (`toolCatalog` out of `v1Carried`, `v1Dependent` −1). — the implementation PR (`rfc0204-mcp-client`); `check-v2-normative-home` 31 resolved / 41 v1-dependent; `check-core-budget` 25,828 / 29,400.
- [x] Both scenarios and the v1 leg ship in a published suite, every row sabotage-proven. — published in suite 2.36.0; sabotage run once per row against the v2 reference host and recorded in `conformance/CHANGELOG.md` 2.36.0 (re-wrap, strip `_meta`, throw on `isError`, merge pages, drop the cursor, drop `annotations`, … — each turned its row `executed-fail`).
- [x] A committed v2 host bundle carries the §A rows at `executed-pass`; a committed v2 host bundle (MyndHyve preferred, or the v2 reference host with a real `toolCatalog` whose `sources` include `mcp` and project the suite's fake MCP server) carries `.annotations-derived` and `.mcp-unclassified-write` at `executed-pass` — `annotations-derived` must be non-vacuous (at least one descriptor carries `annotations`). — the certified public v2-reference bundle on published suite 2.38.0 (`evidence/v2-host-bundles/openwop-host-v2-reference.json`, openwop#1542; build `commit:1157625f`, witness `669d926abf45`, 385 executed-pass / 0 fail / 0 blocked, relaxations `[]`, all three profiles certified): every `0204.*` row `executed-pass`; `.annotations-derived` ran 88 assertions over the reference host's `toolCatalog` projecting the suite's fake MCP server, and `.mcp-unclassified-write` passed.
- [x] `tool-annotations-untrusted` registered. — `SECURITY/invariants.yaml` (protocol, high), tests `v2-tool-catalog-annotations` + the coherence test; threat row in `threat-model-prompt-injection.md` §4.4.
