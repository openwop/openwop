...
| **Created**       | 2026-05-21                                                                                                                                                                                                                                                                                                                                            |
| **Updated**       | **Parked — tripwire:** three independent organizations with a maintainer in good standing (`GOVERNANCE.md` §"Path to working group", condition 1; the binding one). Draft-stage acceptance criteria are **all met** as of 2026-08-13 — no repository work remains. 2026-08-13 — Draft-stage acceptance criteria all met (`docs/working-group/` landed); **status deliberately unchanged**, see §Acceptance criteria. 2026-06-11 — **Parked** (per `RFCS/README.md` §"Parked Drafts"): deliberately idle pending the `GOVERNANCE.md` §"Path to working group" tripwires (≥3 independent-org maintainers + ≥2 non-steward conformant hosts); not abandoned. The charter is authored so ratification is mechanical the moment the tripwire fires. 2026-05-21 — filed `Draft`. |
...
- [x] Spec text merged (this file) — Status: `Draft`.
- [x] `docs/working-group/` directory created with this charter linked. **Landed 2026-08-13** — `docs/working-group/README.md`, which states in its first line that nothing in it is operative, records the three tripwire conditions against their status today, and names condition 1 (three independent organizations) as the binding one.
- [x] `GOVERNANCE.md` updated with the forward-pointer. **Verified present** at `GOVERNANCE.md` §"Decision making": *"The replacement working-group charter is filed as `RFCS/0038-working-group-charter.md` at `Status: Draft` — it ratifies and replaces this section the moment the tripwire defined at §'Path to working group' below fires."* This box was unticked while the work was done, which is the mirror of the defect RFC 0111 carries — **a bare box misleads in both directions**, and an unticked-but-satisfied item costs a reader the same re-derivation as a ticked-but-unmet one.
- [x] CHANGELOG entry under `[Unreleased]`.
- [x] No status flip on Status field — stays `Draft` until tripwire fires (§E). **Satisfied by holding, not by acting.** The status is still `Draft` and this RFC is deliberately not being promoted: `Active` would suggest a governance model somebody is following, and nobody is following this one because there is nobody to follow it yet.

**All five Draft-stage criteria are now met, and the RFC still does not move.** That is not a contradiction — §E ties the status to an external tripwire, not to this checklist. What was closable here was the *work*; what remains is a count of independent organizations, which no amount of repository work produces. Recording the distinction is the point: an RFC parked on an external condition and an RFC parked on unfinished work look identical from the outside, and until now this one looked like the second while being the first.
