...
Every row here is a corpus property. This RFC binds the REGISTRAR and the declaration, never a host's wire behaviour, which is why it claims `corpus gate` and cites no host bundle.

| Requirement | Observable — what an outside party sees | Who can cause the condition | Verdict |
| --- | --- | --- | --- |
| §A.1 the registrar is unambiguous; the refusal predicate is not host-controlled | `check-declaration.mjs` refuses a declaration naming an unregistered org — `openwop.requirement.0169.declaration-parity` is `executed-pass` in `evidence/corpus-ledger.json` | a corpus PR | witnessable — unaided (corpus) |
| §A.3 a reserved segment registered as an org | `check-declaration` refuses the declaration | a corpus PR | witnessable — unaided (corpus) |
| §A.4 deregistration is foreclosed | no procedure exists to remove an `extensions` entry, and the retroactive-unreadability reason is recorded in §A.4 | nobody — the absence is the requirement | witnessable in the negative only — negative-existence, by inspection of §A.4 |
| §A.5 the effective date is the release, not the merge | a declaration merged between releases does not change what any host resolves until the release ships | a corpus PR plus a release | witnessable — corpus gate (release identity) |
