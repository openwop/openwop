...
| **Created** | 2026-08-11 |
| **Updated** | 2026-08-16 (§E suite + SDK provenance verification landed — `scripts/verify-published-provenance.sh`, SLSA v1 attestations on both npm packages verified from a clean directory.) 2026-08-16 (§B negative chain legs landed — `workload-identity-chain-bounds.test.ts` (too-long / cyclic / scope-amplified / narrowing-resolves), OPTIONAL per-hop `scopes` on the schema, three closed §20 reasons, See [Amendment record](#amendment-record). |
...
- [ ] Release/suite/SDK/pack provenance attestations verify from a clean checkout. (**Suite + SDK half landed 2026-08-16:** both `@openwop/openwop-conformance` (`openwop-publish.yml`) and `@openwop/openwop` (openwop-sdks `publish.yml`) publish with `npm publish --provenance` under GitHub OIDC, so the registry holds a SLSA provenance v1 attestation per tarball; NEW `scripts/verify-published-provenance.sh [suite] [sdk]` installs both into an empty directory and asserts `npm audit signatures` reports no invalid/missing signature AND the registry metadata references `https://slsa.dev/provenance/v1` for each — verified for `1.129.0` / `1.7.0`. Carried: pack attestations (`openwop-registry`) and the corpus release itself (a git tag + GitHub release, no tarball to attest); the verifier proves integrity and build provenance, not conformance.)
...
- `delegation-chain-acyclic` + `delegation-no-scope-amplification` registered; suite `1.127.0 -> 1.128.0`.) 2026-08-16 (§A–§D prose landed: `spec/v1/auth.md` §"Workload identity and delegated actor chain" (verify/bind/resolve/fail-closed, chain-is-provenance rules, sender constraint + token exchange, content-free audit facts + hashed-subject privacy rule (G5)); `spec/v1/observability.md` §"Identity.
- and delegation attributes" — canonical `openwop.*` attributes and a **v0, experimental, optional** GenAI projection labelled with the upstream ref (G3/UQ4: the GenAI conventions moved to `semantic-conventions-genai` and are Development-stability with no tagged release, so no stable version exists to pin); NEW `SECURITY/threat-model-workload-identity.md` (T1–T5, adversaries A1–A9, STRIDE per.
- §).
- Not landed: §E provenance (cross-repo), the six unregistered invariants, a host advertiser, proof format (G1), DPoP SDK measurement (G2).) 2026-08-12 (`Active` -> `Accepted`; 7-day comment window waived by the steward per `MAINTAINERS.md` §"Bootstrap-phase RFC waivers".
- **Landed:** RFC text and its gap/risk registers. **Carried forward, not closed:** workload identity, delegation, the OTel mapping, and artifact attestations.).
