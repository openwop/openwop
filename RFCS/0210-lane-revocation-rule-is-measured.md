...
> **Correction (2026-09-25) — the §B rows' gate.** The four rows above read "gated on the harness issuer", and the scenario recorded `blocked` whenever the harness was not the host's trust root. A production host MUST NOT trust a test issuer: a deployed service that accepts tokens signed by a key on a test runner has an authentication bypass. So every honest production bundle advertising `exp-only` carried four `blocked` rows, and RFC 0168 §E.1 makes one `blocked` row deny certification bundle-wide. This was measured on openwop-app (its ADR 0745, corpus defect 1). These rows are `seam-gated` (`SECURITY/invariants.yaml` `lane-exp-only-lifetime-bounded`), and RFC 0168 §C.1 already rules on an unclaimed suite instrument: it is `inapplicable`, and `blocked` is reserved for a claimed instrument that does not answer. A host claims the harness issuer the way it claims any trust root, by listing it in the lane's `issuers[]`. From suite 2.40.1 the scenario therefore:
> - records `inapplicable`, naming the issuers the lane does trust, when the `exp-only` lane's `issuers[]` does not list `OPENWOP_TEST_OIDC_ISSUER_URL`;
> - runs when the lane lists the harness issuer, and records `blocked` or a failure as before.
>
> No MUST, schema or wire shape changes. **Residual, stated:** a production bundle witnesses none of the four rows, exactly as it witnesses no seam-gated row. The §B witness comes from a cut whose lane lists the harness issuer: the reference host, or a colocated boot of the same release image. Linking such a companion cut to the production bundle it vouches for (by `host.build`) would be a verifier and certification change, which is an RFC of its own and is not made here. Register **G4** (no `executed-pass` on a committed bundle) is unchanged by this correction.
