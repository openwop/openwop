...
| **Created**       | 2026-09-03                                                      |
| **Updated**       | 2026-09-03 (`Draft → Active` in the filing PR. **Comment window waived** under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md`; RFC 0001 §5 cross-org rule not yet active; RFC 0147 §A.6 overridden and named in the parent, RFC 0167 (the bundle is the artifact every security claim rests on). Adversarial review recorded below.) · 2026-09-03 (filed) · 2026-09-18 (`Active → Accepted`). **Evidence tier: tier-1 — steward-verified**, corroborated tier-2. Witness: `openwop-host-v2-reference` build `commit:87cb8a007d05`, suite 2.4.2, `witnessSha256 9217e347b85f…`, signed `v2-reference-3`, all three profiles certified. **Stated plainly:** that host is tier-1 by `GOVERNANCE.md` §"Acceptance evidence tiers" (a reference host in `openwop-examples`), but it is run from the pinned example and its bundle's `discovery.url` is `http://127.0.0.1:3838` — it is not a host that serves traffic, and this acceptance does not call it deployed. Corroborated by `myndhyve` build `commit:46917218d19f`, suite **2.1.7** (five of the 79 scenario files applicable at major 2 were not yet in that suite and have no row), both claimed profiles certified. openwop-app's bundle verifies under its published key but is not certified, so it corroborates nothing under §B.1. |
...
**§C.1** `openwop-conformance-seams-v2` is a profile with `api/seams-v2.yaml` and `schemas/v2/seams/*.schema.json`, `$ref`-ing the canonical error and event schemas with no tolerance path, and forbidden from the capability namespace: a host advertises the profile as `conformance.seamsProfile: "openwop-conformance-seams-v2"` at the discovery root, never a `testSeams` flag (**erratum 2026-09-05** — this sentence read "in `profiles[]`" at acceptance, an advert location no host publishes and no code reads: `seamsProfileAdvertised` tests `conformance.seamsProfile` and the reference host serves no `profiles[]` at all. The named-profile requirement, which is what the sentence was for, is unchanged — the advert names the profile rather than setting a boolean). **The advert is also the CLAIM condition**: the profile's predicate requires `conformance` at the discovery root, so a host that does not publish one does not claim the profile, and a scenario that finds the exact advert absent records `inapplicable` — never `blocked`, which is reserved for an advertised profile whose seam does not answer (rc.62; RFC 0168 §E.1 makes `blocked` bundle-wide fatal, so blocking on an unclaimed instrument denied certification of every profile to hosts that had merely not mounted the seams) (the two `:649–650` proposals are closed by deletion). **§C.2** The nine seam/test operations leave `api/v2/openapi.yaml` and `spec/v2/operation-path-manifest.json`; an SDK generated from the canonical document has no seam method. **§C.3** The profile is versioned with the suite (`seams-v2` for 2.x) and its numbering collision with the sample host is resolved by the profile's own path space `/conformance/seams/...`.
...
| §E.2 `signingKeys[]` is reachable from a **v1** root too — a bundle is v3 regardless of major | `openwop.requirement.0168.bundle-signature-attributable.v1-root` | the v1 capabilities schema + a v1-only host's document | witnessable — unaided |
...
- [x] `Active → Accepted` (Phase 3): the suite with `req()` only and the per-`it` ledger; `@openwop/spec-artifacts` pinned exactly; `api/seams-v2.yaml`; the canonical documents without seams; bundle v3 schema, signer and verifier; every other child's scenarios running under it; openwop-app's v2 bundle verifying. — evidence: `api/seams-v2.yaml`, `schemas/v2/certification-bundle.schema.json` and the sign/verify pair are present; `check-path-parity` reports the canonical v2 document carries no seam operation; every child's scenarios run under the suite (231 `executed-pass` rows on the reference bundle). **On the last clause, both facts:** openwop-app's bundle DOES verify — `check-cut-gates.mjs` reports its attestation verifies under the host's published key `openwop-app-self-2026-09-04` — and it does NOT certify: 3 `blocked` rows leave both claimed profiles `certified: false` (RFC 0168 §E.1, this RFC's own rule). The clause says *verifying*, and verifying is what happened; but under RFC 0174 §B.1 an uncertified bundle supplies no acceptance witness, so the §B.1 witnesses here are the reference and MyndHyve bundles. The version numerals in the original text (`2.0.0`) are dropped rather than re-pinned: the obligation is the shape, and the corpus has moved through 2.4.3 since it was written.
