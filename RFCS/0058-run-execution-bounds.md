...
| **Created** | 2026-05-25 |
| **Updated** | 2026-05-25 (Draft → Active — architect Phase-0 clearance per `docs/autonomous-agent-runtime-plan.md` §8 + steward acceptance; wire surface landed atomically. **`runTimeoutMs` reference-host enforcement now landed** in `examples/hosts/in-memory` with the run-duration behavior scenario live + green. `Active → Accepted` awaits the `maxLoopIterations` half, which rides RFC 0061's execution-loop host enforcement.) |
