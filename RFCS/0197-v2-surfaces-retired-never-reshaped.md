...
- [ ] `v2-capability-maturity-bounded` ships in a published suite and, **verified in the published tarball layout**, reads the shipped `spec/v2/declaration.json`. *(Half done, and the box stays open because the conjunction is not: the scenario landed 2026-09-22 and reads the declaration through `SPEC_V2_DIR`, which resolves to the `@openwop/spec-artifacts` peer — `spec/v2/declaration.json` is in that package's `files`, and `check-published-layout` passes. The suite is NOT published: 2.36.0 is open and unpublished under decisions log D3, and the single batch publish is the steward's call, no earlier than 2026-09-29.)*
