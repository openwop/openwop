...
- [x] Registers — 2026-09-22, the RFC 0197 gates PR (wave W5): `v2-minor` trigger and the `retirement` block in `deprecations.schema.json` (REQUIRED whenever the trigger is present, by an `if`/`then`); `spec/v2/migrations.json` + schema (`minItems: 0` — an empty v2→v2 register is a legal state, and is the state at 2.36.0; the v1 `C1–C11` child grammar is not widened); `spec/v2/corrections.json` + schema, seeded with P3 only — P1 is NOT seeded, because a bundle carried its prior shape and the row would fail R3, which is the point; `spec/v2/surface-baseline.json` (GENERATED, 7,960 surfaces, cut on the finished tree at release 2.35.0); `declaration.schema.json` `maturity.until` gains `^2\.[0-9]+$` and the sentence separating the corpus's shape-hold from the wire record's host timeline. No declaration row's maturity changed.
- [x] Gates — 2026-09-22, same PR. All four are green on the clean tree, wired into `openwop:check` stage 10, and every sabotage was run and recorded in the PR body. `check-v2-retirement`: nine sabotages against one base fixture the gate calls `retirable`, and each is held by ITS OWN predicate and no other (a fixture that reddens the gate through a different predicate proves nothing about the one it names). `check-v2-surface-monotone`: a removed enum member, a removed operation, a `required` entry added to a pre-existing object, an open object closed and a narrowed type are each refused, against two positive controls — an OPTIONAL property added to a closed object passes (§B.5), and an RFC 0209-shaped `anyOf` branch re-cut of the root reports ZERO removals (§A.1). `check-removal-dates`: the same `removeIn: "2.99"` row is due-at-once under the pre-0197 `v2.0-cut` trigger and not due under `v2-minor`; the persisted-class legs fail when the member or its `x-openwop-retired-in` annotation is lost. `check-bundle-maturity`: the committed MyndHyve document re-labelled onto suite 2.36.0 fails with 38, promoting one family in a fixture declaration drops it to 37, and the same content at 2.35.1 is reported, not failed (D1). Each is pinned by a coherence twin under `conformance/src/coherence/`.
- [x] `v2-capability-maturity-bounded` ships in a published suite and, **verified in the published tarball layout**, reads the shipped `spec/v2/declaration.json`. *(Half done, and the box stays open because the conjunction is not: the scenario landed 2026-09-22 and reads the declaration through `SPEC_V2_DIR`, which resolves to the `@openwop/spec-artifacts` peer — `spec/v2/declaration.json` is in that package's `files`, and `check-published-layout` passes. The suite is NOT published: 2.36.0 is open and unpublished under decisions log D3, and the single batch publish is the steward's call, no earlier than 2026-09-29.)*
...
- [x] CHANGELOG entry — `CHANGELOG.md` `[Unreleased]` and `conformance/CHANGELOG.md` `[2.36.0]`, both 2026-09-22.
