...
| **Created**       | 2026-09-02                                                      |
| **Updated**       | 2026-09-02 (`Active → Accepted` on the corpus gates alone, as §Acceptance allows for a process RFC: `check-registers`, `generate-gaps --check`, `check-witness-classes`, `generate-assurance-status --check` and `generate-protocol-status --check` all run in `openwop:check` and are green at the flip, and the mechanical witness backfill has been REVIEWED on both registers — 25 invariants and 6 extension records re-judged against what their cited scenarios can actually observe, 10 of them reclassified `witnessable-gated → seam-gated` (the five `anon-actor-*` invariants and `openwop-anonymous-actor` probe only through `/v1/host/sample/anon-surface/*`; `openwop-workload-identity-v1`, `openwop-a2a-1-0`, `openwop-mcp-2026-07-28`, `openwop-compensation-v1` resolve through §20/§21 seams and record `blocked` without them), so the ratchets moved: unreviewed 191 → 166 invariants, 73 → 67 extensions; unwitnessable unchanged at 20. Window waived per GOVERNANCE §"Sole-steward operation".) · 2026-09-02 (`Draft → Active` in the filing PR: tokens on all 919 register rows, 100 backfilled registers, `spec/v1/gaps.json` (538 gaps), `witness` on 191 invariants + 73 extension records with the mechanical-classification marker, `Rejected` reachable, retrospective queue published, three gates in `openwop:check`. Window waived per `GOVERNANCE.md` §"Sole-steward operation" and logged. `Active → Accepted` on the corpus gates alone — see §Acceptance.) · 2026-09-02 (filed)
...
- [x] `Active → Accepted` on the corpus gates alone (no host leg): every gate above runs in `openwop:check` and is green at the flip, AND at least one entry per register has had its mechanical witness class reviewed (the unreviewed ratchets have moved), so the flip is not a status edit over an unreviewed backfill. (2026-09-02: 25 invariants + 6 extension records reviewed, 10 reclassified; ratchets 191/73 → 166/67 — see § Implementation record.)

## Implementation record

| Item | Where | Status |
| --- | --- | --- |
| Register tokens (919 rows, 100 backfilled registers), `spec/v1/gaps.json` + schema + gate, `witness` on 191 invariants / 73 extensions with ratchet baseline, `Rejected`, `docs/RETROSPECTIVE-QUEUE.md`, RFC 0155 §D repeal, `Draft → Active` | `../openwop` #1174 (`b29a61d5`) | merged |
| First witness-class review (the §C.2 "honesty" clause): 25 invariants + 6 extension records re-judged against their cited scenarios; 10 reclassified `witnessable-gated → seam-gated`; `docs/witness-baseline.json` lowered to unreviewed 166 / 67; `Active → Accepted` | `../openwop` (this PR) | merged |

**What the review found.** The mechanical rule "gate helper ⇒ `witnessable-gated`" over-credits scenarios that gate on a capability AND then probe only a `/v1/host/sample/*` seam: a host can advertise `anonymousActor`, `auth.workloadIdentity`, `a2a.protocolVersions`, `mcp.protocolVersions` or `compensation.supported` and still be unfalsifiable by the suite until it wires the seam. Those are `seam-gated`. The remaining 166 / 67 entries keep their mechanical class and the `initial-mechanical-*` mark; the ratchet forbids the count going back up.
