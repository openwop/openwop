...
>
> **Canonical field name (normative; clarified by amendment 2026-05-21).** The advertised multiplier field is `capabilities.envelopes.reliability.completion.truncationBudgetMultiplier`. The name `truncationRetryMultiplier` is NOT permitted — the multiplier applies to the **output budget**, not to the **retry count**, and the semantic distinction matters: retry count is governed by `limits.schemaRounds` per `capabilities.md` §"Engine-enforced limits"; the multiplier here governs per-retry budget growth. Hosts currently emitting `truncationRetryMultiplier` MUST rename to `truncationBudgetMultiplier` before promotion of RFC 0033 to `Accepted` (a 1-line wire change identified by the MyndHyve adoption feedback `docs/handoffs/MYNDHYVE-RFC-0030-0033-ADOPTION-FEEDBACK-2026-05-20.md` §A.4).
>
