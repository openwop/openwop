...
- [ ] Workstreams 1–3 are implemented; affected prior conformance bundles are inventoried, invalidated where necessary, and reissued. (Partially landed — RFC 0148 §C floor enforcement and the bundle inventory; RFC 0149 §A/§B/§C/§E; RFC 0150 §A/§B/§D/§E. 2026-08-16 sweep: the 0148 requirement ledger landed (`5509599f`, `conformance/src/lib/requirement-ledger.ts`), bundle v2 schema + runner emission landed (`b7d9bb8b`, `78167652`), and 0150 §C's semantic-digest recipe v2 + golden vectors landed (`730ff3de` in `spec/v1/replay.md`, `40617a4d` in `conformance/vectors/`). Still carried: 0149 §D's example extraction (no commit; `docs/RFC-0147-SELF-AUDIT.md` A.8), per-scenario ledger wiring and runner-level rejection of unclassified returns (0148 acceptance item 2), and reissue — blocked on `openwop-examples`, whose bundle is formally invalidated and unowned (`docs/CERTIFICATION-BUNDLE-INVENTORY.md`).)
