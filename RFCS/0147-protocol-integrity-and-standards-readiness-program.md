...
1. ~~The project **MUST** freeze new non-essential optional wire capabilities until Workstreams 1–3 are Accepted and every Critical risk in the companion register is Closed or transferred.~~ **SPENT — exit condition met 2026-08-20; this clause no longer binds.** Workstreams 1–3 (RFCs 0148, 0149, 0150) reached `Accepted` on 2026-08-12, and all six Critical rows in the companion register are dispositioned (R1, R2, R9 Closed; R3, R12, R14 transferred to named tracked surfaces). Retained struck-through rather than deleted so a reader meeting a reference to it elsewhere can see what it was and that it is discharged. Provenance and the process findings around its acceptance are recorded in [`docs/WAIVER-AUDIT-2026-08-20.md`](../docs/WAIVER-AUDIT-2026-08-20.md).
...
- [ ] Workstreams 1–3 are implemented; affected prior conformance bundles are inventoried, invalidated where necessary, and reissued. (Partially landed — RFC 0148 §C floor enforcement and the bundle inventory; RFC 0149 §A/§B/§C/§E; RFC 0150 §A/§B/§D/§E. 2026-08-16 sweep: the 0148 requirement ledger landed (`5509599f`, `conformance/src/lib/requirement-ledger.ts`), bundle v2 schema + runner emission landed (`b7d9bb8b`, `78167652`), and 0150 §C's semantic-digest recipe v2 + golden vectors landed (`730ff3de` in `spec/v1/replay.md`, `40617a4d` in `conformance/vectors/`). **2026-08-16 (later): all three former carries landed** — 0149 §D example extraction (`normative-example-extraction.test.ts`, #1020), per-scenario ledger wiring + `--certify` rejection of unclassified returns (#1018, RFC 0148 acceptance now fully checked), and the reissue (all four reference hosts as bundle v2, `openwop-examples#14`, inventory rows 2–5). What remains of this criterion is the host-witness half of Workstream 3 (§D fencing — no fencing host) and the Python/Go digest-vector consumers in `openwop-sdks`.)
- [x] OpenAPI resolves every operation correctly and normative discovery examples are mechanically validated. (Resolution landed — 44 operations were resolving to `/v1/v1/*` and discovery to `/v1/.well-known/openwop`; fixed with a gate. **"Mechanically validated" landed 2026-08-16** — `normative-example-extraction.test.ts` (#1020): every declared whole-instance example in `spec/v1` is extracted at test time and validated against the same schema files conformance uses, undeclared whole-instance examples fail, and every discovery-shaped example validates against `capabilities.schema.json`; plus `spec/v1/operation-path-manifest.json` + `openapi-asyncapi-sdk-parity.test.ts` (#1019) hold OpenAPI, AsyncAPI and the SDK path table to one resolution.)
- [ ] Idempotency, replay digest, canonicalization, and split-brain semantics are internally consistent and pass cross-language/adversarial vectors. (Idempotency and split-brain landed — RFC 0150 §B retired the `attempt`-bearing identity and §D separated record reconciliation from effect authorization. 2026-08-16 sweep: replay digest and canonicalization landed — `730ff3de` (`spec/v1/replay.md` §"Semantic request digest v2", JCS, `recipe: "openwop-semantic-request-v2"`) with golden vectors in THIS repo (`40617a4d`, `conformance/vectors/semantic-request-digest-v2.json`, TS consumer green). Still carried: the Python/Go vector consumers in `openwop-sdks`, §D fenced-effect partition scenarios, and pending-lease recovery (0150 acceptance items 2–4).)
- [ ] A capability-gated compensation profile and non-vacuous reference implementation pass crash, retry, partial-failure, and operator-recovery scenarios. (2026-08-16 sweep: shape landed (`b4796755` — `capabilities.compensation` + node declaration), the six §D events (`76195069`), the behavioral witness `compensation-behavior.test.ts` (`4bec9e07`, six capability-gated legs), the `compensation-replay-no-refire` invariant (`acf67a71`), and UQ3 — `compensationStatus` on `RunSnapshot`, `spec/v1/compensation.md` at `Draft`, seams catalogued in `host-sample-test-seams.md` §21 (`5f1978f2`, #1007). Still carried, and it is the whole of this criterion's substance: a host that advertises `compensation.supported` and exposes the seams (none does; the legs resolve to `blocked`), the crash / retry / partial-manual / operator-recovery legs, §C lifecycle and §E prose, and `schemas/compensation-policy.schema.json`. See RFC 0151's acceptance criteria.)
- [ ] A2A 1.0 and MCP 2026-07-28 profiles pass against real upstream peers; legacy profiles are explicitly named and time-bounded. (2026-08-16 sweep: discovery shape landed for both (`3ce1c04b` — `a2a.protocolVersions`/`mcp.protocolVersions` + `versioned-composition-profiles.test.ts`), the negotiation witnesses landed (`5f1a7112` `a2a-version-negotiation.test.ts`; `7b09e9aa` `mcp-version-negotiation.test.ts`, both with header capture on the fake peers), and the `a2a-version-no-silent-downgrade` / `mcp-version-no-silent-downgrade` invariants (`acf67a71`); openwop-app's live origin advertises both `protocolVersions` blocks and passes the §A legs; it has not wired either invoke seam, so the §B behavioural legs are `blocked` against it (corrected 2026-08-16 from "§A/§B legs pass"). Still carried: the A2A 1.0 profile prose + translation table and the MCP mapping/MRTR prose in `spec/v1/{a2a,mcp}-integration.md` (zero 0152/0153 content there), naming + time-bounding the legacy profiles, and the card/runtime/durable/streaming/push and MRTR/caching/auth legs. Externally gated: a real upstream peer in CI.)
- [ ] Workload identity/delegation, observability mapping, and provenance guidance land with threat-model and invariant coverage. (2026-08-16 sweep: `schemas/workload-identity.schema.json` + `auth.workloadIdentity` landed (`8de95a14`), the behavioral witness `workload-identity-behavior.test.ts` + seam §20 (`f27765cd`), per-section gating with the `delegation` sub-block (`b7c52f75`), and the `delegation-tenant-audience-bound` / `delegation-chain-bounded` invariants (`acf67a71`). Still carried: threat-model prose, the OTel mapping, auth prose, provenance attestations (which span `openwop-sdks` and `openwop-registry`), a host witness (no host advertises `auth.workloadIdentity`; the legs resolve to `blocked`), and twelve invariants named but unregistered (`docs/RFC-0147-SELF-AUDIT.md`).)
- [ ] Core/profile naming and the versioned extension registry make unqualified conformance claims unambiguous. (2026-08-16 sweep: `spec/v1/extensions.json` and the core-standard manifest + generator landed (`c842c71c`; `capabilityPath` corrections `859ef102`), gated by `core-manifest-and-extension-registry.test.ts` and wired into `openwop:check`. Still carried: the `openwop-core` rename/alias definitions (0155 acceptance item 1, gap G6 — the motivating ambiguity is demonstrated rather than argued, and the rename has not landed) and registry completeness for pre-program extensions (0155 acceptance item 3).)
...
- [ ] `INTEROP-MATRIX.md`, `SECURITY.md`, `GOVERNANCE.md`, `MAINTAINERS.md`, `COMPATIBILITY.md`, `ROADMAP.md`, `docs/KNOWN-LIMITS.md`, `docs/PROTOCOL-STATUS.md`, and `CHANGELOG.md` reflect the evidence accurately. (2026-08-16 sweep: `INTEROP-MATRIX.md` (in-memory row) and `docs/KNOWN-LIMITS.md` were swept in `901a766f` (§A.9), with further `INTEROP-MATRIX.md` and `KNOWN-LIMITS.md` edits at `324d0710`, `96971233`, `74e2402c`; CHANGELOG and `docs/PROTOCOL-STATUS.md` track each landing. **2026-08-16 sweep (S10):** `GOVERNANCE.md` (§Security reconciled to `SECURITY.md` §3 as the single response policy per §I; SDK locations corrected; evidence-tier rule extended to bundle v2), `SECURITY.md` (§3 named the single policy; §8 invariant counts refreshed to the live 169-row registry and what the gate does/doesn't prove stated; §9 audit state made explicit — unscheduled, findings ledger empty), `COMPATIBILITY.md` (§2.3 measurement-vs-claim under RFC 0148; §3 safety-fix classifications on record), `ROADMAP.md` (live corpus counts; certification row rewritten to the bundle-v2 state), and — instead of leaving them "deliberately unswept" — **the SQLite/Python/Postgres (and in-memory) rows were re-measured**: all four reference hosts were booted locally and certified as bundle v2 against suite `1.116.0` (`openwop-examples#14`; `docs/CERTIFICATION-BUNDLE-INVENTORY.md` rows 2–5), and `INTEROP-MATRIX.md` now carries the per-profile verdicts plus an evidence vocabulary in its header. Still carried: `MAINTAINERS.md` beyond its existing tripwire record (no change needed by this sweep), and `docs/KNOWN-LIMITS.md` rows that depend on S11's register sweep.)
- [x] Every child RFC's gap and risk registers receive an acceptance sweep. (**2026-08-16 (S11): swept — all twenty registers**, `RFCS/registers/{0147–0156}.{gaps,risks}.md`, 131 rows, each carrying a `Sweep 2026-08-16` disposition against the evidence on `main` that day: *closed* with the landing pointer, *carried* with the owning gap or the missing artifact named, or *externally gated*. Headline dispositions: RFC 0147 G1/G2/G3/G6/G9/G10/G16/G23/G24 closed and R1/R6 closed-or-remediated; the split-brain (G8 → 0150 G8–G10), compensation (R9 → 0151), workload-identity (R12 → 0154) and interop (R11 → 0152/0153) rows are *mitigated in prose + suite but unwitnessed* because no host advertises the family or wires the seam — RFC 0148 §A `blocked`, stated as such rather than as done; the governance / audit / Tier-3 rows (G18–G22, R14–R17, R19; all of 0156) are externally gated and unchanged. Sweep = disposition appended, original row text preserved.)
