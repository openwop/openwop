...
| **Created**       | 2026-06-13                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Updated**       | 2026-06-24 (`Active → Accepted`: dual-witnessed on the durable-task surface per the RFC 0105/0106 graduation pattern — openwop-app (steward reference arm, the ADR 0033 deferred async-A2A item now live: `openwop-app-backend` rev `00299-hns`) **+** MyndHyve (non-steward, `api.myndhyve.ai` rev `00505-seh`), both advertising `a2a:{supported, agentCardUrl, durableTasks:true, pushNotifications:true}` (streaming omitted — no `tasks/resubscribe` subtest in suite `1.34.0`), both **steward-verified** passing the gated `a2a-task-roundtrip` durable-get (a real paused-at-approval run projecting `input-required` with `taskId==runId`, `interruptKind:"approval"`) + push-config SSRF subtests **non-vacuously** vs `1.34.0`, with credential-less GET-resolvable A2A v0.3 agent cards. `streaming` (`tasks/resubscribe`) stays a zero-witness optional arm pending a gated resubscribe subtest. SECURITY invariant `a2a-push-egress-ssrf` was already protocol-tier — unchanged.) · 2026-09-23 — editorial: §4 push-signing citation corrected (A2A 1.0.1 §4.3.3 defines no HMAC; RFC 0214). |
  ...
  "$defs": {
    "PushConfig": {
      ...
      "properties": {
        ...
        "tokenFingerprint": { "type": "string", "maxLength": 32, "description": "MAY — a truncated/salted digest of the caller's push-auth token (NEVER the raw token; same SR-1 rule as RFC 0083's `secretFingerprint`). The token itself is a destination credential held by reference (RFC 0214)." }
      }
    }
  }
}
...
- [x] Reference host persists durable Tasks + passes the gated subtests. **Done 2026-06-24, dual-witnessed:** openwop-app (steward reference arm, the ADR 0033 deferred async-A2A item now live — `openwop-app-backend` rev `00299-hns`) + MyndHyve (non-steward, `api.myndhyve.ai` rev `00505-seh`), both advertising `a2a:{durableTasks:true, pushNotifications:true}` (streaming omitted) and **steward-verified** passing the gated `a2a-task-roundtrip` durable-get (real paused-at-approval run → `input-required`, `taskId==runId`, `interruptKind:"approval"`) + push-config SSRF non-vacuously vs suite `1.34.0`, with credential-less GET-resolvable A2A v0.3 cards.
