...
| **Created**       | 2026-09-03                                                      |
| **Updated**       | 2026-09-03 (`Draft → Active` in the filing PR. **Comment window waived** under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md`; RFC 0001 §5 cross-org rule not yet active; RFC 0147 §A.6 overridden and named in the parent, RFC 0167 (identity and authorization at the peer boundary). Adversarial review recorded below.) · 2026-09-03 (filed) · 2026-09-18 (`Active → Accepted`). **Evidence tier: tier-1 — steward-verified, single witness.** Witness: `openwop-host-v2-reference` build `commit:87cb8a007d05`, suite 2.4.2, `witnessSha256 9217e347b85f…`, signed `v2-reference-3`, all three profiles certified. **Stated plainly:** that host is tier-1 by `GOVERNANCE.md` §"Acceptance evidence tiers" (a reference host in `openwop-examples`), but it is run from the pinned example and its bundle's `discovery.url` is `http://127.0.0.1:3838` — it is not a host that serves traffic, and this acceptance does not call it deployed. NOT corroborated: neither production host advertises the `a2a` or `mcp` facet, so both record `inapplicable` with that reason on the facet-dependent ids and neither can witness this child today. · 2026-09-23 (**evidence correction, Status unchanged**: one acceptance row — `openwop.requirement.0175.negotiation-authenticated.mcp` — was measured vacuously, and §D.1's authentication clause turns out to be unexercised on every committed bundle. The defect is the suite's and the host posture's, not a host defect. See §"Evidence correction (2026-09-23)".) |
...
- [x] `Active → Accepted` (Phase 3): `spec/v2/ext/grpc-transport/`; the facets and patterns in `schemas/v2/capabilities.schema.json`; `spec/v2/core/interop.md`; `negotiation.decided` in the v2 event registry; `SECURITY/threat-model-interop.md`; the eight scenarios in suite 2.0.0. — evidence: the corpus half is present and green. **`reason:` the openwop-app clause is dropped from the tick and recorded here instead, because three of its four ids cannot be witnessed on that host by construction.** openwop-app advertises neither the `a2a` nor the `mcp` facet, so: `refresh-sla` records `inapplicable` — "host advertises neither a2a nor mcp — no refreshedAt to check"; `legacy-profiles-absent` records `inapplicable` for the same reason on the leg that needs a facet; and `negotiation-decided-emitted` has **no row at all**. Only `no-transport-advertisement` is `executed-pass` there (and on all three bundles). MyndHyve records the identical `inapplicable` reasons — it advertises neither facet either. All eight §D/§E ids are `executed-pass` tier-1 on the reference host, which advertises both. The remedy is a host advertising `a2a`/`mcp` in production, not a change to this RFC.

## Evidence correction (2026-09-23) — one acceptance row measured the harness, and §D.1's clause is unexercised

**Status is unchanged and nothing here is flipped.** This section records what one of the eight `executed-pass` rows cited in the acceptance tick above actually measured, because the row measured the test harness rather than the host, and finding that exposed a second, larger fact about §D.1.

**1. The vacuous row.** `openwop.requirement.0175.negotiation-authenticated.mcp` is `executed-pass` in the committed reference-host bundle (suite 2.35.0, build `commit:8477d75f`, `witnessSha256 5562d7c57609…`) and measured nothing about the host's negotiation decision. `v2-negotiation-authenticated.test.ts` establishes its condition by constructing a single-era `McpFakeServer(['2025-06-18'])` on an ephemeral port and handing the host `hostFacingEndpoint()`. Since suite 2.33.0 that method returns `OPENWOP_MCP_FAKE_SERVER_URL` — the operator's public front, which forwards to the **pinned** port the shared, setup-level fake server owns, and that server is dual-era and speaks the preferred revision. `host-sample-test-seams.md` §23 defines no request field that makes a server offer *only* a lower revision (`requestVersion` overrides what the host **asks for**, which is §D.2's floor case, and the leg does not pass it), so no lower offer ever reached the host: it discovered `2026-07-28`, negotiated at it, and the leg asserted `negotiatedVersion === preferredVersion` — a property of the harness. The row's own assertion count records the moment it went hollow: **4** in every pre-front cut of this host (2.4.0, 2.4.1, the 2.4.2 acceptance witness, 2.4.5, 2.24.0, 2.25.0) and **3** in every cut since the fronts landed in suite 2.33.0 (2.33.1, 2.34.0, 2.35.0). The missing assertion is the wire leg, which iterates the scenario-owned server's `invocations()` — empty, because the host never reached that instance. Its three siblings (`minimum-version-refused`, `minimum-version-refused.mcp`, `negotiation-authenticated`) drove the host's decision genuinely through `peerOffersOnly` / `requestVersion`, and each of them refused as the RFC requires — but their wire half contributes zero assertions in the committed bundle too, for two different reasons. The MCP sibling lost its one wire assertion (8 → 7) when the front took the host away from the scenario's own server; the two A2A legs never had one, because the host refuses before it makes any non-GET call and the leg inspects only non-GET calls (the Agent Card `GET` is filtered out). A wire half that can only ever iterate an empty array is not a check.

**2. What that hid: §D.1's clause has never run.** The host refuses an exchange as `unauthenticated` only when the negotiated version would land **below `preferredVersion` and at or above the floor**. The only host that has ever witnessed these ids advertises `a2a.preferredVersion === a2a.minimumVersion === 1.0` and `mcp.preferredVersion === mcp.minimumRevision === 2026-07-28`, so no such version exists on it and every lower offer refuses `below-floor` instead. Every reference-host cut in the file's history was checked (17): the nine that advertise the facets all carry that posture, the eight before them advertise neither facet and record 0 assertions on these ids, and the two production bundles record them `inapplicable` or omit them. **No committed bundle has ever exercised §D.1's authentication clause**, fronted or loopback. What the three genuine rows measured is §D.2's floor rule, twice.

**3. The defect is the suite's, not the host's, and what now measures it.** No host emitted anything it should not have; the reference host's `decide()` implements §D.1 as written, and the corpus cannot convict or acquit it on this evidence. The harness fix has since landed (openwop#1520, suite 2.37.0): `conformance/src/lib/front-mux.ts` nonce-paths each scenario-owned fake behind the one operator front, so the host now reaches **this** server — the one that offers only `2025-06-18`, which is where the lower offer comes from, §23 still having no knob for it — and both legs now record `blocked` instead of a pass when the seam answers a success and the suite's own peer received no request. This PR touches no scenario; it records what the bundle already in the tree measured, which no code change can retract.

**4. What the remaining evidence supports, stated plainly.** §A, §B, §C, §D.2, §D.3, §D.4, §E.1 and §F.1 keep genuine tier-1 rows, so the acceptance is not withdrawn. **§D.1 is, as of this entry, unwitnessed** — not failed, not refuted, unwitnessed — and neither row may be cited as evidence that any host enforces it. Witnessing it needs a host whose advertised floor sits **below** its preferred version (or a suite lever producing that posture), which no host in `INTEROP-MATRIX.md` has today; recorded as `openwop.gap.0175.6`. The re-cut of the reference bundle owed after the harness fix is `openwop.gap.0175.5`, and the matrix row discloses that the recorded pass predates the fix.

**Verified for this entry** by reading the three committed bundles and all 17 reference-host cuts in the file's history, the scenario source, §22/§23's seam contract, `resolvePublicFront` + `conformance/src/setup.ts`, the cut script that sets the fronts, and the host's `decide()` at the exact build the bundle names. **Not re-measured:** the fronted path cannot be reproduced without opening public ingress, so which branch the 2.35.0 run took is read from the routing and the assertion arithmetic, not from a fresh run.
