...
**3. The defect is the suite's, not the host's.** No host emitted anything it should not have; the reference host's `decide()` implements §D.1 as written, and the corpus cannot convict or acquit it on this evidence. The harness fix (the fakes' routing behind the shared front, and a lever on §23 that lets a peer offer only a lower revision) rides the open 2.37.0 suite cycle in a separate PR; this PR touches no scenario.
