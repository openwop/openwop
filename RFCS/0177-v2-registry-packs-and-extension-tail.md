...
| **Created**       | 2026-09-03                                                      |
| **Updated**       | 2026-09-03 (`Draft → Active` in the filing PR. **Comment window waived** under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md`; RFC 0001 §5 cross-org rule not yet active; RFC 0147 §A.6 overridden and named in the parent, RFC 0167 (pack trust path and signing). Adversarial review recorded below.) · 2026-09-03 (filed) · 2026-09-18 (`Active → Accepted`). **Evidence tier: tier-1 AND tier-2 — both required.** This is the only v2 child no single host witnesses: `provider-conflict` is `executed-pass` on `myndhyve` build `commit:46917218d19f`, suite **2.1.7** (five of the 79 scenario files applicable at major 2 were not yet in that suite and have no row), both claimed profiles certified. and `inapplicable` on the reference host, while the pack-manifest ids are `executed-pass` on `openwop-host-v2-reference` build `commit:87cb8a007d05`, suite 2.4.2, `witnessSha256 9217e347b85f…`, signed `v2-reference-3`, all three profiles certified. **Stated plainly:** that host is tier-1 by `GOVERNANCE.md` §"Acceptance evidence tiers" (a reference host in `openwop-examples`), but it is run from the pinned example and its bundle's `discovery.url` is `http://127.0.0.1:3838` — it is not a host that serves traffic, and this acceptance does not call it deployed. and `inapplicable` on MyndHyve. Neither bundle alone satisfies the falsifiability table. |
...
- [x] `Active → Accepted` (Phase 3): `spec/v2/core/packs.md`; the 13 v2 schemas; `registry/v2/` with the core wave and a green `registry-check.sh`; `spec/v2/peer-dependency-aliases.json` generated; the seven scenarios; the registry's schemas pinned. — evidence: `registry-check.sh` green over 156 packs, 191 v2 and 282 v1 version manifests each signed by a key the registry permits for its namespace. **The registry's pin was bumped to `v2.4.3` before this box was ticked** (openwop-registry#67): it stood at `v2.4.1`, and ticking a "schemas pinned" clause against a two-release-stale pin would have claimed something unchecked. **`reason:` the openwop-app clause is dropped from the tick.** All three ids it names record `inapplicable` there — "host does not advertise conformance.seamsProfile: openwop-conformance-seams-v2" — the mounted-vs-built distinction `GOVERNANCE.md` draws, same as RFC 0176. **This child is also the one that genuinely needs BOTH certified bundles:** `openwop.requirement.0177.provider-conflict` is `executed-pass` on MyndHyve ONLY — the reference host records `inapplicable`, it does not advertise the connections profile — while the ceiling, hatch, peer-dependency and chain-pin ids are `executed-pass` on the reference host. Disclosed: `form-when-reuses-edge-conditions` is `inapplicable` on all three with a stated reason, and its falsifiability verdict declares `host-pending`, so RFC 0174 §B.1's second branch accounts for it rather than a pass.
