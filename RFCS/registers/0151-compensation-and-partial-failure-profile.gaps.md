...
| --- | --- | --- | --- | --- | --- |
| G1 | §C | Dependency-graph inclusion in profile v1 is undecided. | Spec Architect | Prototype against three multi-effect fixtures. | Active — **Sweep 2026-08-16:** **Decided for v1:** `reverse-completion` is the floor; dependency-graph ordering MAY be advertised (`compensation.md` §C `orderingModels`); no fixture prototype ran. |
...
| G3 | §E | Operator substitution authorization and plan-version rules are open. | Security Architect | Threat-model and define closed override actions. | Active — **Sweep 2026-08-16:** **Carried** (`compensation.md` G2 — §E operator actions named, authorization/plan-version rules not written). |
| G4 | §B | Irreversible-effect declaration vocabulary is absent. | Spec Architect | Add advisory/required declaration in child revision. | Accepted — **Sweep 2026-08-16:** **Carried** — no irreversible-effect declaration in `compensation-policy.schema.json` or the node declaration. |
| G5 | Conformance | Real-service emulator choice is unset. | Conformance Architect | Use local transactional HTTP emulator with deterministic failures. | Accepted — **Sweep 2026-08-16:** **Carried** — no host advertises `compensation.supported`; the legs are `blocked` (RFC 0148 §A); no emulator chosen. |
