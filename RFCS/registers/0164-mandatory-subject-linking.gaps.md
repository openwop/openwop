...
| --- | --- | --- | --- | --- | --- |
| G1 | §Compatibility | The `additive` classification rests on an empirical fact (no production host advertises both profiles at filing). If a host is found to have advertised both without the flag before this RFC reached `Active`, the classification MUST be re-stated as `safety-fix` and the host given the §3 migration path. | Compatibility Architect | INTEROP-MATRIX + live-discovery sweep at `Active` (done 2026-09-02: MyndHyve no `auth` block; openwop-app seams-only; matrix empty); re-check at `Accepted` (done 2026-09-02: still none) | — |
