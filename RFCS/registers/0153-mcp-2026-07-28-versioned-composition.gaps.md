...
| G3 | Conformance | Real current MCP peer is not selected. | Conformance Architect | Evaluate official implementation for pinned local CI. | Active — **Sweep 2026-08-16:** **Carried, externally gated** — no real current peer selected. |
...
| G5 | §D | Initial mapped extension set is unset. | Spec Architect | Default to opaque; promote only evidenced needs. | Accepted — **Sweep 2026-08-16:** **Closed** — `mcp-integration.md` §D: opaque by default, mapping registry empty until evidenced need (`mcp-extension-opacity.test.ts`). |
