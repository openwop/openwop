...
| G4 | §D | ~~Authorization-aware cache validator rules are undecided.~~ **Closed 2026-08-16:** `mcp-integration.md` §D — scope change ⇒ stale regardless of `ttlMs`; `"private"` never crosses authorization contexts; key = (tenant, workspace, principal, origin, revision, discovery context). | Security Architect | `carried:openwop.gap.0153.4` Threat-model principal/tenant/scope changes. | ~~Active~~ Closed |
| G5 | §D | Initial mapped extension set is unset. | Spec Architect | `carried:openwop.gap.0153.5` Default to opaque; promote only evidenced needs. | Accepted — **Sweep 2026-08-16:** **Closed** — `mcp-integration.md` §D: opaque by default, mapping registry empty until evidenced need (`mcp-extension-opacity.test.ts`). |
