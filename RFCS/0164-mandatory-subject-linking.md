...
- MyndHyve `workflow-runtime` (tier-2), on which RFC 0050 graduated, serves a live `/.well-known/openwop` with **no `auth` block at all**.
  - **Erratum 2026-09-02 (RFC 0165 host-leg discovery).** This was wrong. The live document at `https://api.myndhyve.ai/.well-known/openwop` carries `auth: { profiles: ["openwop-auth-saml", "openwop-auth-scim"] }` at the root (`services/workflow-runtime/src/routes/discovery.ts`, the RFC 0050 advertisement) with no `subjectLinking` and no `subjectLinkKey`. So one production host — steward-owned, tier-2 — DID advertise both profiles without the flag at filing, and has failed the RFC 0164 conditional and the `auth-subject-link` scenario since suite 1.151.0. The §4 harm set was not empty; it was {MyndHyve}. Under `GOVERNANCE.md` §"Sole-steward operation" the consequence is contained to our own host, and the counterfactual `safety-fix` classification recorded in §Compatibility is the one that applied. The host cannot honour the combined contract today (its SAML validator parses no `<saml:Issuer>`; its SCIM seam has no trust root), so its remediation is RFC 0164 §A.2's second exit — drop `openwop-auth-scim` from discovery — carried in the RFC 0165 MyndHyve leg. Register G1 records the finding.
...
- **MyndHyve.** Implements both lanes (RFC 0050 graduated on it) and — **erratum 2026-09-02, see §Motivation** — advertises both in its live document with no flag; it is therefore bound by this RFC and non-conforming until its RFC 0165 leg drops `openwop-auth-scim`. The original text read "advertises neither; unaffected unless it re-advertises both, in which case the contract applies — the runbook names both paths.
