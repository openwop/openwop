...
| **Created**       | 2026-09-21                                                      |
| **Updated**       | 2026-09-21 (filed `Draft` **after** its text and suite implementation were merged — see §Motivation "How this RFC came late"; **the public comment window runs in full, to 2026-09-28** — RFC 0147 §A.6: this RFC affects certification, so bootstrap waiver language MUST NOT shorten its window; suite 2.35.0, which implements it, is not published before the window closes) · 2026-09-21 (later) — `Draft → Active`; **comment window waived** by the steward on 2026-09-21 — an explicit **steward override of RFC 0147 §A.6**, which forbids bootstrap waiver language from shortening the window for an RFC of this risk class; it is outside the `MAINTAINERS.md` waiver grant, recorded there as an override, not as a routine waiver (this RFC affects certification). · 2026-09-22 — **`Active → Accepted`, provisional pending RFC 0156 §B retrospective review** (it went `Active` under a waived window; RFC 0147 requires such cohorts to be reviewed cross-organization or held provisional — the register row stays `not-reviewed`). Evidence tier: tier-1 — the v2 reference host (openwop-examples), a reference example and not a production host; single witness, on the published suite 2.35.0, build `commit:8477d75`, nothing relaxed, all profiles certified: a relaxation-free bundle that verifies under all three rules, and `v2-relaxation-recorded`'s strip-after-signing leg `executed-pass`. · 2026-09-22 (later) — **`Accepted → Active`, reverted by the steward's own measurement.** §3's `opted-out-but-advertised` rule derived an empty set from every real bundle: it read `openwop.profile.<name>` rows that never reach a bundle, and its falsifiability row's "pinned by self-tests" rested on synthetic rows no real bundle contains. Fixed in suite 2.35.1 (the name rides on the signed per-test detail). It returns to `Accepted` when a committed bundle shows `--verify` deriving real opt-outs. · 2026-09-22 (evening) — **`Active → Accepted` again, provisional pending RFC 0156 §B retrospective review.** Evidence tier: tier-2 — MyndHyve `workflow-runtime` (production, steward-affiliated), single witness for §3: its bundle cut on the published suite 2.35.1 (build `commit:bca0e9b9`, verified against the live `/readiness`; nothing relaxed; both profiles certified) makes the published verifier derive exactly its three declared opt-outs — `family.forms`, `family.sandbox`, `family.workflowChainPacks` — all absent from its live discovery document. Negative control: the same bundle with `forms` added to its captured document is rejected `opted-out-but-advertised` (and `signature-invalid`, since the document is signed). §1 and §2 keep the reference host's 2.35.0 evidence. |
...
| --- | --- | --- | --- |
| §1 an unobserved requirement is `blocked` | the bundle row's `result` for a requirement whose observation did not happen | the suite, by construction of the row | claims-check — suite review: a property of the suite's recording, pinned by its self-tests, not of a host |
...
| §3 opt-outs are checked against the captured document | a bundle recording an opt-out its captured discovery document advertises is rejected `opted-out-but-advertised` | the verifier, on any bundle | claims-check — suite review: the verifier's rule, pinned by self-tests on rows shaped like a real bundle (2.35.1 — the 2.35.0 tests used synthetic rows no bundle contains, and the rule derived nothing from real bundles) |
...
- [x] `conformance.md` §Bundle v3 drops its "proposed" marker (#1467).
- [x] Suite 2.35.0 published (2026-09-22).
- [x] A committed host bundle cut on 2.35.0 or later verifies under the new rules (the reference host, `commit:8477d75`).
- [x] A committed bundle cut on 2.35.1 or later shows `--verify` deriving the host's real opt-outs — MyndHyve, 2.35.1: `family.forms`, `family.sandbox`, `family.workflowChainPacks` (added and met 2026-09-22).
