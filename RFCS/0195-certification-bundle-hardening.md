...
| §3 opt-outs are checked against the captured document | a bundle recording an opt-out its captured discovery document advertises is rejected `opted-out-but-advertised` | the verifier, on any bundle | claims-check — suite review: the verifier's rule, pinned by its self-tests |
