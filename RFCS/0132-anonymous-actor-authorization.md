...
5. **Local public-surface only.** The `anonymous` kind is minted only for a *local* public agent surface (§C). A host MUST NOT set `principalKind: "anonymous"` for an A2A-forwarded end-user interaction (RFC 0100/0101) — a forwarded actor keeps the A2A owner model; mapping it to the anonymous kind (and riding the RFC 0128 purpose-propagation label across the anon boundary) is a cross-host follow-on (§Unresolved-Q3).
...
### §F — Safety + SECURITY invariants (normative)

This RFC introduces five new **protocol-tier** MUST-NOTs. All five are **behavioral** guarantees that require a live tool-enabled public surface, so — following the exact RFC 0079 `egress-credential-audience-bound` precedent this RFC cites — each **landed at `reference-impl` tier at `Draft → Active`** (its gated conformance scenario soft-skipped until a reference host wired the surface) and **GRADUATED to `protocol` tier at `Active → Accepted` (2026-07-22)** once the openwop-app tier-1 reference host (PR openwop-app#2403, rev `03d06d1f2`) wired the surface and the five gated scenarios passed **non-vacuously** under `OPENWOP_REQUIRE_BEHAVIOR=true` (steward-verified 2026-07-22: the in-memory host advertised both tiers + a `hitl` control, the seam served `200`, and `@openwop/openwop-conformance` anon-actor scenarios were 10/10 green). The always-on `anonymous-actor-shape.test.ts` proves the wire contract server-free:
...
All five resolved at `Active → Accepted` (2026-07-22), architect-reviewed by the steward. Recorded here; the two that touch normative text (UQ3, UQ5) are folded into §A.4/§E and §C.3/Summary respectively.

1. **`principalKind` for the authenticated cases.** — **RESOLVED: passive.** `user`/`agent` are a passive marker; only `anonymous` binds behavior in this RFC. The enum carries all three (resolving RFC 0048 §UQ1's discriminator) and `openwop.actor.kind` MAY surface them, but no audit or authorization requirement binds `user`/`agent` here. A future consumer that needs the distinction to carry weight files its own additive RFC.
2. **Cross-surface session correlation for legitimate continuity.** — **RESOLVED: out of scope (follow-on).** §A.2's forbid-reuse stands. A host-minted continuity token that survives a session is a de-anonymization pressure and deserves its own security pass; it is explicitly out of this RFC's scope. Any such token would still have to preserve non-PII + non-cross-linkability-to-an-identity.
3. **A2A-forwarded anonymous interactions.** — **RESOLVED: deferred, with a negative pin.** Under this RFC a host receiving an RFC 0100/0101 A2A-forwarded interaction MUST NOT set `principalKind: "anonymous"` for it — the anonymous actor is minted only for a *local* public surface (§A.4, §E). Mapping a forwarded actor to `anonymous` and riding the RFC 0128 purpose-propagation label across the anon boundary is a cross-host follow-on RFC.
4. **Anonymous-actor rate-limit vocabulary.** — **RESOLVED: opaque.** The `rate-limit-session-cap` parameters (window, per-IP budget, per-session cap) stay host-owned and are NOT advertised — publishing the exact cap tells an abuser precisely how much they may do. `writeEgressControls: ["rate-limit-session-cap"]` advertises only *that* a cap exists (§B).
5. **`bounded-write-egress` + BYOK downgrade default.** — **RESOLVED: credential-free by default.** Anon egress is credential-free by default (`downgraded`/`denied`); the conformance floor (`anon-actor-egress-ssrf-guarded`) asserts `credentialAttached:false` on the tested out-of-audience path. A host MAY attach a credential to an anon egress **only** when that credential's RFC 0079 provenance carries an **explicit anon-use permit** AND the destination is in-audience — a host-policy option **outside** the RFC 0132 Accepted floor that MUST NOT weaken the confused-deputy guard (§C.3). "An anon actor never reaches BYOK" (Summary) means the actor never *resolves/reads* credential material; a host-side in-audience attach the actor never sees is the separate, opt-in case.
