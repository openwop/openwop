...
This RFC introduces five new MUST-NOTs whose **end-state is protocol tier**. All five are **behavioral** guarantees that require a live tool-enabled public surface (no host ships one yet), so — following the exact RFC 0079 `egress-credential-audience-bound` precedent this RFC cites — each **lands at `reference-impl` tier at `Draft → Active`** (its gated conformance scenario soft-skips until a reference host wires the surface) and **graduates to `protocol` tier at `Active → Accepted`** once the reference host (openwop-app) passes the scenario **non-vacuously** under `OPENWOP_REQUIRE_BEHAVIOR=true`. The always-on `anonymous-actor-shape.test.ts` proves the wire contract at `Active`. The "Tier" column below is the **end-state**; the `SECURITY/invariants.yaml` rows carry the reference-impl-until-Accepted landing note:
