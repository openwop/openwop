...
| G3 | **Adoption-gated — CONSUMER half built 2026-08-11; advertising half in flight.** The mechanism needs a host that advertises *and* a consumer that acts on a mismatch, and this row tracks both. **Consumer half: done.** `contract-provenance.test.ts` leg C reads a host's advertised revision, compares it against the suite's own `schemas/CORPUS-STAMP.json`, and **reports** the drift. It **never fails on a difference** — requirement 3, not timidity: additive means a host on an older corpus is *conformant*, and reddening it would convert an advisory disclosure into an upgrade mandate inside a version line where being behind is legal. What it *does* assert is that an advertised object carries at least one of the two members, because an object conveying neither is indistinguishable from silence while looking like an answer. Verified in the **published** layout across four host postures — same revision, drifted, silent (inapplicable), and `{}` (fails) — because the suite's stamp exists only in the tarball, so a repo-layout run has nothing to compare and proves nothing. **Advertising half: DONE 2026-08-13** — landed on the **in-memory reference host** (openwop-examples#13), not openwop-app as this row originally anticipated, and the row is corrected rather than left asserting the older expectation. It derives both values from the installed package's `CORPUS-STAMP.json` rather than a constant, so the claim is structurally true instead of merely asserted. Witnessed end-to-end against the published-layout suite: host `1.73.0`/`b4b867e3` vs suite `1.97.0`/`96971233` — a **real** revision difference, reported and not failed. **What remains open is adoption, not mechanism:** one advertiser exists and it is the steward's own, so this demonstrates the mechanism works rather than that the ecosystem uses it. **Second advertiser 2026-08-15 — and the FIRST on a production deployment**: `openwop-app` (tier-1) advertises `{"suiteVersion":"1.106.0"}` at the document root, build-time stamped, live on its public origin. Landing it took three deploys and produced the deployment caveat below. **The consumer leg read a real advertised value for the first time since it landed** — run from the *published* tarball against that live origin, it reported `host: suite=1.106.0 commit=? | this suite: suite=1.106.1 commit=eb487d75adb0 | DIFFERENT corpus revision`, and **passed**, which is requirement 3 behaving exactly as written: real drift, reported, not failed. Until that run the reader had been green on every execution *by never having anything to read* — the vacuity this RFC's own G3 row was recording as "done". |

### Deployment caveat on G2 — every layer between the derivation and the wire can turn this field off

Non-normative. G2 recommends deriving `contractProvenance` from the installed `@openwop/openwop-conformance` package rather than from a constant. The recommendation is right and it is **not sufficient**, for a reason that generalises past any particular packaging step:

> **Every layer between the derivation and the wire can drop this value without erroring, and none of those layers can be observed from the document.**

Dependency pruning, multi-stage `COPY`, build-context ignore files, bundlers, and asset pipelines all discard files silently *by design* — that is their job. A host can derive correctly at every step it wrote and still serve nothing.

**This shipped twice in one day, on the same host, by two different mechanisms.** `openwop-app` (tier-1) is named at its own request: an anonymised version of this warning reads as a hypothesis, and a hypothesis is what G2 already was.

1. **Runtime dependency pruning.** The derivation resolved the installed package from a **lazy, memoised call made per request** — not at module load. The package is a `devDependency` and the Dockerfile runs `npm ci --omit=dev`, so `require.resolve` threw in the final stage. Correct in source, correct in dev, **inert in production.** *The timing is worth stating precisely, because getting it wrong suggests the wrong fix:* a reader told the derivation ran at import time will reach for "derive it later, lazily" — and this host **already did that**, and failed identically. The resolution boundary is the runtime stage, not the moment of the call. **Timing is not the variable; packaging is.**
2. **Build-context exclusion — after fixing (1).** The fix stamped the value at build time into a `build-meta/` directory, which is deliberately gitignored; `.gcloudignore` does `#!include:.gitignore`, so each stamp needs an explicit negation to reach the builder. The negation named `build-meta/commit.txt`. **The new `corpus-suite.txt` was never in the upload set** — confirmed with `gcloud meta list-files-for-upload`: present on disk, absent from the build context. The commit stamp matched, the deploy verifier was green, and the field was still missing.

**The second instance is why this section is written against the class rather than the mechanism.** Naming only `--omit=dev` would let a reader fix that one boundary and believe they were done — which is exactly what happened between instance (1) and instance (2), inside an hour.

The same trap caught the host's own tripwire. A test existed *specifically* to catch a build stamp not reaching the image, carrying a documented near-miss and eight assertions — and it stayed green, because every assertion named `commit.txt`. **A tripwire written against an instance is indistinguishable from one written against the class until the second instance arrives.** The generalised form derives its file list from the writer rather than naming files, and carries a vacuity guard so a parse that stops matching fails loudly instead of covering nothing.
...
1. **Derive at build time, stamp the value into the image — and verify the stamp is actually in the image.** Deriving correctly and packaging the result out are indistinguishable downstream, and instance (2) is what a host gets for doing only the first part. Derived-not-asserted is preserved; only the moment of derivation moves.
...
Half 2 is what found instance (2): the deploy-time check went red against the real wire before the fix and **stayed red after**, which is the only reason the packaging layer was examined at all. A check first exercised on a wire where the field is present would have proven nothing.

**Instance (3) — the detector itself, and it is the reason this section demands a red→green *pair* rather than a fix.** After the packaging fix shipped, the deploy-time check went red a **third** time. The field was being served correctly. `capabilities.schema.json` places `contractProvenance` at the **document root**, beside `protocolVersion`; the host nests other families under a `capabilities` object, so the checker resolved `doc.capabilities ?? doc` and read the wrong path. That is not a bug under some condition — **there is no wire on which that check could have returned OK.** It was structurally incapable of the green half, and its red was byte-identical to the two honest reds before it, carrying a confident and wrong diagnosis (*"most likely the value did not survive the runtime stage"*).

**A broken detector, from the inside, looks exactly like a detector that keeps finding things.** Had the fix been reported the easy and true way — *"the field is on the wire now"*, which one `curl` shows — half 2 would have been recorded as witnessed, the check would have reported `MISSING` on every future deploy of a correct host, and the first person to debug it would have concluded it was noise and deleted it. It had also already failed a healthy deploy closed, which is the same hazard from the other side: **a gate that cries wolf gets removed.**

What preserved the earlier evidence was not the instrument but independent corroboration — the host's own `conformance corpus stamp unresolvable` log line, present at three timestamps and stopping after the fixing deploy. Without it, "the field was missing three times" and "the check was broken the whole time" would be indistinguishable after the fact.

So the honest record of half 2 is:

> **The deploy-time check is the right location and the mechanism works in both directions. Its first implementation read the field from the wrong path and could never have gone green — caught only because the green half was demanded as evidence rather than inferred from the fix landing.**

A host that cannot derive honestly should omit the field rather than reconstruct one — but that is **requirement 2's obligation, not a new one**: inventing a `corpusCommit` to fill the shape is exactly the false statement requirement 2 already forbids, and it already has the conformance leg. A partial derivation advertises only the member it actually knows, per requirement 1 and the §Proposal case for *"a host that knows only one."* Stated here as a restatement rather than a fresh RFC 2119 keyword, because a bolded `MUST` inside a section headed *non-normative*, carrying no requirement number and no test, either gets enforced by accident or teaches readers that this corpus's keywords are decorative.

Reported by `openwop-app` while implementing this RFC's advertising half. Instance (1) was found only because someone went to build a field that already existed; instance (2) only because the deploy-time check from half 2 was run **before** the fix landed, while it could still fail.
