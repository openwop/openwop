...
1. **Runtime dependency pruning.** The derivation ran at module load against the installed package; the package is a `devDependency` and the Dockerfile runs `npm ci --omit=dev`, so `require.resolve` threw in the final stage. Correct in source, correct in dev, **inert in production.**
