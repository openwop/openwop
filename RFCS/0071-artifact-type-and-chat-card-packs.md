...
> | **Supersedes** | — |
> | **Superseded by** | — |
> | **Amended by** | [RFC 0075](./0075-artifact-type-packs-realworld-amendment.md) (artifact-type-packs real-world adoption amendment — host-native registration tier, `additionalProperties` MUST → SHOULD, per-type capability facets); [RFC 0205](./0205-run-artifacts-and-turns-speak-a2a-parts.md) (`exportFormats` accepts lowercase media types beside the reserved aliases, with a normative alias table) |
...
**Remaining for the RFC overall (keeps `Status: Active`):** Phase 2 (`kind: "card"`) is now `Active` (G9 resolved 2026-05-27; R2 invariant + scenario landed). **Phase 2 → `Accepted`** awaits a host implementing `host.chat.cardPacks` and passing the `chat-card-pack-execution.test.ts` R2 trust-tag proof end-to-end. The RFC graduates to `Accepted` overall when both phases are `Accepted`.
