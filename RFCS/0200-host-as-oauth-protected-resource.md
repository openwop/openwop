...
| **Created**       | 2026-09-22                                                      |
| **Updated**       | 2026-09-26 — `Active → Accepted` (provisional: RFC 0156 §B review owed, register row 0200 `not-reviewed`). Evidence tier: tier-2 — MyndHyve's certified major-2 bundle on published suite 2.39.5 (openwop#1585, `api.myndhyve.ai`, build `commit:47685450`, revision `workflow-runtime-00768-sug`, `oidc` lane, nothing relaxed) carries `prm-served`, `prm-consistent`, `challenge-401`, `challenge-403-scope`, `no-challenge-on-nondisclosure-404` and `inbound-credential-no-passthrough` at `executed-pass` on the served front; the v2 reference host's certified 2.40.0 cut is the second `inbound-credential-no-passthrough` host. **Disclosed:** MyndHyve's `id-token-aud` pass ran under a transient, Admin-SDK-gated, ≤ 3 h, runs-only-principal trust of the suite's per-cut issuer (`conformanceOidcIssuer.ts`; 3 accepts in 14 days of prod logs, all from its own cuts), a root its captured discovery does not advertise. It witnesses the production audience-check code under a precondition trust root (GOVERNANCE seam-precondition shape), not the advertised trust posture; after openwop#1581 a production host records the row `inapplicable`, and RFC 0216 bounds where a harness-issuer witness may come from. The openwop-app colocated companion is **not** cited for any row. Earlier: 2026-09-25 — **correction (Status unchanged):** `id-token-aud` records `inapplicable`, not `blocked`, when the `oidc` lane's `issuers[]` does not list the harness issuer (RFC 0168 §C.1's reading; see the note under §Conformance). No MUST changes. · 2026-09-22 — filed `Draft → Active`; **comment window waived** by the steward on 2026-09-22 under GOVERNANCE.md §"Sole-steward operation" — an explicit **steward override of RFC 0147 §A.6** (precedent: RFC 0194), which forbids bootstrap waiver language from shortening the window for an RFC of this risk class; it is outside the `MAINTAINERS.md` waiver grant, recorded there as an override, not as a routine waiver (this RFC affects identity and authorization: which tokens a host accepts, what its refusals disclose, and what credential may leave it). Acceptance under this override is provisional and the §B review is owed (RFC 0156 register, `docs/WAIVER-RETROSPECTIVE-REGISTER.md`). **Updated 2026-09-22 — amended per implementation review** (`review/arch-impl.md` R2, R3; Status unchanged): `.mcp-mount-prm` is minted at major 2 against the v2 MCP mount RFC 0208 builds; §C's `oauth2cc-wrong-aud-401` is a v1-only, non-gating row (v2's audience rule is the pre-existing `identity.md` §2.1); a MyndHyve deploy of §A/§B must precede its first cut on the suite that ships these scenarios. **Updated 2026-09-22 — dependency recorded (Status unchanged):** RFC 0210 (`Draft`, comment window to 2026-09-29) adds the `exp-only` revocation rule and makes a lane's advertised rule measurable. It does **not** block this RFC: MyndHyve advertises an `oidc` lane today and is therefore bound by §A/§B either way, so §A/§B has a production witness. What RFC 0210 changes is that the witness becomes sound — MyndHyve currently reaches this RFC's lane gate through an advertisement `identity.md` §2.2 does not sanction (`INTEROP-MATRIX.md` §"Advertised revocation rule per lane"). **This RFC's first certifying cut waits for RFC 0210's comment window to close**, and the MyndHyve deploy this header already requires should carry both changes in one push. The §Examples lane shape is caveated in place; nothing normative here changes. |
...
> **Correction (2026-09-25) — the `id-token-aud` gate.** The row above is `seam-gated (harness issuer)`, but the scenario recorded `blocked` whenever the host did not trust the harness issuer. A production host MUST NOT trust it, so every honest production bundle with an `oidc` lane carried a `blocked` row, and RFC 0168 §E.1 makes that deny certification bundle-wide. This was measured on openwop-app (its ADR 0745, corpus defect 1), and MyndHyve is in the same position. From suite 2.40.2, `v2-oidc-id-token-audience` applies RFC 0168 §C.1's reading for an unclaimed suite instrument:
> - it records `inapplicable`, naming the issuers the lane trusts, when the `oidc` lane's `issuers[]` does not list `OPENWOP_TEST_OIDC_ISSUER_URL`;
> - it runs when the lane lists the harness issuer. A refused same-audience control on such a lane stays `blocked`.
>
> No MUST, schema or wire shape changes. The residual is the same as for any seam-gated row: a production bundle does not witness §D, and the witness comes from a cut whose `oidc` lane lists the harness issuer.
...
- [x] A committed v2 host bundle carries `openwop.requirement.0200.prm-served`, `.prm-consistent`, `.challenge-401` and `.no-challenge-on-nondisclosure-404` at `executed-pass` from a host with an `oidc` or `oauth2` lane, nothing relaxed. **All four are `executed-pass` on a local boot of the v2 reference host under a configured `oidc` lane** (2026-09-22, loopback, `OPENWOP_REQUIRE_BEHAVIOR=true`); **Met 2026-09-26:** MyndHyve's certified 2.39.5 bundle (openwop#1585) carries all four at `executed-pass` on `api.myndhyve.ai` under its `oidc` lane, nothing relaxed.
- [x] `.inbound-credential-no-passthrough` at `executed-pass` (unaided webhook leg) on at least two hosts. **Met 2026-09-26:** MyndHyve (certified 2.39.5, #1585) and the v2 reference host (certified 2.40.0 cut). One so far: the v2 reference host, local boot, 2026-09-22. The second is an openwop-app v2 re-cut (`review/handoff-openwop-app-0200.md`).
- [x] RFC 0156 §B retrospective review row filed `not-reviewed` — `docs/WAIVER-RETROSPECTIVE-REGISTER.md` row 0200. Until the review is recorded, `Accepted` is **provisional**.
- [x] CHANGELOG entry.
