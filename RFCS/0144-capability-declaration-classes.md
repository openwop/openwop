...
| ID | Gap |
|---|---|
| G1 | **Arm visibility landed — CLOSED 2026-08-11.** `capability-arm-witness.test.ts` reports which of the four resolver arms a host lands on for every declared family, and asserts that none is reachable **only** under the deprecated `capabilities` wrapper. **It enforces an existing MUST rather than adding one:** `capabilities.md` §"Document-root layout (normative — RFC 0073)" already states that *"the conformance suite reads the root only … a host that serves families exclusively under the wrapper is non-conformant and is graded as such"* — but every helper in the suite falls back through the wrapper arms, so the corpus said one thing and the code did another, and a host that never migrated read exactly like one that did. That gap between prose and resolver **was** this gap. Dotted-at-root is **reported, not asserted**: RFC 0137 G16 makes the plain name the discovery key, but the MUST is written about the root rather than the spelling at it, and `additionalProperties: true` tolerates the extra key — failing there would be a v1.x tightening this leg has no license for, so it is surfaced instead, which is the visibility this gap asked for. Sabotage-verified across six placements: canonical passes; root+wrapper *mirrored* passes (mirroring is a permitted v1.x affordance, so a leg that failed it would be wrong); root-dotted passes **and reports**; wrapper-only and wrapper-dotted both fail. **What it still does not do:** it cannot make a host migrate, and a host mirroring onto both arms is indistinguishable from one that has retired the wrapper — that distinction only becomes enforceable when the wrapper retires at v2.0. |
| G2 | **Duplicate sections removed — CLOSED 2026-08-11.** `host-capabilities.md` carried three `§host.knowledge` and three `§host.secrets` sections. They were **not** variants: within each family the copies were byte-identical except for a single `**Used by:**` line, and the duplicated region was one contiguous 284-line block (`knowledge, secrets, knowledge, secrets`) pasted after the originals. Consolidated to one section each, **union-preserving**: the surviving `§host.knowledge` keeps the specific consumer (`vendor.myndhyve.knowledge-tools` with its typeIds) *and* gains the pack categories that appeared only in the deleted copies. One thing was deliberately **not** carried over — the third `§host.secrets` copy listed *"Future consumers: `vendor.myndhyve.ads-publish-platform`, `vendor.myndhyve.ads-metrics-import`"*, which the surviving copy supersedes by recording that the `ads.publish.platform` umbrella **decomposed** into three platform-specific packs. That line was stale rather than complementary; `ads-metrics-import` was speculative and is dropped with it, noted here rather than lost silently. No normative text changed — every MUST, capability flag and method contract in the surviving sections is byte-identical to what the duplicates carried. |
| G3 | **The §A rule is now machine-enforced — CLOSED 2026-08-11.** `scripts/check-capability-declaration-classes.mjs` runs in `openwop:check` against a reviewed ledger, `spec/v1/capability-declaration-classes.json`, classifying all 36 `§host.<name>` sections (25 declared / 11 extension). **The gate deliberately does not classify.** The §A trigger — *binds the shape or content of a wire artifact* — is a semantic judgement, and every heuristic reaching for it has been wrong in this corpus: `§host.http` advertises `httpClient`, so a heading-derived list calls it undeclared; `§host.coordination` names `agentRuntime` in prose **to contrast itself with it**, so a backtick-matching list calls it declared-by-that-key. Both false positives, the second found while building this gate — the same derive-from-a-plausible-proxy error the RFC records twice already. So a human decides once per section in a file a reviewer reads, and the machine guarantees the decision was made at all, still holds, and matches the schema. **What it closes is narrow and exact:** a new `§host.<name>` section can no longer land *unclassified*. It closes nothing about whether a given classification is right. Five checks, each sabotage-verified for its own cause: unclassified section, stale entry, `declared` key absent from the schema, `extension` family present in it, and — where the risk concentrates — an `extension` section stating a MUST without a recorded reason. That last check is what forced `chat` and `knowledge` to say why: both are `ctx.*`-only surfaces, and `knowledge`'s `403 knowledge_workspace_forbidden` is a host-side method error rather than a protocol response body, which is what keeps it out of the core schema. |
