...
| **Created**       | 2026-09-22                                                      |
| **Updated**       | 2026-09-22 (`Draft → Active` in the filing PR. **Comment window waived** (additive, 7-day) by the steward on 2026-09-22 under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md` §"Bootstrap-phase RFC waivers". RFC 0147 §A.6 does not apply. The RFC changes no identity, authorization, isolation, idempotency, replay, external-effect or certification rule. It widens one string grammar, adds one fallback step that only a newly legal key can reach, and adds two canonical-case SHOULDs.) · **Updated 2026-09-24 — correction:** the `delivery-extended-locale` row and the implementation notes named openwop-app as advertising `es`, `pt-BR`, `fr` (it is MyndHyve's set), and the major-2 scenario's gate tested a `supported` field the v2 `content` record cannot carry; both corrected inline, Status unchanged · **Updated 2026-09-22 — amended per implementation review** (`review/arch-impl.md` R3, R4, R7; Status unchanged: the six `(corpus)` ids, including `script-family-fallback` and `old-sections-unchanged`, which test the suite's shared `resolveSection` reference algorithm, are minted by a coherence test in `conformance/src/coherence/`; `delivery-extended-locale` is minted by the major-2 scenario, whose gated leg records `inapplicable` with its reason on a v2 cut; the major-1 legs are non-gating; the v2 twins are re-seeded only after `derive-v2-schemas.mjs` stops clobbering hand edits, or for the three files alone; the scenarios ride the open 2.36.0 suite (decisions log D3)) · 2026-09-24 (`Active → Accepted`). **Evidence tier: corpus gate — no host tier is claimed.** The six `(corpus)` rows are witnessed in `evidence/corpus-ledger.json`; the one host row, `0206.delivery-extended-locale`, is reached and recorded `inapplicable` with its reason on the certified public v2-reference bundle on published suite 2.38.0 (`evidence/v2-host-bundles/openwop-host-v2-reference.json`, openwop#1542; build `commit:1157625f`, witness `669d926abf45`, 385 executed-pass / 0 fail / 0 blocked, relaxations `[]`, all three profiles certified). **Why no host serves an extended content locale (2026-09-24):** openwop-app and MyndHyve each assessed it independently. Neither advertises a tag outside RFC 0103's `ll(-RR)` subset, because neither has content, UI strings or a product decision behind one (`es-419`, `zh-Hant`, `fil`), and advertising one only to witness this row would be a claim with nothing behind it. The v2 reference host serves no `content` family. The row is therefore externally gated on a host adopting an extended locale for real, not on corpus work; openwop-app#4098 (`35b50b5af`) widened that host's locale grammar to RFC 0206's and added the §C script-family step, so adoption is not blocked by the host. |
...
> **Correction (2026-09-24).** Two claims in the `delivery-extended-locale` row above were wrong, and the original text is kept as history.
>
> 1. **Which host advertises `es`, `pt-BR`, `fr`.** That set is MyndHyve's alone: its committed bundle carries the v2 record `content: {baseLocale: "en", supportedLocales: ["es", "pt-BR", "fr"]}` (`evidence/v2-host-bundles/myndhyve.json`). openwop-app advertises `content` only on its **v1** discovery document, only when the operator sets `OPENWOP_I18N_LOCALES` (its conformance run sets `en,pt-BR`; its deployment sets `en,es,pt-BR`), and it publishes no v2 `content` family — `evidence/v2-host-bundles/openwop-workflow-engine.json` carries none. Read "MyndHyve advertises `es`, `pt-BR`, `fr`" wherever this RFC says "openwop-app and MyndHyve".
> 2. **Why the row was `inapplicable`.** The row was `inapplicable` on every host, but not for the reason stated. The scenario `v2-content-locale-keys` gated on `content.supported === true`. The v2 `content` record in `schemas/v2/capabilities.schema.json` is `additionalProperties: false` and has no `supported` property, because at major 2 the presence of the record is the claim (RFC 0169 §A.2). So the gate was unsatisfiable by any schema-valid host, and the row could not execute even on a host advertising `es-419`. The gate now tests presence (`familyAdvertised`). A suite self-test, `conformance/src/lib/v2-family-gate-no-supported.test.ts`, refuses a `supported` comparison in any `v2-*` scenario. The verdict column is unchanged: the row is gated on an advertised tag outside the old subset, and no committed bundle carries one.
...
- openwop-app and MyndHyve advertise `es`, `pt-BR` and `fr`. All three are valid under both grammars, so neither host changes to stay conformant. To witness the gated leg, one of them adds a content locale such as `es-419`.
  > **Correction (2026-09-24).** Only MyndHyve advertises this set; openwop-app advertises `content` on v1 only, operator-gated, with no v2 `content` family. See the correction under §"Conformance". To witness the gated leg, MyndHyve adds a tag such as `es-419`, or openwop-app publishes a v2 `content` family carrying one.
...
- [x] `openwop.requirement.0206.delivery-extended-locale` records `executed-pass` on a committed bundle of a host advertising an extended content locale, or the RFC records why no host does and is accepted on the corpus rows alone (tier: corpus gate, the RFC 0189 precedent for rows witnessable unaided in the corpus). — the second branch: **Updated** records why no host does, and the host row is reached and recorded `inapplicable` with its reason on the certified public v2-reference bundle on published suite 2.38.0 (`evidence/v2-host-bundles/openwop-host-v2-reference.json`, openwop#1542; build `commit:1157625f`, witness `669d926abf45`, 385 executed-pass / 0 fail / 0 blocked, relaxations `[]`, all three profiles certified).
- [x] CHANGELOG entry.
