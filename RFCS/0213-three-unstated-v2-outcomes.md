...
- [x] **§A** — `v2-sse-last-event-id-cursor` `executed-pass` on v2-reference and an openwop-app memory boot, with each sabotage red. — v2-reference: #1575 (certified 2.39.5 public cut) `0213.last-event-id-exclusive-cursor` and `.last-event-id-after-authorization` `executed-pass`; openwop-app memory boot: the colocated companion #1577 (production image `bc0f13f1`, `memory://` storage) records the same rows plus `.last-event-id-malformed` `executed-pass`; sabotages (400 on a future id, resume from 0, cursor read before authorization) each red in #1517.
- [x] **§B** — `v2-idempotency-in-flight` `executed-pass` (not `partial-witness`) on one host. **Seam route (2026-09-26):** `0213.in-flight-refused-under-hold` drives the 409 branch deterministically through `armIdempotencyHold` (host-sample-test-seams.md §26) on a host that mounts the seams profile — an `executed-pass` on it satisfies this box. The record is in flight only while the winning *create request* is handled, so on a host that answers create in milliseconds the `409` branch rarely runs (v2-reference 2026-09-23: 5 of 5 answers were the winner plus four marked replays — `partial-witness`). The one-winner and replay-marker legs execute there; the `409` branch needs a host whose create handling overlaps, or a seam that holds a claim. — **Met 2026-09-26:** `0213.in-flight-refused-under-hold` `executed-pass` on v2-reference in #1575 (certified 2.39.5 public cut; seam `armIdempotencyHold`, suite #1572, host openwop-examples#88): the host's real create path refused the concurrent same-key request `409 idempotency_in_flight`; the seam never answers a create. `0213.in-flight-loser-outcome` stays `partial-witness` on fast hosts by design.
- [x] **§C** — `v2-interrupt-resolve-terminal` `executed-pass` on v2-reference; openwop-app ADR fix merged (run-scoped v2 ⇒ 409). — #1575: `0213.resolve-after-cancel`, `.resolve-after-complete`, `.resolve-unknown-run` `executed-pass`; openwop-app#4097 (ADR 0744 H3) merged and deployed, and the companion #1577 records the same three rows `executed-pass`.
- [x] Invariant `event-cursor-after-authorization` resolves to a test — `conformance/src/scenarios/v2-sse-last-event-id-cursor.test.ts` (protocol tier; `check-security-invariants.sh` green).
- [x] RFC 0156 §B retrospective review row filed `not-reviewed` — `docs/WAIVER-RETROSPECTIVE-REGISTER.md` row 0213.
