...
1. **Invent a fresh permission DSL for roles.** Rejected — openwop already has a scope grammar that API keys match against; a second grammar would drift from the first and double the conformance surface. Roles should resolve _to_ existing scopes.
2. **Put the full RBAC policy (role assignments, resource ACLs) on the wire.** Rejected — assignment is host-internal state; the protocol's job is the _decision shape_ and the _fail-closed invariant_, not the policy store. (Same boundary RFC 0046 drew for credential storage.)
