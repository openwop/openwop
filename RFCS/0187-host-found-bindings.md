...
| §A.1 the id a delivery CARRIES is the id minted *(added 2026-09-22, openwop#1450)* | `openwop.requirement.0187.bound-id-kinds.webhook-emitted` — a delivery's `OpenWOP-Webhook-Id` (and `X-openwop-Webhook-Id` on a dual-major host) equals the minted `webhookId`; the delivery is selected by run, never by the header under test | the suite, gated on `webhooks`, needing a reachable receiver | witnessable — gated on the `webhooks` family |
