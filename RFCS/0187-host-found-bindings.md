...
| **Created**       | 2026-09-18                                                      |
| **Updated**       | 2026-09-18 (`Draft` → `Active` in the filing PR. **Comment window waived** (additive, 7-day) by the steward under `GOVERNANCE.md` §"Sole-steward operation" and logged in `MAINTAINERS.md` §"Bootstrap-phase RFC waivers". RFC 0147 §A.6 does not apply: no high-risk surface is touched.) |
...
- [ ] `Active → Accepted`: `v2-bound-id-kinds` executed-pass on a host bundle for every kind that host advertises; `check-id-kinds-bound` green with `webhookId` bound; the deprecation row present.
