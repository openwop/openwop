...
| --- | --- | --- | --- | --- |
| `a2a-push-unadvertised-refused` | `a2a` 1.0 advertised, `pushNotifications` not `true` | all four push-config operations answer `-32003 PushNotificationNotSupportedError` (map `:104-123` "else PushNotificationNotSupportedError") | host answers `-32601` (openwop-app today for Get/List/Delete) | **yes** — at filing the one host-witnessable leg *(amended 2026-09-26: since openwop-examples#85 every leg here is witnessable)*. It witnesses the map's unadvertised row; for Get/List/Delete that clause was authored by this RFC (`interop-map.json:122`), so it is this RFC's behaviour, not only a pre-existing one |
...
| unadvertised ⇒ `-32003` (Create: pre-existing map row; Get/List/Delete: clause authored by this RFC, `interop-map.json:122`) | JSON-RPC error code | the suite, unaided | witnessable |
...
- [x] `a2a-push-unadvertised-refused` records `executed-pass` on a deployed tier-1 host: the certified public v2-reference cut on published 2.38.0 (openwop#1542, commit `20a8cbd5`, build `1157625f`, witness `669d926abf45`, signed `v2-reference-4`, 385 pass / 0 fail / 0 blocked, relaxations `[]`; discovery advertises `a2a` 1.0 with `pushNotifications: false`). All four push-config operations returned `-32003` (6 assertions). The later push-on cuts (#1558, #1575) replaced this bundle in the tree and record the row, correctly, `inapplicable`. openwop-app, named here at filing, does not advertise `a2a` at major 2 and is not the witness.
- [x] A deployed host advertising `a2a.pushNotifications: true` records `executed-pass` on the §A/§B/§E legs with a suite-owned public HTTPS receiver (a host's own ledger is not a witness): the certified public v2-reference cut on published 2.39.5 (openwop#1575; build `commit:2c6fc3c`, witness `09587c853ea2`, signed `v2-reference-4`, 399 pass / 0 fail / 0 blocked, relaxations `[]`, RFC 0158 rung `durable-single-instance`; receiver = `lib/scoped-receiver.ts` behind the public front): `0214.a2a-push-register-ssrf`, `.a2a-push-config-isolation`, `.a2a-push-secrets-not-returned`, `.a2a-push-delivery-authenticated`, `.a2a-push-no-openwop-signature`, `.a2a-push-no-redirect`, `.a2a-push-discard-on-delete`, `.a2a-push-fork-no-push` — all `executed-pass`. (D2 was reversed 2026-09-24: openwop-examples#85.)
- [x] RFC 0156 §B retrospective row for this override exists (`not-reviewed`) — `docs/WAIVER-RETROSPECTIVE-REGISTER.md` row 0214.
