...
- **Schema descriptions only.** `a2a-task-state.schema.json` `PushConfig.url` still abbreviates the SSRF guard to its address arm (the 2026-08-25 correction reached v1 and the RFC but not this v2 description); `tokenFingerprint` cites a nonexistent HMAC. Both descriptions are corrected; no keyword, type or `required` changes, so `check-v2-surface-monotone` sees nothing.
