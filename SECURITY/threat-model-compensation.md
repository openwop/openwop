...
> **Status of evidence:** shape and the §21 unwind/replay seams exist and `compensation-behavior.test.ts` drives them; **no host advertises the family on a deployed origin yet** (openwop-app's advert is gated on that witness going green). Until one does, every behavioural row below resolves to `blocked` (RFC 0148 §A). One invariant is registered; three are named and wait for a witness (§7).
...
- **Behaviour (gated on `compensation.supported`, hard-fails under `OPENWOP_REQUIRE_BEHAVIOR=true`):** `compensation-behavior.test.ts` via `POST /v1/host/sample/test/compensation/{unwind,replay}` (`host-sample-test-seams.md` §21) — plan before first effect, reverse-completion order, replay no-refire, content-free events, snapshot rollup ⇄ events.
- **Registered invariant:** `compensation-replay-no-refire` (`SECURITY/invariants.yaml`, protocol tier).
- **Named by RFC 0151 §G, not registered** — each needs a witness that exercises the threat, and the seam extension that would provide it:
  ...
  Registering any of these against `compensation-behavior.test.ts` today would be a vacuous pass — the file does not exercise the threat.
