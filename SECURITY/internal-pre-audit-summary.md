...
1. **Findings 002 (SSRF), 004 (ReDoS), 005 (MCP marker), 011 (error args), 012 (cacheable spec gap)** all benefit from external review — they touch deployment context (host egress policy, multi-tenant compute caps), invariant interpretation (MCP markers), and spec-text gaps the steward shouldn't unilaterally resolve.

### Document for advisory disclosure

1. Once the external audit confirms #001 and #003 (or adds Critical findings of its own), the affected versions get a CNA-assigned `openwop-SA-2026-NNNN` advisory ID per `SECURITY.md` §"Advisory IDs", published with the embargo lift.
