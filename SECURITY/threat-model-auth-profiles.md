...
| Elevation of privilege (revoked-but-unexpired) | A host advertising v2 `revocation: "exp-only"` re-checks the trust root **never**, so a subject revoked at the IdP keeps access until the credential's own `exp` — and nothing bounded that `exp`. A long-lived token turns the advertised window into decoration. | A host advertising `exp-only` on a lane **MUST** refuse a credential whose total lifetime (`exp − iat`) **or** remaining lifetime (`exp − now`) exceeds the advertised `revocationWindowSeconds`, with `401 credential_lifetime_exceeded`, and **MUST** refuse one carrying no `iat` (`identity.md` §2.2, RFC 0210 §B). A host that cannot enforce both bounds **MUST NOT** advertise the member; `auth.schema.json` refuses it on the `api-key` and `session` lanes, where the host issued the credential itself. Verified by `v2-lane-exp-only-bound.test.ts`, whose two refusal legs run in the same pass as a control credential that MUST be accepted. Invariant `lane-exp-only-lifetime-bounded`. |
...
| ---------------------- | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| Information disclosure | A host mints an email, display name, or credential into `subjectId`; run records and events leak it | `subject.schema.json` forbids `@` and whitespace; both `owner` sub-objects are closed; hosts MUST use an opaque, stable, issuer-scoped id. Scenario: `owner-subject-shape.test.ts` (claims-check).                            | `subject-record-opaque`            |
| Elevation of privilege | A synthesized legacy subject or an A2A-forwarded anonymous actor is joined into a SAML ⟷ SCIM link, so a SCIM deactivation denies — or a link inherits — an identity nobody attested | Legacy (`issuer: urn:openwop:legacy`) and anonymous subjects MUST NOT participate in a link (RFC 0165 §B.3, §B.6; `auth-profiles.md` §"Subject linking"). Scenario leg in `auth-subject-link.test.ts` — seam-gated, records `blocked` until a seam exists. | `subject-legacy-not-linkable`      |
| Elevation of privilege | A run record with no `owner.subject` (v1 `principal` string, or nothing) is read as owned by whoever asks, or a v2 host emits `principal` beside `subject` and a consumer keys on the weaker one | `RunSnapshot.owner` is the closed `{ tenant, workspace?, subject }` with `subject` REQUIRED and `principal` / `principalKind` removed (RFC 0170 §A.1; `identity.md` §1.1). Scenario: `v2-owner-subject-required.test.ts` (witnessable — unaided). | `subject-required-on-owner`        |
| Spoofing (link shape)  | A link record that is open, joins one subject to itself, or carries a legacy / anonymous subject is stored and later consulted by the leaver path | The SubjectLink is the closed record `{ a, b, keyClass, issuer, tenant, formedAt, deniedAt? }` of two distinct SubjectRefs, never legacy or anonymous (`subject-link.schema.json`; RFC 0170 §C.1). Scenario: `v2-subject-link-record.test.ts` (seam-gated). | `subject-link-record-shape`        |
| Spoofing               | A caller self-asserts `subject.actor` to launder a delegation                                       | `actor` is provenance, never authorization (RFC 0165 §B.5, RFC 0154 §B); depth ≤ 4 refused with `run_forbidden`; the delegation proof is the lane's (RFC 0154 §C). Covered by `workload-identity-*` invariants.                | (RFC 0154 invariants)              |
...
- **The host as an OAuth client (RFC 0199, `oauth-same-user-binding`).** A connector grant is a CSRF target: a callback carrying an attacker's `code` under a victim's session binds the attacker's third-party account to the victim (or the reverse). `spec/v2/core/oauth.md` requires PKCE S256, a single-use `state` bound to the initiating Subject, same-Subject completion and RFC 9207 `iss` validation (or a provider-unique redirect URI). **Residual:** the suite drives the grant through the `authorize-start` seam and the host's production callback; a host whose seam builds its own URL, or whose production session binding differs from the bearer the suite presents on the callback, is measured on a path production does not take — the bundle must state that the seam calls the production URL builder. §A.2's entropy and 10-minute lifetime are unwitnessable from samples; only the refusal half is witnessed.

- **A challenge as an existence oracle (RFC 0200 §B.3, `auth-challenge-no-oracle`).** RFC 6750 §3 and MCP's §"Runtime Insufficient Scope Errors" both push a host toward answering "you lack `tools:read`" where OpenWOP requires a non-disclosing `404` for an unknown **or** unauthorized resource. A host that adds the scope challenge uniformly turns `tool-catalog.md` §`GET /v1/tools/{toolId}`, the RFC 0072/0074/0086/0087 inventory routes and `capabilities-change-detection.md` into an enumeration oracle, and a resource-binding `403` into a hint. The rule is that a challenge attaches only to a response that is already `401`/`403` and never changes a status. **Residual:** the suite probes the routes it knows — an unknown id, a hidden-but-existing id where a fixture supplies one, and a cross-tenant `403` — so a host that leaks on a route the suite does not exercise, or through a timing or body-length difference the header rule does not govern, is not caught. Non-header oracles (latency, `ETag` presence) are out of this invariant's scope.
- **Wrong-audience acceptance (RFC 0200 §C/§D).** The audience substitution of §4.2/§4.4 is now an explicit MUST on both the OAuth2 client-credentials profile and the `oidc` lane's ID-token case, so a token minted for another relying party at the same IdP is `401`. **Residual:** the witness needs a harness issuer the host trusts (`OPENWOP_TEST_OIDC_ISSUER_URL`, `OPENWOP_TEST_OAUTH_ISSUER_TRUSTED`); a production host that trusts no test issuer records the row `blocked`, and its audience check is then attested rather than witnessed. The `aud` a Firebase-style deployment configures is a project id, not the URL form PRM's `resource` carries, so a client using RFC 8707 `resource` against such an IdP still gets a token the host refuses (RFC 0200 UQ1).
