...
- **Unprobed subject-link pairs (RFC 0159 §A.2/§A.4).** The conformance suite witnesses the correct deny / no-pass on the pairs it exercises, but cannot prove a `subjectLinking:true` host never joins on a mutable key for some pair it did not probe — a negative-existence gap. RFC 0159 UQ2 proposes a discovery-declared link-key class to convert this claims-check into a witnessable advertisement.
