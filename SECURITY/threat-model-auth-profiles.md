...
Applies to a host advertising both `openwop-auth-saml` and `openwop-auth-scim` with `capabilities.auth.subjectLinking: true` (`auth-profiles.md` §"Subject linking (SAML ⟷ SCIM)"). Linking the two lanes' principals to one human is the join whose key `auth-profiles.md` §A.2 forbids binding to a mutable or PII attribute — that MUST-NOT exists to close the threats below.
