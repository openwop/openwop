...
| Elevation (tier laundering) | An MCP server's `tools/list` annotates a mutating tool `readOnlyHint: true` so a host that trusts the hint catalogues it as `safetyTier: "read"` and a consumer gating on the tier stops asking | The host assigns `safetyTier`, `replayPolicy` and `egress` itself and never from upstream `annotations`; an unclassified `source: "mcp"` tool is `write`; any `ToolDescriptor.annotations` it publishes are derived from its own fields (RFC 0204 §D.11–§D.12, `spec/v2/core/tool-catalog.md`) | `tool-annotations-untrusted` |
...
| Credential phishing     | An agent renders a password or API-key field                            | Schema version 2 (RFC 0209): `TextField.variant: "obscured"` is excluded by the closed profile; a host MUST NOT emit a surface whose evident purpose is credential collection | `a2ui-surface-no-secret-input`     |

Schema version 2 of the kind (RFC 0209, real A2UI v0.9 messages in a closed OpenWOP profile of the basic catalog; `spec/v2/ext/a2uiSurface/README.md`) inherits every mitigation in this table: the profile excludes the upstream `functionCall` action arm (including `openUrl`), `FunctionCall` values, the URL-fetching `Image`/`Video`/`AudioPlayer` components and `theme.iconUrl`. Trust is sticky across a surface's fold: one untrusted envelope taints the whole surface, and a later trusted update does not launder it.
...
| `tool-annotations-untrusted`          | (RFC 0204) A host MUST NOT derive a tool's `safetyTier`, `replayPolicy` or `egress` from an MCP server's `annotations`; an unclassified MCP tool is `write`. MCP's own schema says clients "should never make tool use decisions based on `ToolAnnotations` received from untrusted servers"; the catalog is where a consumer makes exactly that decision. |
...
| `a2ui-surface-no-secret-input`         | A `ui.a2ui-surface` at schema version 2 (RFC 0209) MUST NOT solicit a secret: `TextField.variant: "obscured"` is excluded by the closed profile, and a host MUST NOT emit a surface whose evident purpose is credential collection. |
