...
> **Last reviewed:** 2026-05-24 (post-`@openwop/openwop-conformance@1.5.0` publish refresh + corpus releases v1.1.2 and v1.1.3 cut). The generated protocol snapshot lives at `docs/PROTOCOL-STATUS.md`: 35 prose specs, 27 JSON Schemas, 19 OpenAPI operations, AsyncAPI 3.1, 160 conformance scenario files, 26 RFCs tracked, and 48 indexed registry packs. RFC status: see `docs/PROTOCOL-STATUS.md` for the authoritative Accepted / Active / Draft counts; RFC 0025 (test-mode registry namespace) promoted Draft → Active 2026-05-25 as the `/v1/packs-test/*` mirror surface + capability `packs.testMode` landed end-to-end. Postgres reference conformance evidence holds at 781/850 default-mode pass count (91.9% total; 96.4% applicable) with one documented webhook flake.
