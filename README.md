...
*Reference-app UX pair (filed 2026-05-25; see [`plans/app-ux-enhancements.md`](./plans/app-ux-enhancements.md)):*
- [**RFC 0055**](./RFCS/0055-multimodal-envelope-variants-and-rendering-hints.md) — Multimodal envelope variants & rendering hints (`Draft`). Promotes RFC 0031 §C's reserved `vision-input` / `audio-input` / `audio-output` / `image-output` identifiers into a closed vocabulary; adds optional `meta.rendering` hint + `media.*` URL-reference payload convention. Explicitly does NOT add real-time A/V transport (out of charter).
- [**RFC 0056**](./RFCS/0056-run-feedback-and-annotation-event.md) — Run feedback & annotation event (`Draft`). New optional `host.feedback` capability + additive `run.annotated` RunEvent + `annotation.schema.json` + capability-gated `POST/GET /v1/runs/{runId}/annotations` for portable, non-blocking human/agent quality signals. Adds two protocol-tier SECURITY invariants (`annotation-cross-tenant-isolation`, `annotation-content-redaction`).
