...
| [`agent-workspace.md`](./spec/v1/agent-workspace.md) | DRAFT v1.x | ~150 | RFC 0059 — the `host.workspace` capability: a versioned, atomic, tenant·workspace-scoped (RFC 0048) ground-truth file store (identity / directives / memory-index), loaded as a read snapshot at run start. §C endpoints (`/v1/host/workspace/files[/{path}]`), §D run-snapshot exposure, §E invariants (WCT-1 cross-tenant + WSR-1 secret-redaction). Complements the transactional `MemoryAdapter` (RFC 0004) with a durable file layer. Milestone 1 (schema + prose) is Active; CRUD/ETag/isolation behavior + SECURITY invariant land at implementation. |
...
Current generated state: 40 prose specs (35 FINAL + 5 DRAFT) · 38 JSON Schemas · 37 OpenAPI operations · AsyncAPI 3.1 · 248 conformance scenario files · 3 reference SDKs. See [docs/PROTOCOL-STATUS.md](./docs/PROTOCOL-STATUS.md) for the machine-generated snapshot.
...
- **[`spec/v1/`](./spec/v1/)** — 40 prose specs (35 `FINAL v1`/`v1.1`, 5 `DRAFT v1.x` extensions). Start with `rest-endpoints.md` and `auth.md`.
