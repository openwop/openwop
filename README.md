...
> **Status: FINAL v1 (2026-05-08), multi-agent extensions Accepted (RFCs 0002–0007, promoted to `Accepted` 2026-05-11 after the integration-seams audit closed; WASM ABI RFC 0008 at `Active` since 2026-05-11).** Protocol contract is locked; multi-agent extension RFCs are at `Accepted` status with schemas published, normative prose under [`RFCS/`](./RFCS/), every per-phase integration surface verified against the canonical schemas, and conformance scenarios passing against the SQLite reference host (see [`docs/MULTI-AGENT-INTEGRATION-GAPS.md`](./docs/MULTI-AGENT-INTEGRATION-GAPS.md), archived). Implementations validate against the conformance suite at their own cadence; multi-agent scenarios are gated on host capability advertisement. Strict-mode conformance is available via `OPENWOP_REQUIRE_BEHAVIOR=true` — see [`conformance/coverage.md`](./conformance/coverage.md) §"Capability-gated scenarios".
...
| [`production-profile.md`](./spec/v1/production-profile.md) | PROVISIONAL v1 | ~900 | Public-release operational profile: durability, backpressure, retry, event retention, debug bundles, observability. Provisional until ≥1 host advertises it on `INTEROP-MATRIX.md`. |
...
| [`host-capabilities.md`](./spec/v1/host-capabilities.md) | DRAFT v1 | ~2,800 | Normative contracts for the 14 `host.*` capabilities that node-pack `peerDependencies` may declare (canvas, kanban, brand, mcp, et al.). Hosts that advertise `host.X: supported` MUST honor the §host.X contract. |
