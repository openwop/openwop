...
> **Status: v1.0 core locked (2026-05-12); v1.x extension surfaces in motion.** The **v1.0 core spec corpus** is locked: 47 of 52 `spec/v1/*.md` documents are at `Stable` (v1.1 / v1.x — `Stable` is the legend's replacement for the retired `FINAL` tag; see §"Status legend" below). **Five additive v1.x extension docs carry a `Draft` tag** (a curated cohort — distinct from the generated maturity tally in [`docs/PROTOCOL-STATUS.md`](./docs/PROTOCOL-STATUS.md)); these are explicitly not part of the v1.0 conformance gate and stay `Draft` because they are phase-partial, non-normative, or thin summaries of an RFC-defined surface: [`workflow-chain-packs.md`](./spec/v1/workflow-chain-packs.md) (RFC 0013 Phase 1), [`prompts.md`](./spec/v1/prompts.md) (RFC 0027 Phase A), [`cloudevents-mapping.md`](./spec/v1/cloudevents-mapping.md) (non-normative export mapping), [`multi-agent-execution.md`](./spec/v1/multi-agent-execution.md) (RFC 0037 Phase 1), [`agent-workspace.md`](./spec/v1/agent-workspace.md) (RFC 0059 is `Accepted`; the doc remains a thin `Draft` summary of the RFC surface). Nine agent/v1.x extension docs graduated `DRAFT → Stable` once their RFC reached `Accepted` (2026-05-31 – 2026-06-02): `agent-evaluation.md` (RFC 0081), `agent-deployment.md` (RFC 0082), `tool-catalog.md` (RFC 0078), `trigger-bridge.md` (RFC 0083), `budget-policy.md` (RFC 0084), `agent-platform-profile.md` (RFC 0085), `agent-roster.md` (RFC 0086), `agent-org-chart.md` (RFC 0087), `conformance-certification.md` (RFC 0089) — joining `artifact-type-packs.md` (DRAFT → FINAL 2026-05-27, RFC 0071) and `core-standard-profile.md` (DRAFT → Stable 2026-06-01, RFC 0088). **RFC status (94 RFCs excluding template):** RFCs that are `Accepted` (89), that are `Active` (4 — RFC 0035, RFC 0043, RFC 0093 + RFC 0094 from the 2026-06-11 corpus-review hardening/reconciliation cycle), and that are `Draft` (1 — RFC 0038, Parked). The full per-RFC status list is the generated table in [`docs/PROTOCOL-STATUS.md`](./docs/PROTOCOL-STATUS.md) (run `node scripts/generate-protocol-status.mjs --write` to refresh); per-RFC graduation history lives in each RFC’s `Updated` field + [`CHANGELOG.md`](./CHANGELOG.md). Conformance gates `Active`-RFC scenarios behind capability advertisements so v1.0-only hosts pass the locked-core suite. **Reference hosts:** four — `examples/hosts/{in-memory,sqlite,postgres,python}/` in [`openwop/openwop-examples`](https://github.com/openwop/openwop-examples). The Postgres reference host satisfies the production-profile predicate per [`examples/hosts/postgres/conformance-full.md`](https://github.com/openwop/openwop-examples/blob/main/examples/hosts/postgres/conformance-full.md) and is positioned as the first non-steward-deployable reference for credible multi-tenant workloads. **Conformance trajectory (2026-06-01 measurement against suite v1.18.1 — retained as historical context; the published suite has since advanced to `1.21.0` (+6 RFC 0090/0091/0092 scenarios, 3 of them always-on), so a re-measure is pending; see [`INTEROP-MATRIX.md`](./INTEROP-MATRIX.md) for the per-host table):** Postgres 95.7% (2068/2161, 0 deterministic failures); SQLite 95.1% (2056/2161, 0 deterministic failures), both re-measured 2026-06-01 against the v1.18.1 suite via pglite; in-memory 93.0% (2010/2161); Python 92.9% total / 100% of applicable when scoped to the host's claimed `openwop-core` + `openwop-stream-poll` + `openwop-stream-sse` profile set (2008/2161). The suite grew 2074 → 2161 tests since the 2026-05-30 v1.10.0 measurement as the 1.11–1.18.1 scenarios (agent-platform graduation, SAML behavioral leg, Core Standard Profile) landed; all four reference hosts gained +18 passes apiece with no new deterministic failures. The remaining failure counts are capability gaps in surfaces the minimal hosts deliberately do not implement (interrupts, stream-mode buffering, multi-agent dispatch, agent-memory, `aiEnvelope.capBreached`), not regressions. **SECURITY surface:** 116 invariants in [`SECURITY/invariants.yaml`](./SECURITY/invariants.yaml) — 90 protocol-tier (verified at the spec gate; every one has at least one public test in [`conformance/src/scenarios/`](./conformance/src/scenarios/)), 24 reference-impl-tier (verified by reference impls' CI), 2 advisory. (2 invariants graduated reference-impl → protocol on 2026-05-21 via RFC 0034; +1 protocol invariant added 2026-05-22 via RFC 0041 — `replay-llm-cache-key-portable`; +1 protocol invariant added 2026-05-24 via RFC 0046 — `credential-payload-redaction`; +1 protocol invariant added 2026-05-25 via RFC 0049 — `authorization-fail-closed`; +2 protocol invariants added 2026-05-25 via RFC 0028 Tier-2 post-promotion strengthening — `prompt-mutation-workspace-membership-enforced` (write path) + `prompt-read-workspace-membership-enforced` (read path), both filed in response to a self-disclosed adopter Admin-SDK-bypasses-DB-rules vulnerability; T1 + T2 also canonicalized `workspace_membership_required` as the 403 envelope error code; +2 protocol invariants added 2026-05-25 via RFC 0057 — `memory-attribution-no-content` + `memory-attribution-tenant-scoped`, the content-free / tenant-scoped guarantees on the new `memory.written` event; +1 protocol invariant added 2026-05-25 via RFC 0059 M2 — `workspace-cross-tenant-isolation`, the WCT-1 cross-owner isolation guarantee on `host.workspace`; +1 protocol invariant added 2026-05-25 via RFC 0063 M2 — `subrun-merge-approval-fail-closed`, the fail-closed sub-workflow output-merge approval gate; +1 protocol invariant added 2026-05-26 via RFC 0069 — `exec-must-not-be-protocol-tier`, the structural carve-out that arbitrary-command execution is host-extension-only, verified by the always-on server-free `exec-not-protocol-tier.test.ts`; +2 protocol invariants added 2026-05-26 via RFC 0071 — `artifact-schema-compile-bounded` (bounded compilation of third-party artifact-type-pack schemas; always-on server-free floor) + `chat-card-input-trust-boundary` (card-input-derived prompt segments carry `contentTrust:"untrusted"`; host-pending, Phase-2 `Active` gate); +1 protocol invariant added 2026-05-30 via RFC 0081 — `eval-summary-no-content-leak`, the content-free guarantee on the `EvalSummary` + `eval.*` events, verified by the always-on `agent-eval-suite-shape.test.ts`; +2 protocol invariants added 2026-05-30 via RFC 0079 (`egress-decision-no-secret-leak`, the no-secret-material guarantee on `egress.decided`) + RFC 0084 (`budget-no-pricing-leak`, the no-provider-pricing guarantee on the `budget.*` events), with RFC 0079's `egress-credential-audience-bound` confused-deputy MUST graduated reference-impl → protocol 2026-06-01 on RFC 0079 Accept (public test: the gated `egress-audience-binding.test.ts`); +1 protocol invariant added 2026-05-30 via RFC 0082 (`deployment-event-no-content-leak`, the content-free guarantee on the four `deployment.*` events), with the behavioral `deployment-promotion-fail-closed` graduated reference-impl → protocol 2026-06-01 on RFC 0082 Accept (public test: the gated `agent-deployment-lifecycle.test.ts`); +1 protocol invariant added 2026-05-30 via RFC 0086 (`roster-attribution-no-content`, the content-free guarantee on the `roster.run.initiated` attribution event), verified by the always-on `agent-roster-shape.test.ts`; +1 protocol invariant added 2026-05-30 via RFC 0087 (`org-position-no-authority-escalation`, the no-authority-from-org-position guarantee — the `agent-org-chart.schema.json` carries no permissions/scopes/canDispatch field and rejects one via `additionalProperties:false`), verified by the always-on `agent-org-chart-shape.test.ts`; +2 invariants added 2026-06-11 via RFC 0093 — `webhook-cross-tenant-isolation` (protocol-tier, the delivery tenant-scope guarantee on webhook subscriptions, public test: the gated `webhook-tenant-isolation.test.ts`) + `webhook-delivery-egress-revalidation` (reference-impl-tier, delivery-time pinned-resolution re-validation + no-redirects).) Strict-mode conformance available via `OPENWOP_REQUIRE_BEHAVIOR=true` — see [`conformance/coverage.md`](./conformance/coverage.md) §"Capability-gated scenarios". **For the deliberately-disagreeable catalog of what the protocol does NOT yet prove** — shape-only conformance, reference-impl-tier invariants pending sandbox/OTel-collector implementation, profile claims pending non-steward adoption, external-action-gated work (audit engagement, vendor-neutral org migration), and in-flight Active-RFC wire surfaces — **see [`docs/KNOWN-LIMITS.md`](./docs/KNOWN-LIMITS.md).**
> **v1.x published artifacts.** [`@openwop/openwop`](https://www.npmjs.com/package/@openwop/openwop) (npm, **v1.2.0**) · [`@openwop/openwop-conformance`](https://www.npmjs.com/package/@openwop/openwop-conformance) (npm, **v1.18.1**) · [`openwop-client`](https://pypi.org/project/openwop-client/) (PyPI, **v1.1.7**) · [`github.com/openwop/openwop-sdks/go`](https://pkg.go.dev/github.com/openwop/openwop-sdks/go) (Go modules, **v1.1.7**). The three SDKs ship from [`openwop/openwop-sdks`](https://github.com/openwop/openwop-sdks) and are at feature parity per its [`sdk/PARITY.md`](https://github.com/openwop/openwop-sdks/blob/main/sdk/PARITY.md) — the `1.1.7` corpus release brought the agent-platform read surface to Python + Go (v1.1.7) and the TS SDK to **v1.2.0** (a minor, since `client.prompts.get` now returns the null sentinel on `404` to match the other SDKs); the wire-feature surface is identical, and the conformance suite versions independently (1.18.1). Package names are stable through any v1.x release per [`PUBLISHING.md`](./PUBLISHING.md). The initial v1.0.0 publication happened 2026-05-11; v1.1.0 adds the Phase H + Phase I capability surfaces (BYOK, MCP, HTTP, agent memory, OAuth2-CC, OIDC, etc.) — additive per [`COMPATIBILITY.md`](./COMPATIBILITY.md) §2.1. A Rust SDK is demand-gated and not in v1.x; the conformance suite is language-agnostic black-box, so any future Rust client tests against the same wire contract. Future moves to a vendor-neutral org are tripwire-gated per [`ROADMAP.md`](./ROADMAP.md) on ≥1 non-steward maintainer.

> **What remains v1.x work (external-action-gated).** External security audit engagement, first non-steward INTEROP-MATRIX row, first third-party node-pack on `packs.openwop.dev`, vendor-neutral org migration. None of these are controllable by the single-steward authoring this protocol; each is tracked with a named tripwire in [`ROADMAP.md`](./ROADMAP.md) §Phase 4. The corpus is **ready for first non-steward adoption** — the four reference hosts + 116 SECURITY invariants + 335 conformance scenario files + 3 SDKs + 1 spec-closure RFC umbrella close every gap a non-steward maintainer can be expected to discover on day one.
...
|---|---|---|---|
| [`auth.md`](./spec/v1/auth.md) | Stable · v1.1 | ~1,000 | API keys, scopes, tenant isolation, rate limits, audit |
| [`auth-profiles.md`](./spec/v1/auth-profiles.md) | Stable · v1.1 | ~900 | Optional production auth profiles: API-key rotation, OAuth2 client credentials, mTLS |
| [`rest-endpoints.md`](./spec/v1/rest-endpoints.md) | Stable · v1.1 | ~1,150 | Endpoint catalog with per-route auth/scope; canonical headers; error codes |
| [`idempotency.md`](./spec/v1/idempotency.md) | Stable · v1.1 | ~1,300 | Two-layer contract: HTTP `Idempotency-Key` + engine `invocationId` |
| [`version-negotiation.md`](./spec/v1/version-negotiation.md) | Stable · v1.1 | ~2,060 | Four version axes (engine, per-run event-log, per-event, runtime pinning); deploy-skew safety |
| [`capabilities.md`](./spec/v1/capabilities.md) | Stable · v1.1 | ~1,480 | `/.well-known/openwop` handshake; in-package vs network-superset shapes |
| [`capabilities-change-detection.md`](./spec/v1/capabilities-change-detection.md) | Stable · v1.1 | ~900 | `Capabilities-Etag`, scoped capability views, and non-HTTP discovery handoff |
| [`observability.md`](./spec/v1/observability.md) | Stable · v1.1 | ~1,260 | Canonical `openwop.*` OTel namespace; span names; metric kinds |
| [`stream-modes.md`](./spec/v1/stream-modes.md) | Stable · v1.1 | ~1,150 | Four SSE consumption modes: `values`/`updates`/`messages`/`debug` |
| [`run-options.md`](./spec/v1/run-options.md) | Stable · v1.1 | ~1,180 | Per-run `configurable` overlay + `tags` + `metadata` (decoupled from versioning) |
| [`interrupt.md`](./spec/v1/interrupt.md) | Stable · v1.1 | ~1,500 | Canonical HITL primitive: 4 `kind`s, 5-action approval vocabulary, signed-token callback |
| [`interrupt-profiles.md`](./spec/v1/interrupt-profiles.md) | Stable · v1.1 | ~850 | Optional HITL profiles: quorum approval, auth-required resume, external events, parent/child cancel cascade |
| [`replay.md`](./spec/v1/replay.md) | Stable · v1.1 | ~1,320 | `POST /v1/runs/{runId}:fork` for time-travel debugging |
| [`channels-and-reducers.md`](./spec/v1/channels-and-reducers.md) | Stable · v1.1 | ~1,500 | Typed state channels with explicit reducers (replaces variable-prefix conventions) |
| [`node-packs.md`](./spec/v1/node-packs.md) | Stable · v1.1 | ~1,750 | Pack manifest format + distribution + signing + registry HTTP API (P2-F5) |
| [`artifact-type-packs.md`](./spec/v1/artifact-type-packs.md) | Stable · v1.1 | ~1,150 | RFC 0071 Phase 1 — `kind: "artifact-type"` packs: distributable typed artifact definitions (schema + rendering hint + lifecycle); `host.artifactTypes` store/render/export negotiation |
| [`chat-card-packs.md`](./spec/v1/chat-card-packs.md) | Stable · v1.1 | ~1,200 | RFC 0071 Phase 2 — `kind: "card"` packs: AI chat cards (prompt template + typed input subset) bound to a typed `outputArtifactType`; `host.chat.cardPacks` execution + untrusted-input trust boundary |
...
| [`webhooks.md`](./spec/v1/webhooks.md) | Stable · v1.1 | ~1,400 | Subscription register/unregister; HMAC `{timestamp}.{rawBody}` signing; replay-attack-resistant verification recipe; best-effort delivery semantics + circuit breaker (post-v1 addition, 2026-04-29) |
| [`storage-adapters.md`](./spec/v1/storage-adapters.md) | Stable · v1.1 | ~1,150 | Normative `RunEventLogIO` + `SuspendIO` contracts for storage backends; in-memory reference impls; compliance checklist for third-party adapter authors (post-v1 addition, 2026-04-29) |
| [`registry-operations.md`](./spec/v1/registry-operations.md) | Stable · v1.1 | ~3,000 | Operator-side normative reference for node-pack registries: submission, validation, deprecation, yank, signing-key rotation, marketplace boundary (post-v1 addition, 2026-04-29 — closes NP4 + NP5 from `node-packs.md`) |
| [`profiles.md`](./spec/v1/profiles.md) | Stable · v1.1 | ~1,200 | Compatibility profiles derived from existing capabilities (no wire-shape change): `openwop-core`, `openwop-interrupts`, `openwop-stream-sse`, `openwop-stream-poll`, `openwop-secrets`, `openwop-provider-policy`, `openwop-node-packs` |
| [`scale-profiles.md`](./spec/v1/scale-profiles.md) | Stable · v1.1 | ~900 | Three normative scale tiers (`minimal` / `production` / `high-throughput`) with floors for concurrency, latency, retry, fan-out, replay |
| [`production-profile.md`](./spec/v1/production-profile.md) | Stable · v1.1 | ~950 | Public-release operational profile: durability, backpressure, retry, event retention, debug bundles, observability. **First satisfying host:** `examples/hosts/postgres/` since 2026-05-11 — see [`examples/hosts/postgres/conformance-full.md`](https://github.com/openwop/openwop-examples/blob/main/examples/hosts/postgres/conformance-full.md). |
| [`debug-bundle.md`](./spec/v1/debug-bundle.md) | Stable · v1.1 | ~900 | Portable JSON export of a single run's diagnostic state: `GET /v1/runs/{runId}/debug-bundle`. Profile-gated on `capabilities.debugBundle.supported: true` |
| [`positioning.md`](./spec/v1/positioning.md) | Stable · v1.1 | ~1,100 | Honest comparison of OpenWOP vs Temporal / Airflow / Argo / Step Functions / BPMN / LangGraph / MCP. When to choose OpenWOP and when not. |
| [`mcp-integration.md`](./spec/v1/mcp-integration.md) | Stable · v1.1 | ~700 | Worked example of OpenWOP + MCP composition. OpenWOP runs the workflow; MCP exposes tools to the LLM nodes inside that workflow. |
| [`a2a-integration.md`](./spec/v1/a2a-integration.md) | Stable · v1.1 | ~1,400 | Worked example of OpenWOP + A2A composition. A2A handles inter-agent message exchange; OpenWOP runs the workflow inside one agent. Includes OpenWOP↔A2A state-projection table with documented drift points. |
| [`host-extensions.md`](./spec/v1/host-extensions.md) | Stable · v1.1 | ~900 | What's in the protocol vs what's a host extension. Canonical-prefix table; `openwop.*` and other vendor namespaces are host-extensions. |
| [`host-capabilities.md`](./spec/v1/host-capabilities.md) | Stable · v1.1 | ~2,800 | Normative contracts for the 14 `host.*` capabilities that node-pack `peerDependencies` may declare (canvas, kanban, brand, mcp, et al.). Hosts that advertise `host.X: supported` MUST honor the §host.X contract. |
| [`host-sample-test-seams.md`](./spec/v1/host-sample-test-seams.md) | Stable · v1.1 | ~1,700 | Consolidated normative reference for conformance-only test seams under `/v1/host/sample/*` (prompt resolver, OTel span scrape, debug-bundle export, LLM cache-key recipe, staged-refusal). Capability gates + request/response shapes for each seam in one place. Production hosts SHOULD return 404/403 unless an env-gate is set. |
| [`agent-memory.md`](./spec/v1/agent-memory.md) | Stable · v1.1 | ~120 | Multi-Agent Shift Phase 3 — `memoryRef` resolution, `MemoryAdapter` host-interface contract, CTI-1 cross-tenant invariant, SR-1 secret-redaction invariant, TTL semantics. |
| [`agent-ref-positioning.md`](./spec/v1/agent-ref-positioning.md) | Stable · v1.1 | ~900 | Non-normative addendum to RFC 0002 — compares `AgentRef` to W3C DIDs, A2A `AgentCard`, and AGNTCY agent identity. |
| [`agent-workspace.md`](./spec/v1/agent-workspace.md) | Draft v1.x | ~250 | RFC 0059 (`Accepted`) — `host.workspace`: a versioned, atomic, tenant·workspace-scoped ground-truth file store (`/v1/host/workspace/files` endpoints + `workspace.updated`), loaded as a read snapshot at run start; complements the transactional `MemoryAdapter` with a durable file layer. |
| [`ai-envelope.md`](./spec/v1/ai-envelope.md) | Stable · v1.1.1 | ~530 | Inbound LLM-emission envelope (`type` / `schemaVersion` / `envelopeId` / `correlationId` / `payload` / `meta` / `partial`). Closes the documented gap where 8 v1 surfaces (`capabilities.md`, `host-capabilities.md` §host.aiEnvelope, `workflow-chain-packs.md`, `profiles.md`, `host-extensions.md`, `positioning.md`, `capabilities.schema.json`, reference host discovery) already advertise the envelope concept but its own wire shape, universal kinds, schema discipline, and Envelope Contract gate were never specified. Distinct from `RunEventDoc` (outbound) and `error-envelope.schema.json` (host HTTP errors). |
| [`structured-output-subset.md`](./spec/v1/structured-output-subset.md) | Stable · v1.1.1 (informative) | ~180 | Informative companion to `ai-envelope.md` introduced by [RFC 0030](./RFCS/0030-envelope-reasoning-and-tier-one-subset.md) (`Active` 2026-05-20). Documents the intersection of JSON-Schema features supported by OpenAI strict mode ∩ Anthropic strict tool use ∩ Google Gemini `responseSchema` as of 2026-05, so vendor-namespaced envelope-kind authors have a single reference for "what schema features are safe for cross-vendor portability." Includes the strict-mode optional-field emulation pattern. Non-normative — vendor behavior evolves, and substantive subset changes require a follow-up RFC. |
...
| [`i18n.md`](./spec/v1/i18n.md) | Stable · v1.1 | ~1,200 | Optional locale-negotiation annex — `Accept-Language` semantics, `locale` field on InterruptPayload + ErrorEnvelope, capability advertisement, fallback rules. Additive to `interrupt.md`. |
| [`compliance.md`](./spec/v1/compliance.md) | Stable · v1.1 | ~1,500 | Non-normative compliance-vocabulary mapping — protocol surfaces ↔ SOC 2 / GDPR / HIPAA / ISO 27001 controls. Operator reference; does not prescribe certification. |
| [`grpc-transport.md`](./spec/v1/grpc-transport.md) | Stable · v1.1 | ~1,400 | Optional alternative transport — `openwop.v1.Engine` gRPC service mirroring the REST surface. REST + SSE remains required; gRPC is opt-in. Canonical `.proto` at `api/grpc/openwop.proto`. |
...
Current generated state: 52 prose specs (47 Stable + 5 Draft) · 56 JSON Schemas · 48 OpenAPI operations · AsyncAPI 3.1 · 335 conformance scenario files · 3 reference SDKs. See [docs/PROTOCOL-STATUS.md](./docs/PROTOCOL-STATUS.md) for the machine-generated snapshot.
...
- **[`spec/v1/`](./spec/v1/)** — 52 prose specs (47 `Stable`, 5 `Draft` extensions). Start with `rest-endpoints.md` and `auth.md`.
