...
| [`core-standard-profile.md`](./spec/v1/core-standard-profile.md) | DRAFT v1.x | ~80 | RFC 0088 (`Active`) — **operational annex** (NOT a closed `profiles.md` catalog predicate) naming the stable **Core Standard Profile**: the floor of normative MUSTs with black-box production-path conformance (`openwop-core` ∧ `openwop-interrupts` ∧ a transport, backed by the always-black-box wire-core scenarios — runs / discovery / auth-401 / event-ordering / errors / idempotency / interrupts / webhooks / audit). Everything else is an extension kept out by one of two levers (RFC 0042 experimental tier for `Active` caps; floor-membership for `Accepted`-but-seam-gated caps), graduating **in** as its black-box proof lands. Orthogonal to `openwop-agent-platform`. The audit-response Core Candidate target. Live aggregate-evidence deferred to Accepted (MyndHyve + all reference hosts already pass the floor). |
