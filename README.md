...
| [`trigger-bridge.md`](./spec/v1/trigger-bridge.md) | DRAFT v1.x | ~90 | RFC 0083 (`Active`) — composes scheduling (RFC 0052) / dead-letter (RFC 0053) / queue-bus (RFC 0017) / webhooks / cross-host causation (RFC 0040) into one durable inbound-work contract: the `TriggerSubscription` four-state machine (active/paused/failed/dead-lettered) + a dedup/retry delivery model + trigger→run causation + the opt-in `webhooks.durable` mode + the derived `openwop-trigger-bridge` profile + two content-free `trigger.*` events. Channels (Slack/email/SMS) stay vendor extensions; only the bridge is uniform. Best-effort webhook default unchanged. Behavioral delivery + endpoints deferred to Accepted. |
