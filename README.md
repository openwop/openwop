...
> **v1.x published artifacts.** [`@openwop/openwop`](https://www.npmjs.com/package/@openwop/openwop) (npm, **v1.1.6**) · [`@openwop/openwop-conformance`](https://www.npmjs.com/package/@openwop/openwop-conformance) (npm, **v1.18.1**) · [`openwop-client`](https://pypi.org/project/openwop-client/) (PyPI, **v1.1.5**) · [`github.com/openwop/openwop/sdk/go`](https://pkg.go.dev/github.com/openwop/openwop/sdk/go) (Go modules, **v1.1.5**). The three SDKs are at feature parity per [`sdk/PARITY.md`](./sdk/PARITY.md) — patch versions now diverge (TS at 1.1.6 for the agent-platform helper surface; Python + Go at 1.1.5), but the wire-feature surface is identical, and the conformance suite versions independently (1.18.1). Package names are stable through any v1.x release per [`PUBLISHING.md`](./PUBLISHING.md). The initial v1.0.0 publication happened 2026-05-11; v1.1.0 adds the Phase H + Phase I capability surfaces (BYOK, MCP, HTTP, agent memory, OAuth2-CC, OIDC, etc.) — additive per [`COMPATIBILITY.md`](./COMPATIBILITY.md) §2.1. A Rust SDK is demand-gated and not in v1.x; the conformance suite is language-agnostic black-box, so any future Rust client tests against the same wire contract. Future moves to a vendor-neutral org are tripwire-gated per [`ROADMAP.md`](./ROADMAP.md) on ≥1 non-steward maintainer.
