...
| [`agent-platform-profile.md`](./spec/v1/agent-platform-profile.md) | DRAFT v1.x | ~90 | RFC 0085 (`Active`) — **operational annex** (NOT a closed `profiles.md` catalog predicate) naming the aggregate `openwop-agent-platform` target with a `none`/`partial`/`full` status: a floor (manifest+live agents / tool catalog+hooks / safe-fetch / provider-usage / prompts / memory / feedback / replay-or-declared-nondeterminism) + a governance `full` tier (RBAC / tenant scoping / memory attribution / debug bundle / durable triggers / egress policy) + an aggregating meta-scenario + a badge. Eval/deploy/budget (RFC 0081/0082/0084) are advisory platform-plus, not hard terms. Composes — does not redefine — every constituent. Live aggregate-evidence + badge rendering deferred to Accepted. |
