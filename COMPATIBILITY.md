...
**Conformance-affecting correction on record (2026-09-23, RFC 0212 — "canonical JSON" named, and the certification preimage pinned).** `spec/v2/core/packs.md` §Signing, `spec/v2/core/conformance.md` §Bundle and eight pack-manifest `scheme` descriptions required signatures over "canonical JSON" without naming an algorithm, and `witnessSha256` was specified only as covering "the reporter record". The suite sorted its rows with `localeCompare` in the process default locale, so a bundle cut on a machine whose locale is Czech, Slovak, Lithuanian or Hawaiian digested differently from the same bundle anywhere else; the RFC 0150 `tools[]` sort had the same comparator. RFC 0212 names RFC 8785 JCS for every signed or hashed preimage, adds the I-JSON refusal set (duplicate names, lone surrogates, non-finite numbers, integer literals beyond ±(2^53 − 1), non-JSON values — refused, never coerced), specifies the whole `witnessSha256` preimage with UTF-16 code-unit row order, and gives `tools[]` the same comparator. **W3C Process Class 3**: the prior text defined no algorithm, so no host could conform to a different one, and every committed artifact already carries JCS bytes. **Census:** 3 committed v3 bundles in `evidence/v2-host-bundles/` plus the v2-reference and openwop-app bundles re-derive their stored `witnessSha256` unchanged under code-unit order (5 of 5); 156 registry `pack.json` canonicalize identically under JCS; **0 affected**. The behavior change is the refusal: the suite's canonicalizer used to turn NaN into `null`, emit the text `undefined`, keep the last of two duplicate names and round `9007199254740993`; no committed input triggers any of them. No shape moves, so no `spec/v2/corrections.json` row. Witnessed by `jcs-vectors.test.ts` (normative `conformance/vectors/jcs-v1.json`) and `v2-bundle-witness-preimage.test.ts`; each named sabotage turns a leg red.
