...
| 18 | **`README.md` prose RFC-status lists lag the actual `Status:` fields** | The README banner at line ~66 is *supposed* to be the generated-status surface that `scripts/generate-protocol-status.mjs --check` keeps honest — but until PR #858 (2026-07-07) the banner-count guard was toothless (see Drift #27); treat "banner counts match" as verified only on a post-#858 tree. Separately, the **per-RFC prose lists below** ("v1.x Capability Profiles", "Active RFCs", "Draft RFCs") are hand-curated and lag promotions. The 2026-05-23 audit caught README:281 marking RFCs 0027/0030/0031/0032/0033 as `Active` after they had all promoted to `Accepted` between 2026-05-21 and 2026-05-23. Generated-status passes; prose lags silently. | Scope the comparison to the prose lists explicitly: `awk '/\*\*Active RFCs/{flag=1} /\*\*Draft RFCs\|\*\*v1 Foundation/{flag=0} flag' README.md \| grep -oE 'RFC [0-9]+' \| sort -u > /tmp/readme-active.txt; for f in RFCS/[0-9][0-9][0-9][0-9]-*.md; do id=$(basename "$f" \| grep -oE '^[0-9]+'); s=$(grep -m1 '^\| \*\*Status\*\*' "$f"); echo "$s" \| grep -q '\`Active\`' && echo "$id"; done \| sort -u > /tmp/actual-active.txt; diff /tmp/readme-active.txt /tmp/actual-active.txt` |
...
| 21 | **`npx -y -p @<pkg>@latest` in `openwop-check.sh` races the npm cache** | The validator toolchain (`@asyncapi/cli`, `@redocly/cli`) was historically invoked via `npx -y -p @<pkg>@latest`, which races itself when two `openwop-check.sh` runs interleave (or when the cache TTL expires mid-fetch), producing `ECOMPROMISED Lock compromised` errors that require `rm -rf /tmp/openwop-npm-cache` to recover. External auditor 2026-05-23 hit this twice. The fix that stuck: **explicit semver pins in the `npx` calls** (`npx -y -p @redocly/cli@2.31.4`, `npx -y -p @asyncapi/cli@4.1.1`) — pinned tarballs are content-addressed in the cache, so no `@latest` metadata round-trip to race; a devDependency vendoring was rejected (AsyncAPI CLI bundles ~100 MB of Studio UI). **Lesson:** ANY validator-toolchain bump must stay version-pinned; never reintroduce `@latest`. | `grep -nE "npx -y[^@]*@latest" scripts/openwop-check.sh .github/workflows/openwop-spec.yml 2>/dev/null` — any match is a regression (pinned `npx -y -p <pkg>@<semver>` is sanctioned; a naive `grep "npx -y"` false-positives on it). |
...
| 27 | **`README.md` `> **Status:**` banner RFC counts + enumerated Active/Draft id-list ↔ generated `docs/PROTOCOL-STATUS.md`** | The hand-written banner RFC-status sentence ("RFCs that are `Accepted` (N — including …), … `Active` (M — RFC 0035, …), and that are `Draft` (K — RFC 0038, Parked)") drifts from the generated table. This is the *banner* — the surface Drift #18 assumed the gate kept honest. It did NOT: `generate-protocol-status.mjs`'s banner guard used anchored regexes (`\((\d+)\)`) that match only a bare `(124)`, but the banner writes `(124 — including …)` with a trailing annotation, so the Accepted/Active/Draft count checks matched nothing (`m === null`) and *silently passed* while the numbers rotted. v1.3.0 shipped `Accepted 113 / Active 8` with RFCs 0114/0116/0117/0119 still enumerated as `Active` after all four graduated to Accepted that cycle — the invariant/conformance banner numbers were bumped, the RFC counts weren't. Caught downstream by openwop-site's homepage reconciliation, fixed in PR #858 (the guard now tolerates the annotation, hard-fails on a present-but-unparseable count phrase, and asserts the enumerated Active/Draft id-list — which can be stale even when the count is right). **Root-cause class: a mechanical guard with `if (m && …)` is toothless the instant the checked prose reformats out from under its regex — the guard was present but matched nothing, so "gate green" meant "gate blind," not "counts correct."** | `node scripts/generate-protocol-status.mjs --check` (post-#858 this covers banner counts + enumerated Active/Draft id-lists). Corroborate with the authoritative disk parse in the Drift #27 Phase-4 section — the CHANGELOG top entry + the disk parse are the two independent sources of truth. |

When auditing a session, walk this table from #1 to #27 and flag any row whose detection command surfaces a hit. If you find one, the corresponding row in the **Phase 4** drift-verification section below has the fix recipe. **Run every detection command on a fresh `origin/main` worktree (Drift #25) — the shared dev checkout drifts and produces false positives.**
...
- **The "toothless guard" failure mode (Drift #27).** A guard existing over a surface does NOT mean the surface is checked. A guard written as `const m = text.match(re); if (m && m[1] !== actual) fail()` *silently passes* the instant `re` stops matching — reformat the checked prose (add a trailing " — including …" annotation after a count) and `m === null`, so the whole check is skipped with zero signal. This is strictly worse than no guard, because everyone downstream *believes* it's covered (Drift #18 literally documented the banner as gate-protected — it wasn't). This is exactly how v1.3.0 shipped a stale banner. Two defenses, both landed in PR #858: (1) when the anchoring phrase IS present but the number can't be parsed, make that a **hard failure** ("phrase present but unparseable — the format changed, update the check"), never a silent skip; (2) **prove the guard bites** — feed it a known-bad input once and confirm it fails (re-introduce a stale count, run `--check`, see it go red). A green gate you've never watched fail is an unverified gate. Applies to every mechanical assertion over prose in this repo, not just the RFC banner.
...
| RFC Status field changed (Draft → Active or Active → Accepted) | Re-scan `docs/KNOWN-LIMITS.md` §"RFCs not yet `Accepted`" — promoted RFCs leave the table; new Active/Draft RFCs join. Run `node scripts/generate-protocol-status.mjs --write` to regenerate `docs/PROTOCOL-STATUS.md`, then **hand-edit the README `> **Status:**` banner** RFC-status sentence — its counts AND its enumerated `Active`/`Draft` id-lists are NOT auto-written by `--write`; `--check` verifies them (Drift #27). A promotion that leaves an RFC in the banner's `Active` enumeration is exactly the v1.3.0 drift. |
...
`scripts/generate-protocol-status.mjs --check` catches the README **banner** counts (line ~66: "34 Accepted / 6 Active / 4 Draft") — but only on a post-PR-#858 tree; see Drift #27 for the window when its regex silently matched nothing and the banner counts drifted anyway. Distinct from that, the **per-RFC prose lists below** ("v1.x Capability Profiles", "Active RFCs", "Draft RFCs") are hand-curated and lag promotions, and no gate covers them. The 2026-05-23 audit caught README:281 marking RFCs 0027/0030/0031/0032/0033 as `Active` after all 5 had promoted to `Accepted` between 2026-05-21 and 2026-05-23. Generated-status passed (banner counts were correct); the prose list silently drifted.
...
External auditor 2026-05-23 hit this twice in a single session. The fix that stuck (see the rationale comment block at the top of `scripts/openwop-check.sh`): **pin the semver explicitly in the `npx` call** — `npx -y -p @redocly/cli@2.31.4` / `npx -y -p @asyncapi/cli@4.1.1` (the last Node-22-compatible AsyncAPI release). A pinned tarball is content-addressed in the cache, so the second invocation hits cleanly with no `@latest` remote-metadata round-trip — that round-trip was what raced. Vendoring as a repo-root devDependency was considered and rejected: the AsyncAPI CLI bundles the entire Studio UI (~100 MB of `node_modules`) for what is effectively a YAML validator. A belt-and-suspenders `rm -rf "$NPM_CACHE/_locks"` at gate start clears any stale locks from killed prior runs.

Detection — only an **`@latest`** (or otherwise unpinned) `npx` invocation in the gate scripts is a regression; pinned `npx -y -p <pkg>@<exact-semver>` is the sanctioned pattern:

```bash
grep -nE "npx -y[^@]*@latest|npx -y -p @[a-z/-]+ " scripts/openwop-check.sh .github/workflows/openwop-spec.yml 2>/dev/null
# Empty output = no regression. (2026-06-12: a naive `grep "npx -y"` false-positives on the two sanctioned pinned calls.)
```

Lesson generalizes: ANY validator-toolchain bump in this repo MUST stay version-pinned — bump the explicit semver in `openwop-check.sh` (both the invocation and its rationale comment) and mirror it in `CONTRIBUTING.md` §"Useful one-liners". Never reintroduce `@latest`.
...
### Drift #27 — README Status-banner RFC counts + enumerated Active/Draft id-list

The `> **Status:**` banner (README line ~66) carries a hand-written RFC-status sentence: "RFCs that are `Accepted` (N — including …), … `Active` (M — RFC 0035, RFC 0043, …), and that are `Draft` (K — RFC 0038, Parked)". `generate-protocol-status.mjs --check` is *meant* to keep it honest, but before PR #858 its count regexes were anchored (`\((\d+)\)`) and matched only a bare `(124)` — the banner's `(124 — including …)` annotation made every count check `m === null` and silently pass. Result: v1.3.0 shipped `Accepted 113 / Active 8` (with 0114/0116/0117/0119 still enumerated `Active`) after those four graduated. Two independent authoritative sources — the top CHANGELOG entry's stated deltas and a clean disk parse — agreed on 124/4/1; only the banner disagreed.

**First, run the gate (post-#858 it now covers this):**

```bash
node scripts/generate-protocol-status.mjs --check
```

**Then corroborate against the two independent sources of truth** (do this even when `--check` is green — it's how you catch a guard that reformatted itself back into blindness, per the toothless-guard meta-lesson):

```bash
# (a) The release's own stated deltas — top CHANGELOG entry:
grep -m1 -E '^## \[?[0-9]+\.[0-9]+\.[0-9]+' CHANGELOG.md
grep -m1 -iE 'Accepted [0-9]+ ?(→|->) ?[0-9]+|Active [0-9]+ ?(→|->) ?[0-9]+' CHANGELOG.md

# (b) Authoritative disk parse — MAIN RFCs only (a main file is NNNN-slug.md with
#     slug in [a-z0-9-]; exclude 0000-template and every NNNN-slug.WORD.md sidecar
#     like .gaps.md / .risks.md):
python3 - <<'PY'
import re, glob
from collections import Counter
main = [f for f in glob.glob("RFCS/*.md")
        if re.match(r'^\d{4}-[a-z0-9-]+\.md$', f.split('/')[-1]) and '0000-template' not in f]
t, ids = Counter(), {}
for f in main:
    m = re.search(r'\*\*Status\*\*\s*\|\s*`([A-Za-z]+)`', open(f, encoding="utf-8", errors="replace").read())
    st = m.group(1) if m else "??"
    t[st] += 1
    ids.setdefault(st, []).append(re.match(r'(\d{4})', f.split('/')[-1]).group(1))
print("Accepted", t['Accepted'], "Active", t['Active'], "Draft", t['Draft'],
      "| live total", t['Accepted']+t['Active']+t['Draft'])
print("Active RFCs:", sorted(ids.get('Active', [])), " Draft RFCs:", sorted(ids.get('Draft', [])))
PY

# (c) What the banner currently claims (count + enumerated id-list):
grep -oE 'RFC status \([0-9]+ RFCs[^)]*\)|are `Accepted` \([0-9]+|`Active` \([0-9]+ [^)]*\)|`Draft` \([0-9]+' README.md
```

The banner's count AND its enumerated id-list must both equal the disk parse (they drift independently — a right count with a stale list, or vice-versa, are both real failures). Fix recipe: edit the banner sentence in place — bump the Accepted/Active/Draft counts and add/remove RFC ids from the `Active`/`Draft` enumerations so both match. The banner counts are NOT auto-written by `--write` (the self-heal covers corpus counts like prose-specs / schemas, not the RFC-status sentence), so this is a hand-edit; `--check` verifies it afterward. **After any fix, prove the guard bites:** re-introduce the stale count, run `--check`, confirm it goes red (all three dimensions — Accepted count, Active count, Active id-list — report), then restore. If `--check` stays green on a known-bad banner, the guard has gone toothless again — patch `generate-protocol-status.mjs` (the count regex must tolerate a trailing annotation after the number: `\((\d+)(?=[\s)])`, and a present-but-unparseable count phrase must hard-fail, not skip).
...
| `drift sweep` | Run the full drift catalog (#1–#27) from top of skill — report every hit before fixing anything |
...
| `rfc-status-sync` | Walk every RFCS/NNNN-*.md, compare its `Status:` field to `docs/KNOWN-LIMITS.md` open-RFC table + `docs/PROTOCOL-STATUS.md` RFC table + the `README.md` `> **Status:**` banner counts AND its enumerated `Active`/`Draft` id-lists (Drift #27); report mismatches |
