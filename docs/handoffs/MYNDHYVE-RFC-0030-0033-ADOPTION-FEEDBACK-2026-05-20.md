...
| 10  | Reference `parseRefusal` helper                                             | `sdk/typescript/` follow-up | ✅ **Resolved 2026-05-21** — Published as `parseRefusal(providerResponse): RefusalSignal \| null` from `@openwop/openwop`. Normalizes per-provider safety-stop signals (Anthropic `stop_reason: "refusal"`, OpenAI `message.refusal` + `finish_reason: "content_filter"`, Gemini `finishReason: "SAFETY"` + `promptFeedback.blockReason: "SAFETY"`) to a canonical `{ refusalText, safetyCategory?, provider }` shape. 24 unit tests covering all three vendors + defensive-shape paths. MyndHyve adopts via `import { parseRefusal } from '@openwop/openwop'`.                                                                                                                                                                           |
| 11  | Reference `buildReasoningDirective` export                                  | `sdk/typescript/` follow-up | ✅ **Resolved 2026-05-21** — Published as `buildReasoningDirective(responseSchema, strength): string \| null` from `@openwop/openwop`. Reference workflow-engine sample's helper (commit `9412f57`) becomes the canonical source. MyndHyve replaces their ported copy with `import { buildReasoningDirective, ReasoningDirectiveStrength } from '@openwop/openwop'`. 15 unit tests covering schema-shape gating + advisory/mandatory wording + the 2026-05-21 amendment's operational note.                                                                                                                                                                                                                                               |
| 12  | `@openwop/conformance-sdk/mint-test-key` helper                             | `sdk/typescript/` follow-up | ✅ **Resolved 2026-05-21 (as runbook, not SDK export)** — Published as `docs/runbooks/CONFORMANCE-API-KEY-MINTING.md`. The credential-acquisition path is per-host stack (MyndHyve: gcloud + Firebase Auth + admin-SDK; Postgres reference: static bearer; mTLS hosts: cert + key pair) — a generalized SDK helper would either delegate back to the operator or hard-code one vendor's flow. The runbook documents what the suite needs from an API key + per-host minting recipes (reference workflow-engine, Postgres reference, MyndHyve) + a template for other host stacks + the validation step + common failure modes (including MyndHyve's `invalid_rapt` reauth issue with `gcloud auth login` vs `application-default login`). |
