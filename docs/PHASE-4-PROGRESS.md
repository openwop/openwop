...
| Today's coverage | `conformance/src/scenarios/replay-divergence-at-refusal.test.ts` — advertisement-shape probes with a `refusalDivergenceEmission: true` MUST when `version >= 4`. Two `it.todo` behavioral assertions documented end-to-end (lines 132–133) with the wire-shape they will probe spelled out in comments. `conformance/src/scenarios/replay-observable-sequence-determinism.test.ts` — analogous shape + boundary. |
| Gap | (a) Reference workflow-engine's mock-AI provider does not have a "refusal on replay" mode. (b) The `conformance-phase4-nondet-tool` fixture does not exist. |
| Unblock criterion | (a) Extend `apps/workflow-engine/backend/typescript/src/aiProviders/aiProvidersHost.ts` (the mock-AI provider lives here, not in a separate `mockAiProvider.ts`) with a `OPENWOP_PHASE4_REFUSAL_ON_REPLAY=true` env-gate that returns valid envelope on the first call + refusal on subsequent calls (replays). (b) Add `conformance/fixtures/conformance-phase4-nondet-tool.json` per the schema documented in the scenario's leading comments. (c) Flip the 2 `it.todo` lines to `it()` with the assertion bodies the scenario header already spells out. |
| Effort | ~1 day end-to-end (workflow-engine wiring is small; fixture is JSON; scenario already-drafted). |
| Closing PR | TBD — `feat(workflow-engine+conformance): RFC 0041 Phase 4 staged-refusal seam + behavioral close-out` |
