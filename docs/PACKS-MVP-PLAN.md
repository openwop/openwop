...
2. **Community.openwop-team ed25519 keypair** — generate fresh + commit the **public key** to `packs/community.openwop-team.demo/keys/community.openwop-team.pub.pem` + serve from `packs.openwop.dev/keys/community.openwop-team.json`. The **private key** lives in the steward team's secrets vault (not the repo).
