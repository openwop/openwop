...
| 2 | `openwop-examples/examples/hosts/in-memory/certification-bundle-v2.json` | `openwop-host-in-memory` 1.1.7 (tier-1) | `@openwop/openwop-conformance` 1.116.0 | 2026-08-16 | `2` | **Valid evidence; certifies** `openwop-discovery-core` (+alias) and `openwop-fixtures`; `openwop-stream-sse` / `openwop-stream-poll` not certified (floor rows `executed-fail`); `openwop-node-packs` **rejected** (`pack-registry-publish` executed-pass, 0 assertions). Totals 311 / 30 / 0 / 77 / 43. Supersedes bundle 1. |
| 3 | `openwop-examples/examples/hosts/sqlite/certification-bundle-v2.json` | `openwop-host-sqlite` 1.1.7 (tier-1) | `@openwop/openwop-conformance` 1.116.0 | 2026-08-16 | `2` | **Valid evidence; certifies** `openwop-discovery-core` (+alias), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-fixtures`; `openwop-secrets` unprovable (no floor); `openwop-node-packs` **rejected** (`pack-registry` + `pack-registry-publish`, 0 assertions). Totals 340 / 0 / 0 / 78 / 43. |
| 4 | `openwop-examples/examples/hosts/postgres/certification-bundle-v2.json` | `openwop-host-postgres` 1.0.0 (tier-1; pglite-backed local boot) | `@openwop/openwop-conformance` 1.116.0 | 2026-08-16 | `2` | **Valid evidence; certifies** `openwop-discovery-core` (+alias), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-fixtures`; `openwop-provider-policy` / `openwop-memory` unprovable (no floor); `openwop-node-packs` **rejected**. Totals 353 / 2 / 0 / 74 / 32 (fails: `stream-subscribe-from-beginning`, `webhook-signed-delivery`). |
| 5 | `openwop-examples/examples/hosts/python/certification-bundle-v2.json` | `openwop-host-in-memory-python` 1.0.0 (tier-1) | `@openwop/openwop-conformance` 1.116.0 | 2026-08-16 | `2` | **Valid evidence; certifies** `openwop-discovery-core` (+alias), `openwop-stream-sse`, `openwop-stream-poll`, `openwop-fixtures`; `openwop-node-packs` **rejected**. Totals 321 / 0 / 0 / 78 / 62. |

Landed by `ad59631` ("spec(rfc-0089): --certify bundle generator + reference-host evidence → Accepted").

Bundles 2–5 landed by `openwop-examples#14` (2026-08-16), generated with `--certify --bundle-version 2` (requirement ledger on, evidence scrubbed) against a local boot of each host; every one validates against `certification-bundle-v2.schema.json` and the consumer verifier `verifyBundleV2` agrees with the emitter row-for-row ("valid evidence" is the verifier's `evidenceValid`; "certifies" is its per-profile `certified`). Totals are executed-pass / executed-fail / skipped / inapplicable / blocked. **The reissue RFC 0148 §D asked for is therefore done for the four reference hosts** — with the honest result that a bundle with `blocked > 0` and a rejected `openwop-node-packs` claim is what the evidence says today. Zero-assertion passes (107 / 128 / 125 / 143 per host) are visible in each bundle and are the next classification target.
