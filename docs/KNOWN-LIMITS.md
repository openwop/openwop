...
| `node-pack-sandbox-*` (8 reference-impl invariants) | **A real-isolation reference host now exists** — `examples/hosts/wasm-sandbox/` (2026-05-31) executes pack-loaded typeIds as WebAssembly with construction-enforced isolation; its `test/sandbox.test.ts` passes **11/11 non-vacuously against real `.wasm` fixtures** (capability-gate both ways, all four `escapeKind`s, a real memory OOB trap, a real worker kill-timeout, fresh-instance isolation). This retires the old `it.todo`/`node:vm`-only rationale. The 8 conformance scenario files (`@openwop/openwop-conformance@1.4.0`) still carry `it.todo` behavioral stubs; a 2026-05-22 premature graduation to `protocol` was reverted (`5864a2f`) precisely because those were vacuous. | **Next step:** a portable sandbox conformance scenario under `conformance/src/scenarios/` (driving the committed fixtures) — `scripts/check-security-invariants.sh` requires a protocol-tier invariant's backing test to live there. Once it lands, 7 of 8 invariants graduate `reference-impl → protocol` (the JS-specific `no-eval` stays exempt — WASM has no `eval`). RFC 0035 `Active → Accepted` separately needs a **non-steward** sandbox-executing host (MyndHyve opted out: no-untrusted-packs). |
...
| 0073 (Capability families at document root) | `Draft` | Safety-fix/corrective — makes the schema's existing root placement a normative `MUST` (`capabilities.schema.json` already roots `agents`/`secrets`/etc. with no wrapper) and marks a top-level `capabilities` wrapper a deprecated v1.x-window legacy shape. Root-first/wrapper-fallback conformance accessor + agent-cohort reader migration + primary reference host landed. Path-to-`Active` is the 7-day comment window; resolves the RFC 0070 discovery-layout caveat openwop-side. No schema change. |
...
| 0077 (Agent Run Lifecycle + Live Manifest Dispatch) | `Active` | Wave-1 keystone's live-execution layer: a normative `AgentManifest` → live-run mapping + additive optional `capabilities.agents.liveRuntime` (⊃ `manifestRuntime`) + a content-free `agent.invocation.started`/`agent.invocation.completed` bracket around the existing `agent.*` family, so a manifest agent emits one identical observable family across workflow node, run API, and chat mention. Composes RFC 0070/0072/0061/0002; all five UQs resolved (MyndHyve T4 co-design; `invocationId` recorded-fact per `replay.md`). Path-to-`Accepted`: a non-steward host (MyndHyve) advertising `liveRuntime` + emitting the invocation pair. |
...
| 0083 (Durable Trigger + Channel Bridge Profile) | `Active` | Composes existing primitives (RFC 0052 scheduling, 0053 dead-letter, 0017 queue, webhooks, 0040 causation, the trigger nodes) into one additive `triggerBridge` capability + a derived `openwop-trigger-bridge` profile: four subscription states, a delivery model (per-attempt tracking + a `dedupKey` for at-least-once de-dup + a `retryPolicy`), an opt-in durable-webhook mode (`webhooks.durable`, best-effort default unchanged), and explicit trigger→run causation. Two content-free `trigger.*` events; all five UQs resolved. Behavioral delivery scenario + subscription-management endpoints + reference-host durable delivery deferred to Accepted. Path-to-`Accepted`: a host advertising `triggerBridge`. |
...
| 0086 (Standing Agent Roster + Workflow Portfolio) | `Draft` | Standardizes the "digital-twin employee" standing instance additively — a roster entry (`rosterId` + `persona` + an `agentRef` + a `workflows[]` portfolio + an owner triple, tenant-scoped per RFC 0074), an inventory projection, one content-free `roster.run.initiated` recorded-fact event attributing trigger-fired runs to the roster member, + an `agents.roster` capability. Triggers compose RFC 0052/0083 (no new `WorkflowTrigger.type`); the concrete work surface (Kanban/queue/inbox) stays a host extension. Merged at Draft (#363); wire surface = Draft → Active. Path-to-`Accepted`: a non-steward host (the adopter already carries a private `AutomationAgent` record) advertising `agents.roster` + emitting the event. |
| 0087 (Agent Org-Chart) | `Draft` | A descriptive org-chart layer over RFC 0086, additively — departments, roles, members (`rosterId`s), `reportsTo` edges + a derived responsibility view (which workflows a department owns transitively), gated behind `agents.orgChart` + tenant-scoped per RFC 0074. Load-bearing constraint: a new protocol-tier SECURITY invariant `org-position-no-authority-escalation` — an org edge is metadata only (a "manager" inherits no report's `toolAllowlist`, gains no RBAC scope by position, auto-satisfies no approval gate; RFC 0049/0051 stay the sole authority/gate sources, fail-closed). Merged at Draft (#363). Path-to-`Accepted`: a non-steward host advertising `agents.orgChart` + passing the behavioral non-authority scenario (the strongest bar — proving live that position grants nothing). |
