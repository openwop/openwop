...
| 0046 (`host.credentials` capability) | `Draft` | MyndHyve protocol-extension batch, Tier 1 (filed 2026-05-24). Portable credential resolution + lifecycle (store-at-rest, workspace sharing, two-key-overlap rotation) with a new `credential-payload-redaction` SECURITY invariant. No deps; foundation for 0047 + 0045. Path-to-`Active` is the 7-day comment window. |
| 0047 (`host.oauth` connector flows) | `Draft` | Tier 1. Host-performed OAuth2 authorization-code + refresh, tokens stored as 0046 credentials. Closes the `auth.md` authorization-code "Open spec gap". Depends on 0046. Path-to-`Active` is the 7-day comment window. |
| 0045 (Connector pack manifest & action model) | `Draft` | Tier 1. Optional `connector` manifest block (typed actions + idempotency/rate-limit metadata) binding to 0047 auth + 0046 credentials. Depends on 0046 + 0047. Path-to-`Active` is the 7-day comment window. |
| 0048 (Tenant·workspace·principal identity model) | `Draft` | MyndHyve batch, Tier 2. Promotes the RFC 0011 tenant dimension to an explicit `{tenant, workspace?, principal}` triple threading discovery + run ownership + events. Builds on 0011; foundation for 0049/0050/0051. Path-to-`Active` is the 7-day comment window. |
| 0049 (RBAC scopes & authorization decisions) | `Draft` | Tier 2. Role→scope binding (reuses the API-key scope grammar) + `authorization.decided` event + fail-closed `authorization-fail-closed` SECURITY invariant. Depends on 0048. Path-to-`Active` is the 7-day comment window. |
| 0050 (SAML / SCIM enterprise identity profiles) | `Draft` | Tier 2. SAML assertion-validation (`alg:none` rejection mirroring OIDC) + SCIM provisioning profiles mapping IdP users/groups onto 0048 principals + 0049 roles. Depends on 0048 + 0049, extends RFC 0010. Path-to-`Active` is the 7-day comment window. |
| 0051 (Approval & deployment-gate primitive) | `Draft` | Tier 2. `core.openwop.governance.approvalGate` interrupt node — role-gated, audited approvals/deploy-promotions composing quorum + auth-required profiles with 0049 authorization. Depends on 0049. Path-to-`Active` is the 7-day comment window. |
| 0052 (Scheduling & time-based triggers) | `Draft` | MyndHyve batch, Tier 3. `host.scheduling` (cron/delayed/calendar + horizon) wiring the `schedule` trigger to a durable, once-per-tick execution contract. Composes with RFC 0017 `host.queueBus`. Path-to-`Active` is the 7-day comment window. |
| 0053 (Dead-letter routing & failure sinks) | `Draft` | Tier 3. `host.deadLetter` + `run.dead_lettered` event; terminally-failed runs land in a durable, fork-eligible sink. Composes with RFC 0009 retry + RFC 0011 fork; distinct from 0017's transport-level DLQ. Path-to-`Active` is the 7-day comment window. |
| 0054 (Run diff & execution comparison) | `Draft` | Tier 3. Read-only `GET /v1/runs/{runId}:diff?against={otherRunId}` returning a deterministic, replay-aware structured diff. Depends on RFC 0011 fork; aligns with `replay.md` `replay.diverged`. Path-to-`Active` is the 7-day comment window. |
...
RFC **0034 (OTel collector test seam)** was promoted **Active → Accepted on 2026-05-23** after MyndHyve workflow-runtime advertised `observability.testSeams.otelScrape: true` and served `GET /v1/host/sample/test/otel/spans` with the RFC 0034 §B Tier-1 empty-buffer shape. See `INTEROP-MATRIX.md` §"Honest correction — MyndHyve `registerHostSampleRoutes` was not called pre-`60b569de`" for the evidence and the route-registration correction.
