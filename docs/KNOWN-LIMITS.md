...
| 0058 (Run execution bounds) | `Active` | Promoted Draft → Active 2026-05-25. Wire surface landed atomically (`capabilities.limits.{maxRunDurationMs, maxLoopIterations}` + `cap.breached{run-duration, loop-iterations}` payloads + `run_timeout` error code). The in-memory + Postgres + SQLite reference hosts now enforce the `runTimeoutMs` wall-clock arm end-to-end (emits `cap.breached{run-duration}` strictly exceeding the limit, then `failed` with `run_timeout`). MyndHyve round-3 did NOT graduate this RFC — they advertise `maxNodeExecutions` (the pre-existing recursionLimit bound, a different `cap.breached` kind), not `limits.maxRunDurationMs`. The `maxLoopIterations` arm is gated on an RFC 0061 stateful agent-loop host; neither reference host runs the orchestrator loop. Path-to-`Accepted` requires a non-steward host advertising `limits.maxRunDurationMs`. |
| 0061 (Stateful agent-loop lifecycle, `multiAgent.executionModel.version: 5`) | `Active` | Promoted Draft → Active 2026-05-25 — the autonomous-agent-runtime cohort's keystone. Wire surface landed atomically: `executionModel.version` ceiling 4 → 5 + additive optional `statefulResume` / `transcriptWindow` fields + additive optional `iteration` counter on the existing `runOrchestrator.decided` payload (no new event type) + normative `## Stateful agent-loop lifecycle (version >= 5)` section in `multi-agent-execution.md`. Composes RFC 0058 bound + RFC 0059 workspace snapshot + RFC 0039 memory snapshot + RFC 0041 replay determinism; no new event type, capability block, or SECURITY invariant. Path-to-`Accepted` requires a genuinely re-entrant agent-loop host — the workflow-engine executor is single-pass `core.dispatch` (consumes a supervisor's `decisions[]` in one node execution), NOT a re-entrant loop, so advertising `version: 5` on a single-pass host would overclaim. Either an executor rearchitecture or a non-steward runtime already running re-entrant loops (MyndHyve at `version: 4` → `5`) is required. |
...
| 0065 (Workflow node primary-output annotation) | `Draft` | Filed 2026-05-25 from the chat-surface architect-review pass. Additive optional `outputRole: "primary" \| "secondary"` on `WorkflowNode` so tooling can disambiguate the canonical artifact on multi-terminal DAGs. Advisory-only — engine behavior unchanged; pure additive per `COMPATIBILITY.md` §2.1. Path-to-`Active` is the 7-day comment window. |
