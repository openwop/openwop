...
| 0055 (Multimodal envelope variants + rendering hints) | `Draft` | Reference-app UX pair (filed 2026-05-25). Promotes RFC 0031 §C's reserved `vision-input` / `audio-input` / `audio-output` / `image-output` model-capability identifiers into a formal closed vocabulary; adds optional `meta.rendering` hint + `media.*` URL-reference payload convention so an LLM node can emit images / audio / files / cards any consumer renders portably. Explicitly **does not** add real-time A/V transport (out of charter). Path-to-`Active` is the 7-day comment window. |
| 0056 (Run feedback & annotation event) | `Draft` | Reference-app UX pair (filed 2026-05-25). New optional `host.feedback` capability + additive `run.annotated` RunEvent + `annotation.schema.json` + capability-gated `POST/GET /v1/runs/{runId}/annotations` for a portable, non-blocking human/agent quality signal (rating / correction / label / flag). Adds two protocol-tier SECURITY invariants (`annotation-cross-tenant-isolation`, `annotation-content-redaction`). Path-to-`Active` is the 7-day comment window. |
