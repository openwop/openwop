...
| Workflow-engine reference — `prompt.composed` not emitted for all four PromptKinds | `prompt-all-four-kinds-events.test.ts` + `prompt-end-to-end-events.test.ts` assert the host emits `prompt.composed` for `system`, `user`, `schema-hint`, AND `few-shot` per `prompts.md` §"Composition + observability". The reference workflow-engine emits for `system` + `user` end-to-end but the `few-shot` + `schema-hint` dispatch paths don't yet wire the composition event. | Wire `few-shot` + `schema-hint` PromptKinds through `host/promptCompose.ts` + `bootstrap/nodes.ts`'s dispatch loop. Estimated ~1 day. Tracked as a reference-host gap; the wire-shape contract is fully specified, only the reference impl's coverage is partial. |
