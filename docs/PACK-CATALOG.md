...
> Snapshot of every signed pack published at [`packs.openwop.dev`](https://packs.openwop.dev) as of **2026-05-21**. Grouped by domain (not alphabetical) so workflow authors can find what they need by use case. Live registry: [`/v1/index.json`](https://packs.openwop.dev/v1/index.json). Authoring guide: [`docs/AUTHORING-CANVAS-PACKS.md`](AUTHORING-CANVAS-PACKS.md). Architecture rationale: [`docs/CANVAS-PACKS-INVENTORY.md`](CANVAS-PACKS-INVENTORY.md).

**Catalog status:** 62 packs published. All signed under one of three keychains (`openwop-registry-root` for framework + community packs, `myndhyve-internal-1` for `vendor.myndhyve.*`, `vendor.openwop.*` for the rust-hello demo). Catalog updates on each merged pack-publishing PR.
