...
**A third shape, found the same day by both hosts: a scoped signal quoted as if it were unscoped, with the scope nowhere in the output.** A preflight tool answered *"competing: none"* at load 146, because it enumerated one repo's worktrees and looked for one process shape — it was right about the set it could see and wrong about the machine. A harness reported exit `0` over a real exit `1` five times in a day. `executedFail: 0` was true at one target major and quoted as if true of the host. In every case the instrument measured something real and the reader took it for something larger, and nothing in the output said which. A fourth instance, from a deploy: a root `typecheck` that carried **no project reference to the service being deployed** passed, the suite passed (the new mount's discovery branch was never exercised), lint passed, and the image failed at `esbuild` fifteen minutes into a remote build on an import of a module-private symbol — the bundle step was the only gate that looked at the tree the image is built from. Run the exact bundle command locally before any deploy; a green typecheck is a statement about the projects it references. A fifth, from a red rather than a green: a `set -euo pipefail` gate script with the conformance lane ahead of the frontend lanes exited 1 on a known conformance artifact in every full run of a day — so **the frontend lanes never executed once**, and the exit code, already discounted as "the known red", was also hiding every lane after it. *"The lane never ran"* and *"the lane passed"* print identically in a summary that only lists what failed. A known red early in a sequential gate is not merely noise; it is a curtain. A sixth, from the front door (openwop-app, ADR 0631's correction): every scenario that read a tenant-bound id back **passed against the direct service URL** and the host's own gate was green — while the public origin's hosting layer decoded `%2F` to `/` before forwarding, so every bound id was unreachable through the origin (`404 No route matches`), and the create response linked `eventsUrl` to `http://<direct service host>/…`. The direct-URL witness could not see either defect because it never crossed the layer that had them. **A hosting layer is part of the wire:** every id encoding and every absolute URL the backend emits must be witnessed *through* it, at the origin a client is given, not at the service behind it (§4.2d) — and the suite's own `blocked` on a non-200 read-back was the same shape one layer up, a routing hole recorded as "the host would not let me look" (`v2-created-run-readable`, rc.44, made it a hard assertion). **When a signal is green, ask what it was scoped to before you ask what it proves — and when it is a red you have learned to ignore, ask what it is standing in front of.** A tool that answers a shared question from a private vantage is not merely incomplete; two sessions running it get the same false answer simultaneously and both act on it.

**A seventh, and the pair is the clearest statement of the whole section.** In one week, from opposite directions: a conformance scenario shipped inside the published package but had no row in `conformance/scenario-majors.json`, the registry `--target-major` selects from — so every run reported 73 files, the scenario never executed, and a host's 260/260 was cited as the acceptance evidence for the RFC it was written to test. And a host built v1 retirement as one flag, covered it with **3,158 unit tests** that pinned the entire atomic set, and passed all of them — then deployed the flag to a throwaway service and cut the suite at it: **106 / 11 / 87 / 22 blocked**, every block *"run did not settle"*, because its own task-queue callbacks were addressed under `/v1` and a retired host refuses its own traffic. Neither instrument was lying. Both were **unasked**: one measured a set that did not contain the scenario, the other measured requests that nobody's queue would ever send.

> **A test suite is not a rehearsal, and a green step is not evidence that the thing was asked.**

The two defences differ, and both are cheap. For the registry: after adding a scenario, read the runner's own `N scenario file(s)` line and confirm it moved — the count is the instrument reporting what it was asked. For the flag: run it against a real cut, in a throwaway copy of production, before you hold the date (Phase 6).
