...
| Conformance scenario files | 371 | `conformance/src/scenarios/*.test.ts` |
| RFCs tracked | 111 | `RFCS/[0-9][0-9][0-9][0-9]-*.md`, excluding template |
...
| Conformance suite `@openwop/openwop-conformance` | 1.38.0 | `conformance/package.json` | minor on scenario add/remove |
...
- 3 RFCs `Active` (RFC 0035, RFC 0043, RFC 0115) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
