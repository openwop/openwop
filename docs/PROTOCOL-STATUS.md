...
| Conformance suite `@openwop/openwop-conformance` | 2.41.2 | `conformance/package.json` | minor on scenario add/remove |
...
| RFC 0197 | v2 surfaces are retired, never reshaped | Accepted |
...
| RFC 0205 | A named A2A `Artifact` / `Part` schema for `getArtifact` (negotiated by `application/a2a+json`) and for conversation turns (by `parts` presence), and media types accepted beside the reserved `exportFormats` aliases | Accepted |
...
| RFC 0217 | after a subscription is unregistered, its dead-letter read answers as though it never existed | Active |
...
- 5 RFCs `Active` (RFC 0111, RFC 0121, RFC 0199, RFC 0210, RFC 0217) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
