...
| RFC 0120 | Connection-pack provider `apiHosts` - declared credential-egress allow-list | Draft |
...
- 2 RFCs still `Draft` (RFC 0038, RFC 0120) — advance with schema/conformance proof or defer.
- 7 RFCs `Active` (RFC 0035, RFC 0043, RFC 0111, RFC 0114, RFC 0116, RFC 0117, RFC 0119) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
