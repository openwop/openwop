...
| JSON Schemas | 53 | `schemas/*.schema.json` |
| OpenAPI operations | 42 | `api/openapi.yaml` |
| AsyncAPI version | 3.1.0 | `api/asyncapi.yaml` |
| Conformance scenario files | 301 | `conformance/src/scenarios/*.test.ts` |
...
|---|---:|---:|---:|---:|---:|---|
| Postgres reference | 1988 | 6 | 92 | 0 | 2086 | 95.3% total; 1 deterministic failure (re-measured 2026-05-30 against suite v1.10.0 via pglite + OPENWOP_WEBHOOK_ALLOW_PRIVATE=true, post #376 + #378). The agent-memory (×5), agent-packs (×3), aiEnvelope.capBreached (×3) and discovery (×1) failures are closed; the lone deterministic residual is orchestratorConservativePath (RFC 0006 CP-1 low-confidence supervisor suspend - the host completes instead of suspending to waiting-approval; not yet implemented). The other 5 (queue-ack-nack-dlq ×2, replay-llm-cache-key ×3) pass in isolation - full-suite-parallelism flakes. |
