...
| Workflow-engine reference (exhaustive-mode, post bundled-path bugfix: 2026-05-23, suite v1.5.0 + commit d09d99c) | 1521 | 6 | 56 | 14 | 1597 | 95.4% total (re-measured 2026-05-23 with OPENWOP_RATELIMIT_DISABLED=true + all Phase 4 + sandbox + multi-region + cross-engine + secret-leakage seams enabled). Prior crash-polluted snapshot (2026-05-23, pre-d09d99c): 1291 / 129 / 161 / 14 / 1595 = 80.9% - retained for delta-comparison context only; the host-side envelopeAcceptor.ts ENOENT crash on first envelope-accept request masked ~120 honest scenarios as failures rather than skips. See d09d99c commit message + docs/CONFORMANCE-RUNS-2026-05-23.md for the diagnosis. The earlier "129 failures" inventory dissolves into 6 real failures. Closes 22 behavioral assertions across the 5 new Phase 4 scenarios end-to-end (multi-region 6 + cross-engine 4 + sandbox 9 + secret-leakage 3). |
...
- 7 RFCs `Active` (RFC 0028, RFC 0029, RFC 0034, RFC 0035, RFC 0036, RFC 0040, RFC 0041) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
