...
| Conformance suite `@openwop/openwop-conformance` | 1.54.0 | `conformance/package.json` | minor on scenario add/remove |
...
| RFC 0132 | An anonymous-actor principal kind for public agent surfaces - an opaque, origin-bound, non-PII actor whose authority is a per-surface tool grant (default-deny, never the default-on baseline), with a read tier and a mandatorily-gated bounded-write/egress tier | Draft |
...
- 2 RFCs still `Draft` (RFC 0038, RFC 0132) — advance with schema/conformance proof or defer.
