...
| Conformance scenario files | 554 | `conformance/src/scenarios/*.test.ts` |
| RFCs tracked | 208 | `RFCS/[0-9][0-9][0-9][0-9]-*.md`, excluding template |
...
| Conformance suite `@openwop/openwop-conformance` | 2.37.0 | `conformance/package.json` | minor on scenario add/remove |
...
| openwop-app corpus pin / suite pin | v2.31.0 / ^2.31.0 | openwop-app `schemas/CORPUS_TAG`, `backend/typescript/package.json` | tier-1 host; both must agree (RFC 0176 sectionE.1) |
...
| RFC 0035 | Sandbox execution contract for pack-loaded typeIds | Superseded |
...
| RFC 0158 | Durable Execution and Disaster-Recovery Qualification | Accepted |
...
| RFC 0188 | `GET /webhooks/{webhookId}/dead-letters`: the read that makes `webhooks.md` sectionDurability observable, gated on a new `webhooks.deadLetter` facet | Accepted |
...
| RFC 0212 | canonical JSON is RFC 8785 JCS, and a certification preimage does not depend on the machine that computed it | Active |
...
- 17 RFCs `Active` (RFC 0111, RFC 0121, RFC 0197, RFC 0198, RFC 0199, RFC 0200, RFC 0201, RFC 0202, RFC 0203, RFC 0204, RFC 0205, RFC 0206, RFC 0207, RFC 0208, RFC 0209, RFC 0210, RFC 0212) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
