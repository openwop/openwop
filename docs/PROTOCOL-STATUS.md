...
| RFCs tracked | 132 | `RFCS/[0-9][0-9][0-9][0-9]-*.md`, excluding template |
...
| Conformance suite `@openwop/openwop-conformance` | 1.55.0 | `conformance/package.json` | minor on scenario add/remove |
...
| RFC 0132 | An anonymous-actor principal kind for public agent surfaces - an opaque, origin-bound, non-PII actor whose authority is a per-surface tool grant (default-deny, never the default-on baseline), with a read tier and a mandatorily-gated bounded-write/egress tier | Active |
...
- 4 RFCs `Active` (RFC 0035, RFC 0111, RFC 0121, RFC 0132) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
