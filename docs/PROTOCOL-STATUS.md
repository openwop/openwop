...
| Conformance suite `@openwop/openwop-conformance` | 1.23.0 | `conformance/package.json` | minor on scenario add/remove |
...
| RFC 0099 | External-Event Trigger Ingestion - extend the RFC 0083 durable-trigger bridge so `webhook` / `email` / `form` subscription sources can deliver an externally-originated event -> start a run, with a normative inbound-event envelope, a subscription-registration contract, and SSRF/replay safety | Draft |
...
- 3 RFCs still `Draft` (RFC 0038, RFC 0099, RFC 0100) — advance with schema/conformance proof or defer.
- 5 RFCs `Active` (RFC 0035, RFC 0043, RFC 0096, RFC 0097, RFC 0098) — wire-shape MAY shift compatibly within v1.x until promotion to `Accepted`.
