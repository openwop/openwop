 ...
 *
 * Mocks `globalThis.fetch` to return synthesized OpenAI-style SSE
 * streams so the dispatcher runs end-to-end without a real provider.
 *
 * Verifies the §B normative ("MUST emit exactly ONE per LLM provider
 * invocation") at the boundary the impl actually uses:
 *
 *   - Plain `callAI`               → 1 dispatchPlain call  → 1 event
 *   - Structured `callAI` w/ retry → N dispatchPlain calls → N events
 *
 * The emission site is `providers/dispatch.ts`'s consumer
 * `aiProviders/aiProvidersHost.ts:dispatchPlain` so the retry loop
 * inside `dispatchStructured` produces one event per attempt rather
 * than collapsing N invocations into 1.
 */
